		...
		match Flavor::current() {
			...
			Flavor::Acorn { .. } => {
				let (base, value, raw) = match self {
					...
					Literal::Null(l) => {
						(&l.base, AcornLiteralValue::Null(None), Cow::Borrowed("null"))
					},
					Literal::Boolean(l) => {
						(
							&l.base,
							AcornLiteralValue::Boolean(l.value),
							if l.value { Cow::Borrowed("true") } else { Cow::Borrowed("false") },
						)
					},
					Literal::RegExp(l) => {
						let mut s = serializer.serialize_map(None)?;
						{
							...
							l.base.serialize(serde::__private::ser::FlatMapSerializer(&mut s))?;
						}
						s.serialize_entry("type", "Literal")?;

						s.serialize_entry(
							"regex",
							&AcornRegex { flags:&l.flags, pattern:&l.pattern },
						)?;
				...
				let acorn = AcornLiteral { type_:"Literal", raw:&raw, base, value };
...
impl<'de> Deserialize<'de> for Literal {
	...
	where
		...
		BabelLiteral::deserialize(deserializer).map(|b| {
			match b {
