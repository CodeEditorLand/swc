...
pub struct ClassMethod {
	...
	#[serde(default, rename = "async", serialize_with = "crate::ser::serialize_as_bool")]
		...
		match Flavor::current() {
			...
			Flavor::Acorn { .. } => {
...

				{
					...
					self.base.serialize(serde::__private::ser::FlatMapSerializer(&mut s))?;
				}
...
impl From<TSExpressionWithTypeArguments> for ClassImpl {
	fn from(expr:TSExpressionWithTypeArguments) -> Self { ClassImpl::TSExpr(expr) }
}
