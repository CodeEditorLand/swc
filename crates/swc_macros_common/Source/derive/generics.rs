...
impl<'a> Derive<'a> {
	pub fn all_generic_fields(&self) -> Vec<&'a Field> {
		...
		impl<'a:'b, 'b> Visit<'a> for FieldVisitor<'b> {
			fn visit_field(&mut self, field:&'a Field) {
				let mut vis = TypeVisitor { params:&self.params, is_generic:false };
	...
	where
		...
		let preds = preds.into_iter().map(|t| Pair::Punctuated(t, Token![,](def_site())));

		match self.out.generics.where_clause {
			Some(WhereClause { ref mut predicates, .. }) => {
