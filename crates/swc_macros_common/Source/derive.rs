...
impl<'a> Derive<'a> {
	...
	/// Set `unsafety`
	pub fn unsafety(&mut self, unsafety:Option<token::Unsafe>) { self.out.unsafety = unsafety; }

	pub fn input(&self) -> &DeriveInput { self.input }
	...
	pub fn append_to(mut self, item:ItemImpl) -> ItemImpl {
		...
		self.out.generics.params.extend(item.generics.params.into_pairs());
