...
pub fn call_site() -> Span { Span::call_site() }

pub fn def_site() -> Span { call_site() }
...
pub fn is_attr_name(attr:&Attribute, name:&str) -> bool { attr.path().is_ident(name) }
...
pub fn access_field(obj:&dyn ToTokens, idx:usize, f:&Field) -> Expr {
	Expr::Field(ExprField {
		...
		member:match &f.ident {
			...
			_ => Member::Unnamed(Index { index:idx as _, span:Span::call_site() }),
		},
	})
}
