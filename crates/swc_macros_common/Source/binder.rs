...
impl<'a> VariantBinder<'a> {
	pub const fn new(
		...
		VariantBinder { enum_name, name, data, attrs }
	}

	pub const fn variant_name(&self) -> &Ident { self.name }

	pub const fn data(&self) -> &Fields { self.data }

	pub const fn attrs(&self) -> &[Attribute] { self.attrs }
	...
	pub fn bind(
		...
		let (pat, bindings) = match *self.data {
			Fields::Unit => {
				...
				let pat = Pat::Path(PatPath { qself:None, path, attrs:Default::default() });

				// Unit struct does not have any field to bind
				(pat, Vec::new())
			},
			Fields::Named(FieldsNamed { named: ref fields, brace_token }) => {
			...
			Fields::Unnamed(FieldsUnnamed { unnamed: ref fields, paren_token }) => {
				// TODO
				let mut bindings = Vec::new();
...
impl<'a> BindedField<'a> {
	pub const fn idx(&self) -> usize { self.idx }

	/// Name of field binding.
	pub const fn name(&self) -> &Ident { &self.binded_ident }

	pub const fn field(&self) -> &Field { self.field }
}

impl<'a> ToTokens for BindedField<'a> {
	fn to_tokens(&self, t:&mut TokenStream) { self.binded_ident.to_tokens(t) }
}
