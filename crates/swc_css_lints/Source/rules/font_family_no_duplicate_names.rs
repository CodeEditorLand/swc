...
use swc_common::{Span, collections::AHashSet};
use swc_css_ast::*;
...
pub fn font_family_no_duplicate_names(
	...
	Ok(visitor_rule(ctx.reaction(), FontFamilyNoDuplicateNames { ctx, ignored }))
}
...
impl FontFamilyNoDuplicateNames {
	fn check_component_values(&self, values:&[ComponentValue]) {
		let (mut fonts, last) = values.iter().fold(
			(Vec::with_capacity(values.len()), Option::<(String, Span)>::None),
			|(mut fonts, last_identifier), item| {
				match item {
		...
		fonts.iter().fold(AHashSet::default(), |mut seen, (font, span)| {
			let name = font.name();

			if seen.contains(&font) && self.ignored.iter().all(|item| !item.is_match(name)) {
				self.ctx.report(span, format!("Unexpected duplicate name '{}'.", name));
			}
