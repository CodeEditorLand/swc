...
pub fn custom_property_no_missing_var_function(ctx:LintRuleContext<()>) -> Box<dyn LintRule> {
	visitor_rule(ctx.reaction(), CustomPropertyNoMissingVarFunction { ctx, ..Default::default() })
}
...
impl Visit for CustomPropertyNoMissingVarFunction {
	...
	fn visit_dashed_ident(&mut self, dashed_ident:&DashedIdent) {
		if let (Some(true), Some(false) | None) =
			(self.in_declaration_value.last(), self.in_var_function.last())
		{
			self.ctx.report(
				dashed_ident,
				format!("Unexpected missing var function for '--{}'.", dashed_ident.value),
			);
		}
