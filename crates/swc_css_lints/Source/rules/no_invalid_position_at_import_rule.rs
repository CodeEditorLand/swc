...
pub fn no_invalid_position_at_import_rule(
	...
	let ignored = ctx
		...
		.collect::<Result<_, _>>()?;
	Ok(visitor_rule(ctx.reaction(), NoInvalidPositionAtImportRule { ctx, ignored }))
}
...
impl Visit for NoInvalidPositionAtImportRule {
	fn visit_stylesheet(&mut self, stylesheet:&Stylesheet) {
		stylesheet.rules.iter().fold(false, |seen, rule| {
			...
			let AtRule { name, prelude, block, .. } = &**at_rule;
			...
			if self.ignored.iter().any(|item| item.is_match(name)) { seen } else { true }
		});
