...
pub fn inline_globals2(
	...
	as_folder(InlineGlobals { envs, globals, global_exprs, typeofs, bindings:Default::default() })
}
...
mod tests {
	...
	test!(
		Default::default(),
		|tester| {
			inline_globals(envs(tester, &[("x", "FOO")]), globals(tester, &[]), Default::default())
		},
		issue_417_2,
		"const test = process.env['x']"
	);
	...
	test!(
		Default::default(),
		|tester| {
			inline_globals(envs(tester, &[("x", "BAR")]), globals(tester, &[]), Default::default())
		},
		issue_2499_1,
		"process.env.x = 'foo'"
	);
