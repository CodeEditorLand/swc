...
fn parse_option(cm:&SourceMap, name:&str, src:String) -> Arc<Expr> {
	...
	let fm =
		cm.new_source_file(FileName::Internal(format!("<const-module-{}.js>", name)).into(), src);
	if let Some(expr) = CACHE.get(&**fm.src) {
		return expr.clone();
	}

	let expr =
		parse_file_as_expr(&fm, Default::default(), Default::default(), None, &mut Vec::new())
			.map_err(|e| {
				if HANDLER.is_set() {
