...
impl CompilerPass for Remover {
	fn name(&self) -> Cow<'static, str> { Cow::Borrowed("dead_branch_remover") }
}
...
impl Parallel for Remover {
	fn create(&self) -> Self { Self { expr_ctx:self.expr_ctx.clone(), ..*self } }
...
impl VisitMut for Remover {
	...
	fn visit_mut_expr(&mut self, e:&mut Expr) {
		...
		match e {
			...
			Expr::Assign(AssignExpr {
				op: op!("="), left: AssignTarget::Pat(left), right, ..
			}) if match &*left {
			...
			Expr::Bin(be) => {
				match (be.left.is_invalid(), be.right.is_invalid()) {
					(true, true) => {
						*e = Expr::dummy();
					},
					(true, false) => {
	...
	fn visit_mut_object_pat(&mut self, p:&mut ObjectPat) {
		...
		fn is_computed(k:&PropName) -> bool { matches!(k, PropName::Computed(..)) }
		...
		p.props.retain(|p| {
			match p {
					...
					} =>
				{
					if cfg!(feature = "debug") {
						debug!(
							"Dropping key-value pattern property because it's an empty object \
							 pattern"
						);
					}
					...
					} =>
				{
					if cfg!(feature = "debug") {
						debug!(
							"Dropping key-value pattern property because it's an empty array \
							 pattern"
						);
					}
	...
	fn visit_mut_object_pat_prop(&mut self, p:&mut ObjectPatProp) {
		...
		match p {
			...
			{
				*p =
					ObjectPatProp::Assign(AssignPatProp { span:*span, key:key.take(), value:None });
			},
			...
			match stmt {
				Stmt::If(IfStmt { span, test, cons, alt }) => {
					if let Stmt::If(IfStmt { alt: Some(..), .. }) = *cons {
						return IfStmt {
							test,
							cons:Box::new(
								BlockStmt { stmts:vec![*cons], ..Default::default() }.into(),
							),
							alt,
							span,
						}
				...
				Stmt::Switch(mut s) => {
										...
										) =>
									{
										return Some(
											VarDecl {
												...
												decls:var.decls.move_map(|decl| {
													VarDeclarator { init:None, ..decl }
												}),
												..Default::default()
											}
											.into(),
						...
						if let Expr::Lit(..) = *s.discriminant {
							...
							if let Some(i) = idx {
								...
								if !has_conditional_stopper(&s.cases[i].cons) {
									...
									let mut block:Stmt =
										BlockStmt { span:s.span, stmts, ..Default::default() }
											.into();
							...
							let res = match case.test {
								Some(e)
									if matches!(
										&*e,
										Expr::Lit(Lit::Num(..))
											| Expr::Lit(Lit::Str(..)) | Expr::Lit(Lit::Null(..))
									) =>
								{
									case.cons
										.into_iter()
...

					{
						...
						let is_all_case_empty =
							s.cases.iter().all(|case| case.test.is_none() || case.cons.is_empty());
							...
							match &case.test {
								Some(e)
									if matches!(
										&**e,
										Expr::Lit(Lit::Str(..))
											| Expr::Lit(Lit::Null(..)) | Expr::Lit(Lit::Num(..))
									) =>
								{
									true
								},
				...
				Stmt::While(s) => {
					if let (purity, Known(v)) = s.test.cast_to_bool(&self.expr_ctx) {
						if v {
							if purity.is_pure() {
								WhileStmt {
									test:Lit::Bool(Bool { span:s.test.span(), value:true }).into(),
									..s
								}
								.into()
							} else {
				...
				Stmt::DoWhile(s) => {
					...
					if let Known(v) = s.test.as_pure_bool(&self.expr_ctx) {
						if v {
							...
							ForStmt { span:s.span, init:None, test:None, update:None, body:s.body }
								.into()
						} else {
	...
	fn visit_mut_stmts(&mut self, n:&mut Vec<Stmt>) { self.fold_stmt_like(n) }
								...
								(purity, Known(val)) => {
									...
									if !purity.is_pure() {
										...
										if let Some(expr) = expr {
											new_stmts.push(T::from(
												ExprStmt { span:DUMMY_SP, expr }.into(),
											));
										}
									}
...
fn ignore_result(e:Box<Expr>, drop_str_lit:bool, ctx:&ExprCtx) -> Option<Box<Expr>> {
	match *e {
		...
		Expr::Assign(AssignExpr {
			op: op!("="), left: AssignTarget::Simple(left), right, ..
		}) if match &left {
			SimpleAssignTarget::Ident(l) => {
				match &*right {
					Expr::Ident(r) => l.sym == r.sym && l.ctxt == r.ctxt,
		...
		Expr::Array(ArrayLit { span, elems, .. }) => {
			...
			let elems = elems.move_flat_map(|v| {
				match v {
		...
		Expr::Object(ObjectLit { span, props, .. }) => {
			let props = props.move_flat_map(|v| {
				match v {
		...
		Expr::New(NewExpr { span, ref callee, args, .. }) if callee.is_pure_callee(ctx) => {
			ignore_result(
				ArrayLit {
					span,
					elems:args
		...
		Expr::Call(CallExpr { span, callee: Callee::Expr(ref callee), args, .. })
			if callee.is_pure_callee(ctx) =>
		{
			ignore_result(
				ArrayLit { span, elems:args.into_iter().map(Some).collect() }.into(),
				true,
				ctx,
			)
		},
		...
		Expr::Cond(CondExpr { span, test, cons, alt }) => {
			let alt = if let Some(alt) = ignore_result(alt, true, ctx) {
				...
				return ignore_result(
					BinExpr { span, left:test, op:op!("&&"), right:cons }.into(),
					true,
					ctx,
				);
			};
			...
			let cons = if let Some(cons) = ignore_result(cons, true, ctx) {
				...
				return ignore_result(
					BinExpr { span, left:test, op:op!("||"), right:alt }.into(),
					true,
					ctx,
				);
			};
...
fn is_ok_to_inline_block(s:&[Stmt]) -> bool {
	...
	let last_stopper = s.iter().rposition(|s| {
		matches!(s, Stmt::Return(..) | Stmt::Throw(..) | Stmt::Break(..) | Stmt::Continue(..))
	});
...
fn has_unconditional_stopper(s:&[Stmt]) -> bool { check_for_stopper(s, false) }

fn has_conditional_stopper(s:&[Stmt]) -> bool { check_for_stopper(s, true) }
