...
impl Parallel for SimplifyExpr {
	...
	fn merge(&mut self, other:Self) { self.changed |= other.changed; }
}
...
impl CompilerPass for SimplifyExpr {
	fn name(&self) -> Cow<'static, str> { Cow::Borrowed("simplify-expr") }
}
...
impl SimplifyExpr {
		...
		match &mut **obj {
				...
				match op {
					...
					KnownOp::Index(idx) => {
						if idx.fract() != 0.0 || idx < 0.0 || idx as usize >= value.len() {
							// Prototype changes affect indexing if the index is out of bounds, so
							// we don't replace out-of-bound indexes.
							return;
						}
			...
			Expr::Array(ArrayLit { elems, span }) => {
				...
				match op {
					KnownOp::Len => {
						...
						*expr = Lit::Num(Number { value:elems.len() as _, span:*span, raw:None })
							.into();
					},
					...
					KnownOp::Index(idx) => {
						...
						let after_has_side_effect =
							elems.iter().skip((idx as usize + 1) as _).any(|elem| {
								match elem {
						...
						if exprs.is_empty() {
							...
							// (0, val)
							*expr = SeqExpr { span:val.span(), exprs:vec![0.into(), val] }.into();
			...
			Expr::Object(ObjectLit { props, span }) => {
				...
				*expr = *self.expr_ctx.preserve_effects(
					*span,
					v,
					once(ObjectLit { props:props.take(), span:*span }.into()),
				);
			},
			...
			op!(bin, "+") => {
				...
				if left.is_str() || left.is_array_lit() || right.is_str() || right.is_array_lit() {
					if let (Known(l), Known(r)) =
						(left.as_pure_string(&self.expr_ctx), right.as_pure_string(&self.expr_ctx))
					{
				...
				if let (_, Known(val)) = left.cast_to_bool(&self.expr_ctx) {
					...
					if !left.may_have_side_effects(&self.expr_ctx) {
						...
						if node.directness_maters() {
							*expr = SeqExpr { span:node.span(), exprs:vec![0.into(), node.take()] }
								.into();
						} else {
							*expr = *node.take();
						}
						...
						let mut seq = SeqExpr { span:*span, exprs:vec![left.take(), node.take()] };
	...
	fn perform_arithmetic_op(&self, op:BinaryOp, left:&Expr, right:&Expr) -> Value<f64> {
		...
		let (lv, rv) = (left.as_pure_number(&self.expr_ctx), right.as_pure_number(&self.expr_ctx));
		...
		match (lt, rt) {
			...
			(NumberType, StringType) | (_, BoolType) => {
				...
				self.perform_abstract_eq_cmp(
					span,
					left,
					&Lit::Num(Number { value:rv, span, raw:None }).into(),
				)
			},
			...
			(StringType, NumberType) | (BoolType, _) => {
				...
				self.perform_abstract_eq_cmp(
					span,
					&Lit::Num(Number { value:lv, span, raw:None }).into(),
					right,
				)
			},
	...
	fn perform_strict_eq_cmp(&mut self, left:&Expr, right:&Expr) -> Value<bool> {
		...
		match lt {
			...
			BoolType => {
				let (lv, rv) =
					(left.as_pure_bool(&self.expr_ctx), right.as_pure_bool(&self.expr_ctx));
...
impl VisitMut for SimplifyExpr {
					...
					match elem {
						Some(ExprOrSpread { spread: Some(..), expr }) if expr.is_array() => {
							...
							e.extend(expr.array().unwrap().elems.into_iter().map(|elem| {
								Some(elem.unwrap_or_else(|| {
									ExprOrSpread { spread:None, expr:Expr::undefined(DUMMY_SP) }
								}))
							}));
						},
	...
	fn visit_mut_with_stmt(&mut self, n:&mut WithStmt) { n.obj.visit_mut_with(self); }
...
fn need_zero_for_this(e:&Expr) -> bool { e.directness_maters() || e.is_seq() }
