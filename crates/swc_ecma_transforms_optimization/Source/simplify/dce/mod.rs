...
use petgraph::{Direction::Incoming, algo::tarjan_scc};
...
use swc_ecma_transforms_base::{
	...
	perf::{ParVisitMut, Parallel, cpu_count},
};
use swc_ecma_utils::{
	ExprCtx,
	ExprExt,
...
pub fn dce(config:Config, unresolved_mark:Mark) -> impl Pass + VisitMut + Repeated + CompilerPass {
...
impl CompilerPass for TreeShaker {
	fn name(&self) -> Cow<'static, str> { Cow::Borrowed("tree-shaker") }
}
		...
		match self.graph.edge_weight_mut(from, to) {
			...
			None => {
				self.graph.add_edge(
					from,
					to,
					VarInfo { usage:u32::from(!assign), assign:u32::from(assign) },
				);
			},
		};
	}
...
impl Analyzer<'_> {
	...
	where
		F: for<'aa> FnOnce(&mut Analyzer<'aa>), {
		let child_scope = {
			let child = Scope { parent:Some(&self.scope), ..Default::default() };
		...
		if self.scope.is_ast_path_empty() {
			...
			while let Some(s) = scope {
				for component in &s.ast_path {
					self.data.add_dep_edge(component.clone(), id.clone(), assign)
				}
...
impl Visit for Analyzer<'_> {
	...
	fn visit_export_decl(&mut self, n:&ExportDecl) {
		let name = match &n.decl {
			...
			Decl::Var(v) => v.decls.iter().flat_map(|d| find_pat_ids(d).into_iter()).collect(),
			_ => Vec::new(),
		};
...
impl TreeShaker {
	...
	fn can_drop_assignment_to(&self, name:Id, is_var:bool) -> bool {
		...
		self.bindings.contains(&name)
			&& self.data.used_names.get(&name).map(|v| v.usage == 0).unwrap_or_default()
	}
}
...
impl VisitMut for TreeShaker {
	...
	fn visit_mut_expr(&mut self, n:&mut Expr) {
		...
		if let Expr::Call(CallExpr { callee: Callee::Expr(callee), args, .. }) = n {
			//
			if args.is_empty() {
				match &mut **callee {
					Expr::Fn(FnExpr { ident: None, function: f })
						if matches!(
							&**f,
							Function { is_async:false, is_generator:false, body:Some(..), .. }
						) =>
					{
	...
	fn visit_mut_exprs(&mut self, n:&mut Vec<Box<Expr>>) { self.visit_mut_par(cpu_count() * 8, n); }
	...
	fn visit_mut_import_specifiers(&mut self, ss:&mut Vec<ImportSpecifier>) {
		ss.retain(|s| {
			...
			if self.can_drop_binding(local.to_id(), false) {
				debug!("Dropping import specifier `{}` because it's not used", local);
	...
	fn visit_mut_module_items(&mut self, s:&mut Vec<ModuleItem>) { self.visit_mut_stmt_likes(s); }
		...
		if let Stmt::Decl(Decl::Var(v)) = s {
			...
			if cnt != 0
				&& v.decls.iter().all(|vd| {
					match &vd.name {
						Pat::Ident(i) => {
							self.can_drop_binding(i.to_id(), v.kind == VarDeclKind::Var)
						},
						_ => false,
					}
				}) {
				let exprs = v.decls.take().into_iter().filter_map(|v| v.init).collect::<Vec<_>>();

				debug!(count = cnt, "Dropping names of variables as they are not used",);
				...
				if exprs.is_empty() {
					...
					*s = ExprStmt { span, expr:Expr::from_exprs(exprs) }.into();
				}
			}
		}
	...
	fn visit_mut_stmts(&mut self, s:&mut Vec<Stmt>) { self.visit_mut_stmt_likes(s); }
