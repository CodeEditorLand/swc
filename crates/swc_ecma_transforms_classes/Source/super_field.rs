...
impl<'a> VisitMut for SuperFieldAccessFolder<'a> {
	...
	fn visit_mut_expr(&mut self, n:&mut Expr) {
		match n {
			Expr::This(ThisExpr { span }) if self.in_nested_scope => {
				*n = quote_ident!(
					SyntaxContext::empty().apply_mark(
						*self.this_alias_mark.get_or_insert_with(|| Mark::fresh(Mark::root()))
					),
					*span,
					"_this"
				)
			...
			Expr::Assign(AssignExpr { ref left, op: op!("="), right, .. })
...
impl<'a> SuperFieldAccessFolder<'a> {
	...
	fn super_to_set_call(
		...
		let this_expr = Box::new(match self.constructor_this_mark {
			Some(mark) => {
				quote_ident!(SyntaxContext::empty().apply_mark(mark), super_token, "_this").into()
			},
...
fn prop_arg(prop:SuperProp) -> Expr {
	match prop {
		SuperProp::Ident(IdentName { sym: value, span, .. }) => {
			Lit::Str(Str { span, raw:None, value }).into()
		},
