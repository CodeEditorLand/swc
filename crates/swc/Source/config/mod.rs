...
impl Options {
	pub fn codegen_target(&self) -> Option<EsVersion> { self.config.jsc.target }
}
...
impl Default for InputSourceMap {
	fn default() -> Self { InputSourceMap::Bool(true) }
}
...
impl Options {
	...
	where
		...
		let mut assumptions = assumptions
			.unwrap_or_else(|| if loose { Assumptions::all() } else { Assumptions::default() });
		...
		program.visit_mut_with(&mut resolver(unresolved_mark, top_level_mark, syntax.typescript()));
		...
		js_minify = js_minify.map(|mut c| {
			...
			JsMinifyOptions { compress, mangle, ..c }
		});
		...
		let preserve_comments = if preserve_all_comments {
			...
			js_minify
				.as_ref()
				.map(|v| {
					match v.format.comments.clone().into_inner() {
						Some(v) => v,
		...
		let preamble = if !cfg.jsc.output.preamble.is_empty() {
			...
			js_minify.as_ref().map(|v| v.format.preamble.clone()).unwrap_or_default()
		};
		...
		.hygiene(if self.disable_hygiene {
			...
			Some(hygiene::Config { keep_class_names, ..Default::default() })
		})
		.fixer(!self.disable_fixer)
		.preset_env(cfg.env)
		.regenerator(regenerator)
		.finalize(syntax, cfg.module, comments.map(|v| v as _), resolver.clone());
		...
		let plugin_transforms = {
			...
			#[cfg(all(feature = "plugin", not(target_arch = "wasm32")))]
			{
				...
				if let Some(plugins) = &experimental.plugins {
					...
					let mut inner_cache =
						PLUGIN_MODULE_CACHE.inner.get().expect("Cache should be available").lock();
			...
			.into_bool()
		{
			...
			let decorator_pass:Box<dyn Fold> = match transform.decorator_version.unwrap_or_default()
			{
				DecoratorVersion::V202112 => {
					Box::new(decorators(decorators::Config {
			...
			Box::new(chain!(
				...
				Optional::new(
					typescript::tsx::<Option<&dyn Comments>>(
						...
						typescript::TsxConfig {
							pragma:Some(
								transform.react.pragma.clone().unwrap_or_else(default_pragma)
							),
							pragma_frag:Some(
								transform
									.react
				...
				Optional::new(dropped_comments_preserver(comments.cloned()), preserve_all_comments),
			))
		};
...
const fn default_swcrc() -> bool { true }
...
impl Default for ConfigFile {
	fn default() -> Self { ConfigFile::Bool(true) }
}
...
#[cfg(not(all(target_arch = "wasm32", not(target_os = "wasi"))))]
fn default_cwd() -> PathBuf { ::std::env::current_dir().unwrap() }
	...
	fn default() -> Self {
		Rc::Multi(vec![
			Config {
				...
				jsc:JscConfig { syntax:Some(Default::default()), ..Default::default() },
				..Default::default()
			},
			Config {
				env:None,
				...
				exclude:None,
				jsc:JscConfig {
					syntax:Some(Syntax::Typescript(TsSyntax { tsx:true, ..Default::default() })),
					..Default::default()
				},
				..Default::default()
			},
			...
			Config {
				...
				exclude:None,
				jsc:JscConfig {
					syntax:Some(Syntax::Typescript(TsSyntax { tsx:false, ..Default::default() })),
					..Default::default()
				},
				..Default::default()
			},
	...
	pub fn build<'cmt>(
		...
		match config {
			...
			Some(ModuleConfig::SystemJs(config)) => {
				Box::new(modules::system_js::system_js(resolver, unresolved_mark, config))
			},
		}
	}
...
impl Default for SimplifyOption {
	fn default() -> Self { SimplifyOption::Bool(true) }
}
...
fn default_preserve_imports_with_side_effects() -> bool { true }
...
fn default_jsonify_min_cost() -> usize { 1024 }
...
impl GlobalPassOption {
	pub fn build(self, cm:&SourceMap, handler:&Handler) -> impl 'static + Fold {
		...
		fn mk_map(
			...
			for (k, v) in values {
				let v = if is_env { format!("'{}'", v) } else { (*v).into() };
				let v_str = v.clone();
		...
		let env_map = if cfg!(target_arch = "wasm32") {
			...
			match &self.envs {
				...
				GlobalInliningPassEnvs::Map(map) => {
					...
					let cache_key =
						self.vars.iter().map(|(k, v)| (k.clone(), v.clone())).collect::<Vec<_>>();
