...
impl DroppedCommentsPreserver {
	...
	/// once.
	fn collect_existing_comments(
		&self,
		comments:&SingleThreadedComments,
	) -> CommentEntries {
		let (mut leading_comments, mut trailing_comments) =
	...
	fn shift_leading_comments(
		...
		for span in self.known_spans.iter() {
			let cut_point =
				existing_comments.partition_point(|(bp, _)| *bp <= span.lo);
	...
	/// similar location
	fn shift_trailing_comments(
		&self,
		remaining_comment_entries:CommentEntries,
	) {
