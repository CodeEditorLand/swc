...
/// comments.
pub fn dropped_comments_preserver(comments:Option<SingleThreadedComments>) -> impl Fold + VisitMut {
	as_folder(DroppedCommentsPreserver { comments, is_first_span:true, known_spans:Vec::new() })
}
...
impl DroppedCommentsPreserver {
	...
	fn shift_leading_comments(&self, comments:&SingleThreadedComments) -> CommentEntries {
		...
		for span in self.known_spans.iter() {
			...
			let collected_comments = existing_comments
				...
				.flat_map(|(_, c)| c)
				.collect::<Vec<Comment>>();
			self.comments.add_leading_comments(span.lo, collected_comments)
		}
	...
	fn shift_trailing_comments(&self, remaining_comment_entries:CommentEntries) {
		...
		self.comments.add_trailing_comments(
			last_trailing.hi,
			remaining_comment_entries.into_iter().flat_map(|(_, c)| c).collect(),
		);
	}
}
