...
impl RustPlugins {
	...
	fn apply_inner(&mut self, n:Program) -> Result<Program, anyhow::Error> {
		...
		// swc_plugin_macro will not inject proxy to the comments if comments is
		// empty
		...
		swc_plugin_proxy::COMMENTS.set(
			...
			|| {
				...
				// We do not serialize / deserialize between each plugin
				// execution but copies raw transformed bytes directly into
				// plugin's memory space. Note: This doesn't mean plugin
				// won't perform any se/deserialization: it still have to
				// construct from raw bytes internally to perform actual
				// transform.
				...
				// Plugin transformation is done. Deserialize transformed bytes
				// back into Program
