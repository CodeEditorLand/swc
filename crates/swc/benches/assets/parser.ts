...
/** @internal */
export function isFileProbablyExternalModule(
...
namespace Parser {
	...
	function parsePropertyNameWorker(
		...
		if (
			...
			const node = parseLiteralNode() as
				| StringLiteral
				| NumericLiteral
				| BigIntLiteral;
	...
	function currentNode(
		...
		if (
			nodeIsMissing(node) ||
			intersectsIncrementalChange(node) ||
	...
	function parseImportOrExportSpecifier(
		...
		if (kind === SyntaxKind.ImportSpecifier) {
			if (name.kind !== SyntaxKind.Identifier) {
				...
				parseErrorAt(
					skipTrivia(sourceText, name.pos),
					name.end,
					Diagnostics.Identifier_expected,
				);
	...
	function parseNamespaceExport(pos: number): NamespaceExport {
		return finishNode(
			factory.createNamespaceExport(
				parseModuleExportName(parseIdentifierName),
			),
			pos,
		);
	}
