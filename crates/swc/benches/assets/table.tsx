...
import {
	...
	type PagePropertiesMetaManager,
...
export const SortableProperties = ({ children }: SortablePropertiesProps) => {
	...
	const properties = useMemo(
		() => manager.sorter.getOrderedItems(),
		[manager],
	);
	...
	const menuItems = useMemo(() => {
		...
		if (!localPropertyMeta.required) {
			...
			options.push({
				...
				text: t[
					visibilityMenuText(
						rotateVisibility(localProperty.visibility),
					)
				](),
				onClick: toggleHide,
			});
...
export const PagePropertiesCreatePropertyMenuItems = ({
	...
	const onAddProperty = useCallback(
		(
			...
			const nameExists = schemaList.some(
				(meta) => meta.name === option.name,
			);
...
const PagePropertiesAddPropertyMenuItems = ({
	...
	const menuItems = useMemo(() => {
		...
		if (nonRequiredMetaList.length > 0) {
			options.push("-");
			const nonRequiredMetaOptions: MenuItemOption =
...
export const PagePropertiesAddProperty = () => {
	...
	const menuOptions = useMemo(() => {
		...
		const items = adding ? (
			<PagePropertiesAddPropertyMenuItems
