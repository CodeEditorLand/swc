...
impl VisitMut for Actual {
			...
			ModuleItem::ModuleDecl(ModuleDecl::ExportDefaultDecl(export_default)) => {
				if let DefaultDecl::Fn(expr) = &mut export_default.decl {
					if expr.function.is_async {
						...
						*item = ExportDefaultDecl { span:export_default.span, decl:name_fn.into() }
							.into();
	...
	fn visit_mut_method_prop(&mut self, prop:&mut MethodProp) {
		...
		let prop_method_body_span =
			if let Some(body) = &prop.function.body { body.span } else { DUMMY_SP };

		prop.function.span = prop_method_body_span;

		let fn_ref = make_fn_ref(Function { params:Vec::new(), ..*prop.function.take() }.into());
		...
		prop.function = Function {
			...
			body:Some(BlockStmt {
				span:DUMMY_SP,
				stmts:vec![Stmt::Return(ReturnStmt { span:DUMMY_SP, arg:Some(Box::new(fn_ref)) })],
				..Default::default()
			}),
			..Default::default()
		}
	...
	fn visit_mut_var_declarator(&mut self, var:&mut VarDeclarator) {
		if let VarDeclarator { name: Pat::Ident(id), init: Some(init), .. } = var {
			match init.as_ref() {
				Expr::Fn(FnExpr { ident: None, ref function })
...
fn handle_await_for(stmt:&mut Stmt, is_async_generator:bool) {
	...
	let finally_block = {
		let throw_iterator_error =
			ThrowStmt { span:DUMMY_SP, arg:iterator_error.clone().into() }.into();
