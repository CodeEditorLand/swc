	...
	R: Resolve,
{
	...
	pub fn new(inner:R, base_url:PathBuf, paths:Vec<(String, Vec<String>)>) -> Self {
		if cfg!(debug_assertions) {
			info!(base_url = tracing::field::display(base_url.display()), "jsc.paths");
		}

		let mut paths:Vec<(Pattern, Vec<String>)> = paths
			.into_iter()
			.map(|(from, to)| {
				assert!(!to.is_empty(), "value of `paths.{}` should not be an empty array", from,);
				...
				let pat = if from.contains('*') {
					...
					Pattern::Wildcard { prefix:from[..pos.unwrap()].to_string() }
				} else {
					assert_eq!(
						to.len(),
						1,
		...
		paths.sort_by(|(a, _), (b, _)| {
			match (a, b) {
				...
				(
					Pattern::Wildcard { prefix: prefix_a },
					Pattern::Wildcard { prefix: prefix_b },
				) => prefix_a.len().cmp(&prefix_b.len()).reverse(),
			}
		});

		Self { inner, base_url_filename:FileName::Real(base_url.clone()), base_url, paths }
	}
	...
	fn invoke_inner_resolver(
		...
		match res {
			Ok(resolved) => {
				info!("Resolved `{}` as `{}` from `{}`", module_specifier, resolved.filename, base);
		...
		for (from, to) in &self.paths {
			match from {
				...
				Pattern::Exact(from) => {
					...
					if tp.is_absolute() {
						return Ok(Resolution { filename:FileName::Real(tp.into()), slug });
					}
