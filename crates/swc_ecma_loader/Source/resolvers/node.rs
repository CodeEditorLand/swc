...
pub fn to_absolute_path(path:&Path) -> Result<PathBuf, Error> {
	let absolute_path =
		if path.is_absolute() { path.to_path_buf() } else { current_dir()?.join(path) }.clean();

	Ok(absolute_path)
}

pub(crate) fn is_core_module(s:&str) -> bool { NODE_BUILTINS.contains(&s) }
...
impl NodeModulesResolver {
	...
	pub fn new(
		...
		Self { target_env, alias, preserve_symlinks, ignore_node_modules:false }
	}
	...
	pub fn without_node_modules(
		...
		Self { target_env, alias, preserve_symlinks, ignore_node_modules:true }
	}
	...
	fn resolve_node_modules(&self, base_dir:&Path, target:&str) -> Result<Option<PathBuf>, Error> {
	...
	fn resolve_filename(&self, base:&FileName, module_specifier:&str) -> Result<FileName, Error> {
		debug!("Resolving {} from {:#?} for {:#?}", module_specifier, base, self.target_env);
		...
		if !module_specifier.starts_with('.') {
			...
			if let Ok(file) =
				self.resolve_as_file(path).or_else(|_| self.resolve_as_directory(path, false))
			{
		...
		let file_name = {
			if target_path.is_absolute() {
				...
				if let Some(Component::CurDir | Component::ParentDir) = components.next() {
					...
					self.resolve_node_modules(base_dir, target).and_then(|path| {
