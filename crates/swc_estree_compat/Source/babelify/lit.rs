...
impl Babelify for Bool {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		BooleanLiteral { base:ctx.base(self.span), value:self.value }
	}
}
...
impl Babelify for Null {
	...
	fn babelify(self, ctx:&Context) -> Self::Output { NullLiteral { base:ctx.base(self.span) } }
}
...
impl Babelify for Number {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		NumericLiteral { base:ctx.base(self.span), value:self.value }
	}
}
...
impl Babelify for Regex {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		RegExpLiteral { base:ctx.base(self.span), pattern:self.exp, flags:self.flags }
	}
}
