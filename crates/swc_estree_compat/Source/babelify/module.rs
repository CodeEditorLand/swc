...
impl Babelify for Module {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		...
		BabelProgram {
			...
			body:self.body.into_iter().map(|stmt| stmt.babelify(ctx).into()).collect(),
			interpreter:self.shebang.map(|s| {
				InterpreterDirective { base:ctx.base(extract_shebang_span(span, ctx)), value:s }
			}),
...
fn base_with_trailing_newline(span:Span, ctx:&Context) -> BaseNode {
	...
	base.loc = base
		.loc
		.map(|loc| Loc { end:LineCol { line:loc.end.line + 1, column:0 }, ..loc });
...
impl Babelify for ModuleItem {
	...
	fn parallel(cnt:usize) -> bool { cnt >= 32 }
...
fn extract_all_comments(program:&Program, ctx:&Context) -> Vec<Comment> {
	let mut collector = CommentCollector { comments:ctx.comments.clone(), collected:Vec::new() };
