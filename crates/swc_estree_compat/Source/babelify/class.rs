...
use swc_estree_ast::{
	...
	ClassBodyEl,
	ClassExpression,
	ClassMethod as BabelClassMethod,
	ClassMethodKind,
	ClassPrivateMethod,
...
impl Babelify for Class {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		...
		ClassExpression {
			...
			body,
			super_class:self.super_class.map(|expr| Box::alloc().init(expr.babelify(ctx).into())),
			...
			implements:Some(
				self.implements.into_iter().map(|imp| imp.babelify(ctx).into()).collect(),
			),
			id:Default::default(),
...
impl Babelify for ClassMember {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		match self {
			...
			ClassMember::Empty(_) => {
				panic!("illegal conversion: Cannot convert {:?} to ClassBodyEl", &self)
			},
...
impl Babelify for ClassProp {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		...
		ClassProperty {
			...
			value:self.value.map(|val| Box::alloc().init(val.babelify(ctx).into())),
			type_annotation:self.type_ann.map(|ann| Box::alloc().init(ann.babelify(ctx).into())),
...
impl Babelify for PrivateProp {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		ClassPrivateProperty {
			base:ctx.base(self.span),
			key:self.key.babelify(ctx),
			value:self.value.map(|expr| Box::alloc().init(expr.babelify(ctx).into())),
			type_annotation:self.type_ann.map(|ann| Box::alloc().init(ann.babelify(ctx).into())),
...
impl Babelify for StaticBlock {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		BabelStaticBlock { base:ctx.base(self.span), body:self.body.stmts.babelify(ctx) }
	}
}
