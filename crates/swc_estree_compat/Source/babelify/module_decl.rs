	...
	asserts.map(|obj| {
		...
		obj.props
			...
			.map(|prop_or_spread| {
				...
				ImportAttribute { base:ctx.base(obj_span), key, value:val }
			})
			.collect()
	})
}
...
impl Babelify for ExportDefaultDecl {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		ExportDefaultDeclaration { base:ctx.base(self.span), declaration:self.decl.babelify(ctx) }
	}
}
...
impl Babelify for DefaultDecl {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		match self {
			...
			DefaultDecl::TsInterfaceDecl(_) => panic!("unimplemented"), /* TODO(dwoznicki): Babel
			                                                             * expects a TSDeclareFunction
			                                                             * here, which does not
			                                                             * map cleanly to
			                                                             * TsInterfaceDecl
			                                                             * expected by swc */
...
impl Babelify for ImportDefaultSpecifier {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		BabelImportDefaultSpecifier { base:ctx.base(self.span), local:self.local.babelify(ctx) }
	}
}
...
impl Babelify for ImportStarAsSpecifier {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		ImportNamespaceSpecifier { base:ctx.base(self.span), local:self.local.babelify(ctx) }
	}
}
...
impl Babelify for ImportNamedSpecifier {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		BabelImportSpecifier {
			...
			import_kind:if self.is_type_only { Some(ImportKind::Type) } else { None },
		}
	}
}
...
impl Babelify for ExportNamespaceSpecifier {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		BabelExportNamespaceSpecifier { base:ctx.base(self.span), exported:self.name.babelify(ctx) }
	}
}
...
impl Babelify for ExportDefaultSpecifier {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		...
		BabelExportDefaultSpecifier { base:exported.base.clone(), exported }
	}
}
...
impl Babelify for ExportNamedSpecifier {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		BabelExportSpecifier {
			...
			export_kind:if self.is_type_only { ExportKind::Type } else { ExportKind::Value },
		}
	}
}
