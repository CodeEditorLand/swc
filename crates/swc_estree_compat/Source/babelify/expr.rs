...
impl Babelify for Expr {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		match self {
			...
			Expr::Lit(lit) => {
				match lit {
					Lit::Str(s) => {
						ExprOutput::Expr(
							Box::alloc()
								.init(Expression::Literal(Literal::String(s.babelify(ctx)))),
						)
					},
					Lit::Bool(b) => {
						ExprOutput::Expr(
							Box::alloc()
								.init(Expression::Literal(Literal::Boolean(b.babelify(ctx)))),
						)
					},
					Lit::Null(n) => {
						ExprOutput::Expr(
					...
					Lit::Num(n) => {
						ExprOutput::Expr(
							Box::alloc()
								.init(Expression::Literal(Literal::Numeric(n.babelify(ctx)))),
						)
					},
					Lit::BigInt(i) => {
						ExprOutput::Expr(
							Box::alloc()
								.init(Expression::Literal(Literal::BigInt(i.babelify(ctx)))),
						)
					},
					Lit::Regex(r) => {
						ExprOutput::Expr(
							Box::alloc()
								.init(Expression::Literal(Literal::RegExp(r.babelify(ctx)))),
						)
					},
					Lit::JSXText(_) => {
						panic!("illegal conversion: Cannot convert {:?} to ExprOutput", &lit)
			...
			Expr::JSXMember(_) => {
				panic!(
					"illegal conversion: Cannot convert {:?} to ExprOutput - babel has no \
					 equivalent",
					&self
				)
			},
...
impl From<ExprOutput> for Expression {
	fn from(o:ExprOutput) -> Self {
		match o {
			...
			ExprOutput::Private(_) => {
				panic!(
					"illegal conversion: Cannot convert {:?} to Expression - babel has no \
					 equivalent",
					&o
				)
			},
...
impl From<ExprOutput> for ObjectKey {
	fn from(o:ExprOutput) -> Self {
		match o {
			...
			ExprOutput::Private(_) => {
				panic!(
					"illegal conversion: Cannot convert {:?} to ObjectKey - babel has no \
					 equivalent",
					&o
				)
			},
...
impl Babelify for ThisExpr {
	...
	fn babelify(self, ctx:&Context) -> Self::Output { ThisExpression { base:ctx.base(self.span) } }
}
...
impl Babelify for ArrayLit {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		ArrayExpression { base:ctx.base(self.span), elements:self.elems.babelify(ctx) }
	}
}
...
impl Babelify for ObjectLit {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		ObjectExpression { base:ctx.base(self.span), properties:self.props.babelify(ctx) }
	}
}
...
impl Babelify for BinExpr {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		match self.op {
			...
			_ => {
				BinaryOrLogicalExpr::Binary(BinaryExpression {
					base:ctx.base(self.span),
					operator:self.op.babelify(ctx).into(),
					left:Box::alloc().init(self.left.babelify(ctx).into()),
					right:Box::alloc().init(self.right.babelify(ctx).into()),
...
impl Babelify for ClassExpr {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		ClassExpression { id:self.ident.map(|id| id.babelify(ctx)), ..self.class.babelify(ctx) }
	}
}
...
impl Babelify for CallExpr {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		CallExpression {
			...
			arguments:self.args.into_iter().map(|arg| arg.babelify(ctx).into()).collect(),
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		NewExpression {
			...
			arguments:match self.args {
				Some(args) => args.into_iter().map(|arg| arg.babelify(ctx).into()).collect(),
				None => Vec::new(),
			},
			type_parameters:self.type_args.map(|t| t.babelify(ctx)),
			type_arguments:Default::default(),
...
impl Babelify for ArrowExpr {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		ArrowFunctionExpression {
			base:ctx.base(self.span),
			params:self.params.into_iter().map(|p| p.babelify(ctx).into()).collect(),
			...
			return_type:self.return_type.map(|t| Box::alloc().init(t.babelify(ctx).into())),
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		let (meta, property) = match self.kind {
			MetaPropKind::ImportMeta => {
				(
					Ident {
						span:Span { hi:self.span.hi - BytePos(5), ..self.span },
						sym:"import".into(),
						..Default::default()
					}
					.babelify(ctx),
					Ident {
						span:Span { lo:self.span.lo + BytePos(7), ..self.span },
						sym:"meta".into(),
						..Default::default()
					}
					.babelify(ctx),
			...
			MetaPropKind::NewTarget => {
				(
					Ident {
						span:Span { hi:self.span.hi - BytePos(7), ..self.span },
						sym:"new".into(),
						..Default::default()
					}
					.babelify(ctx),
					Ident {
						span:Span { hi:self.span.hi + BytePos(4), ..self.span },
						sym:"target".into(),
						..Default::default()
					}
					.babelify(ctx),
		...
		MetaProperty { base:ctx.base(self.span()), meta, property }
	}
}
...
impl Babelify for TplElement {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		TemplateElement {
			base:ctx.base(self.span),
			tail:self.tail,
			value:TemplateElVal { raw:self.raw, cooked:self.cooked },
		}
	}
}
...
impl Babelify for Super {
	...
	fn babelify(self, ctx:&Context) -> Self::Output { BabelSuper { base:ctx.base(self.span) } }
}

impl Babelify for Import {
	type Output = BabelImport;

	fn babelify(self, ctx:&Context) -> Self::Output { BabelImport { base:ctx.base(self.span) } }
}
...
impl Babelify for ExprOrSpread {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		match self.spread {
			Some(_) => {
				ArrayExprEl::Spread(BabelSpreadElement {
					base:ctx.base(self.span()),
					argument:Box::alloc().init(self.expr.babelify(ctx).into()),
				})
			},
