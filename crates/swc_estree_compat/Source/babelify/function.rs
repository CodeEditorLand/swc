...
impl Babelify for Param {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		match self.pat {
			...
			Pat::Array(a) => {
				BabelParam::Pat(Pattern::Array(ArrayPattern {
					base:ctx.base(self.span),
					decorators:Some(self.decorators.babelify(ctx)),
					..a.babelify(ctx)
				}))
			...
			Pat::Object(o) => {
				BabelParam::Pat(Pattern::Object(ObjectPattern {
					base:ctx.base(self.span),
					decorators:Some(self.decorators.babelify(ctx)),
					..o.babelify(ctx)
				}))
			...
			Pat::Assign(a) => {
				BabelParam::Pat(Pattern::Assignment(AssignmentPattern {
					base:ctx.base(self.span),
					decorators:Some(self.decorators.babelify(ctx)),
					..a.babelify(ctx)
				}))
			...
			Pat::Expr(_) => {
				panic!("illegal conversion: Cannot convert {:?} to BabelParam", &self.pat)
			},
			Pat::Invalid(_) => {
				panic!("illegal conversion: Cannot convert {:?} to BabelParam", &self.pat)
			},
