...
impl Babelify for Stmt {
	...
	fn parallel(cnt:usize) -> bool { cnt >= 16 }
...
impl Babelify for EmptyStmt {
	...
	fn babelify(self, ctx:&Context) -> Self::Output { EmptyStatement { base:ctx.base(self.span) } }
}
...
impl Babelify for DebuggerStmt {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		DebuggerStatement { base:ctx.base(self.span) }
	}
}
...
impl Babelify for ReturnStmt {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		ReturnStatement {
			base:ctx.base(self.span),
			argument:self.arg.map(|expr| Box::alloc().init(expr.babelify(ctx).into())),
		}
	}
}
...
impl Babelify for BreakStmt {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		BreakStatement { base:ctx.base(self.span), label:self.label.map(|id| id.babelify(ctx)) }
	}
}
...
impl Babelify for ContinueStmt {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		ContinueStatement { base:ctx.base(self.span), label:self.label.map(|id| id.babelify(ctx)) }
	}
}
...
impl Babelify for ForStmt {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		ForStatement {
			...
			test:self.test.map(|expr| Box::alloc().init(expr.babelify(ctx).into())),
			update:self.update.map(|expr| Box::alloc().init(expr.babelify(ctx).into())),
			body:Box::alloc().init(self.body.babelify(ctx)),
		}
	}
}
