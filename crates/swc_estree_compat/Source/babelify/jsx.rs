...
use swc_estree_ast::{
	JSXAttrName as BabelJSXAttrName,
	JSXAttrVal,
	JSXAttribute,
	JSXClosingElement as BabelJSXClosingElement,
	JSXClosingFragment as BabelJSXClosingFragment,
	JSXElement as BabelJSXElement,
	JSXElementChild as BabelJSXElementChild,
	JSXElementName as BabelJSXElementName,
	JSXEmptyExpression,
	JSXExprContainerExpr,
	JSXExpressionContainer,
	JSXFragment as BabelJSXFragment,
	JSXMemberExprObject,
	JSXMemberExpression,
	JSXNamespacedName as BabelJSXNamespacedName,
	JSXOpeningElAttr,
	JSXOpeningElement as BabelJSXOpeningElement,
	JSXOpeningFragment as BabelJSXOpeningFragment,
	JSXSpreadAttribute,
	JSXSpreadChild as BabelJSXSpreadChild,
	JSXText as BabelJSXText,
	flavor::Flavor,
};
...
impl Babelify for JSXEmptyExpr {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		JSXEmptyExpression { base:ctx.base(self.span) }
	}
}
...
impl Babelify for JSXExprContainer {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		JSXExpressionContainer { base:ctx.base(self.span), expression:self.expr.babelify(ctx) }
	}
}
...
impl Babelify for JSXClosingElement {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		BabelJSXClosingElement { base:ctx.base(self.span), name:self.name.babelify(ctx) }
	}
}
...
impl Babelify for JSXText {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		BabelJSXText { base:ctx.base(self.span), value:self.value }
	}
}
...
impl Babelify for JSXOpeningFragment {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		BabelJSXOpeningFragment { base:ctx.base(self.span) }
	}
}
...
impl Babelify for JSXClosingFragment {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		BabelJSXClosingFragment { base:ctx.base(self.span) }
	}
}
