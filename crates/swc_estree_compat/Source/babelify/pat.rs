...
impl Babelify for Pat {
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		match self {
			...
			Pat::Invalid(_) => {
				panic!("illegal conversion: Cannot convert {:?} to PatOutput", &self)
			},
		}
	}
}
...
impl From<PatOutput> for AssignmentPatternLeft {
	fn from(pat:PatOutput) -> Self {
		match pat {
			...
			PatOutput::Rest(_) => {
				panic!("illegal conversion: Cannot convert {:?} to AssignmentPatternLeft", &pat)
			},
			PatOutput::Assign(_) => {
				panic!("illegal conversion: Cannot convert {:?} to AssignmentPatternLeft", &pat)
			},
	...
	fn babelify(self, ctx:&Context) -> Self::Output {
		ArrayPattern {
			...
			elements:self
				...
				.collect(),
			type_annotation:self.type_ann.map(|a| Box::alloc().init(a.babelify(ctx).into())),
