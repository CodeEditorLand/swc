...
impl Context {
	...
	fn line_col(&self, pos:BytePos) -> Option<LineCol> {
		...
		Some(LineCol { line:loc.line, column:loc.col.0 })
	}
...
pub trait Babelify: Send + Sync {
	...
	fn parallel(_cnt:usize) -> bool { false }
	...
	T: Babelify,
{
	...
	fn babelify(self, ctx:&Context) -> Self::Output { self.map(|v| v.babelify(ctx)) }
}
	...
	T: Babelify,
{
	...
	fn babelify(self, ctx:&Context) -> Self::Output { (*self).babelify(ctx) }
}
