...
impl Swcify for RestElement {
	...
	fn swcify(self, ctx:&Context) -> Self::Output {
		...
		RestPat { span, dot3_token:span, arg:Box::new(self.argument.swcify(ctx)), type_ann:None }
	}
}
	...
	fn swcify(self, ctx:&Context) -> Self::Output {
		match self {
			...
			swc_estree_ast::Param::Pat(v) => {
				...
				swc_ecma_ast::Param { span:pat.span(), decorators:Default::default(), pat }
			},
			swc_estree_ast::Param::Rest(v) => {
				swc_ecma_ast::Param {
