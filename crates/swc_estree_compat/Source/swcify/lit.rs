...
impl Swcify for StringLiteral {
	...
	fn swcify(self, ctx:&Context) -> Self::Output {
		Str { span:ctx.span(&self.base), value:self.value, raw:Some(self.raw) }
	}
}
...
impl Swcify for NullLiteral {
	...
	fn swcify(self, ctx:&Context) -> Self::Output { Null { span:ctx.span(&self.base) } }
}
...
impl Swcify for BooleanLiteral {
	...
	fn swcify(self, ctx:&Context) -> Self::Output {
		Bool { span:ctx.span(&self.base), value:self.value }
	}
}
...
impl Swcify for RegExpLiteral {
	...
	fn swcify(self, ctx:&Context) -> Self::Output {
		Regex { span:ctx.span(&self.base), exp:self.pattern, flags:self.flags }
	}
}
...
impl Swcify for DecimalLiteral {
	...
	fn swcify(self, ctx:&Context) -> Self::Output {
		Number {
			span:ctx.span(&self.base),
			value:self.value.parse().expect("failed to parse the value of DecimalLiteral"),
			// TODO improve me
			raw:None,
		}
	}
