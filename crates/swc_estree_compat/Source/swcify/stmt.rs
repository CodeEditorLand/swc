...
impl Swcify for BlockStatement {
	...
	fn swcify(self, ctx:&Context) -> Self::Output {
		BlockStmt {
			...
			stmts:self.body.swcify(ctx).into_iter().map(|v| v.expect_stmt()).collect(),
			..Default::default()
		}
	}
}
...
impl Swcify for BreakStatement {
	...
	fn swcify(self, ctx:&Context) -> Self::Output {
		BreakStmt { span:ctx.span(&self.base), label:self.label.swcify(ctx).map(|v| v.into()) }
	}
}
...
impl Swcify for ContinueStatement {
	...
	fn swcify(self, ctx:&Context) -> Self::Output {
		ContinueStmt { span:ctx.span(&self.base), label:self.label.swcify(ctx).map(|v| v.into()) }
	}
}
...
impl Swcify for DebuggerStatement {
	...
	fn swcify(self, ctx:&Context) -> Self::Output { DebuggerStmt { span:ctx.span(&self.base) } }
}
...
impl Swcify for EmptyStatement {
	...
	fn swcify(self, ctx:&Context) -> Self::Output { EmptyStmt { span:ctx.span(&self.base) } }
}
...
impl Swcify for ExpressionStatement {
	...
	fn swcify(self, ctx:&Context) -> Self::Output {
		ExprStmt { span:ctx.span(&self.base), expr:self.expression.swcify(ctx) }
	}
}
...
impl Swcify for IfStatement {
	...
	fn swcify(self, ctx:&Context) -> Self::Output {
		IfStmt {
			...
			alt:self.alternate.swcify(ctx).map(|v| v.expect_stmt()).map(Box::new),
		}
	}
}
...
impl Swcify for ReturnStatement {
	...
	fn swcify(self, ctx:&Context) -> Self::Output {
		ReturnStmt { span:ctx.span(&self.base), arg:self.argument.swcify(ctx) }
	}
}
...
impl Swcify for swc_estree_ast::SwitchCase {
	...
	fn swcify(self, ctx:&Context) -> Self::Output {
		swc_ecma_ast::SwitchCase {
			span:ctx.span(&self.base),
			test:self.test.swcify(ctx),
			cons:self.consequent.swcify(ctx).into_iter().map(|v| v.expect_stmt()).collect(),
		}
	}
}
...
impl Swcify for ThrowStatement {
	...
	fn swcify(self, ctx:&Context) -> Self::Output {
		ThrowStmt { span:ctx.span(&self.base), arg:self.argument.swcify(ctx) }
	}
}
	...
	fn swcify(self, ctx:&Context) -> Self::Output {
		ExportAll {
			...
			with:self
				...
				.map(|props| ObjectLit { span:DUMMY_SP, props }.into()),
		}
	}
}
...
impl Swcify for ImportAttribute {
	...
	fn swcify(self, ctx:&Context) -> Self::Output {
		KeyValueProp { key:self.key.swcify(ctx), value:Lit::Str(self.value.swcify(ctx)).into() }
	}
}
	...
	fn swcify(self, ctx:&Context) -> Self::Output {
		match self.declaration {
			ExportDefaultDeclType::Func(v) => {
				...
				ExportDefaultDecl {
					span:ctx.span(&self.base),
					decl:DefaultDecl::Fn(FnExpr { ident:Some(d.ident), function:d.function }),
				}
				.into()
			},
			ExportDefaultDeclType::Class(v) => {
				...
				ExportDefaultDecl {
					span:ctx.span(&self.base),
					decl:DefaultDecl::Class(ClassExpr { ident:Some(d.ident), class:d.class }),
				}
				.into()
			},
			ExportDefaultDeclType::Expr(v) => {
				ExportDefaultExpr { span:ctx.span(&self.base), expr:v.swcify(ctx) }.into()
			},
			_ => {
				todo!("swcify: {:?}", self)
			},
...
impl Swcify for swc_estree_ast::ExportDefaultSpecifier {
	...
	fn swcify(self, ctx:&Context) -> Self::Output {
		swc_ecma_ast::ExportDefaultSpecifier { exported:self.exported.swcify(ctx).into() }
	}
}
	...
	fn swcify(self, ctx:&Context) -> Self::Output {
		ImportDecl {
			...
			with:self
				...
				.map(|props| ObjectLit { span:DUMMY_SP, props }.into()),
			phase:self.phase.map(|phase| Swcify::swcify(phase, ctx)).unwrap_or_default(),
		}
	}
}
...
impl Swcify for ImportNamespaceSpecifier {
	...
	fn swcify(self, ctx:&Context) -> Self::Output {
		ImportStarAsSpecifier { span:ctx.span(&self.base), local:self.local.swcify(ctx).into() }
	}
}
...
impl Swcify for DeclareClass {
	...
	fn swcify(self, _:&Context) -> Self::Output { unimplemented!("flow") }
}
