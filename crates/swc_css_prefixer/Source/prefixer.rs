...
static PREFIXES_AND_BROWSERS:Lazy<
	AHashMap<String, [BrowserData<Option<Version>>; 2]>,
> = Lazy::new(|| {
...
fn should_enable(
	...
	if zip!(target, low_versions, high_versions).all(
		|((_, target_version), ((_, l), (_, h)))| {
	...
	zip!(target, low_versions, high_versions).any(
		|(
			...
			maybe_target_version.map_or(false, |target_version| {
				let low_or_fallback_version = maybe_low_version.or_else(|| {
					match target_name {
						// Fall back to Chrome versions if Android browser data
						// is missing from the feature data. It appears the
						// Android browser has aligned its versioning with
						// Chrome.
				...
				if let Some(low_or_fallback_version) = low_or_fallback_version {
					if target_version >= low_or_fallback_version {
						let high_or_fallback_version =
							maybe_high_version.or(match target_name {
								// Fall back to Chrome versions if Android
								// browser data is missing from the
								// feature data. It appears the
								// Android browser has aligned its versioning
								// with Chrome.
								"android" => high_versions.chrome,
								_ => None,
							});

						if let Some(high_or_fallback_version) =
							high_or_fallback_version
						{
...
pub fn prefixer(options:Options) -> impl VisitMut {
	let env:Versions =
		targets_to_versions(options.env).expect("failed to parse targets");
...
impl VisitMut for CrossFadeFunctionReplacerOnLegacyVariant<'_> {
	fn visit_mut_function(&mut self, n:&mut Function) {
		...
		if n.name == *self.from {
			...
			let transparency_value =
				match (transparency_values[0], transparency_values[1]) {
					(None, None) => 0.5,
...
fn replace_cross_fade_function_on_legacy_variant<N>(
	node:&mut N,
	from:&str,
	to:&str,
) where
	N: for<'aa> VisitMutWith<CrossFadeFunctionReplacerOnLegacyVariant<'aa>>, {
	node.visit_mut_with(&mut CrossFadeFunctionReplacerOnLegacyVariant {
		from,
		to,
	});
}
...
fn replace_image_set_function_on_legacy_variant<N>(
	node:&mut N,
	from:&str,
	to:&str,
) where
	...
	fn visit_mut_function(&mut self, n:&mut Function) {
		...
		if n.name == *self.from {
					...
					if ident.value.as_ref().eq_ignore_ascii_case("to") =>
				{
					...
					match (n.value.get(1), n.value.get(2)) {
						(
							...
							if let (
								Some(new_first_direction),
								Some(new_second_direction),
							) = (
						...
						(Some(ComponentValue::Ident(ident)), Some(_)) => {
							...
							if let Some(new_direction) =
								get_old_direction(value)
							{
								let new_value = vec![ComponentValue::Ident(
									Box::new(Ident {
										span:*span,
				...
				Some(ComponentValue::Dimension(dimension)) => {
					...
					}) = &**dimension
					{
						...
						if angle == 0.0 {
							...
							let new_value = ((450.0 - angle).abs() % 360.0
								* 1000.0)
								.round() / 1000.0;

							n.value[0] = ComponentValue::Dimension(Box::new(
			...
			if matches!(
				...
				if let (Some(at_index), Some(first_comma_index)) =
					(at_index, first_comma_index)
				{
					let mut new_value = Vec::new();

					new_value.append(
						&mut n.value[at_index + 1..first_comma_index].to_vec(),
					);
					new_value.append(&mut vec![ComponentValue::Delimiter(
						Box::new(Delimiter {
							span:DUMMY_SP,
							value:DelimiterValue::Comma,
						}),
...
fn replace_gradient_function_on_legacy_variant<N>(
	node:&mut N,
	from:&str,
	to:&str,
) where
	N: for<'aa> VisitMutWith<
		LinearGradientFunctionReplacerOnLegacyVariant<'aa>,
	>, {
	node.visit_mut_with(&mut LinearGradientFunctionReplacerOnLegacyVariant {
		from,
		to,
	});
}
...
impl VisitMut for MediaFeatureResolutionReplacerOnLegacyVariant<'_> {
	fn visit_mut_media_feature_plain(&mut self, n:&mut MediaFeaturePlain) {
		...
		if let MediaFeatureValue::Dimension(Dimension::Resolution(
			Resolution {
				value: resolution_value, unit: resolution_unit, ..
			},
		)) = &*n.value
		{
			...
			if feature_name_value == self.from {
				...
				let left = match &*resolution_unit.value {
					"dpi" => {
						(resolution_value.value / 96.0 * 100.0).round() / 100.0
					},
					"dpcm" => {
						(((resolution_value.value * 2.54) / 96.0) * 100.0)
							.round() / 100.0
					},
...
fn replace_media_feature_resolution_on_legacy_variant<N>(
	node:&mut N,
	from:&str,
	to:&str,
) where
	N: for<'aa> VisitMutWith<
		MediaFeatureResolutionReplacerOnLegacyVariant<'aa>,
	>, {
	node.visit_mut_with(&mut MediaFeatureResolutionReplacerOnLegacyVariant {
		from,
		to,
	});
}
		...
		if let CalcValue::Function(function) = n {
			if matches_eq!(function.name, "calc", "-webkit-calc", "-moz-calc") {
				let calc_sum = match function.value.first() {
					Some(ComponentValue::CalcSum(calc_sum)) => {
						*calc_sum.clone()
					},
					_ => return,
				};
	...
	fn visit_mut_function(&mut self, n:&mut Function) {
		...
		if let Some(ComponentValue::Ident(ident)) = n.value.first() {
			...
			let new_value = match &*new_value {
				"woff" | "truetype" | "opentype" | "woff2"
				| "embedded-opentype" | "collection" | "svg" => new_value,
				_ => {
					return;
				},
			};
			...
			*n = Function {
				...
				value:vec![
					...
					ComponentValue::CalcSum(Box::new(CalcSum {
						span:DUMMY_SP,
						expressions:vec![CalcProductOrOperator::Product(
							CalcProduct {
								span:DUMMY_SP,
								expressions:vec![CalcValueOrOperator::Value(
									CalcValue::Function(min_function),
								)],
							},
						)],
					})),
...
impl Prefixer {
	...
	fn add_declaration3<'a>(
		...
		if should_prefix(property, self.env, true) {
			...
			// don't use `-moz` prefix for properties in `@-webkit-keyframes`
			// at-rule
...
impl VisitMut for Prefixer {
	fn visit_mut_stylesheet(&mut self, stylesheet:&mut Stylesheet) {
		...
		for mut rule in take(&mut stylesheet.rules) {
			...
			for mut added_rule in take(&mut self.added_top_rules) {
				let need_skip = new_rules.iter().any(|existing_rule| {
					added_rule.1.eq_ignore_span(existing_rule)
				});
	...
	fn visit_mut_import_conditions(
		&mut self,
		import_conditions:&mut ImportConditions,
	) {
		...
		if !self.added_declarations.is_empty() {
			if let Some(supports) = import_conditions.supports.take() {
				...
				for n in take(&mut self.added_declarations) {
					let supports_condition_type =
						SupportsConditionType::Or(SupportsOr {
							span:DUMMY_SP,
							keyword:None,
				...
				import_conditions.supports = Some(Box::new(Function {
					...
					value:vec![ComponentValue::SupportsCondition(Box::new(
						SupportsCondition { span:DUMMY_SP, conditions },
					))],
				}));
			}
		}
	...
	fn visit_mut_supports_condition(
		&mut self,
		supports_condition:&mut SupportsCondition,
	) {
	...
	fn visit_mut_supports_in_parens(
		&mut self,
		supports_in_parens:&mut SupportsInParens,
	) {
		...
		if let Some(supports_condition) = &self.supports_condition {
			match supports_in_parens {
					...
					if !self.added_declarations.is_empty() =>
				{
					...
					conditions.push(
						swc_css_ast::SupportsConditionType::SupportsInParens(
					...
					for n in take(&mut self.added_declarations) {
						...
						let need_skip = supports_condition
							.conditions
							.iter()
							.any(|existing_condition_type| {
								supports_condition_type
									.eq_ignore_span(existing_condition_type)
							});
					...
					if conditions.len() > 1 {
						*supports_in_parens =
							SupportsInParens::SupportsCondition(
								SupportsCondition { span:DUMMY_SP, conditions },
							);
					}
				},
	...
	fn visit_mut_media_query_list(
		&mut self,
		media_query_list:&mut MediaQueryList,
	) {
		...
		for n in &media_query_list.queries {
			if should_prefix("-webkit-min-device-pixel-ratio", self.env, false)
			{
				...
				let need_skip = media_query_list.queries.iter().any(
	...
	fn visit_mut_qualified_rule(&mut self, n:&mut QualifiedRule) {
		...
		if self.rule_prefix == Some(Prefix::Webkit)
		...
		if self.rule_prefix == Some(Prefix::Moz) || self.rule_prefix.is_none() {
			...
			if should_prefix(":-moz-placeholder", self.env, false) {
				...
				if !new_moz_prelude_with_previous
					.eq_ignore_span(&new_moz_prelude)
				{
					let qualified_rule = Box::new(QualifiedRule {
						span:DUMMY_SP,
			...
			if !n.prelude.eq_ignore_span(&new_moz_prelude) {
				...
				if self.simple_block.is_none() {
					self.added_top_rules.push((
						Prefix::Moz,
						Rule::QualifiedRule(Box::new(qualified_rule)),
					));
				} else {
					self.added_qualified_rules
	...
	fn visit_mut_declaration(&mut self, n:&mut Declaration) {
		...
		match &**name {
			...
			"animation" => {
				let need_prefix = n.value.iter().all(|n| {
					match n {
			...
			"cursor" => {
				...
				if self.rule_prefix == Some(Prefix::Moz)
					|| self.rule_prefix.is_none()
				{
				...
				if n.value.len() == 1 {
						...
						|| self.rule_prefix.is_none()
					{
						...
						if !n.value.eq_ignore_span(&old_spec_webkit_value) {
							self.added_declarations.push(Box::new(
								Declaration {
									span:n.span,
									name:n.name.clone(),
					...
					if self.rule_prefix == Some(Prefix::Ms)
						|| self.rule_prefix.is_none()
					{
						...
						Box::new(|| {
							...
							if let Some(span) = value
								...
								.and_then(|component_value| {
									component_value.as_integer()
								})
								.filter(|integer| integer.value == 0)
								.map(|integer| integer.span)
							{
								value[2] = ComponentValue::Dimension(Box::new(
			...
			"flex-direction" => {
				let old_values = match n.value.first() {
					...
					Some(ComponentValue::Ident(ident))
						if ident
							.value
							.eq_ignore_ascii_case("column-reverse") =>
					{
			...
			"flex-flow" => {
				...
				let old_values = match is_single_flex_wrap {
					true => None,
					_ => {
						let get_old_values = |index:usize| {
							match n.value.get(index) {
								Some(ComponentValue::Ident(ident))
									if ident
										.value
			...
			"justify-content" => {
				...
				if need_old_spec {
					add_declaration!(
						...
						"-webkit-box-pack",
						Box::new(|| {
							let mut old_spec_webkit_new_value =
								webkit_value.clone();
			...
			"image-rendering" => {
				...
				{
					if should_prefix(
						"-webkit-optimize-contrast:fallback",
						self.env,
						false,
					) {
			...
			"transform" => {
				...
				let has_3d_function = n.value.iter().any(|n| {
					match n {
			...
			"writing-mode" if n.value.len() == 1 => {
				if let Some(simple_block) = &self.simple_block {
					let direction = match simple_block.value.iter().rev().find(
						|declaration| {
							declaration
								.as_declaration()
								.and_then(|declaration| {
									declaration.name.as_ident()
								})
								.filter(|ident| {
									ident
										.value
			...
			"overflow"
				if should_prefix("overflow", self.env, false)
					&& n.value.len() == 2 =>
			{
				if let (
			...
			"break-inside" => {
				if let ComponentValue::Ident(ident) = &n.value[0] {
					match &*ident.value.to_ascii_lowercase() {
						"auto" | "avoid" | "initial" | "inherit" | "revert"
						| "revert-layer" => {
							add_declaration!(
								Prefix::Webkit,
			...
			"src"
				if should_prefix("font-face-format-ident", self.env, false) =>
			{
			...
			"place-content"
				if should_prefix("place-content", self.env, false) =>
			{
