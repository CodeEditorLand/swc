...
impl Deriver {
	...
	fn make_arm_from_fields(&self, pat_path:Path, fields:&Fields) -> Arm {
		...
		for (i, field) in fields.iter().enumerate().filter(|(_, f)| !(self.ignore_field)(f)) {
			...
			} else if field.attrs.iter().any(|attr| attr.path().is_ident("use_eq_ignore_span")) {
			...
			let make_pat_field = |ident:&Ident| {
				FieldPat {
					attrs:Default::default(),
					member:match &field.ident {
						Some(v) => Member::Named(v.clone()),
						None => Member::Unnamed(Index { index:i as _, span:field.ty.span() }),
					},
		...
		Arm {
			...
			body:Box::new(Expr::Block(ExprBlock {
				...
				block:Block { brace_token:Default::default(), stmts:vec![Stmt::Expr(expr, None)] },
			})),
