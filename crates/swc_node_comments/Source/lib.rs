...
impl Comments for SwcComments {
	...
	fn has_leading(&self, pos:BytePos) -> bool {
		if let Some(v) = self.leading.get(&pos) { !v.is_empty() } else { false }
	}
	...
	fn has_trailing(&self, pos:BytePos) -> bool {
		if let Some(v) = self.trailing.get(&pos) { !v.is_empty() } else { false }
	}
	...
	where
		...
		let ret = if let Some(cmts) = self.leading.get(&pos) { f(&cmts) } else { f(&[]) };
	...
	where
		Self: Sized,
		F: FnOnce(&[Comment]) -> Ret, {
		let ret = if let Some(cmts) = &self.trailing.get(&pos) { f(cmts) } else { f(&[]) };
