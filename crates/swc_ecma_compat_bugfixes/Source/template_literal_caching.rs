...
pub fn template_literal_caching() -> impl Fold { TemplateLiteralCaching::default() }
...
mod tests {
	...
	fn tr() -> impl Fold {
		chain!(resolver(Mark::new(), Mark::new(), false), template_literal_caching())
	}

	test!(::swc_ecma_parser::Syntax::default(), |_| tr(), single_tag, "t`a`;");

	test!(::swc_ecma_parser::Syntax::default(), |_| tr(), single_tag_empty, "x``;");
	...
	test!(::swc_ecma_parser::Syntax::default(), |_| tr(), dynamic_tag, "fn()``;");
