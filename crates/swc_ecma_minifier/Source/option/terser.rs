...
use swc_common::{DUMMY_SP, FileName, SourceMap, collections::AHashMap, sync::Lrc};
use swc_ecma_ast::*;
use swc_ecma_parser::parse_file_as_expr;
use swc_ecma_utils::drop_span;

use super::{CompressOptions, TopLevelOptions, default_passes, true_by_default};
...
impl From<TerserTopRetainOption> for Vec<JsWord> {
	fn from(v:TerserTopRetainOption) -> Self {
		match v {
			TerserTopRetainOption::Str(s) => {
				s.split(',').filter(|s| s.trim() != "").map(|v| v.into()).collect()
			},
...
fn value_to_expr(v:Value) -> Box<Expr> {
	match v {
		...
		Value::Bool(value) => Lit::Bool(Bool { span:DUMMY_SP, value }).into(),
		Value::Number(v) => {
			trace_op!("Creating a numeric literal from value");

			Lit::Num(Number { span:DUMMY_SP, value:v.as_f64().unwrap(), raw:None }).into()
		},

		Value::String(v) => {
			let value:JsWord = v.into();

			Lit::Str(Str { span:DUMMY_SP, raw:None, value }).into()
		},
		...
		Value::Array(arr) => {
			...
			ArrayLit { span:DUMMY_SP, elems }.into()
		},
		...
		Value::Object(obj) => {
			let props = obj
				...
				.map(|(key, value)| {
					KeyValueProp {
						key:PropName::Str(Str { span:DUMMY_SP, raw:None, value:key.into() }),
						value,
					}
				})
				.map(Prop::KeyValue)
			...
			ObjectLit { span:DUMMY_SP, props }.into()
		},
	}
}
