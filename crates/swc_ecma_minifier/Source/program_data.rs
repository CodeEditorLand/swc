...
impl VarUsageInfo {
	pub(crate) fn is_infected(&self) -> bool { !self.infects_to.is_empty() }
...
impl Storage for ProgramData {
	...
	fn top_scope(&mut self) -> &mut Self::ScopeData { &mut self.top }

	fn var_or_default(&mut self, id:Id) -> &mut Self::VarData { self.vars.entry(id).or_default() }
	...
	fn report_usage(&mut self, ctx:Ctx, i:Id) {
		...
		let e = self
			.vars
			.entry(i.clone())
			.or_insert_with(|| VarUsageInfo { used_above_decl:true, ..Default::default() });
	...
	fn get_initialized_cnt(&self) -> usize { self.initialized_vars.len() }

	fn truncate_initialized_cnt(&mut self, len:usize) { self.initialized_vars.truncate(len) }
...
impl ScopeDataLike for ScopeData {
	...
	fn mark_used_arguments(&mut self) { self.used_arguments = true; }

	fn mark_eval_called(&mut self) { self.has_eval_call = true; }

	fn mark_with_stmt(&mut self) { self.has_with_stmt = true; }
}

impl VarDataLike for VarUsageInfo {
	fn mark_declared_as_fn_param(&mut self) { self.declared_as_fn_param = true; }

	fn mark_declared_as_fn_decl(&mut self) { self.declared_as_fn_decl = true; }

	fn mark_declared_as_fn_expr(&mut self) { self.declared_as_fn_expr = true; }

	fn mark_declared_as_for_init(&mut self) { self.declared_as_for_init = true; }

	fn mark_has_property_access(&mut self) { self.has_property_access = true; }

	fn mark_used_as_callee(&mut self) { self.callee_count += 1; }
	...
	fn mark_indexed_with_dynamic_key(&mut self) { self.indexed_with_dynamic_key = true; }

	fn add_accessed_property(&mut self, name:swc_atoms::JsWord) {
		*self.accessed_props.entry(name).or_default() += 1;
	}

	fn mark_used_as_ref(&mut self) { self.used_as_ref = true; }

	fn add_infects_to(&mut self, other:Access) { self.infects_to.push(other); }

	fn prevent_inline(&mut self) { self.inline_prevented = true; }

	fn mark_as_exported(&mut self) { self.exported = true; }

	fn mark_initialized_with_safe_value(&mut self) { self.no_side_effect_for_member_access = true; }

	fn mark_as_pure_fn(&mut self) { self.pure_fn = true; }

	fn mark_used_above_decl(&mut self) { self.used_above_decl = true; }

	fn mark_used_recursively(&mut self) { self.used_recursively = true; }
}
