...
where
	...
	let mut v = VarAnalyzer { all:&mut data, cur:Default::default() };
...
impl<'a> Scope<'a> {
	pub fn new(parent:Option<&'a Scope<'a>>) -> Self { Scope { parent, data:Default::default() } }
...
macro_rules! scoped {
	($v:expr, $n:expr) => {
		let data = {
			...
			let mut v = VarAnalyzer { all:&mut $v.all, cur:child };
