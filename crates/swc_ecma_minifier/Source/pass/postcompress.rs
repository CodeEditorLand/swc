...
use crate::{LIGHT_TASK_PARALLELS, maybe_par, option::CompressOptions};

pub fn postcompress_optimizer(options:&CompressOptions) -> impl '_ + VisitMut {
	PostcompressOptimizer { options, ctx:Default::default() }
}
...
impl Parallel for PostcompressOptimizer<'_> {
	fn create(&self) -> Self { Self { options:self.options, ctx:self.ctx } }
			...
			match &mut *e.expr {
				Expr::Fn(f) => {
					if f.ident.is_some() {
						if self.options.top_level() {
							*m = ExportDefaultDecl { span:e.span, decl:DefaultDecl::Fn(f.take()) }
								.into()
						}
					} else {
						*m =
							ExportDefaultDecl { span:e.span, decl:DefaultDecl::Fn(f.take()) }.into()
					}
				},
				...
				Expr::Class(c) => {
					if c.ident.is_some() {
						if self.options.top_level() {
							*m =
								ExportDefaultDecl { span:e.span, decl:DefaultDecl::Class(c.take()) }
									.into()
						}
					} else {
						*m = ExportDefaultDecl { span:e.span, decl:DefaultDecl::Class(c.take()) }
							.into()
					}
				},
