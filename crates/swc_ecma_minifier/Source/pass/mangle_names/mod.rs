...
pub(crate) fn mangle_names(
	...
	program.visit_mut_with(&mut renamer(
		...
		ManglingRenamer { chars, preserved, cache, mangle_name_cache },
	));
}
...
impl Renamer for ManglingRenamer {
	...
	fn preserved_ids_for_module(&mut self, _:&Module) -> FxHashSet<Id> { self.preserved.clone() }

	fn preserved_ids_for_script(&mut self, _:&Script) -> FxHashSet<Id> { self.preserved.clone() }

	fn new_name_for(&self, _:&Id, n:&mut usize) -> Atom { self.chars.encode(n, true) }

	fn get_cached(&self) -> Option<Cow<RenameMap>> { self.cache.as_ref().map(Cow::Borrowed) }
