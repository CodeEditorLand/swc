...
impl<'b> Deref for WithCtx<'_, 'b> {
	...
	fn deref(&self) -> &Self::Target { self.pass }
}

impl DerefMut for WithCtx<'_, '_> {
	fn deref_mut(&mut self) -> &mut Self::Target { self.pass }
}

impl Drop for WithCtx<'_, '_> {
	fn drop(&mut self) { self.pass.ctx = self.orig_ctx; }
}
