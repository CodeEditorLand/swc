...
use swc_ecma_ast::*;
use swc_ecma_utils::{ExprExt, IsEmpty, Value, number::ToJsString};
			...
			MemberProp::Computed(p) => {
				if let Expr::Lit(Lit::Str(s)) = &*p.expr {
					if let Ok(value) = s.value.parse::<u32>() {
						p.expr =
							Lit::Num(Number { span:s.span, value:value as f64, raw:None }).into();
					}
				}
			},
		}
	...
	pub(super) fn eval_number_method_call(&mut self, e:&mut Expr) {
		...
		if &*method.sym == "toPrecision" {
			...
			if let Some(precision) =
				args.first().and_then(|arg| eval_as_number(&self.expr_ctx, &arg.expr))
			{
