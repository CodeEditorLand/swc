...
impl Pure<'_> {
							...
							match &mut stmt.init {
									...
									.unwrap_or(true) =>
								{
									stmt.init = cur
										.take()
										.and_then(
											|v| if v.decls.is_empty() { None } else { Some(v) },
										)
	...
	where
		...
		let vars = {
			let mut v =
				VarMover { target, vars:Default::default(), var_decl_kind:Default::default() };
