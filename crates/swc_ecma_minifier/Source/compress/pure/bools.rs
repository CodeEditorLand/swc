...
impl Pure<'_> {
	...
	pub(super) fn optimize_bools(&mut self, e:&mut Expr) {
		...
		if let Expr::Unary(UnaryExpr { op: op!("!"), arg, .. }) = e {
			match &mut **arg {
				...
				Expr::Unary(UnaryExpr { op: op!("!"), arg: arg_of_arg, .. }) => {
					if let Expr::Bin(BinExpr { op: op!("||"), left, right, .. }) = &mut **arg_of_arg
					{
						if negate_cost(&self.expr_ctx, left, false, false) > 0
							|| negate_cost(&self.expr_ctx, right, false, false) > 0
						{
	...
	pub(super) fn optimize_expr_in_bool_ctx(&mut self, n:&mut Expr, is_ignore:bool) {
		...
		match n {
			Expr::Unary(UnaryExpr { span, op: op!("!"), arg }) => {
				match &mut **arg {
			...
			Expr::Unary(UnaryExpr { span, op: op!("typeof"), arg }) => {
				...
				match &**arg {
					Expr::Ident(..) => {
						*n = Lit::Num(Number { span:*span, value:1.0, raw:None }).into()
					},
	...
	fn can_swap_bin_operands(&mut self, l:&Expr, r:&Expr, is_for_rel:bool) -> bool {
		match (l, r) {
			...
			| (
				...
				Expr::Unary(UnaryExpr { op: op!("void") | op!("!"), .. }),
			)
