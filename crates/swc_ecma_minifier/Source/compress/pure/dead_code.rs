...
impl Pure<'_> {
	...
	pub(super) fn drop_instant_break(&mut self, s:&mut Stmt) {
		if let Stmt::Labeled(ls) = s {
			if let Stmt::Break(BreakStmt { label: Some(label), .. }) = &*ls.body {
	...
	where
		...
		let idx = stmts.iter().position(|stmt| {
			match stmt.as_stmt() {
				Some(s) => s.terminates(),
				_ => false,
			}
	...
	pub(super) fn drop_unused_stmt_at_end_of_fn(&mut self, s:&mut Stmt) {
		if let Stmt::Return(r) = s {
			if let Some(Expr::Unary(UnaryExpr { span, op: op!("void"), arg })) =
				r.arg.as_deref_mut()
			{
					...
					match stmt {
						Stmt::If(mut s) => {
							if let Value::Known(v) = s.test.cast_to_bool(&self.expr_ctx).1 {
								...
								new.push(T::from(
									ExprStmt { span:DUMMY_SP, expr:s.test.take() }.into(),
								));
