...
impl Pure<'_> {
	...
	pub(super) fn eval_str_addition(&mut self, e:&mut Expr) {
		let (span, l_l, r_l, r_r) = match e {
			Expr::Bin(e @ BinExpr { op: op!(bin, "+"), .. }) => {
				match &mut *e.right {
					Expr::Bin(r @ BinExpr { op: op!(bin, "+"), .. }) => {
	...
	pub(super) fn eval_nested_tpl(&mut self, e:&mut Expr) {
		...
		if !tpl.exprs.iter().any(|e| {
			match &**e {
				Expr::Tpl(t) => {
					t.quasis.iter().all(|q| q.cooked.is_some() && !q.raw.contains('\\'))
				},
				_ => false,
			}
		}) {
		...
		let mut new_tpl =
			Tpl { span:tpl.span, quasis:Default::default(), exprs:Default::default() };
