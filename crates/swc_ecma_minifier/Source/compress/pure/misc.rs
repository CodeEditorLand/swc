...
impl Pure<'_> {
	...
	pub(super) fn eval_spread_array(&mut self, args:&mut Vec<ExprOrSpread>) {
		if args.iter().all(|arg| arg.spread.is_none() || !arg.expr.is_array()) {
			return;
		}
		...
		for arg in args.take() {
			match arg {
				ExprOrSpread { spread: Some(spread), expr } => {
					match *expr {
	...
	pub(super) fn compress_array_join(&mut self, e:&mut Expr) {
		...
		if cannot_join_as_str_lit {
			...
			let mut res = Lit::Str(Str { span:DUMMY_SP, raw:None, value:js_word!("") }).into();
				...
				match &**expr {
					Expr::Lit(Lit::Num(num)) => {
						if num.value <= 5_f64 && num.value >= 0_f64 {
							Some(
								ArrayLit { span:*span, elems:vec![None; num.value as usize] }
									.into(),
							)
						} else {
							None
					...
					Expr::Lit(_) => {
						Some(
							ArrayLit { span:*span, elems:vec![args.take().into_iter().next()] }
								.into(),
						)
					},
					_ => None,
	...
	pub(super) fn optimize_opt_chain(&mut self, e:&mut Expr) {
		...
		if let OptChainBase::Member(base) = &mut *opt.base {
			if match &*base.obj {
				...
				*e =
					MemberExpr { span:opt.span, obj:base.obj.take(), prop:base.prop.take() }.into();
			}
		}
	}
	...
	pub(super) fn optimize_builtin_object(&mut self, e:&mut Expr) {
		...
		match e {
			...
			{
				let new_expr = match &**callee {
					Expr::Ident(Ident { sym, .. }) if &**sym == "Boolean" => {
						match &mut args[..] {
							...
							[ExprOrSpread { spread: None, expr }] => {
								Some(
									UnaryExpr {
										span:*span,
										op:op!("!"),
										arg:UnaryExpr { span:*span, op:op!("!"), arg:expr.take() }
											.into(),
									}
									.into(),
								)
					...
					Expr::Ident(Ident { sym, .. }) if &**sym == "Number" => {
						match &mut args[..] {
							...
							[ExprOrSpread { spread: None, expr }] if self.options.unsafe_math => {
								Some(
									UnaryExpr { span:*span, op:op!(unary, "+"), arg:expr.take() }
										.into(),
								)
							},
							_ => None,
	...
	fn make_ignored_expr(
		...
		let mut exprs = exprs
			.filter_map(|mut e| {
				self.ignore_return_value(
					&mut e,
					DropOpts { drop_global_refs_if_unused:true, drop_str_lit:true, drop_zero:true },
				);
	...
	pub(super) fn ignore_return_value(&mut self, e:&mut Expr, opts:DropOpts) {
		...
		match e {
			...
			Expr::TaggedTpl(TaggedTpl { span, ctxt, tpl, .. })
			...
			Expr::New(NewExpr { span, ctxt, args, .. }) if ctxt.has_mark(self.marks.pure) => {
				report_change!("ignore_return_value: Dropping a pure call");
		...
		match e {
			...
			Expr::Call(CallExpr { callee: Callee::Expr(callee), .. }) if callee.is_fn_expr() => {
				match &mut **callee {
					Expr::Fn(callee) => {
