...
use swc_common::{DUMMY_SP, Mark, SyntaxContext, collections::AHashSet, util::take::Take};
use swc_ecma_ast::*;
use swc_ecma_transforms_base::perf::{Parallel, ParallelExt};
use swc_ecma_utils::{ExprCtx, ExprExt, Remapper, collect_decls};
...
impl<'b> Optimizer<'b> {
	...
	pub(super) fn try_remove_label(&mut self, s:&mut LabeledStmt) {
		...
		let mut analyer = LabelAnalyzer { label:s.label.sym.clone(), ..Default::default() };
...
impl<'b> Deref for WithCtx<'_, 'b> {
	...
	fn deref(&self) -> &Self::Target { self.reducer }
}

impl DerefMut for WithCtx<'_, '_> {
	fn deref_mut(&mut self) -> &mut Self::Target { self.reducer }
}

impl Drop for WithCtx<'_, '_> {
	fn drop(&mut self) { self.reducer.ctx = self.orig_ctx.clone(); }
}
...
pub(crate) fn extract_class_side_effect(expr_ctx:&ExprCtx, c:Class) -> Vec<Box<Expr>> {
	...
	for m in c.body {
		match m {
			ClassMember::Method(ClassMethod { key: PropName::Computed(key), .. }) => {
			...
			ClassMember::PrivateProp(PrivateProp { value: Some(v), is_static: true, .. }) => {
...
pub(crate) fn is_valid_for_lhs(e:&Expr) -> bool { !matches!(e, Expr::Lit(..) | Expr::Unary(..)) }
	...
	fn var(&mut self, i:&Id, mode:FinalizerMode) -> Option<Box<Expr>> {
		...
		match &*e {
			Expr::Ident(Ident { sym, .. }) if &**sym == "eval" => {
				Some(SeqExpr { span:DUMMY_SP, exprs:vec![0.into(), e] }.into())
			},
			_ => Some(e),
		}
	}
...
impl<'a> NormalMultiReplacer<'a> {
	...
	pub fn new(vars:&'a mut FxHashMap<Id, Box<Expr>>) -> Self {
		NormalMultiReplacer { vars, changed:false }
	}
	...
	fn visit_mut_prop(&mut self, p:&mut Prop) {
		...
		if let Prop::Shorthand(i) = p {
			if self.from.0 == i.sym && self.from.1 == i.ctxt {
				...
				*p = Prop::KeyValue(KeyValueProp { key:PropName::Ident(i.clone().into()), value });
			}
		}
	}
}
...
impl SynthesizedStmts {
	pub fn take_stmts(&mut self) -> Vec<Stmt> { take(&mut self.0) }
}
...
impl SynthesizedStmts {
	pub fn push(&mut self, stmt:Stmt) { self.0.push(stmt); }

	pub fn extend(&mut self, stmts:impl IntoIterator<Item = Stmt>) { self.0.extend(stmts); }

	pub fn append(&mut self, other:&mut SynthesizedStmts) { self.0.append(&mut other.0); }
}
...
impl VisitMut for LabelAnalyzer {
	...
	fn visit_mut_for_stmt(&mut self, n:&mut ForStmt) { self.visit_mut_loop(n) }

	fn visit_mut_for_in_stmt(&mut self, n:&mut ForInStmt) { self.visit_mut_loop(n) }

	fn visit_mut_for_of_stmt(&mut self, n:&mut ForOfStmt) { self.visit_mut_loop(n) }

	fn visit_mut_while_stmt(&mut self, n:&mut WhileStmt) { self.visit_mut_loop(n) }

	fn visit_mut_do_while_stmt(&mut self, n:&mut DoWhileStmt) { self.visit_mut_loop(n) }
