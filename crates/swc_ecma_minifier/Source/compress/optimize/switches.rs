...
impl Optimizer<'_> {
	...
	pub(super) fn optimize_switch_cases(&mut self, cases:&mut Vec<SwitchCase>) {
		...
		if let Some(default) = default {
			let end =
				cases.iter().skip(default).position(|case| !case.cons.is_empty()).unwrap_or(0)
					+ default;
	...
	fn merge_cases_with_same_cons(&mut self, cases:&mut Vec<SwitchCase>) {
		...
		while i < len {
			...
			for j in (i + 1)..len {
				...
				let case_i_terminates =
					cases[i].cons.last().map(|s| s.terminates()).unwrap_or(false);
			...
			match &mut *sw.cases {
				...
				[case] => {
					...
					if let Some(test) = case.test {
						let test =
							BinExpr { left:discriminant, right:test, op:op!("==="), span:DUMMY_SP }
								.into();
				...
				[first, second] if first.test.is_none() || second.test.is_none() => {
					...
					if terminate {
						...
						let (def, case) =
							if first.test.is_none() { (first, second) } else { (second, first) };
						*s = IfStmt {
							span:sw.span,
							test:BinExpr {
								span:DUMMY_SP,
