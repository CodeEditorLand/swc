...
impl Optimizer<'_> {
	...
	pub(crate) fn should_preserve_property_access(&self, e:&Expr, opts:PropertyAccessOpts) -> bool {
		...
		match e {
			...
			Expr::Object(o) => {
				...
				return o.props.iter().any(|p| {
					match p {
						PropOrSpread::Spread(p) => {
							self.should_preserve_property_access(&p.expr, opts)
						},
						PropOrSpread::Prop(p) => {
							match &**p {
								Prop::Assign(_) => true,
			...
			Pat::Array(arr) => {
				for (idx, arr_elem) in arr.elems.iter_mut().enumerate() {
					if let Some(p) = arr_elem {
						let elem =
							init.as_mut().and_then(|expr| self.access_numeric_property(expr, idx));
	...
	pub(super) fn drop_unused_decl(&mut self, decl:&mut Decl) {
		...
		match decl {
			Decl::Class(ClassDecl { ident, class, .. }) => {
				...
				let may_have_side_effect = class.body.iter().any(|m| {
					match m {
		...
		if let AssignTarget::Simple(SimpleAssignTarget::Ident(i)) = &mut assign.left {
			...
			if let Some(var) = self.data.vars.get(&i.to_id()) {
				...
				if !var.inline_prevented
					&& !var.exported
					&& var.usage_count == 0
					&& var.declared && (!var.declared_as_fn_param
					|| !used_arguments
					|| self.ctx.expr_ctx.in_strict)
				{
					...
					if self.ctx.is_this_aware_callee {
						*e = SeqExpr { span:DUMMY_SP, exprs:vec![0.into(), assign.right.take()] }
							.into()
					} else {
						*e = *assign.right.take();
					}
	...
	pub(super) fn drop_unused_properties(&mut self, v:&mut VarDeclarator) -> Option<()> {
		...
		let should_preserve = |key:&PropName| {
			match key {
...
fn can_remove_property(sym:&str) -> bool { !matches!(sym, "toString" | "valueOf") }
