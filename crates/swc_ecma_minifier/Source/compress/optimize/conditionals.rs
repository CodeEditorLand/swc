...
impl Optimizer<'_> {
	...
	where
		...
		let has_work = stmts.windows(2).any(|stmts| {
	...
	fn compress_similar_cons_alt(
		...
		if cons.eq_ignore_span(alt) && !matches!(&*cons, Expr::Yield(..) | Expr::Fn(..)) {
			...
			return Some(
				SeqExpr { span:DUMMY_SP, exprs:vec![test.take(), Box::new(cons.take())] }.into(),
			);
		}
		...
		match (cons, alt) {
			...
			(Expr::New(cons), Expr::New(alt)) => {
				...
				if cons.callee.eq_ignore_span(&alt.callee)
					...
					&& (cons.args.is_some()
						&& cons.args.as_ref().unwrap().iter().all(|arg| arg.spread.is_none()))
					&& (alt.args.is_some()
						&& alt.args.as_ref().unwrap().iter().all(|arg| arg.spread.is_none()))
				{
		...
		stmts.take().into_iter().for_each(|stmt| {
			match stmt.try_into_stmt() {
				Ok(stmt) => {
					match stmt {
						Stmt::If(IfStmt {
							span, mut test, mut cons, alt: Some(mut alt), ..
						}) if cons.terminates() => {
							if let (
