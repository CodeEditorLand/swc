	...
	fn ignore_return_value(&mut self, e:&mut Expr) -> Option<Expr> {
		...
		match e {
			...
			Expr::Cond(cond) => {
				...
				let cons = self.with_ctx(ctx).ignore_return_value(&mut cond.cons).map(Box::new);
				let alt = self.with_ctx(ctx).ignore_return_value(&mut cond.alt).map(Box::new);
			...
			Expr::Seq(seq) => {
				...
				let mut exprs = seq
					...
					.filter_map(|(idx, expr)| {
						...
						let ctx = Ctx { dont_use_negated_iife:idx != 0, ..self.ctx };
...
impl VisitMut for Optimizer<'_> {
	...
	fn visit_mut_arrow_expr(&mut self, n:&mut ArrowExpr) {
		...
		{
			let ctx = Ctx { in_param:true, ..self.ctx };
	...
	fn visit_mut_assign_expr(&mut self, e:&mut AssignExpr) {
		{
			let ctx = Ctx { is_lhs_of_assign:true, is_exact_lhs_of_assign:true, ..self.ctx };
	...
	fn visit_mut_bin_expr(&mut self, n:&mut BinExpr) {
		{
			let ctx = Ctx { in_cond:self.ctx.in_cond || n.op.may_short_circuit(), ..self.ctx };
	...
	fn visit_mut_block_stmt(&mut self, n:&mut BlockStmt) {
		let ctx = Ctx { top_level:false, in_block:true, scope:n.ctxt, in_param:false, ..self.ctx };
		n.visit_mut_children_with(&mut *self.with_ctx(ctx));
	}
	...
	fn visit_mut_class(&mut self, n:&mut Class) {
		...
		{
			let ctx = Ctx { dont_invoke_iife:true, is_update_arg:false, ..self.ctx };
			n.super_class.visit_mut_with(&mut *self.with_ctx(ctx));
		}

		{
			let ctx = Ctx { in_strict:true, is_update_arg:false, ..self.ctx };
	...
	fn visit_mut_do_while_stmt(&mut self, n:&mut DoWhileStmt) {
		{
			let ctx = Ctx { executed_multiple_time:true, ..self.ctx };
			n.visit_mut_children_with(&mut *self.with_ctx(ctx));
		}
	}
	...
	fn visit_mut_export_decl(&mut self, n:&mut ExportDecl) {
		...
		let ctx = Ctx { is_exported:true, ..self.ctx };
		n.visit_mut_children_with(&mut *self.with_ctx(ctx));
	}
	...
	fn visit_mut_expr(&mut self, e:&mut Expr) {
		...
		let ctx = Ctx { is_exported:false, is_callee:false, ..self.ctx };
	...
	fn visit_mut_function(&mut self, n:&mut Function) {
		...
		{
			let ctx = Ctx { in_fn_like:true, scope:n.ctxt, top_level:false, ..self.ctx };
	...
	fn visit_mut_if_stmt(&mut self, n:&mut IfStmt) {
		...
		let ctx = Ctx { in_cond:true, ..self.ctx };
	...
	fn visit_mut_module_items(&mut self, stmts:&mut Vec<ModuleItem>) {
		let ctx = Ctx { top_level:true, ..self.ctx };
	...
	fn visit_mut_new_expr(&mut self, n:&mut NewExpr) {
		...
		{
			let ctx = Ctx { is_exact_lhs_of_assign:false, is_lhs_of_assign:false, ..self.ctx };
	...
	fn visit_mut_param(&mut self, n:&mut Param) {
		let ctx = Ctx { var_kind:None, in_param:true, ..self.ctx };
	...
	fn visit_mut_seq_expr(&mut self, n:&mut SeqExpr) {
		...
		let ctx = Ctx { dont_use_negated_iife:true, ..self.ctx };
	...
	fn visit_mut_tagged_tpl(&mut self, n:&mut TaggedTpl) {
		n.tag
			.visit_mut_with(&mut *self.with_ctx(Ctx { is_this_aware_callee:true, ..self.ctx }));
	...
	fn visit_mut_tpl(&mut self, n:&mut Tpl) {
		...
		{
			let ctx = Ctx { in_tpl_expr:true, ..self.ctx };
			let mut o = self.with_ctx(ctx);
			n.visit_mut_children_with(&mut *o);
		}
	...
	fn visit_mut_try_stmt(&mut self, n:&mut TryStmt) {
		let ctx = Ctx { in_try_block:true, ..self.ctx };
	...
	fn visit_mut_unary_expr(&mut self, n:&mut UnaryExpr) {
		let ctx =
			Ctx { in_bang_arg:n.op == op!("!"), is_delete_arg:n.op == op!("delete"), ..self.ctx };
	...
	fn visit_mut_update_expr(&mut self, n:&mut UpdateExpr) {
		let ctx = Ctx { is_update_arg:true, ..self.ctx };
	...
	fn visit_mut_using_decl(&mut self, n:&mut UsingDecl) {
		let ctx = Ctx { is_update_arg:false, has_const_ann:false, var_kind:None, ..self.ctx };
	...
	fn visit_mut_with_stmt(&mut self, n:&mut WithStmt) {
		...
		{
			let ctx = Ctx { in_with_stmt:true, ..self.ctx };
			n.body.visit_mut_with(&mut *self.with_ctx(ctx));
		}
	}
