...
use crate::{
	...
	util::{
		...
		ModuleItemExt,
		contains_eval,
		contains_leaping_continue_with_label,
		make_number,
	},
};
...
impl Ctx {
	...
	pub fn in_top_level(&self) -> bool { self.top_level || !self.in_fn_like }
}
...
impl Optimizer<'_> {
	...
	fn ignore_return_value(&mut self, e:&mut Expr) -> Option<Expr> {
		...
		match e {
			...
			Expr::Unary(UnaryExpr { op: op!("void"), .. }) if !self.options.unused => {
				return Some(e.take());
			},
			...
			Expr::Bin(
				...
				return Some(
					SeqExpr { span, exprs:vec![Box::new(left.unwrap()), Box::new(right.unwrap())] }
						.into(),
				);
			},
			...
			Expr::Call(CallExpr { callee: Callee::Expr(callee), args, .. })
				if match &**callee {
					Expr::Fn(f) => {
						f.function.body.as_ref().map(|body| body.stmts.is_empty()).unwrap_or(false)
					},
					Expr::Arrow(f) => {
						match &*f.body {
				...
				if let AssignTarget::Simple(expr) = left {
					if let SimpleAssignTarget::Member(m) = expr {
						if !m.obj.may_have_side_effects(&self.ctx.expr_ctx)
							&& (m.obj.is_object()
								|| m.obj.is_fn_expr() || m.obj.is_arrow()
								|| m.obj.is_class())
						{
					...
					&& (self.options.top_level() || !self.ctx.in_top_level()) =>
			{
				if self.should_preserve_property_access(
					obj,
					PropertyAccessOpts { allow_getter:true, only_ident:true },
				) {
					return Some(e.take());
				} else {
					return None;
								...
								Prop::KeyValue(KeyValueProp { key, mut value, .. }) => {
									match key {
										...
										PropName::Computed(mut key) => {
											exprs.extend(
												self.ignore_return_value(&mut key.expr)
													.map(Box::new),
											);
										},
									...
									exprs
										.extend(self.ignore_return_value(&mut value).map(Box::new));
								},
			...
			Expr::Bin(BinExpr {
				...
				let mut seq =
					SeqExpr { span:*span, exprs:left.into_iter().chain(right).collect() }.into();
			...
			Expr::Cond(cond) => {
				...
				let cons =
					self.with_ctx(ctx.clone()).ignore_return_value(&mut cond.cons).map(Box::new);

				let alt =
					self.with_ctx(ctx.clone()).ignore_return_value(&mut cond.alt).map(Box::new);
				...
				if exprs.len() <= 1 {
					...
					if is_last_undefined {
						...
						if let Some(last) = exprs.last_mut() {
							...
							self.changed = true;
							*last =
								UnaryExpr { span:DUMMY_SP, op:op!("void"), arg:last.take() }.into();
						}
					}
					...
					&& bs.stmts.iter().all(|stmt| {
						match stmt {
							Stmt::Decl(Decl::Var(v))
								if matches!(&**v, VarDecl { kind:VarDeclKind::Var, .. }) =>
							{
								true
							},
...
impl VisitMut for Optimizer<'_> {
	...
	fn visit_mut_assign_expr(&mut self, e:&mut AssignExpr) {
		{
			let ctx =
				Ctx { is_lhs_of_assign:true, is_exact_lhs_of_assign:true, ..self.ctx.clone() };
	...
	fn visit_mut_bin_expr(&mut self, n:&mut BinExpr) {
		{
			let ctx =
				Ctx { in_cond:self.ctx.in_cond || n.op.may_short_circuit(), ..self.ctx.clone() };
		...
		if is_this_undefined {
			if let Callee::Expr(callee) = &mut e.callee {
				if let Expr::Member(..) = &mut **callee {
					...
					*callee =
						SeqExpr { span:callee.span(), exprs:vec![zero, callee.take()] }.into();
				}
			}
		}
	...
	fn visit_mut_expr_stmt(&mut self, n:&mut ExprStmt) {
		...
		if need_ignore_return_value
			...
			|| (self.options.conditionals
				&& matches!(&*n.expr, Expr::Bin(BinExpr { op:op!("||") | op!("&&"), .. })))
		{
	...
	fn visit_mut_new_expr(&mut self, n:&mut NewExpr) {
		...
		{
			let ctx =
				Ctx { is_exact_lhs_of_assign:false, is_lhs_of_assign:false, ..self.ctx.clone() };
	...
	fn visit_mut_seq_expr(&mut self, n:&mut SeqExpr) {
		let should_preserve_zero =
			n.exprs.last().map(|v| &**v).map_or(false, Expr::directness_maters);

		let ctx = Ctx { dont_use_negated_iife:true, ..self.ctx.clone() };
		...
		let exprs =
			n.exprs
				...
				.filter_map(|(last, (idx, expr))| {
					#[cfg(feature = "debug")]
					let _span = tracing::span!(tracing::Level::ERROR, "seq_expr_with_children").entered();
					...
					let can_remove = !last
						&& (idx != 0
							|| !is_injected_zero || !self.ctx.is_this_aware_callee
	...
	fn visit_mut_str(&mut self, s:&mut Str) { s.visit_mut_children_with(self); }
	...
	fn visit_mut_tagged_tpl(&mut self, n:&mut TaggedTpl) {
		n.tag.visit_mut_with(
			&mut *self.with_ctx(Ctx { is_this_aware_callee:true, ..self.ctx.clone() }),
		);
	...
	fn visit_mut_using_decl(&mut self, n:&mut UsingDecl) {
		let ctx =
			Ctx { is_update_arg:false, has_const_ann:false, var_kind:None, ..self.ctx.clone() };
