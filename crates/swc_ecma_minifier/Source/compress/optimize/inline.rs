...
use swc_common::{EqIgnoreSpan, Mark, collections::AHashSet, util::take::Take};
use swc_ecma_ast::*;
				...
				&& ref_count == 1
			{
				match init {
					Expr::Fn(FnExpr { function: f, .. })
						if matches!(
							&**f,
							Function { is_async:true, .. } | Function { is_generator:true, .. }
						) =>
					{
						return;
					},
					...
					if let Some(body) = &f.function.body {
                            ...
                            )
						{
							...
							self.vars.simple_functions.insert(
								i.to_id(),
								FnExpr { ident:None, function:f.function.clone() }.into(),
							);
		...
		match e {
			...
			Expr::Ident(i) => {
					...
					.cloned()
				{
					...
					for id in bindings {
						let new_ctxt =
							cache.entry(id.1).or_insert_with(|| id.1.apply_mark(new_mark));
...
fn is_block_stmt_of_fn_simple_enough_for_copy(b:&BlockStmt) -> bool {
	if b.stmts.len() == 1 {
		if let Stmt::Return(ret) = &b.stmts[0] {
			return ret.arg.as_deref().map(is_arrow_body_simple_enough_for_copy).unwrap_or(true);
		}
	}
