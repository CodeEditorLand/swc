...
impl Optimizer<'_> {
					...
					Pat::Rest(rest_pat) => {
						if let Pat::Ident(param_id) = &*rest_pat.arg {
							if let Some(usage) = self.data.vars.get(&param_id.to_id()) {
								if usage.reassigned
									|| usage.ref_count != 1 || !usage.has_property_access
								{
									continue;
								}
	...
	pub(super) fn invoke_iife(&mut self, e:&mut Expr) {
		...
		match callee {
			Expr::Arrow(f) => {
				...
				let param_ids =
					f.params.iter().map(|p| p.clone().ident().unwrap().id).collect::<Vec<_>>();
		...
		if !body.stmts.iter().all(|stmt| {
			...
			match stmt {
				Stmt::Decl(Decl::Var(var))
					if matches!(
						&**var,
						VarDecl { kind:VarDeclKind::Var | VarDeclKind::Let, .. }
					) =>
				{
					for decl in &var.decls {
						match &decl.name {
	...
	fn inline_fn_param(
		...
		for (idx, param) in params.iter().enumerate() {
			...
			if let Some(arg) = arg {
				...
				exprs.push(
					AssignExpr { span:DUMMY_SP, op:op!("="), left:param.clone().into(), right:arg }
						.into(),
				)
			};
	...
	fn can_be_inlined_for_iife(&self, arg:&Expr) -> bool {
		match arg {
			...
			| Expr::Unary(UnaryExpr { op: op!("!"), arg, .. }) => self.can_be_inlined_for_iife(arg),
			...
			Expr::Arrow(ArrowExpr {
				params, body, is_async: false, is_generator: false, ..
			}) if body.is_expr() => {
...
fn find_params(callee:&mut Expr) -> Option<Vec<&mut Pat>> {
	match callee {
		...
		Expr::Fn(callee) => {
			Some(callee.function.params.iter_mut().map(|param| &mut param.pat).collect())
		},
		_ => None,
	}
}
