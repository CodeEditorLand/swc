...
use swc_common::{DUMMY_SP, Spanned, pass::Either, util::take::Take};
use swc_ecma_ast::*;
use swc_ecma_usage_analyzer::{
	alias::{AccessKind, AliasConfig, collect_infects_from},
...
use crate::{
	...
	util::{ExprOptExt, ModuleItemExt, idents_used_by, idents_used_by_ignoring_nested},
};
...
impl Optimizer<'_> {
	...
	where
		...
		{
			let can_work = stmts.windows(2).any(|stmts| {
			...
			match stmt.try_into_stmt() {
				Ok(stmt) => {
					...
					match stmt {
						...
						| Stmt::For(
							mut stmt @ ForStmt { init: Some(VarDeclOrExpr::Expr(..)), .. },
						) => {
							match &mut stmt.init {
				...
				Err(item) => {
					if !exprs.is_empty() {
						new_stmts.push(T::from(
							ExprStmt { span:DUMMY_SP, expr:Expr::from_exprs(take(&mut exprs)) }
								.into(),
						))
					}
	...
	where
		...
		let need_work = stmts.iter().any(|stmt| {
			match stmt.as_stmt() {
				Some(Stmt::Expr(e)) => {
					match &*e.expr {
						Expr::Seq(seq) => {
		...
		if let Expr::Assign(AssignExpr { op: op!("="), left, right, span }) = e {
			if let (Some(id), Expr::Seq(seq)) = (left.as_ident(), &mut **right) {
				...
				if let Some(last) = seq.exprs.last_mut() {
					**last =
						AssignExpr { span:*span, op:op!("="), left:left.take(), right:last.take() }
							.into()
				}
	...
	pub(super) fn shift_void(&mut self, e:&mut SeqExpr) {
		...
		if let Expr::Unary(UnaryExpr { op: op!("void"), .. }) = &*e.exprs[e.exprs.len() - 2] {
			return;
		}
		...
		if let Some(a) = a {
			...
			let ids_used_by_a_init = match a {
				...
				Mergable::FnDecl(a) => {
					Some(collect_infects_from(
						&a.function,
						AliasConfig { marks:Some(self.marks), ignore_nested:true, need_all:true },
					))
				},
					...
					match p {
						...
						PropOrSpread::Prop(p) => {
							match &**p {
								...
								_ => {
									log_abort!(
										"handler is not implemented for this kind of property"
									);
				...
				if self.merge_sequential_expr(a, b_callee)? {
					if is_this_undefined {
						if let Expr::Member(..) = &**b_callee {
							let zero =
								Lit::Num(Number { span:DUMMY_SP, value:0.0, raw:None }).into();

							report_change!("injecting zero to preserve `this` in call");

							*b_callee =
								SeqExpr { span:b_callee.span(), exprs:vec![zero, b_callee.take()] }
									.into();
						}
					}
				...
				match a {
					Expr::Assign(AssignExpr { left, right, .. }) => {
						...
						if let Some(usage) = self.data.vars.get(&left_id.to_id()) {
							...
							if !usage.reassigned
								&& usage.usage_count == 1
								&& usage.declared && !usage.used_recursively
							{
								can_remove = true;
							}
						} else {
		...
		match b {
				...
				if let Some(b_left) = b.left.as_ident() {
					if b_left.to_id() == left_id.to_id() {
						...
						if let Some(a) = a_expr {
							b.right =
								SeqExpr { span:DUMMY_SP, exprs:vec![Box::new(a), b.right.take()] }
									.into();
						}
			...
			Expr::Assign(b) => {
				if let Some(b_left) = b.left.as_ident() {
					...
					let var_type =
						self.data.vars.get(&left_id.to_id()).and_then(|info| info.merged_var_type);
