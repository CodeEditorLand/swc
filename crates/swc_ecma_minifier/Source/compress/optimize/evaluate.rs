				...
				if self.options.unsafe_passes {
					match &*prop.sym {
						"length" => {
							...
							*e = Lit::Num(Number { span:*span, value:metadata.len as _, raw:None })
								.into();
						...
						"name" => {
							...
							*e = Lit::Str(Str { span:*span, value:obj.sym.clone(), raw:None })
								.into();
				...
				match &**obj {
					Expr::Ident(Ident { sym, .. }) if &**sym == "String" => {
						if &*prop.sym == "fromCharCode" {
							...
							if let Known(char_code) =
								args[0].expr.as_pure_number(&self.ctx.expr_ctx)
							{
				...
				if let Known(l) = l {
					if let Known(r) = r {
						...
						if l.is_nan() || r.is_nan() {
							...
							*e = Lit::Num(Number { span:bin.span, value:l.powf(r), raw:None })
								.into();
						};
					}
				}
