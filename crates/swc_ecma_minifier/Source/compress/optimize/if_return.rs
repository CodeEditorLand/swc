...
impl Optimizer<'_> {
	pub(super) fn merge_nested_if(&mut self, s:&mut IfStmt) {
		...
		if let Stmt::If(IfStmt { test, cons, alt: None, .. }) = &mut *s.cons {
			...
			s.test =
				BinExpr { span:s.test.span(), op:op!("&&"), left:s.test.take(), right:test.take() }
					.into();
	...
	fn merge_if_returns_inner(&mut self, stmts:&mut Vec<Stmt>, should_preserve_last_return:bool) {
		...
		let idx_of_not_mergable = stmts.iter().enumerate().rposition(|(idx, stmt)| {
			match stmt.as_stmt() {
...

		{
			...
			if stmts.len() >= 2 {
				match (&stmts[stmts.len() - 2].as_stmt(), &stmts[stmts.len() - 1].as_stmt()) {
					...
					(Some(Stmt::If(IfStmt { cons, alt: None, .. })), Some(Stmt::Return(..))) => {
						match &**cons {
							Stmt::Return(ReturnStmt { arg: Some(..), .. }) => {},

							_ => {
								log_abort!(
									"if_return: [x] Aborting because stmt before last is an if \
									 stmt and cons of it is not a return stmt"
								);
...

		{
			...
			let can_merge = stmts.iter().enumerate().skip(skip).all(|(idx, stmt)| {
				match stmt.as_stmt() {
