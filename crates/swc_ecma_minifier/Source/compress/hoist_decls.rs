...
use rayon::prelude::*;
use swc_common::{DUMMY_SP, collections::AHashSet, pass::Repeated, util::take::Take};
use swc_ecma_ast::*;
use swc_ecma_usage_analyzer::analyzer::UsageAnalyzer;
use swc_ecma_utils::{StmtLike, find_pat_ids};
use swc_ecma_visit::{VisitMut, VisitMutWith, VisitWith, noop_visit_mut_type};

use super::util::drop_invalid_stmts;
use crate::{
	program_data::ProgramData,
	util::{IsModuleItem, ModuleItemExt, is_hoisted_var_decl_without_init, sort::is_sorted_by},
};
...
pub(super) fn decl_hoister(config:DeclHoisterConfig, data:&ProgramData) -> Hoister {
	Hoister { config, changed:false, data }
}
...
impl VisitMut for Hoister<'_> {
	...
	fn visit_mut_stmts(&mut self, stmts:&mut Vec<Stmt>) { self.handle_stmt_likes(stmts); }
}
