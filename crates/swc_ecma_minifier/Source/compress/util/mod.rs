...
use swc_ecma_ast::*;
use swc_ecma_utils::{ExprCtx, ExprExt, IdentUsageFinder, Value, number::JsNumber};
use swc_ecma_visit::{
	Visit,
	VisitMut,
	VisitMutWith,
...
fn negate_inner(expr_ctx:&ExprCtx, e:&mut Expr, in_bool_ctx:bool, is_ret_val_ignored:bool) -> bool {
	...
	match e {
		...
		Expr::Bin(BinExpr { left, right, op: op @ op!("&&"), .. })
...
pub(crate) fn is_ok_to_negate_for_cond(e:&Expr) -> bool { !matches!(e, Expr::Update(..)) }
	...
	fn cost(
		...
		let cost = (|| {
			match e {
				...
				Expr::Cond(..)
				| Expr::Update(..)
				| Expr::Bin(BinExpr { op: op!("in") | op!("instanceof"), .. }) => 3,
...
pub(crate) fn is_pure_undefined(expr_ctx:&ExprCtx, e:&Expr) -> bool {
	match e {
		Expr::Unary(UnaryExpr { op: UnaryOp::Void, arg, .. })
			if !arg.may_have_side_effects(expr_ctx) =>
		{
			true
		},
	...
	match e {
		...
		Expr::Call(CallExpr { callee: Callee::Expr(callee), args, .. }) => {
			...
			if let Expr::Member(MemberExpr { obj, prop: MemberProp::Ident(prop), .. }) = &**callee {
				match &**obj {
					Expr::Ident(obj) if &*obj.sym == "Math" => {
						match &*prop.sym {
							"cos" => {
...
pub(super) fn is_fine_for_if_cons(s:&Stmt) -> bool {
	match s {
		Stmt::Decl(Decl::Fn(FnDecl { ident: Ident { sym, .. }, .. })) if &**sym == "undefined" => {
			false
		},
...
where
	T: ModuleItemExt, {
	stmts.retain(|s| {
		match s.as_module_decl() {
			Ok(s) => {
				match s {
					ModuleDecl::ExportDecl(ExportDecl { decl: Decl::Var(v), .. }) => {
						!v.decls.is_empty()
					},
					_ => true,
				}
