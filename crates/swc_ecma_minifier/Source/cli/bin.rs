...
use swc_common::{FilePathMapping, Mark, SourceMap, input::SourceFileInput, sync::Lrc};
...
use swc_ecma_parser::{Parser, error::Error as ParseError, lexer::Lexer};
...
fn parse_js(cm:&Lrc<SourceMap>, filename:String) -> Result<Module, ParseError> {
	let fm = cm.load_file(Path::new(&filename)).expect("Failed to load file");

	let lexer =
		Lexer::new(Default::default(), Default::default(), SourceFileInput::from(&*fm), None);
