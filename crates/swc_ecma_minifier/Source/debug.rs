...
pub use swc_ecma_transforms_optimization::{AssertValid, debug_assert_valid};
...
pub(crate) fn invoke_module(module:&Module) {
	...
	let module = Program::Module(module.clone()).apply(hygiene()).apply(fixer(None));
	...
	if should_check {
...

		{
			...
			child_stdin.write_all(code.as_bytes()).expect("failed to write");
		}
...
pub(crate) fn invoke_script(script:&Script) {
	...
	let script = Program::Script(script.clone()).apply(hygiene()).apply(fixer(None));
