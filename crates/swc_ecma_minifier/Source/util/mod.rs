...
use swc_ecma_ast::*;
use swc_ecma_utils::{ModuleItemLike, StmtLike, Value, stack_size::maybe_grow_default};
...
pub(crate) fn make_number(span:Span, value:f64) -> Expr {
	...
	Lit::Num(Number { span, value, raw:None }).into()
}
...
impl ModuleItemExt for Stmt {
	fn as_module_decl(&self) -> Result<&ModuleDecl, &Stmt> { Err(self) }

	fn from_module_item(item:ModuleItem) -> Self { item.expect_stmt() }

	fn into_module_decl(self) -> Result<ModuleDecl, Stmt> { Err(self) }
}
...
impl ModuleItemExt for ModuleItem {
	...
	fn from_module_item(item:ModuleItem) -> Self { item }
...
pub(crate) fn make_bool(span:Span, value:bool) -> Expr {
	...
	UnaryExpr {
		span,
		op:op!("!"),
		arg:Lit::Num(Number { span:DUMMY_SP, value:if value { 0.0 } else { 1.0 }, raw:None })
			.into(),
	}
	.into()
}
...
pub(crate) trait ExprOptExt: Sized {
	...
	fn force_seq(&mut self) -> &mut SeqExpr {
		...
		match expr {
			...
			_ => {
				let inner = expr.take();
				*expr = SeqExpr { span:DUMMY_SP, exprs:vec![Box::new(inner)] }.into();
	...
	fn prepend_exprs(&mut self, mut exprs:Vec<Box<Expr>>) {
		...
		match to {
			...
			_ => {
				...
				*to = SeqExpr { span:DUMMY_SP, exprs }.into();
			},
		}
	}
}
...
impl ExprOptExt for Box<Expr> {
	...
	fn as_mut(&mut self) -> &mut Expr { self }
}
...
where
	...
	let mut v = LeapFinder { target_label:Some(label), ..Default::default() };
	...
	fn visit_continue_stmt(&mut self, n:&ContinueStmt) {
		...
		if let Some(label) = &n.label {
			self.found_continue_with_label |=
				self.target_label.as_ref().map_or(false, |l| *l == label.sym);
		}
	}
...
impl IsModuleItem for Stmt {
	fn is_module_item() -> bool { false }
}

impl IsModuleItem for ModuleItem {
	fn is_module_item() -> bool { true }
}
...
impl Visit for IdentUsageCollector {
	...
	fn visit_ident(&mut self, n:&Ident) { self.ids.insert(n.to_id()); }
...
where
	...
	let mut v = CapturedIdCollector { is_nested:false, ..Default::default() };
...
where
	...
	let mut v = IdentUsageCollector { ignore_nested:false, ..Default::default() };
...
where
	N: VisitWith<IdentUsageCollector>, {
	let mut v = IdentUsageCollector { ignore_nested:true, ..Default::default() };
...
where
	...
	let mut v = EvalFinder { found:false, include_with };
