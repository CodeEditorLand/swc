...
impl Size for AssignOp {
	fn size(&self) -> usize { self.as_str().len() }
}
...
impl SizeWithCtxt for Expr {
	fn size(&self, unresolved:SyntaxContext) -> usize {
		match self {
			...
			Expr::Seq(e) => e.exprs.iter().map(|v| v.size(unresolved) + 1).sum::<usize>() - 1,
			...
			Expr::Arrow(ArrowExpr { params, body, is_async, .. }) => {
				match &**body {
					...
					BlockStmtOrExpr::Expr(e) => {
						...
						let a = if *is_async { 5 + usize::from(params.len() != 1) } else { 0 };
...
impl SizeWithCtxt for Ident {
	fn size(&self, unresolved:SyntaxContext) -> usize {
		if self.ctxt == unresolved { self.sym.len() } else { 1 }
	}
}
