...
impl Default for CharFreq {
	fn default() -> Self { CharFreq([0; 64]) }
}

struct DummySourceMap;

impl SourceMapper for DummySourceMap {
	fn lookup_char_pos(&self, _:BytePos) -> Loc { unreachable!() }

	fn span_to_lines(&self, _:Span) -> Result<FileLines, Box<SpanLinesError>> { unreachable!() }

	fn span_to_string(&self, _:Span) -> String { String::new() }

	fn span_to_filename(&self, _:Span) -> Lrc<FileName> { FileName::Anon.into() }

	fn merge_spans(&self, _:Span, _:Span) -> Option<Span> { None }

	fn call_span_if_macro(&self, sp:Span) -> Span { sp }

	fn doctest_offset_line(&self, line:usize) -> usize { line }
...
impl WriteJs for CharFreq {
	...
	#[inline(always)]
	fn write_semi(&mut self, _:Option<Span>) -> io::Result<()> { Ok(()) }

	#[inline(always)]
	fn write_space(&mut self) -> io::Result<()> { Ok(()) }
	...
	#[inline(always)]
	fn write_line(&mut self) -> io::Result<()> { Ok(()) }
...
impl CharFreq {
	...
	pub fn compute(p:&Program, preserved:&FxHashSet<Id>, unresolved_ctxt:SyntaxContext) -> Self {
		...
		// Subtract
		p.visit_with(&mut CharFreqAnalyzer { freq:&mut freq, preserved, unresolved_ctxt });
	...
	pub fn compile(self) -> Base54Chars {
		...
		Base54Chars { chars:all.try_into().unwrap() }
	}
}
