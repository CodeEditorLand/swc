...
impl<'a> Tester<'a> {
	...
	pub fn apply_transform<T:Fold>(
		...
		let module = {
			...
			let res = p.parse_module().map_err(|e| e.into_diagnostic(self.handler).emit());
...
macro_rules! test_inline {
	(ignore, $syntax:expr, $tr:expr, $test_name:ident, $input:expr, $output:expr) => {
		#[test]
		#[ignore]
		fn $test_name() { $crate::test_inline_input_output($syntax, $tr, $input, $output) }
	};
...
test_inline!(Syntax::default(), |_| noop(), test_inline_pass, "class Foo {}", "class Foo {}");
...
macro_rules! test_exec {
	(ignore, $syntax:expr, $tr:expr, $test_name:ident, $input:expr) => {
		#[test]
		#[ignore]
		fn $test_name() { $crate::exec_tr(stringify!($test_name), $syntax, $tr, $input) }
	};
