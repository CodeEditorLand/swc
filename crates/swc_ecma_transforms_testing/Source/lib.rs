...
impl Tester<'_> {
			...
			HANDLER.set(handler, || {
				HELPERS.set(&Default::default(), || {
					...
					swc_common::comments::COMMENTS
						.set(&c, || op(&mut Tester { cm, handler, comments }))
				})
			})
		});
	...
	where
		...
		let output = ::testing::Tester::new().print_errors(|cm, handler| -> Result<(), _> {
			HANDLER.set(&handler, || {
				HELPERS.set(&Default::default(), || {
					let result =
						op(&mut Tester { cm, handler:&handler, comments:Default::default() });
	...
	pub fn apply_transform<T:Pass>(
		...
		let program = self.with_parser(name, syntax, src, |parser:&mut Parser<Lexer>| {
			match is_module {
...
impl VisitMut for RegeneratorHandler {
	...
	fn visit_mut_module_item(&mut self, item:&mut ModuleItem) {
		if let ModuleItem::ModuleDecl(ModuleDecl::Import(import)) = item {
			...
			let s = import.specifiers.iter().find_map(|v| {
				match v {
...
pub fn test_inline_input_output<F, P>(
	...
	let actual_src = Tester::run_captured(|tester| {
		...
		match ::std::env::var("PRINT_HYGIENE") {
			Ok(ref s) if s == "1" => {
				let hygiene_src = tester
					.print(&actual.clone().fold_with(&mut HygieneVisualizer), &Default::default());
...
macro_rules! test_inline {
	(ignore, $syntax:expr, $tr:expr, $test_name:ident, $input:expr, $output:expr) => {
		#[test]
		#[ignore]
		fn $test_name() { $crate::test_inline_input_output($syntax, None, $tr, $input, $output) }
	};
...
where
	...
	Tester::run(|tester| {
		...
		let mut program =
			program.apply(hygiene::hygiene()).apply(fixer::fixer(Some(&tester.comments)));
...
macro_rules! compare_stdout {
	($syntax:expr, $tr:expr, $test_name:ident, $input:expr) => {
		#[test]
		fn $test_name() { $crate::compare_stdout($syntax, $tr, $input) }
	};
}
...
impl SourceMapGenConfig for SourceMapConfigImpl {
	fn file_name_to_source(&self, f:&swc_common::FileName) -> String { f.to_string() }
