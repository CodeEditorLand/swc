...
where
	I: Input,
{
	fn start_pos(&mut self) -> swc_common::BytePos { self.input.cur_pos() }

	fn last_pos(&mut self) -> swc_common::BytePos { self.input.last_pos() }
	...
	I: Input,
{
	...
	fn create_doctype_token(&mut self, name_c:Option<char>) {
		...
		self.current_doctype_token =
			Some(Doctype { name:new_name, public_id:None, system_id:None });
	}
	...
	fn append_to_doctype_token(
		...
		if let Some(ref mut token) = self.current_doctype_token {
			if let Some(name) = name {
				if let Doctype { name: Some(old_name), .. } = token {
					old_name.push(name);
				}
			}

			if let Some(public_id) = public_id {
				if let Doctype { public_id: Some(old_public_id), .. } = token {
					old_public_id.push(public_id);
				}
			}

			if let Some(system_id) = system_id {
				if let Doctype { system_id: Some(old_system_id), .. } = token {
	...
	fn update_attribute_span(&mut self) {
		if let Some(attribute_start_position) = self.attribute_start_position {
			if let Some(Tag { ref mut attributes, .. }) = self.current_tag_token {
	...
	fn emit_tag_token(&mut self, kind:Option<TagKind>) {
		if let Some(mut current_tag_token) = self.current_tag_token.take() {
			...
			let attributes = current_tag_token
				...
				.map(|attribute| {
					...
					if already_seen.contains(&name) {
						self.errors.push(Error::new(attribute.span, ErrorKind::DuplicateAttribute));
					}
			...
			match current_tag_token.kind {
				TagKind::Start => {
					let start_tag_token =
						Token::StartTag { tag_name:current_tag_token.tag_name.into(), attributes };
				...
				TagKind::End => {
					...
					let end_tag_token =
						Token::EndTag { tag_name:current_tag_token.tag_name.into(), attributes };
				...
				TagKind::Empty => {
					let empty_tag =
						Token::EmptyTag { tag_name:current_tag_token.tag_name.into(), attributes };
	...
	fn emit_comment_token(&mut self, raw_end:Option<&str>) {
		...
		self.emit_token(Token::Comment { data:comment.data.into(), raw:comment.raw.into() });
	}
	...
	fn emit_cdata_token(&mut self) {
		...
		self.emit_token(Token::Cdata { data:cdata.data.into(), raw:cdata.raw.into() });
	}
	...
	fn handle_raw_and_emit_character_token(&mut self, c:char) {
		...
		if is_cr {
			...
			self.emit_token(Token::Character { value:c, raw:Some(String::from(c).into()) });
		}
	}
	...
	fn emit_character_token(&mut self, value:(char, char)) {
		self.emit_token(Token::Character { value:value.0, raw:Some(String::from(value.1).into()) });
	}

	#[inline(always)]
	fn emit_character_token_with_entity(&mut self, c:char, raw:&str) {
		self.emit_token(Token::Character { value:c, raw:Some(raw.into()) });
	}
...
#[inline(always)]
fn is_whitespace(c:char) -> bool { matches!(c, '\x20' | '\x09' | '\x0d' | '\x0a') }
