	...
	I: ParserInput,
{
	...
	pub fn parse_document(&mut self) -> PResult<Document> {
		...
		Ok(Document { span:Span::new(start.lo(), last), children })
	}

	fn create_document(&self) -> RcNode { Node::new(Data::Document, DUMMY_SP) }
	...
	fn node_to_child(&mut self, node:RcNode) -> Child {
		...
		match node.data.clone() {
			...
			Data::Element { tag_name, attributes } => {
				...
				Child::Element(Element { span, tag_name, attributes, children:new_children })
			},
			Data::Text { data, raw } => {
			...
			Data::CdataSection { data, raw } => {
				Child::CdataSection(CdataSection { span:start_span, data, raw })
			},
			_ => {
				unreachable!();
			},
				...
				match &token_and_info.token {
					...
					Token::ProcessingInstruction { .. } => {
						let processing_instruction =
							self.create_processing_instruction(token_and_info);
	...
	fn get_current_element(&self) -> &RcNode {
		self.open_elements_stack.items.last().expect("no current element")
	}
	...
	fn create_document_type_for_token(&self, token_and_info:&mut TokenAndInfo) -> RcNode {
		let (name, public_id, system_id, raw) = match &token_and_info.token {
			Token::Doctype { name, public_id, system_id, raw } => {
				(name.clone(), public_id.clone(), system_id.clone(), raw.clone())
			},
			_ => {
				unreachable!()
			},
		...
		Node::new(Data::DocumentType { name, public_id, system_id, raw }, token_and_info.span)
	}
	...
	fn create_element_for_token(&self, token_and_info:TokenAndInfo) -> RcNode {
		let element = match token_and_info.token {
			...
			| Token::EndTag { tag_name, attributes, .. }
			| Token::EmptyTag { tag_name, attributes, .. } => {
				...
				Data::Element { tag_name, attributes:RefCell::new(attributes) }
			},
			_ => {
				unreachable!();
			},
	...
	fn create_processing_instruction(&self, token_and_info:&mut TokenAndInfo) -> RcNode {
		...
		Node::new(Data::ProcessingInstruction { target, data }, token_and_info.span)
	}
