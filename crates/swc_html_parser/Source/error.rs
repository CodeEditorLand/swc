...
impl Error {
	pub fn kind(&self) -> &ErrorKind { &self.inner.1 }
	...
	pub fn message(&self) -> Cow<'static, str> {
		match &self.inner.1 {
			...
			ErrorKind::UnclosedElements(tag_name) => {
				format!("End tag \"{}\" seen, but there were open elements", tag_name).into()
			},
			ErrorKind::UnclosedElementsImplied(tag_name) => {
				format!("End tag \"{}\" implied, but there were open elements", tag_name).into()
			},
			...
			ErrorKind::BadStartTagInNoscriptInHead(tag_name) => {
				format!("Bad start tag in \"{}\" in \"noscript\" in \"head\"", tag_name).into()
			},
			ErrorKind::UnexpectedImageStartTag => {
				"Saw a start tag \"image\", \"img\" element is outdated".into()
			},

			ErrorKind::SomethingSeenWhenSomethingOpen(tag_name) => {
				format!(
			...
			ErrorKind::NoElementToCloseButEndTagSeen(tag_name) => {
				format!("No \"{}\" element in scope but a \"{}\" end tag seen", tag_name, tag_name)
					.into()
			},
			ErrorKind::HtmlStartTagInForeignContext(tag_name) => {
				format!("HTML start tag \"{}\" in a foreign namespace context", tag_name).into()
			},
			...
			ErrorKind::StartTagSeenWithoutRuby(tag_name) => {
				format!("Start tag \"{}\" seen without a \"ruby\" element being open", tag_name)
					.into()
			},
			...
			ErrorKind::EndTagWithUnclosedElements(tag_name) => {
				format!("Unexpected end tag for \"{}\", but there were unclosed elements", tag_name)
					.into()
			},
			ErrorKind::NonSpaceCharacterWithoutDoctype => {
				"Non-space characters found without seeing a doctype first, expected \"<!DOCTYPE \
				 html>\""
					.into()
			},
