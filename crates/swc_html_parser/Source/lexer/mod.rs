...
use swc_atoms::{Atom, JsWord, js_word};
use swc_common::{BytePos, Span, collections::AHashSet, input::Input};
...
where
	I: Input,
{
	fn start_pos(&mut self) -> BytePos { self.input.cur_pos() }

	fn last_pos(&mut self) -> BytePos { self.input.last_pos() }

	fn take_errors(&mut self) -> Vec<Error> { take(&mut self.errors) }
	...
	fn set_input_state(&mut self, state:State) { self.state = state; }
}
...
{
	#[inline(always)]
	fn next(&mut self) -> Option<char> { self.input.cur() }
	...
	fn emit_error(&mut self, kind:ErrorKind) {
		self.errors
			.push(Error::new(Span::new(self.cur_pos, self.input.cur_pos()), kind));
	}
	...
	fn emit_temporary_buffer_as_character_tokens(&mut self) {
		for c in take(&mut self.temporary_buffer).chars() {
			self.emit_token(Token::Character { value:c, raw:Some(Raw::Same) });
		}
	}
	...
	fn flush_code_points_consumed_as_character_reference(&mut self, raw:Option<String>) {
		if self.is_consumed_as_part_of_an_attribute() {
			...
			let is_value_eq_raw =
				if let Some(raw) = &once_raw { *raw == self.temporary_buffer } else { true };
	...
	fn finish_tag_token_name(&mut self) {
		if let Some(
			Token::StartTag { tag_name, raw_tag_name, .. }
			| Token::EndTag { tag_name, raw_tag_name, .. },
	...
	fn finish_attribute_token_name(&mut self) {
		if let Some(attribute_start_position) = self.attribute_start_position {
			if let Some(
				Token::StartTag { ref mut attributes, .. }
				| Token::EndTag { ref mut attributes, .. },
			) = self.current_token
			{
				if let Some(last) = attributes.last_mut() {
					...
					if self.attributes_validator.contains(&name) {
						self.errors.push(Error::new(span, ErrorKind::DuplicateAttribute));
					}
	...
	fn emit_tag_token(&mut self) {
		if let Some(current_tag_token) = self.current_token.take() {
			...
			match &current_tag_token {
				...
				Token::EndTag { ref is_self_closing, ref attributes, .. } => {
	...
	fn emit_character_token(&mut self, value:char) {
		self.emit_token(Token::Character { value, raw:Some(Raw::Same) });
	}
	...
	fn emit_character_token_with_raw(&mut self, c:char, raw_c:char) {
		...
		self.emit_token(Token::Character { value:c, raw:Some(Raw::Atom(Atom::new(&**buf))) });
		...
		match self.state {
			...
			State::NamedCharacterReference => {
				...
				while let Some(c) = &self.consume_next_char() {
					...
					if let Some(found_entity) = HTML_ENTITIES.get(&entity_temporary_buffer) {
						...
						self.temporary_buffer.replace_range(1.., &entity_temporary_buffer[1..]);
					} else {
						// We stop when:
						//
				...
				match entity {
					Some(entity) => {
						...
						if self.is_consumed_as_part_of_an_attribute()
							&& !is_last_semicolon && is_next_equals_sign_or_ascii_alphanumeric
						{
							self.flush_code_points_consumed_as_character_reference(None);
			...
			State::HexademicalCharacterReference => {
				...
				match self.consume_next_char() {
					...
					Some(c) if c.is_ascii_digit() => {
						match &mut self.character_reference_code {
					...
					Some(c) if is_upper_hex_digit(c) => {
						match &mut self.character_reference_code {
							Some(character_reference_code) => {
					...
					// character's code point) to the character reference code.
					Some(c) if is_lower_hex_digit(c) => {
						match &mut self.character_reference_code {
							Some(character_reference_code) => {
...
#[inline(always)]
fn is_spacy(c:char) -> bool { matches!(c, '\x09' | '\x0a' | '\x0d' | '\x0c' | '\x20') }
...
#[inline(always)]
fn is_surrogate(c:u32) -> bool { matches!(c, 0xD800..=0xDFFF) }
...
#[inline(always)]
fn is_upper_hex_digit(c:char) -> bool { matches!(c, '0'..='9' | 'A'..='F') }

#[inline(always)]
fn is_lower_hex_digit(c:char) -> bool { matches!(c, '0'..='9' | 'a'..='f') }

#[inline(always)]
fn is_ascii_hex_digit(c:char) -> bool { is_upper_hex_digit(c) || is_lower_hex_digit(c) }

#[inline(always)]
fn is_ascii_upper_alpha(c:char) -> bool { c.is_ascii_uppercase() }

#[inline(always)]
fn is_ascii_lower_alpha(c:char) -> bool { c.is_ascii_lowercase() }

#[inline(always)]
fn is_ascii_alpha(c:char) -> bool { is_ascii_upper_alpha(c) || is_ascii_lower_alpha(c) }

#[inline(always)]
fn is_allowed_control_character(c:u32) -> bool { c != 0x00 && is_control(c) }
