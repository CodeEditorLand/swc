...
use crate::parser::{RcNode, TokenAndInfo, is_same_node};
...
impl ActiveFormattingElementStack {
	pub fn new() -> Self { ActiveFormattingElementStack { items:Vec::with_capacity(8) } }
	...
	pub fn insert_marker(&mut self) { self.items.push(ActiveFormattingElement::Marker); }

	pub fn get_position(&self, element:&RcNode) -> Option<usize> {
		self.items.iter().position(|n| {
			match *n {
