	...
	I: ParserInput,
{
	...
	pub fn dump_cur(&mut self) -> String { format!("{:?}", self.input.cur()) }

	pub fn take_errors(&mut self) -> Vec<Error> { mem::take(&mut self.errors) }
	...
	pub fn parse_document(&mut self) -> PResult<Document> {
		...
		Ok(Document { span:Span::new(start.lo(), last), mode, children })
	}
	...
	pub fn parse_document_fragment(
		...
		if is_html_element!(context_node, "template") {
			self.template_insertion_mode_stack.push(InsertionMode::InTemplate);
		}
		...
		Ok(DocumentFragment { span:Span::new(start.lo(), last), children })
	}

	fn create_document(&self, mode:Option<DocumentMode>) -> RcNode {
		Node::new(
			Data::Document { mode:RefCell::new(mode.unwrap_or(DocumentMode::NoQuirks)) },
			DUMMY_SP,
		)
	}
	...
	fn node_to_child(&mut self, node:RcNode) -> Child {
		...
		match node.data.clone() {
			Data::DocumentType { name, public_id, system_id, raw } => {
			...
			Data::Element { namespace, tag_name, attributes, is_self_closing } => {
					...
					_ => {
						let span = if start_span.is_dummy() {
							...
							let end_span = match node.end_span.take() {
								...
								_ => {
									match self.get_deep_end_span(&new_children) {
										Some(end_span) => end_span,
										_ => start_span,
									}
								},
						...
						let (children, content) =
							if namespace == Namespace::HTML && &tag_name == "template" {
								(Vec::new(), Some(DocumentFragment { span, children:new_children }))
							} else {
								(new_children, None)
							};
			...
			Data::Text { data, raw } => {
				...
				Child::Text(Text { span, data:data.take().into(), raw:Some(raw.take().into()) })
			},

			Data::Comment { data, raw } => Child::Comment(Comment { span:start_span, data, raw }),
			_ => {
				unreachable!();
			},
		}
		...
		while !self.stopped {
			...
			let mut token_and_info = match self.input.cur()? {
				Some(_) => {
					...
					TokenAndInfo { span:span!(self, span.lo()), acknowledged:false, token }
				},
			...
			if let Token::StartTag { is_self_closing, .. } = &token_and_info.token {
	...
	fn process_token_in_foreign_content(
		...
		match token {
			...
			Token::Character { value, .. } if *value == '\x00' => {
				self.errors
					.push(Error::new(token_and_info.span, ErrorKind::UnexpectedNullCharacter));
			...
			Token::Character { value: '\x09' | '\x0A' | '\x0C' | '\x0D' | '\x20', .. } => {
				self.insert_character(token_and_info)?;
			},
			...
			Token::Doctype { .. } => {
				self.errors.push(Error::new(token_and_info.span, ErrorKind::StrayDoctype));
			},
			...
			//      self-closing flag.
			Token::StartTag { tag_name, raw_tag_name, is_self_closing, attributes } => {
			...
			Token::EndTag { tag_name, .. } => {
				...
				loop {
					...
					match &inner_node.data {
						Data::Element { tag_name: node_tag_name, .. }
							if node_tag_name.to_ascii_lowercase() == **tag_name =>
						{
		...
		match insertion_mode {
			...
			InsertionMode::Initial => {
				...
				match token {
					...
					Token::Doctype { name, public_id, system_id, force_quirks, raw, .. } => {
						let is_html_name =
						...
						if !self.config.iframe_srcdoc
							&& (*force_quirks
								|| !is_html_name || matches!(public_id, Some(public_id) if QUIRKY_PUBLIC_MATCHES
			...
			InsertionMode::BeforeHtml => {
				...
				match token {
					...
					Token::StartTag { tag_name, attributes, is_self_closing, .. }
						if tag_name == "html" =>
					{
			...
			InsertionMode::InHead => {
				...
				match token {
					...
					Token::StartTag { tag_name, is_self_closing, .. }
					...
					// extracted encoding.
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "meta" => {
						let is_self_closing = *is_self_closing;
					...
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "script" => {
					...
					Token::EndTag { tag_name, .. } if tag_name == "template" => {
						if !self.open_elements_stack.contains_template_element() {
							...
							self.open_elements_stack.generate_implied_end_tags_thoroughly();
							...
							let popped =
								self.open_elements_stack.pop_until_tag_name_popped(&["template"]);
			...
			InsertionMode::AfterHead => {
				...
				match token {
					...
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "body" => {
						self.insert_html_element(token_and_info)?;
					...
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "frameset" => {
						self.insert_html_element(token_and_info)?;
						...
						) =>
					{
						...
						let head =
							self.head_element_pointer.as_ref().expect("no head element").clone();
			...
			InsertionMode::InBody => {
				...
				match token {
					...
					// element created.
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "form" => {
					...
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "li" => {
						self.frameset_ok = false;
					...
					Token::EndTag { tag_name, .. } if tag_name == "li" => {
						if !self.open_elements_stack.has_in_list_item_scope("li") {
							...
							self.open_elements_stack.generate_implied_end_tags_with_exclusion("li");
					...
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "a" => {
						...
						self.active_formatting_elements.push(ActiveFormattingElement::Element(
							element,
					...
					// Insert an HTML element for the token. Push onto the list of active formatting
					// elements that element.
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "nobr" => {
						self.reconstruct_active_formatting_elements()?;
					...
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "table" => {
					...
					Token::EndTag { tag_name, is_self_closing, .. } if tag_name == "br" => {
						let is_self_closing = *is_self_closing;

						self.errors.push(Error::new(token_and_info.span, ErrorKind::EndTagBr));
					...
					Token::StartTag { tag_name, is_self_closing, attributes, .. }
						if tag_name == "input" =>
					{
						let is_self_closing = *is_self_closing;
						...
						let is_hidden = match &input_type {
							Some(input_type) => {
								match &input_type.value {
									Some(value)
										if value.as_ref().eq_ignore_ascii_case("hidden") =>
									{
										true
									},
					...
					//
					// Set the frameset-ok flag to "not ok".
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "hr" => {
						let is_self_closing = *is_self_closing;
					...
					Token::StartTag { tag_name, .. } if tag_name == "image" => {
						...
						match token_and_info {
							TokenAndInfo { token: Token::StartTag { tag_name, .. }, .. } => {
								*tag_name = "img".into();
							},
					...
					//
					// Follow the generic raw text element parsing algorithm.
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "xmp" => {
						if self.open_elements_stack.has_in_button_scope("p") {
							self.close_p_element(token_and_info, false);
						}
					...
					//
					// Follow the generic raw text element parsing algorithm.
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "iframe" => {
						self.frameset_ok = false;
					...
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "select" => {
						self.reconstruct_active_formatting_elements()?;
					...
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "math" => {
						let is_self_closing = *is_self_closing;
					...
					// If the token has its self-closing flag set, pop the current node off the
					// stack of open elements and acknowledge the token's self-closing flag.
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "svg" => {
						let is_self_closing = *is_self_closing;
					...
					//
					// Insert an HTML element for the token.
					Token::StartTag { is_self_closing, tag_name, .. } => {
						self.reconstruct_active_formatting_elements()?;
			...
			InsertionMode::Text => {
				...
				match token {
					...
					// token.
					Token::Eof => {
						self.errors.push(Error::new(token_and_info.span, ErrorKind::EofInText));
			...
			InsertionMode::InTable => {
				...
				match token {
					...
					// caption".
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "caption" => {
					...
					// Insert an HTML element for the token, then switch the insertion mode to "in
					// column group".
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "colgroup" => {
						self.open_elements_stack.clear_back_to_table_context();
				...
				match token {
					...
					Token::EndTag { tag_name, .. } if tag_name == "caption" => {
						if !self.open_elements_stack.has_in_table_scope("caption") {
							...
							let popped =
								self.open_elements_stack.pop_until_tag_name_popped(&["caption"]);
			...
			InsertionMode::InColumnGroup => {
				...
				match token {
					...
					//
					// Acknowledge the token's self-closing flag, if it is set.
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "col" => {
						let is_self_closing = *is_self_closing;
			...
			InsertionMode::InTableBody => {
				...
				match token {
					...
					// Insert an HTML element for the token, then switch the insertion mode to "in
					// row".
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "tr" => {
			...
			InsertionMode::InSelect => {
				match token {
					...
					//
					// Insert an HTML element for the token.
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "option" => {
						match self.open_elements_stack.items.last() {
							Some(node) if is_html_element!(node, "option") => {
								self.open_elements_stack.pop();
							},
					...
					//
					// Insert an HTML element for the token.
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "optgroup" => {
						match self.open_elements_stack.items.last() {
							Some(node) if is_html_element!(node, "option") => {
								self.open_elements_stack.pop();
							},
			...
			InsertionMode::InTemplate => {
				...
				match token {
						...
						) =>
					{
						...
						self.template_insertion_mode_stack.push(InsertionMode::InTable);
					...
					Token::StartTag { tag_name, .. } if tag_name == "col" => {
						...
						self.template_insertion_mode_stack.push(InsertionMode::InColumnGroup);
					...
					Token::StartTag { tag_name, .. } if tag_name == "tr" => {
						...
						self.template_insertion_mode_stack.push(InsertionMode::InTableBody);
					...
					Token::StartTag { tag_name, .. } if matches!(&**tag_name, "td" | "th") => {
						...
						self.template_insertion_mode_stack.push(InsertionMode::InRow);
					...
					Token::StartTag { .. } => {
						...
						self.template_insertion_mode_stack.push(InsertionMode::InBody);
			...
			InsertionMode::InFrameset => {
				...
				match token {
					...
					//
					// Acknowledge the token's self-closing flag, if it is set.
					Token::StartTag { tag_name, is_self_closing, .. } if tag_name == "frame" => {
						let is_self_closing = *is_self_closing;
	...
	fn process_token_in_table_insertion_mode_anything_else(
		...
		match &token_and_info.token {
			...
			Token::Character { .. } => {
				self.errors
					.push(Error::new(token_and_info.span, ErrorKind::NonSpaceCharacterInTable));
			},
	...
	fn any_other_end_tag_for_in_body_insertion_mode(&mut self, token_and_info:&TokenAndInfo) {
		...
		// 2. - 1.
		self.open_elements_stack.generate_implied_end_tags_with_exclusion(tag_name);
	...
	fn create_element_for_token(
		...
		let element = match token {
			Token::StartTag { tag_name, attributes, is_self_closing, .. }
			| Token::EndTag { tag_name, attributes, is_self_closing, .. } => {
	...
	fn run_the_adoption_agency_algorithm(
		...
		loop {
			...
			if formatting_element_stack_index.is_none() {
				...
				self.active_formatting_elements.remove(&formatting_element.1);
			...
			if formatting_element_stack_index.is_some()
				&& !self.open_elements_stack.has_node_in_scope(&formatting_element.1)
			{
				self.errors.push(Error::new(
					token_and_info.span,
					ErrorKind::NoElementToCloseButEndTagSeen(subject),
			...
			loop {
				...
				self.open_elements_stack.replace(node_index, new_element.clone());
			...
			self.open_elements_stack.insert(new_furthest_block_index + 1, new_element);
		}
	}
	...
	fn reparent_children(&mut self, node:&RcNode, new_parent:&RcNode) {
		...
		for child in children.iter() {
			...
			if let Some(previous_parent) = previous_parent {
				assert!(is_same_node(node, &previous_parent.upgrade().expect("dangling weak")));
			}
		}
	...
	fn is_marker_or_open(&self, entry:&ActiveFormattingElement) -> bool {
		match *entry {
			...
			ActiveFormattingElement::Element(ref node, _) => {
				self.open_elements_stack.items.iter().rev().any(|n| is_same_node(n, node))
			},
		}
	}
	...
	fn close_p_element(&mut self, token_and_info:&TokenAndInfo, is_close_p:bool) {
		...
		self.open_elements_stack.generate_implied_end_tags_with_exclusion("p");
	...
	fn close_the_cell(&mut self) {
		...
		match self.open_elements_stack.items.last() {
			Some(node) if !is_html_element!(node, "td" | "th") => {
				self.errors
					.push(Error::new(*node.start_span.borrow(), ErrorKind::UnclosedElementsCell));
			},
		...
		// Pop elements from the stack of open elements stack until a td
		// element or a th element has been popped from the stack.
		self.open_elements_stack.pop_until_tag_name_popped(&["td", "th"]);
	...
	fn get_appropriate_place_for_inserting_node(
		...
		adjusted_insertion_location = match &adjusted_insertion_location {
			InsertionPosition::LastChild(node) | InsertionPosition::BeforeSibling(node) => {
				match &node.data {
					Data::Element { namespace, tag_name, .. }
			...
			InsertionPosition::LastChild(parent) => {
				...
				if let Some(last) = children.last() {
					if let Data::Text { data, raw: raw_data } = &last.data {
						match &token_and_info.token {
							Token::Character { value: c, raw: raw_c } => {
				...
				if let Some((parent, i)) = self.get_parent_and_index(node) {
					if i > 0 {
						...
						if let Some(previous) = children.get(i - 1) {
							if let Data::Text { data, raw: raw_data } = &previous.data {
								match &token_and_info.token {
									Token::Character { value: c, raw: raw_c } => {
										data.borrow_mut().push(*c);
...
fn is_same_node(a:&RcNode, b:&RcNode) -> bool { Rc::ptr_eq(a, b) }
...
fn is_html_integration_point(node:Option<&RcNode>) -> bool {
	if let Some(node) = node {
		match &node.data {
			Data::Element { namespace, tag_name, attributes, .. }
				if *namespace == Namespace::MATHML && tag_name == "annotation-xml" =>
			{
