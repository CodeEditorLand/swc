...
impl Compressor {
			...
			*name = name.to_ascii_lowercase();
			...
			match &**name {
					...
					if declaration.value.len() > 1 =>
				{
					...
					let right = declaration.value.get(1).or_else(|| declaration.value.first());
					let bottom = declaration.value.get(2).or_else(|| declaration.value.first());
	...
	fn is_same_length_nodes(
		...
		match (node_1, node_2) {
			(ComponentValue::Dimension(dimension_1), ComponentValue::Dimension(dimension_2)) => {
				let result_1 =
					dimension_1.as_length().map(|length| (&length.value.value, &length.unit.value));

				let result_2 =
					dimension_2.as_length().map(|length| (&length.value.value, &length.unit.value));
		...
		match (node_1, node_2) {
			(ComponentValue::Dimension(dimension_1), ComponentValue::Dimension(dimension_2)) => {
				...
				result_1.value.value == result_2.value.value
					&& result_1.unit.value.eq_ignore_ascii_case(&result_2.unit.value)
			},
			(
	...
	fn compress_from_initial(&self, declaration:&mut Declaration, span:Span) {
		...
		match &**name {
			...
			| "transition-duration" => {
				declaration.value =
					vec![ComponentValue::Dimension(Box::new(Dimension::Time(Time {
						...
						unit:Ident { span:DUMMY_SP, value:"s".into(), raw:None },
					})))];
			},
			"shape-image-threshold" | "opacity" => {
				declaration.value =
					vec![ComponentValue::AlphaValue(Box::new(AlphaValue::Number(Number {
						span,
						value:0.0,
						raw:None,
					})))];
			...
			"animation-iteration-count" => {
				declaration.value =
					vec![ComponentValue::Number(Box::new(Number { span, value:1.0, raw:None }))];
			},
			...
			| "border-spacing" => {
				declaration.value =
					vec![ComponentValue::Dimension(Box::new(Dimension::Length(Length {
						span,
						value:Number { span:DUMMY_SP, value:0.0, raw:None },
						unit:Ident { span:DUMMY_SP, value:"px".into(), raw:None },
					})))];
			...
			"mask-border-slice" | "mask-border-outset" => {
				declaration.value =
					vec![ComponentValue::Number(Box::new(Number { span, value:0.0, raw:None }))];
			},
			"border-image-slice" => {
				declaration.value = vec![ComponentValue::Percentage(Box::new(Percentage {
					span,
					value:Number { span:DUMMY_SP, value:100.0, raw:None },
				}))];
			},
			"border-image-width" => {
				declaration.value =
			...
			"image-resolution" => {
				declaration.value =
					vec![ComponentValue::Dimension(Box::new(Dimension::Resolution(Resolution {
						span,
						value:Number { span:DUMMY_SP, value:1.0, raw:None },
						unit:Ident { span:DUMMY_SP, value:"dppx".into(), raw:None },
					})))];
			},
			"justify-items" => {
				declaration.value = vec![ComponentValue::Ident(Box::new(Ident {
			...
			"math-depth" | "order" => {
				declaration.value =
					vec![ComponentValue::Integer(Box::new(Integer { span, value:0, raw:None }))];
			},
			"object-fit" => {
				declaration.value = vec![ComponentValue::Ident(Box::new(Ident {
					span,
			...
			"orphans" | "widows" => {
				declaration.value =
					vec![ComponentValue::Integer(Box::new(Integer { span, value:2, raw:None }))];
			},
			_ => {},
		}
	}
	...
	fn _compress_to_initial(&self, declaration:&mut Declaration) {
		...
		match &**name {
			...
			"transform-origin" => {
				if let (
					Some(Percentage { value: Number { value: first, .. }, span, .. }),
					Some(Percentage { value: Number { value: second, .. }, .. }),
					Some(Integer { value: third, .. }),
				) = (
					declaration.value.first().and_then(|x| x.as_percentage()).map(|x| x.as_ref()),
					declaration.value.get(1).and_then(|x| x.as_percentage()).map(|x| x.as_ref()),
					declaration.value.get(2).and_then(|x| x.as_integer()).map(|x| x.as_ref()),
				) {
					if *first == 50.0
						&& *second == 50.0 && *third == 0
