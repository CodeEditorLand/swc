...
impl Compressor {
	...
	fn is_first_or_supports_type(&self, supports_condition:&SupportsCondition) -> bool {
		matches!(supports_condition.conditions.get(1), Some(SupportsConditionType::Or(_)))
	}

	fn is_first_and_supports_type(&self, supports_condition:&SupportsCondition) -> bool {
		matches!(supports_condition.conditions.get(1), Some(SupportsConditionType::And(_)))
	}
			...
			Some(SupportsConditionType::And(_)) => {
				let need_compress = n.conditions.iter().any(|item| {
					match item {
						...
						SupportsConditionType::And(supports_and) => {
							match &*supports_and.condition {
								SupportsInParens::SupportsCondition(supports_condition)
									if self.is_first_and_supports_type(supports_condition)
										&& self.is_first_supports_in_parens(supports_condition) =>
								{
						...
						match item {
							...
							SupportsConditionType::And(supports_and) => {
								match *supports_and.condition {
									...
									_ => {
										new_conditions
											.push(SupportsConditionType::And(supports_and));
									},
								}
							},
							_ => {
