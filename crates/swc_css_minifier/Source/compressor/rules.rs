...
use swc_common::{EqIgnoreSpan, Span, Spanned, collections::AHashMap, util::take::Take};
use swc_css_ast::*;
...
impl Default for CompatibilityChecker {
	fn default() -> Self { CompatibilityChecker { allow_to_merge:true } }
}
...
impl Compressor {
	...
	fn is_same_declaration_name(&self, left:&Declaration, right:&Declaration) -> bool {
		match (&left.name, &right.name) {
			(
				DeclarationName::Ident(Ident { value: left_value, .. }),
				DeclarationName::Ident(Ident { value: right_value, .. }),
			) => left_value.eq_ignore_ascii_case(right_value),
			(
				DeclarationName::DashedIdent(DashedIdent { value: left_value, .. }),
				DeclarationName::DashedIdent(DashedIdent { value: right_value, .. }),
	...
	fn collect_names(&self, at_rule:&AtRule, names:&mut AHashMap<Name, isize>) {
		...
		match &**prelude {
			...
			prelude => {
				...
				names
					.entry(Name::Keyframes(self.get_at_rule_name(at_rule), name.clone()))
					.and_modify(|mana| *mana += 1)
					.or_insert(1);
			},
		}
		...
		let mut discarder = |at_rule:&AtRule| {
			...
			match &**prelude {
				...
				prelude => {
					...
					let counter = names
						.get_mut(&Name::Keyframes(self.get_at_rule_name(at_rule), name.clone()));
	...
	fn merge_selector_list(&self, left:&mut SelectorList, right:&mut SelectorList) -> SelectorList {
		let mut children = left.children.take();

		children.extend(right.children.take());

		SelectorList { span:Span::new(left.span_lo(), right.span_hi()), children }
	}
	...
	fn merge_relative_selector_list(
		...
		RelativeSelectorList { span:Span::new(left.span_lo(), right.span_hi()), children }
	}
	...
	pub(super) fn compress_stylesheet(&mut self, stylesheet:&mut Stylesheet) {
		...
		if !remove_rules_list.is_empty() {
			stylesheet.rules = take(&mut stylesheet.rules)
				...
				.filter_map(
					|(idx, value)| {
						if remove_rules_list.contains(&idx) { None } else { Some(value) }
					},
				)
				.collect::<Vec<_>>();
...
#[inline]
fn need_keep_by_name(name:&JsWord) -> bool { *name == "color-profile" }
