...
pub fn compressor() -> impl VisitMut { Compressor::default() }
...
impl VisitMut for Compressor {
	...
	fn visit_mut_calc_sum(&mut self, n:&mut CalcSum) {
		n.visit_mut_children_with(&mut *self.with_ctx(Ctx { in_math_function:true, ..self.ctx }));
	...
	fn visit_mut_declaration(&mut self, n:&mut Declaration) {
		...
		if let DeclarationName::Ident(Ident { value, .. }) = &n.name {
			if matches_eq_ignore_ascii_case!(
				...
				n.visit_mut_children_with(
					&mut *self.with_ctx(Ctx { preserve_alpha_value:false, ..self.ctx }),
				);
			} else {
				n.visit_mut_children_with(self);
			}
	...
	fn visit_mut_function(&mut self, n:&mut Function) {
		...
		if matches_eq!(
			...
			n.visit_mut_children_with(
				&mut *self.with_ctx(Ctx { in_transform_function:true, ..self.ctx }),
			);
		} else {
			n.visit_mut_children_with(self);
		}
	...
	fn visit_mut_keyframe_block(&mut self, n:&mut KeyframeBlock) {
		n.visit_mut_children_with(&mut *self.with_ctx(Ctx { in_keyframe_block:true, ..self.ctx }));
	}
	...
	fn visit_mut_pseudo_class_selector(&mut self, n:&mut PseudoClassSelector) {
		match &mut n.name {
				...
				) =>
			{
				...
				n.visit_mut_children_with(
					&mut *self.with_ctx(Ctx { in_logic_combinator_selector:true, ..self.ctx }),
				);
			},
			_ => {
				n.visit_mut_children_with(self);
...
fn contains_only_ascii_characters(string:&str) -> bool { string.is_ascii() }
