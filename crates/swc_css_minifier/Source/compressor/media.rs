...
impl Compressor {
	...
	fn is_first_or_media_type(&self, media_condition:&MediaCondition) -> bool {
		matches!(media_condition.conditions.get(1), Some(MediaConditionAllType::Or(_)))
	}

	fn is_first_and_media_type(&self, media_condition:&MediaCondition) -> bool {
		matches!(media_condition.conditions.get(1), Some(MediaConditionAllType::And(_)))
	}
	...
	pub(super) fn compress_media_condition(&mut self, n:&mut MediaCondition) {
		match n.conditions.get(1) {
			Some(MediaConditionAllType::Or(_)) => {
				let need_compress = n.conditions.iter().any(|item| {
					match item {
							...
							MediaConditionAllType::Or(media_or) => {
								match media_or.condition {
											...
											&& self.is_first_media_in_parens(&media_condition) =>
									{
										...
										{
											new_conditions.push(MediaConditionAllType::Or(
												MediaOr {
													span:DUMMY_SP,
													keyword:None,
							...
							MediaConditionAllType::And(media_and) => {
								match media_and.condition {
											...
											&& self.is_first_media_in_parens(&media_condition) =>
									{
										...
										{
											new_conditions.push(MediaConditionAllType::And(
												MediaAnd {
													span:DUMMY_SP,
													keyword:None,
													condition:media_in_parens,
	...
	pub(super) fn compress_media_feature(&mut self, n:&mut MediaFeature) {
		match n {
			MediaFeature::Plain(MediaFeaturePlain {
				...
				if matches!(&*name.value, "min-color" | "min-color-index" | "min-monochrome")
					&& value.as_number().map(|value| value.value == 1.0).unwrap_or_default()
				{
