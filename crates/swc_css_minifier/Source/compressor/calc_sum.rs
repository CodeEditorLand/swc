use swc_atoms::Atom;
use swc_common::{Span, collections::AHashMap};
use swc_css_ast::*;
...
fn get_precision(n:f64) -> u32 {
	...
	n_as_str.find('.').map_or(0, |sep| (n_as_str.len() - sep) as u32 - 1)
}
	...
	match &calc_node {
		CalcNode::Number(n) => {
			...
			if let Some(result) = try_to_divide_values(1.0, n.value) {
				CalcNode::Number(Number { value:result, span:n.span, raw:n.raw.clone() })
			} else {
...
fn try_to_switch_sign_of_nodes(nodes:&[CalcNode]) -> Option<CalcNode> {
	...
	if nodes.len() == 1 {
		...
		Some(CalcNode::OperatorNode(Box::new(CalcOperatorNode::Sum(nodes.to_vec()))))
	}
}
...
fn try_to_multiply_all_numeric_sum_children_by_value(
	...
	Some(CalcNode::OperatorNode(Box::new(CalcOperatorNode::Sum(operands))))
}
		...
		match (&children[0], &children[1]) {
			...
			(numerator, CalcNode::OperatorNode(op)) => {
				if let CalcOperatorNode::Invert(denominator) = &**op {
					Some(CalcNode::OperatorNode(Box::new(CalcOperatorNode::Product(vec![
						denominator.clone(),
						CalcNode::OperatorNode(Box::new(CalcOperatorNode::Invert(
							numerator.clone(),
						))),
...
fn try_to_multiply_values(v1:f64, v2:f64) -> Option<f64> {
	...
	if result_precision <= (precision1 + precision2) { Some(result) } else { None }
}
...
fn try_to_sum_values(n1:f64, n2:f64, ratio:Option<f64>) -> Option<f64> {
	...
	if result_precision <= precision1.max(precision2) { Some(result) } else { None }
}
...
fn try_to_divide_values(numerator:f64, denominator:f64) -> Option<f64> {
	...
	if result_precision <= f64::DIGITS { Some(result) } else { None }
}
		...
		CalcNode::OperatorNode(op) => {
			match &**op {
				CalcOperatorNode::Sum(nodes) => {
					...
					CalcSum { expressions:expr, span:Span::dummy_with_cmt() }
				},
	...
	match calc_node {
		...
		| CalcNode::Function(_) => {
			CalcProductOrOperator::Product(CalcProduct {
				expressions:vec![CalcValueOrOperator::Value(serialize_calc_node_into_calc_value(
					calc_node,
				))],
				span:Span::dummy_with_cmt(),
			})
		...
		CalcNode::OperatorNode(op) => {
			match &**op {
				CalcOperatorNode::Negate(_) => {
					CalcProductOrOperator::Product(CalcProduct {
						expressions:vec![
							...
							CalcValueOrOperator::Value(serialize_calc_node_into_calc_value(
								calc_node,
							)),
						],
						span:Span::dummy_with_cmt(),
				...
				CalcOperatorNode::Product(nodes) => {
					...
					if let Some(calc_node) = nodes_iter.next() {
						expr.push(CalcValueOrOperator::Value(serialize_calc_node_into_calc_value(
							calc_node,
						)))
					}
