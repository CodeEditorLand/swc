...
impl Compressor {
	...
	pub(super) fn compress_an_plus_b(&mut self, an_plus_b:&mut AnPlusB) {
		match &an_plus_b {
			...
			AnPlusB::AnPlusBNotation(AnPlusBNotation { a: Some(a), b: Some(b), span, .. })
				if *a == 2 && (*b == 1 || b % 2 == -1) =>
			{
				*an_plus_b = AnPlusB::Ident(Ident { span:*span, value:"odd".into(), raw:None });
			},
			...
			// `0n+5` => `5`, `0n-5` => `-5`, etc
			AnPlusB::AnPlusBNotation(AnPlusBNotation { a: Some(a), b, span, .. }) if *a == 0 => {
				*an_plus_b = AnPlusB::AnPlusBNotation(AnPlusBNotation {
					span:*span,
					a:None,
					a_raw:None,
			...
			// `-5n+0` => `-5n`, etc
			AnPlusB::AnPlusBNotation(AnPlusBNotation { a, b: Some(b), span, .. }) if *b == 0 => {
				*an_plus_b = AnPlusB::AnPlusBNotation(AnPlusBNotation {
					span:*span,
					a:*a,
					a_raw:None,
	...
	pub(super) fn compress_subclass_selector(&mut self, subclass_selector:&mut SubclassSelector) {
		match &subclass_selector {
			SubclassSelector::PseudoElement(PseudoElementSelector { name, span, .. }) => {
				if matches!(&*name.value, "before" | "after" | "first-letter" | "first-line") {
			...
			SubclassSelector::PseudoClass(PseudoClassSelector {
				...
				span,
				..
			}) if name.value == "nth-child" && children.len() == 1 => {
				match children.first() {
					Some(PseudoClassSelectorChildren::AnPlusB(AnPlusB::AnPlusBNotation(
						AnPlusBNotation { a: None, b: Some(b), .. },
					))) if *b == 1 => {
						*subclass_selector = SubclassSelector::PseudoClass(PseudoClassSelector {
							span:*span,
							name:Ident { span:DUMMY_SP, value:"first-child".into(), raw:None },
							children:None,
						})
					},
			...
			SubclassSelector::PseudoClass(PseudoClassSelector {
				...
				span,
				..
			}) if name.value == "nth-last-child" && children.len() == 1 => {
				match children.first() {
					Some(PseudoClassSelectorChildren::AnPlusB(AnPlusB::AnPlusBNotation(
						AnPlusBNotation { a: None, b: Some(b), .. },
					))) if *b == 1 => {
						*subclass_selector = SubclassSelector::PseudoClass(PseudoClassSelector {
							span:*span,
							name:Ident { span:DUMMY_SP, value:"last-child".into(), raw:None },
							children:None,
						})
					},
			...
			SubclassSelector::PseudoClass(PseudoClassSelector {
				...
				span,
				..
			}) if name.value == "nth-of-type" && children.len() == 1 => {
				match children.first() {
					Some(PseudoClassSelectorChildren::AnPlusB(AnPlusB::AnPlusBNotation(
						AnPlusBNotation { a: None, b: Some(b), .. },
					))) if *b == 1 => {
						*subclass_selector = SubclassSelector::PseudoClass(PseudoClassSelector {
							span:*span,
							name:Ident { span:DUMMY_SP, value:"first-of-type".into(), raw:None },
							children:None,
						})
					},
			...
			SubclassSelector::PseudoClass(PseudoClassSelector {
				...
				match children.first() {
					Some(PseudoClassSelectorChildren::AnPlusB(AnPlusB::AnPlusBNotation(
						...
						*subclass_selector = SubclassSelector::PseudoClass(PseudoClassSelector {
							span:*span,
							name:Ident { span:DUMMY_SP, value:"last-of-type".into(), raw:None },
							children:None,
						})
					},
