...
impl Compressor {
	pub(super) fn length_to_zero(&mut self, n:&mut Length) -> Option<Number> {
		match &n {
			Length { value: Number { value: number_value, .. }, span, .. }
				if *number_value == 0.0 =>
			{
				Some(Number { span:*span, value:0.0, raw:None })
			},
			_ => None,
		}
	}
		...
		match &*length.unit.value {
			"cm" => {
				if value % 2.54 == 0.0 {
					...
					length.value = Number { span:length.value.span, value:new_value, raw:None };
					length.unit = Ident { span:length.unit.span, value:"in".into(), raw:None };
				} else if value <= 0.1 {
					let new_value = self.convert_length(value, &length.unit.value, "mm");

					length.value = Number { span:length.value.span, value:new_value, raw:None };
					length.unit = Ident { span:length.unit.span, value:"mm".into(), raw:None };
				}
			},
			"mm" => {
				if value % 25.4 == 0.0 {
					...
					length.value = Number { span:length.value.span, value:new_value, raw:None };
					length.unit = Ident { span:length.unit.span, value:"cm".into(), raw:None };
				}
			},
			"q" => {
			...
			"pt" => {
				if value % 72.0 == 0.0 {
					...
					length.value = Number { span:length.value.span, value:new_value, raw:None };
					length.unit = Ident { span:length.unit.span, value:"pc".into(), raw:None };
				} else if value % 0.75 == 0.0 {
					let new_value = self.convert_length(value, &length.unit.value, "px");

					length.value = Number { span:length.value.span, value:new_value, raw:None };
					length.unit = Ident { span:length.unit.span, value:"px".into(), raw:None };
				}
			},
			_ => {},
		}
