...
impl Compressor {
	pub(super) fn compress_angle_in_component_value(
		...
		{
			*component_value =
				ComponentValue::Number(Box::new(Number { span, value:0.0, raw:None }));
		}
	}
	...
	pub(super) fn compress_angle(&mut self, angle:&mut Angle) {
		...
		angle.value = Number { value:deg, raw:None, span:angle.span };

		angle.unit = Ident { span:angle.unit.span, value:"deg".into(), raw:None };
	}
}
