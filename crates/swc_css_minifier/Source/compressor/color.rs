...
fn get_short_hex(v:u32) -> u32 { ((v & 0x0FF00000) >> 12) | ((v & 0x00000FF0) >> 4) }
	...
	($span:expr, $r:expr, $g:expr, $b:expr, $a:expr) => {{
		...
		if need_alpha_value {
			...
			if is_alpha_hex_supported {
				...
				let mut alpha_value =
					AlphaValue::Number(Number { span:DUMMY_SP, value:$a, raw:None });
		...
		match &alpha_value {
			ComponentValue::AlphaValue(alpha_value) => {
				match &**alpha_value {
					...
					AlphaValue::Percentage(Percentage { value: Number { value, .. }, .. }) => {
						if *value > 100.0 {
							return Some(1.0);
						} else if *value < 0.0 {
							return Some(0.0);
			...
			Color::AbsoluteColorBase(AbsoluteColorBase::HexColor(HexColor {
				...
				if let Some(value) = self.get_named_color_by_hex(value) {
					*color = Color::AbsoluteColorBase(AbsoluteColorBase::NamedColorOrTransparent(
						Ident { span:*span, value:value.into(), raw:None },
					));
