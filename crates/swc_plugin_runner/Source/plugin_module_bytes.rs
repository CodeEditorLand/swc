...
impl PluginModuleBytes for RawPluginModuleBytes {
	fn get_module_name(&self) -> &str { &self.plugin_name }
...
impl RawPluginModuleBytes {
	pub fn new(identifier:String, bytes:Vec<u8>) -> Self { Self { plugin_name:identifier, bytes } }
}
	...
	fn clone(&self) -> Self {
		Self {
			...
			store:Some(Store::new(
				self.store.as_ref().expect("Store should be available").engine().clone(),
			)),
		}
	}
}
...
impl CompiledPluginModuleBytes {
	pub fn new(identifier:String, bytes:wasmer::Module, store:wasmer::Store) -> Self {
		Self { plugin_name:identifier, bytes:Some(bytes), store:Some(store) }
	}
}
...
impl PluginModuleBytes for CompiledPluginModuleBytes {
	...
	fn compile_module(&self) -> Result<(Store, Module), Error> {
		Ok((
			Store::new(self.store.as_ref().expect("Store should be available").engine().clone()),
			self.bytes.as_ref().expect("Module should be available").clone(),
		))
	}
}
