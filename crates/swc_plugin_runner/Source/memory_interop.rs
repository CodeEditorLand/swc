...
pub fn copy_bytes_into_host(memory:&MemoryView, bytes_ptr:i32, bytes_ptr_len:i32) -> Vec<u8> {
	...
	values.read_to_vec().expect("Should able to read memory from given ptr")
}
...
where
	...
	(ptr_start, serialized_len.try_into().expect("Should be able to convert to i32"))
}
...
pub fn allocate_return_values_into_guest(
	...
	let guest_memory_ptr = alloc_guest_memory
		.call(store, serialized_bytes_len.try_into().expect("Should be able to convert size"))
	...
	write_into_memory_view(memory, store, &comment_ptr_serialized, |_, _| allocated_ret_ptr);
}
