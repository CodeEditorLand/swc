...
use wasmer_wasix::{Runtime, WasiEnv, WasiFunctionEnv, default_fs_backing, is_wasi_module};
...
impl PluginTransformState {
	fn run(
		...
		let guest_program_ptr = write_into_memory_view(
			...
			|s, serialized_len| {
				self.exported_plugin_alloc
					.call(s, serialized_len.try_into().expect("Should able to convert size"))
					.unwrap_or_else(|_| {
		...
		let ret = if returned_ptr_result == 0 {
			...
			match err {
				PluginError::SizeInteropFailure(msg) => {
					Err(anyhow!("Failed to convert pointer size to calculate: {}", msg))
				},
				PluginError::Deserialize(msg) | PluginError::Serialize(msg) => {
					Err(anyhow!("{}", msg))
				},

				_ => Err(anyhow!("Unexpected error occurred while running plugin transform")),
			}
		};
...
impl TransformExecutor {
	...
	fn setup_plugin_env_exports(&mut self) -> Result<PluginTransformState, Error> {
		...
		let transform_env =
			FunctionEnv::new(&mut store, TransformResultHostEnvironment::new(&transform_result));
		...
		// Main transform interface plugin exports
		let exported_plugin_transform:TypedFunction<(u32, u32, u32, u32), u32> =
			instance.exports.get_typed_function(&store, "__transform_plugin_process_impl")?;
