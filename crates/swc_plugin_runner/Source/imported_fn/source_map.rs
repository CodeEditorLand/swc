...
pub fn lookup_char_pos_proxy(
	...
	let ret = VersionedSerializable::new(PartialLoc {
		source_file:if should_include_source_file == 0 { None } else { Some(original_loc.file) },
...
pub fn merge_spans_proxy(
	...
	let sp_lhs = Span { lo:BytePos(lhs_lo), hi:BytePos(lhs_hi) };

	let sp_rhs = Span { lo:BytePos(rhs_lo), hi:BytePos(rhs_hi) };
	...
	if let Some(span) = ret {
		...
		write_into_memory_view(memory, &mut env.as_store_mut(), &serialized_bytes, |_, _| {
			allocated_ptr
		});
		1
	} else {
...
pub fn span_to_lines_proxy(
	...
	let span = Span { lo:BytePos(span_lo), hi:BytePos(span_hi) };

	let ret = (env.data().source_map.lock()).span_to_lines(span).map(|lines| {
		PartialFileLines {
			file:if should_request_source_file == 0 { None } else { Some(lines.file) },
			lines:lines.lines,
		}
	});
