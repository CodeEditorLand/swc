...
impl AsCollapsibleDecl for Stmt {
	fn get_decl_id(&self) -> Option<Id> { self.as_decl().and_then(AsCollapsibleDecl::get_decl_id) }
}
...
#[inline]
pub(crate) fn stmt_is_enum(stmt:&Stmt) -> bool { matches!(stmt, Stmt::Decl(Decl::TsEnum(..))) }

/// this.prop = value
pub(crate) fn assign_value_to_this_prop(prop_name:PropName, value:Expr) -> Box<Expr> {
	let target =
		MemberExpr { obj:ThisExpr { span:DUMMY_SP }.into(), span:DUMMY_SP, prop:prop_name.into() };
