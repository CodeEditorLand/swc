...
use swc_ecma_visit::{Visit, VisitMut, VisitMutWith, VisitWith, noop_visit_type};

use crate::{ImportsNotUsedAsValues, strip_type::IsConcrete};
...
impl From<AHashSet<Id>> for UsageCollect {
	fn from(id_usage:AHashSet<Id>) -> Self { Self { id_usage, import_chain:Default::default() } }
}

impl Visit for UsageCollect {
	noop_visit_type!();

	fn visit_ident(&mut self, n:&Ident) { self.id_usage.insert(n.to_id()); }

	fn visit_expr(&mut self, n:&Expr) { maybe_grow_default(|| n.visit_children_with(self)) }
...
impl UsageCollect {
	fn has_usage(&self, id:&Id) -> bool { self.id_usage.contains(id) }
...
impl Visit for DeclareCollect {
	fn visit_binding_ident(&mut self, n:&BindingIdent) { self.id_value.insert(n.to_id()); }

	fn visit_assign_pat_prop(&mut self, n:&AssignPatProp) { self.id_value.insert(n.key.to_id()); }

	fn visit_var_declarator(&mut self, n:&VarDeclarator) { n.name.visit_with(self); }

	fn visit_fn_decl(&mut self, n:&FnDecl) { self.id_value.insert(n.ident.to_id()); }

	fn visit_class_decl(&mut self, n:&ClassDecl) { self.id_value.insert(n.ident.to_id()); }

	fn visit_ts_enum_decl(&mut self, n:&TsEnumDecl) { self.id_value.insert(n.id.to_id()); }
	...
	fn visit_export_default_decl(&mut self, n:&ExportDefaultDecl) {
		match &n.decl {
			...
			DefaultDecl::Fn(FnExpr { ident: Some(ident), .. }) => {
				self.id_value.insert(ident.to_id());
			},
	...
	fn visit_ts_interface_decl(&mut self, n:&TsInterfaceDecl) { self.id_type.insert(n.id.to_id()); }
	...
	fn visit_import_decl(&mut self, n:&ImportDecl) {
		n.specifiers.iter().for_each(|import_specifier| {
...
impl DeclareCollect {
	...
	fn has_value(&self, id:&Id) -> bool { self.id_value.contains(id) }
}
		...
		n.retain_mut(|module_item| {
			match module_item {
				ModuleItem::ModuleDecl(ModuleDecl::Import(ImportDecl {
					...
					specifiers.retain(|import_specifier| {
						match import_specifier {
							ImportSpecifier::Named(named) => {
				...
				ModuleItem::ModuleDecl(ModuleDecl::ExportNamed(NamedExport {
					...
					specifiers.retain(|export_specifier| {
...
impl VisitMut for StripTsImportEquals {
	fn visit_mut_module_items(&mut self, n:&mut Vec<ModuleItem>) {
		let has_ts_import_equals = n.iter().any(|module_item| {
			matches!(module_item, ModuleItem::ModuleDecl(ModuleDecl::TsImportEquals(..)))
		});

		// TS1235: A namespace declaration is only allowed at the top level of a
		// namespace or module.
		let has_ts_module = n.iter().any(|module_item| {
			matches!(module_item, ModuleItem::Stmt(Stmt::Decl(Decl::TsModule(..))))
		});
