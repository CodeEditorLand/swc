...
impl Visit for Transform {
	...
	fn visit_expr(&mut self, node:&Expr) { maybe_grow_default(|| node.visit_children_with(self)); }
}
...
impl VisitMut for Transform {
	...
	fn visit_mut_program(&mut self, node:&mut Program) {
		...
		if !self.exported_binding.is_empty() {
			self.ref_rewriter = Some(RefRewriter {
				query:ExportQuery { export_name:self.exported_binding.clone() },
			});
		}
	...
	fn visit_mut_module(&mut self, node:&mut Module) {
		...
		if !self.export_var_list.is_empty() {
			...
			node.body.push(
				ExportDecl { decl:VarDecl { decls, ..Default::default() }.into(), span:DUMMY_SP }
					.into(),
			)
		}
	}
	...
	fn visit_mut_constructor(&mut self, node:&mut Constructor) {
		node.params.iter_mut().for_each(|param_or_ts_param_prop| {
			match param_or_ts_param_prop {
				ParamOrTsParamProp::TsParamProp(ts_param_prop) => {
					...
					*param_or_ts_param_prop =
						Param { span:*span, decorators:decorators.take(), pat }.into();
				},
	...
	fn visit_mut_module_item(&mut self, node:&mut ModuleItem) {
		...
		if let Some(ExportDecl { decl, .. }) =
			node.as_mut_module_decl().and_then(ModuleDecl::as_mut_export_decl)
		{
...
impl InitArg<'_> {
	...
	fn empty() -> ExprOrSpread { ExprOrSpread { spread:None, expr:ObjectLit::default().into() } }
	...
	fn or_assign_empty(&self) -> ExprOrSpread {
		...
		let bin =
			BinExpr { op:op!("||"), left:expr.into(), right:assign.into(), ..Default::default() };
	...
	fn transform_ts_enum(
		...
		let body = if !iife {
			let return_stmt:Stmt =
				ReturnStmt { arg:Some(id.clone().into()), ..Default::default() }.into();
		...
		let init_arg = 'init_arg: {
			let init_arg =
				InitArg { id:&id, namespace_id:self.namespace_id.as_ref().filter(|_| is_export) };
		...
		if iife {
			...
			let var_declarator =
				VarDeclarator { span, name:id.into(), init:Some(expr.into()), definite:false };

			FoldedDecl::Decl(
				VarDecl { span, kind:var_kind, decls:vec![var_declarator], ..Default::default() }
					.into(),
			)
		}
	}
...
impl Transform {
	fn transform_ts_module(&self, ts_module:TsModuleDecl, is_export:bool) -> FoldedDecl {
		...
		let TsModuleDecl { span, id: TsModuleName::Ident(module_ident), body: Some(body), .. } =
			ts_module
		else {
			unreachable!();
		};
		...
		for module_item in &mut node.body {
			match module_item {
				...
				ModuleItem::ModuleDecl(ModuleDecl::TsExportAssignment(..)) => {
					let ts_export_assign =
						module_item.take().module_decl().unwrap().ts_export_assignment().unwrap();
...
impl QueryRef for ExportQuery {
	...
	fn query_lhs(&self, ident:&Ident) -> Option<Box<Expr>> { self.query_ref(ident) }
...
impl EnumMemberItem {
	fn is_const(&self) -> bool { self.value.is_const() }
	...
	fn build_assign(self, enum_id:&Id) -> Stmt {
		...
		let inner_assign = value.make_assign_to(
			op!("="),
			Ident::from(enum_id.clone()).computed_member(self.name.clone()).into(),
		);
		...
		let outer_assign = if is_string {
			...
			value.make_assign_to(
				op!("="),
				Ident::from(enum_id.clone()).computed_member(inner_assign).into(),
			)
		};
