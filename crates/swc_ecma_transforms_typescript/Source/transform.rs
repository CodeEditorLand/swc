...
use crate::{
	...
	utils::{
		assign_value_to_this_private_prop,
		assign_value_to_this_prop,
		stmt_is_enum,
		AsCollapsibleDecl,
		AsEnumOrModule,
		Factory,
...
impl VisitMut for Transform {
	...
	fn visit_mut_constructor(&mut self, n:&mut Constructor) {
		n.params.iter_mut().for_each(|param_or_ts_param_prop| {
	...
	fn visit_mut_expr(&mut self, n:&mut Expr) {
		...
		if !decls.is_empty() {
			...
			*n = SeqExpr { span:DUMMY_SP, exprs }.into();
	...
	fn visit_mut_if_stmt(&mut self, node:&mut IfStmt) {
		if stmt_is_enum(&node.cons) {
			node.cons = BlockStmt { stmts:vec![*node.cons.take()], ..Default::default() }.into();
		}

		if let Some(ref mut alt) = node.alt {
			if stmt_is_enum(alt) {
				*alt = BlockStmt { stmts:vec![*alt.take()], ..Default::default() }.into();
			}
		}
	...
	fn visit_mut_for_of_stmt(&mut self, node:&mut ForOfStmt) {
		if stmt_is_enum(&node.body) {
			node.body = BlockStmt { stmts:vec![*node.body.take()], ..Default::default() }.into();
		}
		node.visit_mut_children_with(self);
	}
...
impl Transform {
	...
	fn visit_mut_module_items_for_enum_or_module(&mut self, n:&mut Vec<ModuleItem>) {
		let found = (**n).iter().any(|module_item| module_item.is_enum_or_module());
		...
		for mut module_item in n.take().drain(..) {
			if let Some(id) = &module_item.get_enum_or_module_id() {
				...
				let var_stmt = if self.namespace_id.is_none()
					&& matches!(module_item, ModuleItem::ModuleDecl(ModuleDecl::ExportDecl(..)))
				{
					// export var foo;
					var_decl
						.map(Box::new)
						.map(Decl::Var)
						.map(|decl| ExportDecl { span:DUMMY_SP, decl })
			...
			let is_export = id.as_ref().map(|id| export_names.contains(id)).unwrap_or_default();
...
impl Transform {
	fn transform_ts_enum(&mut self, ts_enum:TsEnumDecl, is_export:bool) -> Stmt {
		...
		for m in members {
			...
			default_init = value.inc();
			{
				...
				let value = if value.is_const() { value.clone() } else { TsEnumRecordValue::Void };
		...
		let expr = Self::wrap_enum_or_module_with_iife(
			&self.namespace_id,
			id,
			BlockStmt { span:DUMMY_SP, stmts, ..Default::default() },
			is_export,
		);
	...
	fn transform_ts_enum_member(
		...
		member
			.init
			.map(|expr| EnumValueComputer { enum_id, unresolved_mark, record }.compute(expr))
...
impl Transform {
	...
	fn transform_export_var_decl_in_ts_module_block(
		...
		var_decl.decls.visit_mut_with(&mut ExportedPatRewriter { id:id.clone().into() });
		...
		let expr = if expr_list.len() == 1 {
			...
			SeqExpr { span:DUMMY_SP, exprs:expr_list }.into()
		};
...
impl Transform {
	...
	fn add_var_for_enum_or_module_declaration(
		...
		let kind = if id.1 != self.top_level_ctxt { VarDeclKind::Let } else { VarDeclKind::Var };
		...
		BinExpr {
			...
			right:Box::new(
				AssignExpr {
					...
					right:Box::new(ObjectLit { span:DUMMY_SP, props:Vec::new() }.into()),
				}
				.into(),
			),
		}
		...
		for item in n {
			match item {
				ModuleItem::ModuleDecl(ModuleDecl::ExportNamed(NamedExport {
					...
					names.extend(specifiers.iter().map(|s| {
						match s {
...
impl VisitMut for ExportedPatRewriter {
	...
	fn visit_mut_var_declarator(&mut self, n:&mut VarDeclarator) {
		...
		n.init = Some(right.make_assign_to(op!("="), left.try_into().unwrap()).into());
	}

	fn visit_mut_pat(&mut self, n:&mut Pat) {
		if let Pat::Ident(bid) = n {
			*n = Pat::Expr(self.id.clone().make_member(IdentName::from(take(bid))).into());
			return;
		}
	...
	fn visit_mut_object_pat_prop(&mut self, n:&mut ObjectPatProp) {
		if let ObjectPatProp::Assign(AssignPatProp { key, value, .. }) = n {
			...
			let value = if let Some(right) = value.take() {
				AssignPat { span:DUMMY_SP, left:left.into(), right }.into()
			} else {
				left
			};
...
impl QueryRef for ExportQuery {
	fn query_ref(&self, ident:&Ident) -> Option<Box<Expr>> {
		self.export_id_list
			...
			.then(|| self.namesapce_id.clone().make_member(ident.clone().into()).into())
	}
...
where
	...
	let mut v = RefRewriter { query:ExportQuery { namesapce_id, export_id_list } };
