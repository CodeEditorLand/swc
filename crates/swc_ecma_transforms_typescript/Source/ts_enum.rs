...
impl TsEnumRecordValue {
	...
	pub fn is_string(&self) -> bool { matches!(self, TsEnumRecordValue::String(..)) }

	pub fn has_value(&self) -> bool { !matches!(self, TsEnumRecordValue::Void) }
}
	...
	fn from(value:TsEnumRecordValue) -> Self {
		match value {
			...
			TsEnumRecordValue::Number(num) if num.is_infinite() => {
				let value:Expr =
					Ident { span:DUMMY_SP, sym:"Infinity".into(), ..Default::default() }.into();
			...
			TsEnumRecordValue::Number(num) => {
				Lit::Num(Number { span:DUMMY_SP, value:*num, raw:None }).into()
			},
...
impl InlineEnum {
	pub fn new(record:TsEnumRecord) -> Self { Self { record, is_lhs:false } }
}
