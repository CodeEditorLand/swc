...
use swc_common::{DUMMY_SP, FileName, FilePathMapping, Mark, SourceMap, SyntaxContext};
use swc_ecma_ast::*;
...
fn parse(code:&str) -> Vec<Stmt> {
	...
	let fm = cm.new_source_file(FileName::Custom(stringify!($name).into()).into(), code.into());
...
impl Helpers {
	pub fn new(external:bool) -> Self {
		Helpers { external, mark:Default::default(), inner:Default::default() }
	}

	pub const fn mark(&self) -> Mark { self.mark.0 }

	pub const fn external(&self) -> bool { self.external }

	pub fn data(&self) -> HelperData {
		HelperData { inner:*self.inner.borrow(), external:self.external, mark:self.mark }
	}

	pub fn from_data(data:HelperData) -> Self {
		Helpers { external:data.external, mark:data.mark, inner:RefCell::new(data.inner) }
	}
}
...
impl Default for HelperMark {
	fn default() -> Self { HelperMark(Mark::new()) }
}
...
define_helpers!(Helpers {
	...
	class_private_field_update:(class_extract_field_descriptor, class_apply_descriptor_update),
	...
	wrap_native_super:(construct, get_prototype_of, set_prototype_of, is_native_function),
	write_only_error:(),
...
pub fn inject_helpers(global_mark:Mark) -> impl Pass + VisitMut {
	visit_mut_pass(InjectHelpers { global_mark, helper_ctxt:None })
}
...
impl InjectHelpers {
	fn make_helpers_for_module(&mut self) -> Vec<ModuleItem> {
		...
		if external {
			...
			self.build_helpers().into_iter().map(ModuleItem::Stmt).collect()
		}
	}
	...
	fn build_reqire(&self, name:&str, mark:Mark) -> Stmt {
		let c = CallExpr {
			...
			.as_callee(),
			args:vec![
				Str { span:DUMMY_SP, value:format!("@swc/helpers/_/_{}", name).into(), raw:None }
					.as_arg(),
			],
			..Default::default()
		};
	...
	fn map_helper_ref_ident(&mut self, ref_ident:&Ident) -> Option<Expr> {
		self.helper_ctxt.filter(|ctxt| ctxt == &ref_ident.ctxt).map(|_| {
...
impl VisitMut for Marker {
	...
	fn visit_mut_fn_decl(&mut self, n:&mut FnDecl) {
		let old_decl_ctxt =
			replace(&mut self.decl_ctxt, SyntaxContext::empty().apply_mark(Mark::new()));
...
_throw();",
				...
				assert_eq!(DebugUsingDisplay(&actual_src), DebugUsingDisplay(&expected_src));
