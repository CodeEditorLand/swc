...
/// value of [Config].
pub fn hygiene() -> impl Pass + VisitMut { hygiene_with_config(Default::default()) }
...
pub fn hygiene_with_config(config:Config) -> impl 'static + Pass + VisitMut {
	(renamer(config, HygieneRenamer), visit_mut_pass(HygieneRemover))
}
...
impl Renamer for HygieneRenamer {
	...
	fn new_name_for(&self, orig:&Id, n:&mut usize) -> swc_atoms::JsWord {
		let res = if *n == 0 { orig.0.clone() } else { format!("{}{}", orig.0, n).into() };
		*n += 1;
...
impl VisitMut for HygieneRemover {
	...
	fn visit_mut_ident(&mut self, i:&mut Ident) { i.ctxt = Default::default(); }
}
