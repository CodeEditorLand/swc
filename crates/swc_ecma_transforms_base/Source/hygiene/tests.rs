...
use swc_common::{DUMMY_SP, collections::AHashMap, hygiene::*};
...
fn marker(markers:&[(&str, Mark)]) -> Marker {
	Marker { map:markers.iter().map(|(k, v)| ((*k).into(), *v)).collect() }
}
...
impl OnceMarker {
	fn new(markers:&[(&str, &[Mark])]) -> OnceMarker {
		OnceMarker { map:markers.iter().map(|(k, v)| ((*k).into(), (*v).into())).collect() }
	}
}
...
where
	...
	crate::tests::Tester::run(|tester| {
		...
		let expected = {
			let expected = tester
				.with_parser("expected.js", Syntax::default(), expected, |p| p.parse_module())
				.map(Program::Module)?;
...
fn var_class_decl() {
	test(
		|tester| Ok(vec![tester.parse_stmt("actual1.js", "var Foo = function Foo(){}")?]),
		...
		|_tester| {
			...
			Ok(vec![Stmt::Decl(Decl::Fn(FnDecl {
				...
				function:Box::new(Function {
					...
					is_generator:false,
					decorators:Vec::new(),
					body:Some(BlockStmt { ..Default::default() }),
					params:vec![
						Param {
							span:DUMMY_SP,
	...
	test_module(
		|tester| {
			...
			Ok(tester
				...
				.fold_with(&mut OnceMarker::new(&[("e", &[mark1, mark2, mark3, mark2])])))
		},
		"function foo(e1) {
            e: {
                try {
	...
	test(
		|tester| {
			...
			let stmts = tester
				...
				.fold_with(&mut OnceMarker::new(&[("foo", &[mark1, mark2, mark1, mark2, mark1])]));
	...
	test(
		|tester| {
			...
			let stmts = tester
				...
				.fold_with(&mut OnceMarker::new(&[("_bar", &[mark1, mark2, mark1, mark2])]));
	...
	test(
		|tester| {
			...
			let stmts = tester
				...
				.fold_with(&mut OnceMarker::new(&[("_bar", &[mark1, mark2, mark2, mark1])]));
