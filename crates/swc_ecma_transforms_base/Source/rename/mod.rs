...
use self::{
	analyzer::Analyzer,
	collector::{CustomBindingCollector, IdCollector, collect_decls},
	eval::contains_eval,
	ops::Operator,
};
...
pub trait Renamer: Send + Sync {
	...
	fn preserved_ids_for_module(&mut self, _:&Module) -> FxHashSet<Id> { Default::default() }

	fn preserved_ids_for_script(&mut self, _:&Script) -> FxHashSet<Id> { Default::default() }

	fn get_cached(&self) -> Option<Cow<RenameMap>> { None }
...
pub fn rename_with_config(map:&RenameMap, config:Config) -> impl '_ + Pass + VisitMut {
	visit_mut_pass(Operator { rename:map, config, extra:Default::default() })
}
	...
	R: Renamer,
{
	...
	where
		...
		let usages = {
			let mut v = IdCollector { ids:Default::default() };
		...
		let (decls, preserved) =
			collect_decls(n, if has_eval { Some(self.config.top_level_mark) } else { None });
	...
	where
		...
		if !self.preserved.is_empty() {
			unresolved.to_mut().extend(self.preserved.iter().map(|v| v.0.clone()));
		}

		if !self.config.preserved_symbols.is_empty() {
			unresolved.to_mut().extend(self.config.preserved_symbols.iter().cloned());
		}
