...
impl Analyzer {
	...
	fn add_usage(&mut self, id:Id) { self.scope.add_usage(id); }

	fn reserve_usage(&mut self, len:usize) { self.scope.reserve_usage(len); }
	...
	where
		F: FnOnce(&mut Analyzer), {
		{
			let mut v = Analyzer {
				...
				scope:Scope { kind, ..Default::default() },
			...
			if !v.hoisted_vars.is_empty() {
				...
				match self.scope.kind {
					ScopeKind::Fn => {
						...
						v.hoisted_vars.into_iter().for_each(|id| self.add_decl(id, true));
					},
					ScopeKind::Block => {
...
impl Visit for Analyzer {
	...
	fn visit_fn_decl(&mut self, f:&FnDecl) {
		...
		let has_rest = f.function.params.iter().any(|p| p.pat.is_rest() || p.pat.is_assign());
		if has_rest {
		...
		if let Some(id) = &f.ident {
			self.with_fn_scope(|v| {
				...
				v.with_fn_scope(|v| {
					...
					// We need to check for assign pattern because safari has a bug.
					// https://github.com/swc-project/swc/issues/9015
					if f.function.params.iter().any(|p| p.pat.is_rest() || p.pat.is_assign()) {
