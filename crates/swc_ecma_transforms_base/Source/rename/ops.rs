	...
	I: IdentLike,
{
	fn keep_class_name(&mut self, ident:&mut Ident, class:&mut Class) -> Option<ClassExpr> {
...

		{
			...
			let mut operator =
				Operator { rename:&rename, config:self.config.clone(), extra:Default::default() };
		...
		let class_expr = ClassExpr { ident:Some(orig_name), class:Box::new(class.take()) };
...
{
	fn create(&self) -> Self {
		Self { rename:self.rename, config:self.config.clone(), extra:Default::default() }
	}

	fn merge(&mut self, other:Self) { self.extra.extend(other.extra); }

	fn after_module_items(&mut self, stmts:&mut Vec<ModuleItem>) { stmts.append(&mut self.extra); }
}
	...
	fn visit_mut_module_item(&mut self, item:&mut ModuleItem) {
		...
		macro_rules! export {
			($orig:expr, $ident:expr) => {
				self.extra.push(ModuleItem::ModuleDecl(ModuleDecl::ExportNamed(NamedExport {
					span,
		...
		match item {
			ModuleItem::ModuleDecl(ModuleDecl::ExportDecl(ExportDecl {
				span,
				decl: Decl::Class(ClassDecl { ident, class, declare }),
			})) => {
				...
				match self.rename_ident(&mut ident) {
					Ok(..) => {
						*item =
							ClassDecl { ident:ident.clone(), class:class.take(), declare:*declare }
								.into();
			...
			ModuleItem::ModuleDecl(ModuleDecl::ExportDecl(ExportDecl {
				span,
				decl: Decl::Fn(FnDecl { ident, function, declare }),
			})) => {
				let mut ident = ident.take();
				...
				match self.rename_ident(&mut ident) {
					Ok(..) => {
						*item = FnDecl { ident:ident.clone(), function, declare:*declare }.into();
					...
					Err(..) => {
						*item = ExportDecl {
							span:*span,
							decl:FnDecl { ident, function, declare:*declare }.into(),
						}
						.into()
					},
				}
			...
			ModuleItem::ModuleDecl(ModuleDecl::ExportDecl(ExportDecl {
				...
				let decls = decls.move_map(|mut decl| {
					decl.name.visit_mut_with(&mut VarFolder { orig:self, renamed:&mut renamed });
				...
				if renamed.is_empty() {
					*item =
						ExportDecl { span, decl:VarDecl { decls, ..*var.take() }.into() }.into();

					return;
				}
				*item = VarDecl { decls, ..*var.take() }.into();

				self.extra.push(
					NamedExport { span, specifiers:renamed, src:None, type_only:false, with:None }
						.into(),
				);
			},
			...
			if self.rename_ident(&mut renamed).is_ok() {
				...
				*n = KeyValuePatProp {
					...
					value:match p.value.take() {
						Some(default_expr) => {
							AssignPat { span:p.span, left:renamed.into(), right:default_expr }
								.into()
						},
						None => renamed.into(),
					},
	...
	I: IdentLike,
{
	...
	fn visit_mut_ident(&mut self, i:&mut Ident) {
		...
		if self.orig.rename_ident(i).is_ok() {
			self.renamed.push(ExportSpecifier::Named(ExportNamedSpecifier {
				span:i.span,
