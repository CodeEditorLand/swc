...
pub fn resolver(
	...
	visit_mut_pass(Resolver {
		...
		strict_mode:false,
		config:InnerConfig { handle_types:typescript, unresolved_mark, top_level_mark },
	})
}
...
impl<'a> Scope<'a> {
	...
	fn is_declared(&self, symbol:&JsWord) -> Option<&DeclKind> {
		self.declared_symbols.get(symbol).or_else(|| self.parent?.is_declared(symbol))
	}
}
...
impl<'a> Resolver<'a> {
	...
	where
		...
		let mut child = Resolver {
			current:Scope::new(kind, Mark::fresh(self.config.top_level_mark), Some(&self.current)),
			ident_type:IdentType::Ref,
			config:self.config,
	...
	fn mark_for_ref(&self, sym:&JsWord) -> Option<Mark> { self.mark_for_ref_inner(sym, false) }
	...
	fn modify(&mut self, id:&mut Ident, kind:DeclKind) {
		if cfg!(debug_assertions) && LOG {
			debug!("Binding (type = {}) {}{:?} {:?}", self.in_type, id.sym, id.ctxt, kind);
		}
...
impl VisitMut for Resolver<'_> {
	...
	typed_ref_init!(visit_mut_ts_type_param_instantiation, TsTypeParamInstantiation);
	...
	typed!(visit_mut_ts_union_or_intersection_type, TsUnionOrIntersectionType);
			...
			match &mut *e.body {
				BlockStmtOrExpr::BlockStmt(s) => {
					...
					if !child.strict_mode {
						child.strict_mode =
							s.stmts.first().map(|stmt| stmt.is_use_strict()).unwrap_or(false);
					}
	...
	fn visit_mut_expr(&mut self, expr:&mut Expr) {
		let _span = if LOG { Some(span!(Level::ERROR, "visit_mut_expr").entered()) } else { None };
	...
	fn visit_mut_function(&mut self, f:&mut Function) {
		...
		if let Some(body) = &mut f.body {
			...
			if !self.strict_mode {
				self.strict_mode =
					body.stmts.first().map(|stmt| stmt.is_use_strict()).unwrap_or(false);
			}
			// Prevent creating new scope.
	...
	fn visit_mut_pat(&mut self, p:&mut Pat) { p.visit_mut_children_with(self); }
	...
	fn visit_mut_script(&mut self, script:&mut Script) {
		self.strict_mode = script.body.first().map(|stmt| stmt.is_use_strict()).unwrap_or(false);
	...
	fn visit_mut_stmts(&mut self, stmts:&mut Vec<Stmt>) {
		let _span = if LOG { Some(span!(Level::ERROR, "visit_mut_stmts").entered()) } else { None };

		// Phase 1: Handle hoisting
		{
			let _span = if LOG { Some(span!(Level::ERROR, "hoist").entered()) } else { None };
	...
	fn visit_mut_ts_enum_decl(&mut self, decl:&mut TsEnumDecl) {
		...
		self.with_child(ScopeKind::Block, |child| {
			...
			// Ex. `enum Foo { a, b = a }`
			let member_names = decl.members.iter().filter_map(|m| {
				match &m.id {
...
impl VisitMut for Hoister<'_, '_> {
	...
	fn visit_mut_catch_clause(&mut self, c:&mut CatchClause) {
		...
		self.catch_param_decls.extend(params.into_iter().map(|v| v.0));
	...
	fn visit_mut_class_decl(&mut self, node:&mut ClassDecl) {
		...
		if self.resolver.config.handle_types {
			self.resolver.current.declared_types.insert(node.ident.sym.clone());
		}
	}
		...
		if self.resolver.config.handle_types {
			match decl {
				...
				Decl::TsModule(v)
					if matches!(
						&**v,
						TsModuleDecl { global:false, id:TsModuleName::Ident(_), .. },
					) =>
				{
					if !self.in_block {
						let old_in_type = self.resolver.in_type;
	...
	fn visit_mut_import_default_specifier(&mut self, n:&mut ImportDefaultSpecifier) {
		...
		if self.resolver.config.handle_types {
			self.resolver.current.declared_types.insert(n.local.sym.clone());
		}
	}
	...
	fn visit_mut_var_decl_or_expr(&mut self, n:&mut VarDeclOrExpr) {
		match n {
			VarDeclOrExpr::VarDecl(v)
				if matches!(&**v, VarDecl { kind:VarDeclKind::Let | VarDeclKind::Const, .. }) => {},
