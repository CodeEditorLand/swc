...
use swc_common::{DUMMY_SP, SourceMap, SyntaxContext, collections::AHashMap};
use swc_ecma_ast::*;
...
impl Modules {
	...
	pub fn add_dep(&mut self, dep:Modules) { self.push_all(dep) }
	...
	pub fn iter(&self) -> impl Iterator<Item = (ModuleId, &ModuleItem)> {
		self.prepended_stmts
			...
			.chain(self.modules.iter().flat_map(|(id, m)| m.body.iter().map(move |v| (*id, v))))
			.chain(
				self.appended_stmts
					.iter()
					.flat_map(|(id, stmts)| stmts.iter().map(move |stmt| (*id, stmt))),
	...
	where
		...
		self.prepended_stmts = p.into_iter().map(|(id, items)| (id, op(id, items))).collect();
		...
		self.appended_stmts = a.into_iter().map(|(id, items)| (id, op(id, items))).collect();
	}
	...
	pub fn prepend(&mut self, module_id:ModuleId, item:ModuleItem) {
		self.prepended_stmts.entry(module_id).or_default().push(item);
	}
	...
	where
		...
		rayon::scope(|s| {
			s.spawn(|_| {
				pre.par_iter_mut().for_each(|(_, stmts)| stmts.visit_mut_with(&mut v.clone()));
			});
			...
			s.spawn(|_| {
				app.par_iter_mut().for_each(|(_, stmts)| stmts.visit_mut_with(&mut v.clone()));
			});
		});
	}
	...
	where
		...
		self.modules = self.modules.into_iter().map(|m| (m.0, m.1.fold_with(&mut *v))).collect();
	...
	pub(crate) fn print(&self, cm:&swc_common::sync::Lrc<SourceMap>, event:impl std::fmt::Display) {
		let files = self
			.modules
			.iter()
			.map(|(_, m)| m.span)
		...
		for (id, mut module) in cloned.modules {
			swc_ecma_utils::prepend_stmts(
				&mut module.body,
				cloned.prepended_stmts.get(&id).cloned().unwrap_or_default().into_iter(),
			);

			module.body.extend(cloned.appended_stmts.get(&id).cloned().unwrap_or_default());
		...
		crate::debug::print_hygiene(
			&format!("{}\n{}", event, files),
			cm,
			&Module { span:DUMMY_SP, body:stmts, shebang:None },
		);
	}
}
...
impl From<Modules> for Module {
	fn from(modules:Modules) -> Self {
		// TODO
		Self { span:DUMMY_SP, body:modules.into_items(), shebang:None }
	}
}
