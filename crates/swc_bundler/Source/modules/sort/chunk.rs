...
impl Modules {
	...
	pub(super) fn take_chunks(
		...
		chunks.extend(toposort_real_modules(injected_ctxt, modules, entry_id, graph, cycle, cm));
	...
	for ids in sorted_ids {
		...
		for id in ids.iter().copied().rev() {
			if let Some((_, module)) = modules.iter_mut().find(|(module_id, _)| *module_id == id) {
				module.body.retain(|item| !matches!(item, ModuleItem::Stmt(Stmt::Empty(..))));
		...
		if ids.len() == 1 && graph.neighbors_directed(ids[0], Outgoing).count() == 0 {
			chunks.push(Chunk { stmts:stmts.into_iter().next().unwrap() });
