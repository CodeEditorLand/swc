	...
	R: Resolve,
{
	...
	pub(super) fn resolve(
		...
		self.run(|| {
			let path =
				self.resolver.resolve(base, module_specifier).map(|v| v.filename).with_context(
				...
				import.specifiers.iter().any(|specifier| {
					match specifier {
						ImportSpecifier::Namespace(ns) => {
							ns.local.sym == id.0 && ns.local.ctxt == id.1
						},
						_ => false,
					}
				})
	...
	fn analyze_usage(&mut self, e:&mut Expr) {
		if let Expr::Member(e) = e {
			if let Expr::Ident(obj) = &*e.obj {
				...
				self.usages.entry(obj.to_id()).or_default().push(prop.to_id());
			}
		}
	}
		...
		if self.deglob_phase {
			...
			for import in self.info.imports.iter_mut() {
				let use_ns = self.info.forced_ns.contains(&import.src.value)
					|| self.bundler.config.external_modules.contains(&import.src.value);

				if use_ns {
					wrapping_required.push(import.src.value.clone());
				} else {
					// De-glob namespace imports
					import.specifiers.retain(|s| !matches!(s, ImportSpecifier::Namespace(_)));
				}
			}
