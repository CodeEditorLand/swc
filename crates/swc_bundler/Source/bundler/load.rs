...
impl TransformedModule {
	...
	pub fn export_ctxt(&self) -> SyntaxContext { self.export_ctxt }

	/// Top level contexts.
	pub fn local_ctxt(&self) -> SyntaxContext { self.local_ctxt }
}
	...
	R: Resolve,
{
	...
	pub(super) fn load_transformed(
		...
		self.run(|| {
			...
			let (v, mut files) =
				self.analyze(file_name, data).context("failed to analyze module")?;
	...
	fn load(&self, file_name:&FileName) -> Result<(ModuleId, ModuleData), Error> {
		self.run(|| {
			let (module_id, ..) = self.scope.module_id_gen.gen(file_name);
	...
	fn analyze(
		...
		self.run(|| {
			...
			let mut module =
				data.module.fold_with(&mut resolver(self.unresolved_mark, local_mark, false));
			...
			let is_es6 = if !self.config.require {
				...
				let mut v = Es6ModuleDetector { forced_es6:false, found_other:false };
	...
	fn resolve_imports(
		...
		self.run(|| {
			...
			let RawImports { imports, lazy_imports, dynamic_imports, forced_ns } = info;
			...
			let loaded = imports
				...
				.map(|(decl, dynamic, unconditional)| -> Result<_, Error> {
					self.run(|| {
						...
						Ok((id, local_mark, export_mark, file_name, decl, dynamic, unconditional))
					})
				})
				.collect::<Vec<_>>();
...
impl VisitMut for ClearMark {
	...
	fn visit_mut_ident(&mut self, ident:&mut Ident) { ident.ctxt = SyntaxContext::empty(); }
}
