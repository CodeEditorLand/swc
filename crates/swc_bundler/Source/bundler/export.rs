...
use swc_common::{FileName, SyntaxContext, collections::ARandomState};
use swc_ecma_ast::*;
	...
	R: Resolve,
{
	...
	pub(super) fn extract_export_info(
		...
		self.run(|| {
			let mut v =
				ExportFinder { info:Default::default(), file_name, bundler:self, export_ctxt };
	...
	R: Resolve,
{
	...
	fn ctxt_for(&self, src:&JsWord) -> Option<(SyntaxContext, SyntaxContext)> {
		...
		if self.bundler.config.external_modules.iter().any(|v| v == src) {
			return None;
		}
	...
	fn mark_as_wrapping_required(&self, src:&JsWord) {
		...
		let (id, ..) = self.bundler.scope.module_id_gen.gen(&path);
	...
	fn visit_mut_module_item(&mut self, item:&mut ModuleItem) {
		match item {
				...
				v.push({
					let i = match decl.decl {
						...
						Decl::Var(ref var) => {
							...
							for id in ids {
								v.push(Specifier::Specific { local:id, alias:None });
							}
					...
					Specifier::Specific { local:i.into(), alias:None }
				});
			},

			ModuleItem::ModuleDecl(ModuleDecl::ExportDefaultDecl(_decl)) => {
				self.info.items.entry(None).or_default().push(Specifier::Specific {
			...
			ModuleItem::ModuleDecl(ModuleDecl::ExportNamed(named)) => {
				let ctxt = named.src.as_ref().map(|s| &s.value).and_then(|src| self.ctxt_for(src));

				let mut need_wrapping = false;

				let v = self.info.items.entry(named.src.clone().map(|v| *v)).or_default();
			...
			ModuleItem::ModuleDecl(ModuleDecl::ExportAll(all)) => {
				...
				if let Some((_, export_ctxt)) = ctxt {
					ExportMetadata { export_ctxt:Some(export_ctxt), ..Default::default() }
						.encode(&mut all.with);
				}
