...
use swc_common::{FileName, GLOBALS, SourceMap, Span, collections::ARandomState, sync::Lrc};
use swc_ecma_ast::*;
...
impl Load for Loader {
	fn load(&self, f:&FileName) -> Result<ModuleData, Error> {
		...
		let lexer =
			Lexer::new(Default::default(), EsVersion::Es2020, StringInput::from(&*fm), None);
...
impl Resolve for Resolver {
	fn resolve(&self, _:&FileName, s:&str) -> Result<Resolution, Error> {
		...
		Ok(Resolution { filename:FileName::Real(path), slug:None })
	}
}
...
impl Tester<'_> {
	...
	pub fn parse(&self, s:&str) -> Module {
		...
		let lexer =
			Lexer::new(Default::default(), Default::default(), StringInput::from(&*fm), None);
...
pub(crate) fn suite() -> TestBuilder { TestBuilder::default() }
		...
		testing::run_test2(true, |cm, _| {
			GLOBALS.with(|globals| {
				let bundler = Bundler::new(
					globals,
					cm.clone(),
					Loader { cm:cm.clone(), files:self.files.clone() },
					Default::default(),
					Config {
						require:true,
				...
				for (name, _) in self.files {
					bundler.load_transformed(&FileName::Real(name.clone().into())).unwrap();
				}

				let mut t = Tester { cm:cm.clone(), bundler };
