...
use swc_common::{DUMMY_SP, FileName, Mark, collections::AHashMap, util::move_map::MoveMap};
use swc_ecma_ast::*;
	...
	R: Resolve,
{
	...
	pub(super) fn finalize(
		...
		self.run(|| {
			...
			for mut bundle in bundles {
...

				{
					...
					HELPERS.set(&Helpers::from_data(swc_helpers), || {
						bundle.module.visit_mut_with(&mut inject_helpers(unresolved_mark));
					});
				}
				...
				match bundle.kind {
					...
					BundleKind::Lib { name } => {
						...
						new.push(Bundle {
							kind:BundleKind::Named { name:file_name.display().to_string() },
							..bundle
						})
					},
			...
			new = new.move_map(|bundle| {
				...
				let module = {
					// Change imports
					let mut v = Renamer { resolver:&self.resolver, base:&path, renamed:&renamed };
	...
	fn may_wrap_with_iife(&self, module:Module) -> Module {
						...
						ModuleDecl::ExportDefaultDecl(export) => {
							match export.decl {
								DefaultDecl::Class(expr) => {
									...
									Some(
										ClassDecl { ident, class:expr.class, declare:false }.into(),
									)
								},
								...
								DefaultDecl::Fn(expr) => {
									...
									Some(
										FnDecl { ident, function:expr.function, declare:false }
											.into(),
									)
								},
		...
		body.stmts.push(
			ReturnStmt { span:DUMMY_SP, arg:Some(ObjectLit { span:DUMMY_SP, props }.into()) }
				.into(),
		);
		...
		let invoked_fn_expr = FnExpr { ident:None, function:Box::new(f) };
		...
		Module {
			span:DUMMY_SP,
			shebang:None,
			body:vec![ModuleItem::Stmt(Stmt::Expr(ExprStmt { span:DUMMY_SP, expr:iife }))],
		}
	}
}
	...
	R: Resolve,
{
	...
	fn fold_import_decl(&mut self, import:ImportDecl) -> ImportDecl {
		let resolved =
			match self.resolver.resolve(&FileName::Real(self.base.clone()), &import.src.value) {
				Ok(v) => {
					match v.filename {
						FileName::Real(v) => v,
		...
		if let Some(v) = self.renamed.get(&resolved) {
			...
			// entry-a.js
			// common.js
			let base = self.base.parent().unwrap_or(self.base).as_os_str().to_string_lossy();
