...
use swc_common::{DUMMY_SP, FileName, Mark, SyntaxContext, collections::AHashMap, sync::Lock};
...
impl Id {
	pub fn new(sym:JsWord, ctxt:SyntaxContext) -> Self { Id(sym, ctxt) }

	pub fn sym(&self) -> &JsWord { &self.0 }

	pub fn ctxt(&self) -> SyntaxContext { self.1 }

	pub fn into_ident(self) -> Ident { Ident::new(self.0, DUMMY_SP, self.1) }
...
impl IdentLike for Id {
	fn from_ident(i:&Ident) -> Self { i.into() }

	fn to_id(&self) -> (JsWord, SyntaxContext) { (self.0.clone(), self.1) }

	fn into_id(self) -> (JsWord, SyntaxContext) { (self.0, self.1) }
}

impl From<Ident> for Id {
	fn from(i:Ident) -> Self { Id(i.sym, i.ctxt) }
}

impl From<&Ident> for Id {
	fn from(i:&Ident) -> Self { Id(i.sym.clone(), i.ctxt) }
}
...
impl PartialEq<JsWord> for Id {
	fn eq(&self, other:&JsWord) -> bool { self.0 == *other }
}

impl From<Id> for Ident {
	#[inline]
	fn from(id:Id) -> Self { id.into_ident() }
}

impl From<Id> for Expr {
	#[inline]
	fn from(id:Id) -> Self { id.into_ident().into() }
}
