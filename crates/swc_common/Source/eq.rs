...
impl EqIgnoreSpan for Span {
	/// Always returns true
	#[inline]
	fn eq_ignore_span(&self, _:&Self) -> bool { true }
}

impl EqIgnoreSpan for swc_atoms::Atom {
	#[inline]
	fn eq_ignore_span(&self, r:&Self) -> bool { self == r }
}
...
impl TypeEq for Span {
	/// Always returns true
	#[inline]
	fn type_eq(&self, _:&Self) -> bool { true }
}
...
swc_allocator::nightly_only!(
	...
	{
		#[inline]
		fn type_eq(&self, other:&Self) -> bool { (**self).type_eq(&**other) }
	}
);
...
impl TypeEq for BigInt {
	fn type_eq(&self, other:&Self) -> bool { self == other }
}
