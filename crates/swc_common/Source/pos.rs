...
pub trait Spanned {
	...
	#[inline]
	fn span_lo(&self) -> BytePos { self.span().lo }

	#[inline]
	fn span_hi(&self) -> BytePos { self.span().hi }
}
	...
	T: Spanned + Clone,
{
	...
	#[inline]
	fn span_lo(&self) -> BytePos { (**self).span_lo() }

	#[inline]
	fn span_hi(&self) -> BytePos { (**self).span_hi() }
}
...
impl Spanned for BytePos {
	...
	#[inline(always)]
	fn span(&self) -> Span { Span::new(*self, *self) }
}
...
where
	S: ?Sized + Spanned,
{
	fn span(&self) -> Span { <S as Spanned>::span(self) }

	#[inline]
	fn span_lo(&self) -> BytePos { <S as Spanned>::span_lo(self) }

	#[inline]
	fn span_hi(&self) -> BytePos { <S as Spanned>::span_hi(self) }
}
