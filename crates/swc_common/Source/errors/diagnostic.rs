...
impl DiagnosticStyledString {
	pub fn new() -> DiagnosticStyledString { Default::default() }
	...
	pub fn content(&self) -> String { self.0.iter().map(|x| x.content()).collect::<String>() }
}
...
impl Diagnostic {
	...
	pub fn note_expected_found_extra(
		...
		msg.extend(expected.0.iter().map(|x| {
			match *x {
	...
	pub fn get_code(&self) -> Option<DiagnosticId> { self.code.clone() }

	pub fn message(&self) -> String {
		self.message.iter().map(|i| i.0.as_str()).collect::<String>()
	}

	pub fn styled_message(&self) -> &Vec<Message> { &self.message }
