...
impl FatalError {
	pub fn raise(self) -> ! { panic::resume_unwind(Box::new(FatalErrorMarker)) }
}

impl fmt::Display for FatalError {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result { write!(f, "parser fatal error") }
}

impl error::Error for FatalError {
	fn description(&self) -> &str { "The parser has encountered a fatal error" }
}
...
impl fmt::Display for ExplicitBug {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result { write!(f, "parser internal bug") }
}

impl error::Error for ExplicitBug {
	fn description(&self) -> &str { "The parser has encountered an internal bug" }
}
...
impl Handler {
	...
	pub fn with_emitter_writer(
		...
		Handler::with_emitter(true, false, Box::new(EmitterWriter::new(dst, cm, false, true)))
	}
	...
	pub fn cancel(&self, err:&mut DiagnosticBuilder<'_>) { err.cancel(); }
	...
	pub fn span_err<S:Into<MultiSpan>>(&self, sp:S, msg:&str) { self.emit(&sp.into(), msg, Error); }
	...
	pub fn failure(&self, msg:&str) { DiagnosticBuilder::new(self, FailureNote, msg).emit() }
	...
	pub fn unimpl(&self, msg:&str) -> ! { self.bug(&format!("unimplemented {}", msg)); }
	...
	pub fn err_count(&self) -> usize { self.err_count.load(SeqCst) }

	pub fn has_errors(&self) -> bool { self.err_count() > 0 }
...
impl Level {
	...
	pub fn is_failure_note(self) -> bool { matches!(self, FailureNote) }
}
