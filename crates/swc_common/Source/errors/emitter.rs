...
impl Emitter for EmitterWriter {
	...
	fn should_show_explain(&self) -> bool { !self.short_message }
}
...
impl EmitterWriter {
	...
	fn render_source_line(
		...
		for (i, annotation) in annotations.iter().enumerate() {
			...
			for (j, next) in annotations.iter().enumerate() {
				if j > i {
					let l = if let Some(ref label) = next.label { label.len() + 2 } else { 0 };
	...
	fn fix_multispan_in_std_macros(&mut self, span:&mut MultiSpan, always_backtrace:bool) -> bool {
