...
#[non_exhaustive]
#[cfg_attr(feature = "__plugin", derive(rkyv::Archive, rkyv::Serialize, rkyv::Deserialize))]
...
impl PluginSerializedBytes {
	...
	pub fn as_slice(&self) -> &[u8] { self.field.as_slice() }

	pub fn as_ptr(&self) -> (*const u8, usize) { (self.field.as_ptr(), self.field.len()) }
	...
	// where
	// W: rkyv::Archive,
	// W::Archived: rkyv::Deserialize<W,
	// rkyv::de::deserializers::SharedDeserializeMap>
	// + for<'a> rkyv::CheckBytes<rkyv::validation::validators::DefaultValidator<'
	//   a>>,
	// {
	// use anyhow::Context;
	//
	// let archived =
	// rkyv::check_archived_root::<VersionedSerializable<W>>(&self.field[..])
	// .map_err(|err| {
	// anyhow::format_err!("wasm plugin bytecheck failed {:?}", err.to_string())
	// })?;
	//
