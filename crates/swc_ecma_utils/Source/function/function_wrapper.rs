...
impl<T> FunctionWrapper<T> {
	...
	fn build_anonymous_expression_wrapper(&mut self) -> Expr {
		...
		let return_fn_stmt = {
			...
			ReturnStmt { span:DUMMY_SP, arg:Some(Box::new(fn_expr.into())) }
		}
		.into();

		let block_stmt =
			BlockStmt { stmts:vec![ref_decl.into(), return_fn_stmt], ..Default::default() };
		...
		FnExpr { ident:None, function }.as_iife().into()
	}
	...
	fn build_named_expression_wrapper(&mut self, name_ident:Ident) -> Expr {
		...
		let fn_decl_stmt = {
			...
			FnDecl { ident:name_ident.clone(), declare:false, function }.into()
		};

		let return_stmt =
			ReturnStmt { span:DUMMY_SP, arg:Some(Box::new(name_ident.into())) }.into();

		let block_stmt =
			BlockStmt { stmts:vec![ref_stmt, fn_decl_stmt, return_stmt], ..Default::default() };
	...
	fn build_declaration_wrapper(&mut self, name_ident:Option<Ident>) -> (FnExpr, FnDecl) {
		...
		let ref_fn_block_stmt =
			BlockStmt { stmts:vec![assign_stmt, return_ref_apply_stmt], ..Default::default() };
	...
	fn build_function_forward(&mut self, ref_ident:Ident, name_ident:Option<Ident>) -> FnExpr {
		...
		FnExpr {
			...
			function:Box::new(Function {
				params:self.params.take(),
				span:DUMMY_SP,
				body:Some(BlockStmt { stmts:vec![apply], ..Default::default() }),
				is_generator:false,
				is_async:false,
...
impl From<ArrowExpr> for FunctionWrapper<Expr> {
	fn from(ArrowExpr { span, params, body, is_async, is_generator, .. }:ArrowExpr) -> Self {
		let body = Some(match *body {
			BlockStmtOrExpr::BlockStmt(block) => block,
			BlockStmtOrExpr::Expr(expr) => {
				BlockStmt {
					stmts:vec![Stmt::Return(ReturnStmt { span:expr.span(), arg:Some(expr) })],
					..Default::default()
				}
			},
		});
		...
		let fn_expr = FnExpr { ident:None, function };
...
impl From<FnDecl> for FunctionWrapper<FnDecl> {
	fn from(mut fn_decl:FnDecl) -> Self {
		...
		Self {
			...
			ignore_function_name:false,
			ignore_function_length:false,
			function:FnExpr { ident:None, function:fn_decl.function }.into(),
			_type:Default::default(),
		}
	}
}
...
impl From<FunctionWrapper<FnDecl>> for FnWrapperResult<FnDecl, FnDecl> {
	fn from(mut value:FunctionWrapper<FnDecl>) -> Self {
		...
		FnWrapperResult { name_fn:FnDecl { ident:name_ident, declare:false, function }, ref_fn }
	}
}

impl From<FunctionWrapper<Expr>> for FnWrapperResult<FnExpr, FnDecl> {
	fn from(mut value:FunctionWrapper<Expr>) -> Self {
		let name_ident = value.function_ident.clone().or_else(|| value.binding_ident.clone());
