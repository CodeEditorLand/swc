...
impl Visit for ThisVisitor {
	...
	/// Don't recurse into fn
	fn visit_getter_prop(&mut self, n:&GetterProp) { n.key.visit_with(self); }
	...
	fn visit_this_expr(&mut self, _:&ThisExpr) { self.found = true; }
}
...
where
	...
	let mut visitor = IdentRefFinder { found:false, ident };
...
impl StmtOrModuleItem for Stmt {
	#[inline]
	fn into_stmt(self) -> Result<Stmt, ModuleDecl> { Ok(self) }

	#[inline]
	fn as_stmt(&self) -> Result<&Stmt, &ModuleDecl> { Ok(self) }

	#[inline]
	fn as_stmt_mut(&mut self) -> Result<&mut Stmt, &mut ModuleDecl> { Ok(self) }

	#[inline]
	fn from_stmt(stmt:Stmt) -> Self { stmt }

	#[inline]
	fn try_from_module_decl(decl:ModuleDecl) -> Result<Self, ModuleDecl> { Err(decl) }
}
...
impl StmtOrModuleItem for ModuleItem {
	...
	#[inline]
	fn from_stmt(stmt:Stmt) -> Self { stmt.into() }

	#[inline]
	fn try_from_module_decl(decl:ModuleDecl) -> Result<Self, ModuleDecl> { Ok(decl.into()) }
}

pub trait ModuleItemLike: StmtLike {
	fn try_into_module_decl(self) -> Result<ModuleDecl, Self> { Err(self) }
...
impl StmtLike for Stmt {
	#[inline]
	fn try_into_stmt(self) -> Result<Stmt, Self> { Ok(self) }

	#[inline]
	fn as_stmt(&self) -> Option<&Stmt> { Some(self) }

	#[inline]
	fn as_stmt_mut(&mut self) -> Option<&mut Stmt> { Some(self) }
}
...
impl IsEmpty for CatchClause {
	fn is_empty(&self) -> bool { self.body.stmts.is_empty() }
}
...
impl<T:IsEmpty> IsEmpty for Box<T> {
	#[inline]
	fn is_empty(&self) -> bool { <T as IsEmpty>::is_empty(self) }
}
...
pub trait StmtExt {
	...
	fn extract_var_ids_as_var(&self) -> Option<VarDecl> {
		...
		Some(VarDecl {
			...
			decls:ids
				.into_iter()
				.map(|i| VarDeclarator { span:i.span, name:i.into(), init:None, definite:false })
				.collect(),
			..Default::default()
		})
	}
	...
	fn may_have_side_effects(&self, ctx:&ExprCtx) -> bool {
		match self.as_stmt() {
			Stmt::Block(block_stmt) => {
				block_stmt.stmts.iter().any(|stmt| stmt.may_have_side_effects(ctx))
			},
			Stmt::Empty(_) => false,
			...
			Stmt::If(if_stmt) => {
				if_stmt.test.may_have_side_effects(ctx)
					...
					|| if_stmt.alt.as_ref().map_or(false, |stmt| stmt.may_have_side_effects(ctx))
			},

			Stmt::Switch(switch_stmt) => {
				switch_stmt.discriminant.may_have_side_effects(ctx)
					|| switch_stmt.cases.iter().any(|case| {
						case.test.as_ref().map_or(false, |expr| expr.may_have_side_effects(ctx))
							|| case.cons.iter().any(|con| con.may_have_side_effects(ctx))
					})
			},

			Stmt::Try(try_stmt) => {
				try_stmt.block.stmts.iter().any(|stmt| stmt.may_have_side_effects(ctx))
					|| try_stmt.handler.as_ref().map_or(false, |handler| {
						handler.body.stmts.iter().any(|stmt| stmt.may_have_side_effects(ctx))
					}) || try_stmt.finalizer.as_ref().map_or(false, |finalizer| {
					finalizer.stmts.iter().any(|stmt| stmt.may_have_side_effects(ctx))
				})
			},
...
impl StmtExt for Stmt {
	fn as_stmt(&self) -> &Stmt { self }

	fn extract_var_ids(&self) -> Vec<Ident> { extract_var_ids(self) }

	fn terminates(&self) -> bool {
		match self {
			Stmt::Break(_) | Stmt::Continue(_) | Stmt::Throw(_) | Stmt::Return(_) => true,
			Stmt::Block(block) => block.stmts.iter().rev().any(|s| s.terminates()),
			Stmt::If(IfStmt { cons, alt: Some(alt), .. }) => cons.terminates() && alt.terminates(),
			_ => false,
		}
	}
}
...
impl StmtExt for Box<Stmt> {
	...
	fn terminates(&self) -> bool { (**self).terminates() }
}
...
impl Visit for Hoister {
	...
	fn visit_assign_expr(&mut self, node:&AssignExpr) { node.right.visit_children_with(self); }
	...
	fn visit_fn_decl(&mut self, f:&FnDecl) { self.vars.push(f.ident.clone()); }
...
pub trait ExprExt {
	...
	fn is_immutable_value(&self) -> bool {
		...
		match *self.as_expr() {
			...
			Expr::Unary(UnaryExpr { op: op!("!"), ref arg, .. })
			| Expr::Unary(UnaryExpr { op: op!("~"), ref arg, .. })
			| Expr::Unary(UnaryExpr { op: op!("void"), ref arg, .. }) => arg.is_immutable_value(),
	...
	fn is_number(&self) -> bool { matches!(*self.as_expr(), Expr::Lit(Lit::Num(..))) }

	// TODO: remove this after a proper evaluator
	fn is_str(&self) -> bool {
		match self.as_expr() {
			Expr::Lit(Lit::Str(..)) | Expr::Tpl(_) => true,
			Expr::Unary(UnaryExpr { op: op!("typeof"), .. }) => true,
			Expr::Bin(BinExpr { op: op!(bin, "+"), left, right, .. }) => {
				left.is_str() || right.is_str()
			},
			Expr::Assign(AssignExpr { op: op!("=") | op!("+="), right, .. }) => right.is_str(),
	...
	fn is_array_lit(&self) -> bool { matches!(*self.as_expr(), Expr::Array(..)) }
	...
	fn is_undefined(&self, ctx:&ExprCtx) -> bool { self.is_global_ref_to(ctx, "undefined") }

	fn is_void(&self) -> bool {
		matches!(*self.as_expr(), Expr::Unary(UnaryExpr { op:op!("void"), .. }))
	}
	...
	fn cast_to_bool(&self, ctx:&ExprCtx) -> (Purity, BoolValue) {
		...
		let val = match expr {
			...
			Expr::Assign(AssignExpr { ref right, op: op!("="), .. }) => {
				let (_, v) = right.cast_to_bool(ctx);

				return (MayBeImpure, v);
			},

			Expr::Unary(UnaryExpr { op: op!(unary, "-"), arg, .. }) => {
			...
			Expr::Bin(BinExpr { left, op: op!(bin, "-"), right, .. }) => {
			...
			Expr::Bin(BinExpr { left, op: op!("/"), right, .. }) => {
			...
			Expr::Bin(BinExpr { ref left, op: op @ op!("&"), ref right, .. })
			| Expr::Bin(BinExpr { ref left, op: op @ op!("|"), ref right, .. }) => {
				...
				let v = if *op == op!("&") { lv.and(rv) } else { lv.or(rv) };
			...
			Expr::Bin(BinExpr { ref left, op: op!("||"), ref right, .. }) => {
				let (lp, lv) = left.cast_to_bool(ctx);
			...
			Expr::Unary(UnaryExpr { op: op!("void"), .. }) => Known(false),
			...
			Expr::Lit(ref lit) => {
				return (
					...
					Known(match *lit {
						...
						Lit::BigInt(ref v) => {
							v.value.to_string().contains(|c:char| matches!(c, '1'..='9'))
						},
						Lit::Bool(b) => b.value,
		...
		if expr.may_have_side_effects(ctx) { (MayBeImpure, val) } else { (Pure, val) }
	}
	...
	fn cast_to_number(&self, ctx:&ExprCtx) -> (Purity, Value<f64>) {
		...
		let v = match expr {
			...
			Expr::Unary(UnaryExpr { op: op!("void"), ref arg, .. }) => {
	...
	fn as_pure_string(&self, ctx:&ExprCtx) -> Value<Cow<'_, str>> {
		...
		match *expr {
			...
			Expr::Unary(UnaryExpr { op: op!("void"), .. }) => Known(Cow::Borrowed("undefined")),
			Expr::Unary(UnaryExpr { op: op!("!"), ref arg, .. }) => {
					...
					let e = match *elem {
						Some(ref elem) => {
							...
							match &**expr {
								...
								Expr::Unary(UnaryExpr { op: op!("void"), arg, .. }) => {
									if arg.may_have_side_effects(ctx) {
	...
	fn get_type(&self) -> Value<Type> {
		...
		match expr {
			Expr::Assign(AssignExpr { ref right, op: op!("="), .. }) => right.get_type(),
			...
			Expr::Seq(SeqExpr { ref exprs, .. }) => {
				exprs.last().expect("sequence expression should not be empty").get_type()
			},

			Expr::Bin(BinExpr { ref left, op: op!("&&"), ref right, .. })
			| Expr::Bin(BinExpr { ref left, op: op!("||"), ref right, .. })
			| Expr::Cond(CondExpr { cons: ref left, alt: ref right, .. }) => {
				and(left.get_type(), right.get_type())
			},

			Expr::Bin(BinExpr { ref left, op: op!(bin, "+"), ref right, .. }) => {
			...
			Expr::Assign(AssignExpr { op: op!("+="), ref right, .. }) => {
			...
			| Expr::Update(UpdateExpr { op: op!("--"), .. })
			| Expr::Unary(UnaryExpr { op: op!(unary, "+"), .. })
			| Expr::Unary(UnaryExpr { op: op!(unary, "-"), .. }) => Known(NumberType),
			...
			| Expr::Bin(BinExpr { op: op!("instanceof"), .. })
			| Expr::Unary(UnaryExpr { op: op!("!"), .. })
			| Expr::Unary(UnaryExpr { op: op!("delete"), .. }) => Known(BoolType),

			Expr::Unary(UnaryExpr { op: op!("typeof"), .. })
			| Expr::Lit(Lit::Str { .. })
			| Expr::Tpl(..) => Known(StringType),

			Expr::Lit(Lit::Null(..)) => Known(NullType),

			Expr::Unary(UnaryExpr { op: op!("void"), .. }) => Known(UndefinedType),
		...
		match self.as_expr() {
			Expr::Member(MemberExpr { obj, prop: MemberProp::Ident(prop), .. }) => {
				obj.is_global_ref_to(ctx, "Math")
					|| match &**obj {
						// Allow dummy span
						Expr::Ident(Ident { ctxt, sym: math, .. }) => {
						...
						Expr::Lit(Lit::Str(..)) => {
							match &*prop.sym {
								...
								| "indexOf" | "lastIndexOf" | "localeCompare" | "slice"
	...
	fn may_have_side_effects(&self, ctx:&ExprCtx) -> bool {
		...
		match self.as_expr() {
			...
			Expr::Unary(UnaryExpr { op: op!("delete"), .. }) => true,
			...
			Expr::Call(CallExpr { callee: Callee::Expr(callee), ref args, .. })
			...
			Expr::OptChain(OptChainExpr { base, .. })
				if matches!(&**base, OptChainBase::Call(..))
					&& OptChainBase::as_call(base).unwrap().callee.is_pure_callee(ctx) =>
			{
	...
	for m in &c.body {
		match m {
			...
			ClassMember::StaticBlock(s) => {
				if s.body.stmts.iter().any(|stmt| stmt.may_have_side_effects(expr_ctx)) {
					return true;
				}
			},
...
impl Purity {
	...
	pub fn is_pure(self) -> bool { self == Pure }
}
...
impl Visit for RestPatVisitor {
	...
	fn visit_rest_pat(&mut self, _:&RestPat) { self.found = true; }
}
...
where
	T: VisitWith<LiteralVisitor>, {
	let mut v = LiteralVisitor { is_lit:true, cost:0, allow_non_json_value };
...
impl Visit for LiteralVisitor {
	...
	fn visit_arrow_expr(&mut self, _:&ArrowExpr) { self.is_lit = false }

	fn visit_assign_expr(&mut self, _:&AssignExpr) { self.is_lit = false; }

	fn visit_await_expr(&mut self, _:&AwaitExpr) { self.is_lit = false }

	fn visit_bin_expr(&mut self, _:&BinExpr) { self.is_lit = false }

	fn visit_call_expr(&mut self, _:&CallExpr) { self.is_lit = false }

	fn visit_class_expr(&mut self, _:&ClassExpr) { self.is_lit = false }

	fn visit_cond_expr(&mut self, _:&CondExpr) { self.is_lit = false; }
	...
	fn visit_fn_expr(&mut self, _:&FnExpr) { self.is_lit = false; }

	fn visit_invalid(&mut self, _:&Invalid) { self.is_lit = false; }

	fn visit_member_expr(&mut self, _:&MemberExpr) { self.is_lit = false; }

	fn visit_meta_prop_expr(&mut self, _:&MetaPropExpr) { self.is_lit = false }

	fn visit_new_expr(&mut self, _:&NewExpr) { self.is_lit = false }
	...
	fn visit_opt_chain_expr(&mut self, _:&OptChainExpr) { self.is_lit = false }

	fn visit_private_name(&mut self, _:&PrivateName) { self.is_lit = false }
	...
	fn visit_seq_expr(&mut self, _:&SeqExpr) { self.is_lit = false }

	fn visit_spread_element(&mut self, _:&SpreadElement) { self.is_lit = false; }

	fn visit_tagged_tpl(&mut self, _:&TaggedTpl) { self.is_lit = false }

	fn visit_this_expr(&mut self, _:&ThisExpr) { self.is_lit = false; }

	fn visit_ts_const_assertion(&mut self, _:&TsConstAssertion) { self.is_lit = false }

	fn visit_ts_non_null_expr(&mut self, _:&TsNonNullExpr) { self.is_lit = false }

	fn visit_unary_expr(&mut self, _:&UnaryExpr) { self.is_lit = false; }

	fn visit_update_expr(&mut self, _:&UpdateExpr) { self.is_lit = false; }

	fn visit_yield_expr(&mut self, _:&YieldExpr) { self.is_lit = false }
}

pub fn is_simple_pure_expr(expr:&Expr, pure_getters:bool) -> bool {
	match expr {
		Expr::Ident(..) | Expr::This(..) | Expr::Lit(..) => true,
		Expr::Unary(UnaryExpr { op: op!("void") | op!("!"), arg, .. }) => {
...
fn sym_for_expr(expr:&Expr) -> Option<String> {
	match expr {
		...
		Expr::Ident(ident)
		| Expr::Fn(FnExpr { ident: Some(ident), .. })
		| Expr::Class(ClassExpr { ident: Some(ident), .. }) => Some(ident.sym.to_string()),

		Expr::OptChain(OptChainExpr { base, .. }) => {
			match &**base {
				OptChainBase::Call(OptCall { callee: expr, .. }) => sym_for_expr(expr),
				OptChainBase::Member(MemberExpr {
					prop: MemberProp::Ident(ident), obj, ..
				}) => Some(format!("{}_{}", sym_for_expr(obj).unwrap_or_default(), ident.sym)),
		...
		Expr::Call(CallExpr { callee: Callee::Expr(expr), .. }) => sym_for_expr(expr),

		Expr::SuperProp(SuperPropExpr { prop: SuperProp::Ident(ident), .. }) => {
		...
		Expr::Member(MemberExpr { prop: MemberProp::Ident(ident), obj, .. }) => {
			Some(format!("{}_{}", sym_for_expr(obj).unwrap_or_default(), ident.sym))
		},
...
pub fn alias_ident_for_simple_assign_tatget(expr:&SimpleAssignTarget, default:&str) -> Ident {
	...
	let mut sym = match expr {
		...
		SimpleAssignTarget::SuperProp(SuperPropExpr { prop: SuperProp::Ident(ident), .. }) => {
			Some(format!("super_{}", ident.sym))
		},
		...
		SimpleAssignTarget::Member(MemberExpr { prop: MemberProp::Ident(ident), obj, .. }) => {
			Some(format!("{}_{}", sym_for_expr(obj).unwrap_or_default(), ident.sym))
		},
...
pub fn prop_name_to_expr_value(p:PropName) -> Expr {
	match p {
		PropName::Ident(i) => Lit::Str(Str { span:i.span, raw:None, value:i.sym }).into(),
		PropName::Str(s) => Lit::Str(s).into(),
		PropName::Num(n) => Lit::Num(n).into(),
		PropName::BigInt(b) => Lit::BigInt(b).into(),
		PropName::Computed(c) => *c.expr,
...
pub fn prop_name_to_member_prop(prop_name:PropName) -> MemberProp {
	match prop_name {
		...
		PropName::Str(s) => MemberProp::Computed(ComputedPropName { span:DUMMY_SP, expr:s.into() }),
		PropName::Num(n) => MemberProp::Computed(ComputedPropName { span:DUMMY_SP, expr:n.into() }),
		PropName::Computed(c) => MemberProp::Computed(c),
		PropName::BigInt(b) => {
			MemberProp::Computed(ComputedPropName { span:DUMMY_SP, expr:b.into() })
		},
	}
}
...
pub fn opt_chain_test(left:Box<Expr>, right:Box<Expr>, span:Span, no_document_all:bool) -> Expr {
	if no_document_all {
		BinExpr { span, left, op:op!("=="), right:Lit::Null(Null { span:DUMMY_SP }).into() }.into()
	} else {
		BinExpr {
			span,
			left:BinExpr {
...
#[inline]
pub fn prepend_stmt<T:StmtLike>(stmts:&mut Vec<T>, stmt:T) { stmts.prepend_stmt(stmt); }
...
pub trait IsDirective {
	...
	fn is_directive(&self) -> bool {
		match self.as_ref() {
			Some(Stmt::Expr(expr)) => {
				match &*expr.expr {
					Expr::Lit(Lit::Str(Str { raw: Some(value), .. })) => {
	...
	fn is_use_strict(&self) -> bool { self.as_ref().map_or(false, Stmt::is_use_strict) }
}

impl IsDirective for Stmt {
	fn as_ref(&self) -> Option<&Stmt> { Some(self) }
}

impl IsDirective for ModuleItem {
	fn as_ref(&self) -> Option<&Stmt> { self.as_stmt() }
}
...
impl<I:IdentLike> Visit for DestructuringFinder<I> {
	...
	fn visit_ident(&mut self, i:&Ident) { self.found.push(I::from_ident(i)); }

	fn visit_jsx_member_expr(&mut self, n:&JSXMemberExpr) { n.obj.visit_with(self); }
	...
	F: for<'a> FnMut(&'a BindingIdent),
{
	...
	fn visit_binding_ident(&mut self, i:&BindingIdent) { (self.op)(i); }
}
...
impl Pass for DropSpan {
	fn process(&mut self, program:&mut Program) { program.visit_mut_with(self); }
}

impl VisitMut for DropSpan {
	fn visit_mut_span(&mut self, span:&mut Span) { *span = DUMMY_SP; }
}
...
impl<'a> IdentUsageFinder<'a> {
	...
	where
		N: VisitWith<Self>, {
		let mut v = IdentUsageFinder { ident, found:false };
...
impl ExprCtx {
	...
	pub fn extract_side_effects_to(&self, to:&mut Vec<Box<Expr>>, expr:Expr) {
		match expr {
			...
			Expr::Unary(UnaryExpr { op: op!("typeof"), arg, .. }) => {
			...
			Expr::Seq(SeqExpr { exprs, .. }) => {
				exprs.into_iter().for_each(|e| self.extract_side_effects_to(to, *e))
			},
						...
						match prop {
							PropOrSpread::Prop(node) => {
								match *node {
									...
									Prop::Assign(..) => {
										unreachable!(
											"assign property in object literal is not a valid \
											 syntax"
										)
									},
								}
			...
			Expr::TaggedTpl(TaggedTpl { tag, tpl, .. }) => {
				...
				tpl.exprs.into_iter().for_each(|e| self.extract_side_effects_to(to, *e));
			},
	...
	I: IdentLike + Eq + Hash + Send + Sync,
{
	...
	fn visit_export_default_decl(&mut self, e:&ExportDefaultDecl) {
		match &e.decl {
			DefaultDecl::Class(ClassExpr { ident: Some(ident), .. }) => {
				self.add(ident);
			},

			DefaultDecl::Fn(FnExpr { ident: Some(ident), function: f }) if f.body.is_some() => {
				self.add(ident);
			},
...
where
	...
	let mut v = BindingCollector { only:None, bindings:Default::default(), is_pat_decl:false };
...
where
	...
	N: VisitWith<BindingCollector<I>>, {
	let mut v =
		BindingCollector { only:Some(ctxt), bindings:Default::default(), is_pat_decl:false };
...
impl Visit for TopLevelAwait {
	...
	fn visit_class_member(&mut self, prop:&ClassMember) {
		match prop {
			ClassMember::ClassProp(ClassProp { key: PropName::Computed(computed), .. })
			| ClassMember::Method(ClassMethod { key: PropName::Computed(computed), .. }) => {
	...
	fn visit_prop(&mut self, prop:&Prop) {
		match prop {
			Prop::KeyValue(KeyValueProp { key: PropName::Computed(computed), .. })
			| Prop::Getter(GetterProp { key: PropName::Computed(computed), .. })
			| Prop::Setter(SetterProp { key: PropName::Computed(computed), .. })
			| Prop::Method(MethodProp { key: PropName::Computed(computed), .. }) => {
				computed.visit_children_with(self)
			},
			_ => {},
		}
...
impl<'a> Remapper<'a> {
	pub fn new(vars:&'a FxHashMap<Id, SyntaxContext>) -> Self { Self { vars } }
}
...
impl<'a> IdentRenamer<'a> {
	pub fn new(map:&'a FxHashMap<Id, Id>) -> Self { Self { map } }
}
...
pub trait QueryRef {
	fn query_ref(&self, _ident:&Ident) -> Option<Box<Expr>> { None }

	fn query_lhs(&self, _ident:&Ident) -> Option<Box<Expr>> { None }

	/// ref used in JSX
	fn query_jsx(&self, _ident:&Ident) -> Option<JSXElementName> { None }

	/// when `foo()` is replaced with `bar.baz()`,
	/// should `bar.baz` be indirect call?
	fn should_fix_this(&self, _ident:&Ident) -> bool { false }
}
	...
	pub fn exit_prop(&mut self, n:&mut Prop) {
		if let Prop::Shorthand(shorthand) = n {
			if let Some(expr) = self.query.query_ref(shorthand) {
				*n = KeyValueProp { key:shorthand.take().into(), value:expr }.into()
			}
		}
	}
...
mod test {
	...
	fn test_collect_decls() {
		run_collect_decls("const { a, b = 1, inner: { c }, ...d } = {};", &["a", "b", "c", "d"]);
	...
	fn test_extract_var_ids() {
		run_extract_var_ids("var { a, b = 1, inner: { c }, ...d } = {};", &["a", "b", "c", "d"]);
	...
	#[test]
	fn top_level_await_block() { assert!(has_top_level_await("if (maybe) { await test; }")) }

	#[test]
	fn top_level_await_for_of() { assert!(has_top_level_await("for await (let iter of []){}")) }
