...
pub trait ExprFactory: Into<Box<Expr>> {
	...
	fn as_arg(self) -> ExprOrSpread { ExprOrSpread { expr:self.into(), spread:None } }

	/// Creates an expression statement with `self`.
	#[cfg_attr(not(debug_assertions), inline(always))]
	fn into_stmt(self) -> Stmt { ExprStmt { span:DUMMY_SP, expr:self.into() }.into() }

	/// Creates a statement whcih return `self`.
	#[cfg_attr(not(debug_assertions), inline(always))]
	fn into_return_stmt(self) -> ReturnStmt { ReturnStmt { span:DUMMY_SP, arg:Some(self.into()) } }

	#[cfg_attr(not(debug_assertions), inline(always))]
	fn as_callee(self) -> Callee { Callee::Expr(self.into()) }

	#[cfg_attr(not(debug_assertions), inline(always))]
	fn as_iife(self) -> CallExpr {
		CallExpr { span:DUMMY_SP, callee:self.as_callee(), ..Default::default() }
	}
	...
	fn into_var_decl(self, kind:VarDeclKind, name:Pat) -> VarDecl {
		let var_declarator =
			VarDeclarator { span:DUMMY_SP, name, init:Some(self.into()), definite:false };

		VarDecl { kind, decls:vec![var_declarator], ..Default::default() }
	}

	#[cfg_attr(not(debug_assertions), inline(always))]
	fn into_new_expr(self, span:Span, args:Option<Vec<ExprOrSpread>>) -> NewExpr {
		NewExpr { span, callee:self.into(), args, ..Default::default() }
	}
	...
	fn call_fn(self, span:Span, args:Vec<ExprOrSpread>) -> Expr {
		CallExpr {
			span,
			args,
			callee:self.make_member(IdentName::new("call".into(), span)).as_callee(),
			..Default::default()
		}
		.into()
	}
	...
	fn as_call(self, span:Span, args:Vec<ExprOrSpread>) -> Expr {
		CallExpr { span, args, callee:self.as_callee(), ..Default::default() }.into()
	}

	#[cfg_attr(not(debug_assertions), inline(always))]
	fn as_fn_decl(self) -> Option<FnDecl> {
		match *self.into() {
			Expr::Fn(FnExpr { ident: Some(ident), function }) => {
				Some(FnDecl { ident, declare:false, function })
			},
			_ => None,
		}
	}
	...
	fn as_class_decl(self) -> Option<ClassDecl> {
		match *self.into() {
			Expr::Class(ClassExpr { ident: Some(ident), class }) => {
				Some(ClassDecl { ident, declare:false, class })
			},
			_ => None,
		}
	}
	...
	where
		...
		BinExpr { span:DUMMY_SP, left:self.into(), op, right }.into()
	}
	...
	fn make_assign_to(self, op:AssignOp, left:AssignTarget) -> Expr {
		...
		AssignExpr { span:DUMMY_SP, left, op, right }.into()
	}

	#[cfg_attr(not(debug_assertions), inline(always))]
	fn make_member(self, prop:IdentName) -> MemberExpr {
		MemberExpr { obj:self.into(), span:DUMMY_SP, prop:MemberProp::Ident(prop) }
	}
	...
	where
		...
		MemberExpr {
			obj:self.into(),
			span:DUMMY_SP,
			prop:MemberProp::Computed(ComputedPropName { span:DUMMY_SP, expr:prop.into() }),
		}
	}
}
...
impl IntoIndirectCall for Callee {
	...
	fn into_indirect(self) -> Callee {
		SeqExpr { span:DUMMY_SP, exprs:vec![0f64.into(), self.expect_expr()] }.as_callee()
	}
}
...
impl IntoIndirectCall for TaggedTpl {
	...
	fn into_indirect(mut self) -> Self {
		Self {
			tag:Box::new(
				SeqExpr { span:DUMMY_SP, exprs:vec![0f64.into(), self.tag.take()] }.into(),
			),
			..self
		}
	}
...
pub trait FunctionFactory: Into<Box<Function>> {
	#[cfg_attr(not(debug_assertions), inline(always))]
	fn into_fn_expr(self, ident:Option<Ident>) -> FnExpr { FnExpr { ident, function:self.into() } }

	#[cfg_attr(not(debug_assertions), inline(always))]
	fn into_fn_decl(self, ident:Ident) -> FnDecl {
		FnDecl { ident, declare:false, function:self.into() }
	}

	#[cfg_attr(not(debug_assertions), inline(always))]
	fn into_method_prop(self, key:PropName) -> MethodProp {
		MethodProp { key, function:self.into() }
	}
}
