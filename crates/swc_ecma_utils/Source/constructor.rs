...
pub fn inject_after_super(c:&mut Constructor, exprs:Vec<Box<Expr>>) {
	...
	let mut injector = Injector { exprs, ..Default::default() };
	...
	if !injector.injected {
		...
		body.stmts.splice(0..0, exprs.into_iter().map(|e| e.into_stmt()));
	}
}
...
impl VisitMut for Injector {
	...
	fn visit_mut_expr(&mut self, node:&mut Expr) {
		...
		if let Expr::Call(CallExpr { callee: Callee::Super(..), .. }) = node {
			self.injected = true;
