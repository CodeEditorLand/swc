...
impl FromStr for Version {
	...
	fn from_str(v:&str) -> Result<Self, Self::Err> {
		...
		Ok(Version { major:v.major as _, minor:v.minor as _, patch:v.patch as _ })
	}
}
...
impl<'de> Visitor<'de> for SerdeVisitor {
	...
	where
		E: de::Error, {
		v.parse().map_err(|_| de::Error::invalid_type(de::Unexpected::Str(v), &self))
	}
	...
	target.iter().zip(feature.iter()).any(
		|((target_name, maybe_target_version), (_, maybe_feature_version))| {
			maybe_target_version.map_or(false, |target_version| {
				let feature_or_fallback_version = maybe_feature_version.or_else(|| {
					match target_name {
						// Fall back to Chrome versions if Android browser
						// data is missing from the feature data. It
						// appears the Android browser has aligned
						// its versioning with Chrome.
...
mod tests {
	...
	fn should_enable_android_falls_back_to_chrome() {
		assert!(!should_enable(
			BrowserData { android:Some("51.0.0".parse().unwrap()), ..Default::default() },
			BrowserData { chrome:Some("51.0.0".parse().unwrap()), ..Default::default() },
			false
		));
	}
}
