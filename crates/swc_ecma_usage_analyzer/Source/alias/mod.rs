...
use swc_common::{SyntaxContext, collections::AHashSet};
use swc_ecma_ast::*;
use swc_ecma_utils::{BindingCollector, collect_decls};
...
impl InfectableNode for Function {
	fn is_fn_or_arrow_expr(&self) -> bool { false }
}

impl InfectableNode for Expr {
	fn is_fn_or_arrow_expr(&self) -> bool { matches!(self, Expr::Arrow(..) | Expr::Fn(..)) }
}
...
where
	T: InfectableNode,
{
	fn is_fn_or_arrow_expr(&self) -> bool { (**self).is_fn_or_arrow_expr() }
}
...
where
	...
	let unresolved_ctxt =
		config.marks.map(|m| SyntaxContext::empty().apply_mark(m.unresolved_mark));
	...
	let mut visitor = InfectionCollector {
		...
		exclude:&decls,
		ctx:Ctx { track_expr_ident:true, ..Default::default() },
...
impl InfectionCollector<'_> {
	fn add_id(&mut self, e:&Id) {
		...
		self.accesses.insert((
			e.clone(),
			if self.ctx.is_callee { AccessKind::Call } else { AccessKind::Reference },
		));
	}
}
...
impl Visit for InfectionCollector<'_> {
	...
	fn visit_bin_expr(&mut self, e:&BinExpr) {
		match e.op {
			...
			| op!(">>")
			| op!(">>>") => {
				let ctx = Ctx { track_expr_ident:false, is_callee:false, ..self.ctx };

				e.visit_children_with(&mut *self.with_ctx(ctx));
			},

			_ => {
				let ctx = Ctx { track_expr_ident:true, is_callee:false, ..self.ctx };
	...
	fn visit_cond_expr(&mut self, e:&CondExpr) {
		...
		{
			let ctx = Ctx { track_expr_ident:true, ..self.ctx };
	...
	fn visit_ident(&mut self, n:&Ident) { self.add_id(&n.to_id()); }
	...
	fn visit_member_expr(&mut self, n:&MemberExpr) {
		{
			let ctx = Ctx { track_expr_ident:self.config.need_all, ..self.ctx };
	...
	fn visit_member_prop(&mut self, n:&MemberProp) {
		if let MemberProp::Computed(c) = &n {
			c.visit_with(&mut *self.with_ctx(Ctx { is_callee:false, ..self.ctx }));
		}
	}
