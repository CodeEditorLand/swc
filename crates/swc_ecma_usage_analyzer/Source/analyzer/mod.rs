use swc_common::{SyntaxContext, collections::AHashMap};
use swc_ecma_ast::*;
use swc_ecma_utils::{ExprCtx, ExprExt, IsEmpty, StmtExt, Type, Value, find_pat_ids};
	...
	S: Storage,
{
	...
	where
		...
		let mut child = UsageAnalyzer {
			data:Default::default(),
			marks:self.marks,
			ctx:Ctx { is_top_level:false, ..self.ctx },
	...
	fn visit_pat_id(&mut self, i:&Ident) {
		let Ctx { in_left_of_for_loop, in_pat_of_param, in_pat_of_var_decl, .. } = self.ctx;
	...
	fn report_assign_pat(&mut self, p:&Pat, is_read_modify:bool) {
		for id in find_pat_ids(p) {
			...
			self.data.report_assign(self.ctx, id, is_read_modify, Value::Unknown)
		}

		if let Pat::Expr(e) = p {
			match &**e {
				Expr::Ident(i) => {
					self.data.report_assign(self.ctx, i.to_id(), is_read_modify, Value::Unknown)
				},
	...
	fn mark_mutation_if_member(&mut self, e:Option<&MemberExpr>) {
		if let Some(m) = e {
			for_each_id_ref_in_expr(&m.obj, &mut |id| self.data.mark_property_mutation(id.to_id()));
		}
	}
}
	...
	S: Storage,
{
	...
	fn visit_array_lit(&mut self, n:&ArrayLit) {
		let ctx = Ctx { is_id_ref:true, ..self.ctx };
	...
	fn visit_arrow_expr(&mut self, n:&ArrowExpr) {
		self.with_child(n.ctxt, ScopeKind::Fn, |child| {
			{
				let ctx = Ctx { in_pat_of_param:true, inline_prevented:true, ..child.ctx };
	...
	fn visit_assign_expr(&mut self, n:&AssignExpr) {
		...
		match &n.left {
			AssignTarget::Pat(p) => {
				for id in find_pat_ids(p) {
					self.data.report_assign(self.ctx, id, is_op_assign, n.right.get_type())
				}
			},
		...
		if n.op == op!("=") {
			...
			if let Some(left) = left {
				for id in collect_infects_from(
					&n.right,
					AliasConfig { marks:self.marks, ..Default::default() },
				) {
					self.data.var_or_default(left.clone()).add_infects_to(id.clone());
				}
			}
		}
	}
	...
	fn visit_assign_pat(&mut self, p:&AssignPat) {
		...
		{
			let ctx = Ctx { in_pat_of_param:false, var_decl_kind_of_pat:None, ..self.ctx };
	...
	fn visit_await_expr(&mut self, n:&AwaitExpr) {
		let ctx = Ctx { in_await_arg:true, ..self.ctx };
	...
	fn visit_bin_expr(&mut self, e:&BinExpr) {
		if e.op.may_short_circuit() {
			...
			let ctx = Ctx { in_cond:true, is_id_ref:true, ..self.ctx };
			...
			let ctx = Ctx { is_id_ref:false, ..self.ctx };
	...
	#[cfg_attr(feature = "tracing-spans", tracing::instrument(skip_all))]
	fn visit_binding_ident(&mut self, n:&BindingIdent) { self.visit_pat_id(&Ident::from(n)); }
	...
	fn visit_call_expr(&mut self, n:&CallExpr) {
		let inline_prevented = self.ctx.inline_prevented
			|| self.marks.map(|marks| n.ctxt.has_mark(marks.noinline)).unwrap_or_default();

		{
			let ctx = Ctx { inline_prevented, ..self.ctx };
		...
		{
			let ctx = Ctx { inline_prevented, is_id_ref:true, ..self.ctx };
	...
	fn visit_catch_clause(&mut self, n:&CatchClause) {
		{
			let ctx = Ctx { in_cond:true, in_catch_param:true, ..self.ctx };
	...
	fn visit_class(&mut self, n:&Class) {
		...
		{
			let ctx = Ctx { inline_prevented:true, ..self.ctx };
	...
	fn visit_class_method(&mut self, n:&ClassMethod) {
		...
		self.with_child(n.function.ctxt, ScopeKind::Fn, |a| {
			n.key.visit_with(a);
			{
				let ctx = Ctx { in_pat_of_param:true, ..a.ctx };
	...
	fn visit_constructor(&mut self, n:&Constructor) {
		self.with_child(n.ctxt, ScopeKind::Fn, |child| {
			{
				let ctx = Ctx { in_pat_of_param:true, ..child.ctx };
	...
	fn visit_do_while_stmt(&mut self, n:&DoWhileStmt) {
		n.body
			.visit_with(&mut *self.with_ctx(Ctx { executed_multiple_time:true, ..self.ctx }));
	...
	fn visit_expr_or_spread(&mut self, e:&ExprOrSpread) {
		...
		if e.spread.is_some() {
			for_each_id_ref_in_expr(&e.expr, &mut |i| {
				self.data.var_or_default(i.to_id()).mark_indexed_with_dynamic_key();
			});
		}
	}
	...
	fn visit_fn_decl(&mut self, n:&FnDecl) {
		let ctx = Ctx { in_decl_with_no_side_effect_for_member_access:true, ..self.ctx };
		...
		self.used_recursively.insert(id.clone(), RecursiveUsage::FnOrClass);
...

		{
			for id in collect_infects_from(
				...
				self.data.var_or_default(n.ident.to_id()).add_infects_to(id.clone());
			}
		}
	}
	...
	fn visit_fn_expr(&mut self, n:&FnExpr) {
		if let Some(n_id) = &n.ident {
			self.data.var_or_default(n_id.to_id()).mark_declared_as_fn_expr();

			self.used_recursively.insert(n_id.to_id(), RecursiveUsage::FnOrClass);
	...
	fn visit_for_in_stmt(&mut self, n:&ForInStmt) {
		...
		self.with_child(SyntaxContext::empty(), ScopeKind::Block, |child| {
			...
			let ctx = Ctx { executed_multiple_time:true, in_cond:true, ..child.ctx };
	...
	fn visit_for_stmt(&mut self, n:&ForStmt) {
		...
		let ctx = Ctx { executed_multiple_time:true, in_cond:true, ..self.ctx };
	...
	fn visit_function(&mut self, n:&Function) {
		...
		self.with_ctx(ctx).with_child(n.ctxt, ScopeKind::Fn, |child| {
	...
	fn visit_module(&mut self, n:&Module) {
		let ctx = Ctx { is_top_level:true, ..self.ctx };
	...
	fn visit_param(&mut self, n:&Param) {
		let ctx = Ctx { in_pat_of_param:false, ..self.ctx };

		n.decorators.visit_with(&mut *self.with_ctx(ctx));

		let ctx =
			Ctx { in_pat_of_param:true, var_decl_kind_of_pat:None, is_id_ref:true, ..self.ctx };
	...
	fn visit_pat(&mut self, n:&Pat) {
		match n {
			...
			_ => {
				let ctx = Ctx { in_decl_with_no_side_effect_for_member_access:false, ..self.ctx };
	...
	fn visit_stmt(&mut self, n:&Stmt) {
		let ctx = Ctx { in_await_arg:false, is_id_ref:true, ..self.ctx };
	...
	fn visit_stmts(&mut self, stmts:&[Stmt]) {
		...
		for stmt in stmts {
			let ctx = Ctx { in_cond:self.ctx.in_cond || had_cond, is_id_ref:true, ..self.ctx };
	...
	fn visit_var_decl(&mut self, n:&VarDecl) {
		let ctx = Ctx { var_decl_kind_of_pat:Some(n.kind), in_await_arg:false, ..self.ctx };
		...
		for decl in &n.decls {
			if let (Pat::Ident(var), Some(init)) = (&decl.name, decl.init.as_deref()) {
				for id in collect_infects_from(
					...
					self.data.var_or_default(var.to_id()).add_infects_to(id.clone());
				}
			}
		}
	}
