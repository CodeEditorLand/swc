...
impl Compiler {
	...
	fn append_compound(
		...
		if c.nesting_selector.is_some() {
			...
			to.children.extend(base.children.iter().cloned().enumerate().map(
				|(idx, mut children)| {
					if idx == len - 1 {
			...
			to.children.push(ComplexSelectorChildren::CompoundSelector(c.clone()));
		}
	}
	...
	fn relative_selector_list_to_selector_list(
		...
		for base_complex in &base.children {
			for relative_selector in &relative_selector_list.children {
				let mut complex_selector =
					ComplexSelector { span:relative_selector.span, children:Default::default() };

				let is_non_relative = relative_selector.combinator.is_none()
					&& relative_selector.selector.children.iter().any(|s| {
						match s {
				...
				if let Some(combinator) = &relative_selector.combinator {
					complex_selector.children.extend(base_complex.children.clone());
					...
					complex_selector.children.push(ComplexSelectorChildren::Combinator(
						Combinator { span:DUMMY_SP, value:CombinatorValue::Descendant },
					))
				}
		...
		SelectorList { span:relative_selector_list.span, children }
	}
						...
						if let Some(block) = &at_rule.block {
							...
							if !decls_of_media.is_empty() {
								let rule = Box::new(QualifiedRule {
									...
									block:SimpleBlock { value:decls_of_media, ..block.clone() },
								});
							...
							nested_rules.push(Rule::AtRule(Box::new(AtRule {
								block:Some(SimpleBlock { value:nested_of_media, ..block.clone() }),
								..*at_rule.clone()
							})));
