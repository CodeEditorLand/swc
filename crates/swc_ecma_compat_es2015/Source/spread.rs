...
pub fn spread(c:Config) -> impl Fold + VisitMut { as_folder(Spread { c, vars:Default::default() }) }
...
impl VisitMut for Spread {
	...
	fn visit_mut_expr(&mut self, e:&mut Expr) {
		...
		match e {
			...
			Expr::Call(CallExpr { callee: Callee::Expr(callee), args, span, .. }) => {
				let has_spread = args.iter().any(|ExprOrSpread { spread, .. }| spread.is_some());
				if !has_spread {
					return;
				}
			...
			Expr::New(NewExpr { callee, args: Some(args), span, .. }) => {
				let has_spread = args.iter().any(|ExprOrSpread { spread, .. }| spread.is_some());
				if !has_spread {
					return;
				}
