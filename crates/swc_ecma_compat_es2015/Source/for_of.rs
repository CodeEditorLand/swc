...
impl ForOf {
	fn fold_for_stmt(
		&mut self,
		label:Option<Ident>,
		ForOfStmt { span, left, right, body, .. }:ForOfStmt,
	) -> Stmt {
...
impl Parallel for ForOf {
	fn create(&self) -> Self { ForOf { c:self.c, top_level_vars:Default::default() } }

	fn merge(&mut self, other:Self) { self.top_level_vars.extend(other.top_level_vars); }
}
