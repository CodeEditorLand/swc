...
use swc_common::{Spanned, collections::AHashSet};
use swc_ecma_ast::*;
...
pub fn duplicate_keys() -> impl Pass { visit_mut_pass(DuplicateKeys) }
		...
		match name {
			...
			PropName::Str(s) => {
				if !self.props.insert(s.value.clone()) {
					*name =
						PropName::Computed(ComputedPropName { span:s.span, expr:s.clone().into() })
				}
			},
