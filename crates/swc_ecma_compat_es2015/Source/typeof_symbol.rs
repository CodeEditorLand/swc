...
pub fn typeof_symbol() -> impl VisitMut + Fold { as_folder(TypeOfSymbol) }
...
impl VisitMut for TypeOfSymbol {
	...
	fn visit_mut_bin_expr(&mut self, expr:&mut BinExpr) {
		...
		if let Expr::Unary(UnaryExpr { op: op!("typeof"), .. }) = *expr.left {
			if is_non_symbol_literal(&expr.right) {
				return;
			}
		}
		if let Expr::Unary(UnaryExpr { op: op!("typeof"), .. }) = *expr.right {
