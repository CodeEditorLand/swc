...
impl BlockScoping {
	...
	fn mark_as_used(&mut self, i:Id) {
		...
		{
			if let ScopeKind::Loop { lexical_var, used, .. } = scope {
	...
	fn handle_capture_of_vars(&mut self, body:&mut Box<Stmt>) {
		...
		if let Some(ScopeKind::Loop { args, used, mutated, .. }) = self.scope.pop() {
			if used.is_empty() {
				return;
			}
			...
			let mut body_stmt = match &mut body_stmt.take() {
				...
				body => BlockStmt { span:DUMMY_SP, stmts:vec![body.take()], ..Default::default() },
			};

			if !flow_helper.mutated.is_empty() {
				let no_modification = flow_helper.mutated.is_empty();

				let mut v = MutationHandler { map:&mut flow_helper.mutated, in_function:false };
			...
			if flow_helper.has_await {
				call = AwaitExpr { span:DUMMY_SP, arg:call.into() }.into();
			}

			if flow_helper.has_yield {
				call = YieldExpr { span:DUMMY_SP, arg:Some(call.into()), delegate:true }.into();
			}
			...
			if flow_helper.has_return || flow_helper.has_break || !flow_helper.label.is_empty() {
				...
				if flow_helper.has_break {
					stmts.push(
						IfStmt {
							...
							cons:BreakStmt { span:DUMMY_SP, label:None }.into(),
							alt:None,
						}
						.into(),
					);
				...
				*body = Box::new(BlockStmt { span:DUMMY_SP, stmts, ..Default::default() }.into());
...
impl VisitMut for BlockScoping {
	...
	fn visit_mut_for_in_stmt(&mut self, node:&mut ForInStmt) {
		...
		let kind =
			ScopeKind::Loop { lexical_var, args, used:Vec::new(), mutated:Default::default() };
	...
	fn visit_mut_for_of_stmt(&mut self, node:&mut ForOfStmt) {
		...
		let kind =
			ScopeKind::Loop { lexical_var:vars, args, used:Vec::new(), mutated:Default::default() };
...
impl FlowHelper<'_> {
	fn check(&mut self, i:Id) {
		if self.all.contains(&i) {
			self.mutated
				.insert(i, SyntaxContext::empty().apply_mark(Mark::fresh(Mark::root())));
		}
	}
		...
		match node {
			Stmt::Continue(ContinueStmt { label, .. }) => {
				...
				let value = if let Some(label) = label {
					...
					self.label.insert(value.clone(), Label::Continue(label.clone()));
				...
				*node =
					ReturnStmt { span, arg:Some(Lit::Str(Str { span, value, raw:None }).into()) }
						.into();
			},
			...
			Stmt::Break(BreakStmt { label, .. }) => {
				...
				let value = if let Some(label) = label {
					...
					self.label.insert(value.clone(), Label::Break(label.clone()));
