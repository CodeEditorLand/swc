...
use swc_common::{Mark, SyntaxContext, collections::ARandomState};
use swc_ecma_ast::*;
...
pub(super) fn block_scoped_vars() -> impl VisitMut { BlockScopedVars::default() }
...
impl BlockScopedVars {
	...
	where
		...
		let parent = ParentScope { parent:None, vars:&empty_vars };
	...
	fn with_scope(&mut self, kind:ScopeKind, op:impl FnOnce(&mut Self)) {
		let scope = Scope { kind, ..Default::default() };
...
impl Scope {
	fn rename(&mut self, parent:ParentScope, rename_map:&mut FxHashMap<Id, Id>, fn_only:bool) {
		for s in self.children.iter_mut() {
			let parent = ParentScope { parent:Some(&parent), vars:&self.vars };
	...
	fn collect_candidates(&mut self, parent:ParentScope, symbols:&mut Vec<JsWord>) {
		...
		self.children.iter_mut().for_each(|s| s.collect_candidates(parent, symbols));
	}
	...
	fn rename_decls(&self, symbols:&[JsWord], rename_map:&mut FxHashMap<Id, Id>) {
		...
		self.children.iter().for_each(|s| s.rename_decls(symbols, rename_map));
	}
}
...
impl VisitMut for BlockScopedVars {
	...
	fn visit_mut_module(&mut self, n:&mut Module) { self.handle_program(n) }
	...
	fn visit_mut_script(&mut self, n:&mut Script) { self.handle_program(n) }
