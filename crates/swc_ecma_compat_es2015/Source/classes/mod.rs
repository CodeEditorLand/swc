						...
						Ok(mut decl) => {
							match decl {
								ModuleDecl::ExportDefaultDecl(ExportDefaultDecl {
									...
									let mut decl =
										self.fold_class_as_var_decl(ident.clone(), class);
...
impl VisitMut for Classes {
	...
	fn visit_mut_stmts(&mut self, items:&mut Vec<Stmt>) { self.visit_mut_stmt_like(items) }

	fn visit_mut_decl(&mut self, n:&mut Decl) {
		if let Decl::Class(decl) = n {
			*n = self.fold_class_as_var_decl(decl.ident.take(), decl.class.take()).into()
		};
	...
	fn visit_mut_var_declarator(&mut self, d:&mut VarDeclarator) {
		...
		if let VarDeclarator { name: Pat::Ident(i), init: Some(init), .. } = d {
	...
	fn visit_mut_assign_expr(&mut self, a:&mut AssignExpr) {
		if let AssignExpr { op: op!("=") | op!("||=") | op!("??="), left, right, .. } = a {
...
#[swc_trace]
impl Classes {
	fn add_pure_comments(&mut self, start:&mut BytePos) { *start = BytePos::PURE; }
	...
	fn fold_class(&mut self, class_name:Option<Ident>, class:Box<Class>) -> Expr {
		...
		let super_ident = class.super_class.as_ref().map(|e| alias_if_required(e, "_superClass").0);
		...
		if !has_super && cnt_of_non_directive == 1 {
			...
			match stmt {
				Stmt::Decl(Decl::Fn(FnDecl { ident, mut function, .. })) => {
					...
					return FnExpr { ident:Some(ident), function }.into();
				},
		...
		let body = BlockStmt { span:DUMMY_SP, stmts, ..Default::default() };
	...
	fn class_to_stmts(
		...
		// constructor
		stmts.push(
			fold_constructor(class.span, constructor, &class_name, &super_class_ident, self.config)
				.into(),
		);
		...
		// `return Foo`
		stmts.push(ReturnStmt { span:DUMMY_SP, arg:Some(class_name_sym.into()) }.into());
	...
	fn fold_class_methods(
		...
		fn mk_key_prop(key:PropName) -> Box<Prop> {
			Box::new(Prop::KeyValue(KeyValueProp {
				...
				value:match key {
					...
					PropName::BigInt(b) => {
						Str { span:b.span, raw:None, value:b.value.to_string().into() }.into()
					},
		...
		fn mk_key_prop_member(key:PropName) -> MemberProp {
			match key {
				...
				PropName::Str(s) => {
					MemberProp::Computed(ComputedPropName { span:s.span, expr:Lit::Str(s).into() })
				},
				PropName::Num(n) => {
					MemberProp::Computed(ComputedPropName { span:n.span, expr:Lit::Num(n).into() })
				},
				PropName::BigInt(b) => {
					MemberProp::Computed(ComputedPropName {
						span:b.span,
						expr:Str { span:b.span, raw:None, value:b.value.to_string().into() }.into(),
					})
				},
			...
			ArrayLit {
				...
				elems:props
					...
					.map(|(_, data)| {
						...
						ObjectLit { span:DUMMY_SP, props }.as_arg()
					})
					.map(Some)
					.collect(),
			}
		...
		for mut m in methods {
			...
			let append_to:&mut IndexMap<_, _> =
				if m.is_static { &mut static_props } else { &mut props };
			...
			let data = append_to
				.entry(key)
				.or_insert_with(|| Data { key_prop, get:None, set:None, method:None });
...
fn is_always_initialized(body:&[Stmt]) -> bool {
	...
	impl Visit for SuperFinder {
		...
		fn visit_super_prop_expr(&mut self, _:&SuperPropExpr) { self.found = true }
	}

	let pos = match body.iter().position(|s| {
		match s {
			Stmt::Expr(ExprStmt { expr, .. }) => {
				matches!(&**expr, Expr::Call(CallExpr { callee:Callee::Super(..), .. }))
			},
			_ => false,
		}
	}) {
...
impl Visit for ClassFinder {
	...
	fn visit_class(&mut self, _:&Class) { self.found = true }
}
