...
pub(super) fn fold_constructor(
	...
	if let Some(class_super_name) = class_super_name {
				...
				if let Some(expr_stmt) = stmt.as_mut_expr() {
					...
					match expr_stmt.expr.as_mut() {
						Expr::Assign(assign) => {
							...
							*stmt = ReturnStmt { span, arg:arg.into() }.into();
						},
						arg @ Expr::Seq(..) | arg @ Expr::Paren(..) => {
							*stmt = ReturnStmt { span, arg:Some(Box::new(arg.take())) }.into();
						},
		...
		if constructor_folder.this_ref_count > 0 || constructor_folder.super_prop_found {
			let this =
				constructor_folder.this.get_or_insert_with(|| private_ident!("_this")).clone();
			...
			let var = VarDecl { decls:vec![var], ..Default::default() };
		...
		if !is_last_super && !is_last_return {
			...
			let return_this = ReturnStmt { span:DUMMY_SP, arg:Some(this.into()) };
	...
	FnDecl { ident:class_name.clone(), declare:false, function:function.into() }
}
...
impl VisitMut for ConstructorFolder {
	...
	fn visit_mut_return_stmt(&mut self, node:&mut ReturnStmt) {
		...
		if !self.in_arrow {
			...
			args.extend(arg);
			node.arg =
				Some(helper_expr!(possible_constructor_return).as_call(DUMMY_SP, args).into());
		}
	}
...
impl ConstructorFolder {
	...
	fn transform_super_call(&self, node:&mut Expr) -> bool {
		...
		let CallExpr { callee: callee @ Callee::Super(..), args: origin_args, .. } = call_expr
		else {
			return false;
		};
		...
		if self.is_super_callable_constructor {
			if self.is_constructor_default || is_spread_arguements(origin_args) {
				*callee =
					self.class_super_name.clone().make_member(quote_ident!("apply")).as_callee();
				...
				*callee =
					self.class_super_name.clone().make_member(quote_ident!("call")).as_callee();
		...
		*callee = helper!(call_super);
		...
		let mut args = vec![ThisExpr { span:DUMMY_SP }.as_arg(), self.class_name.clone().as_arg()];
...
fn is_spread_arguements(args:&[ExprOrSpread]) -> bool {
	...
	return arg.expr.as_ident().filter(|ident| ident.sym == *"arguments").is_some();
}
...
impl SuperLastCall for &Expr {
	fn is_super_last_call(&self) -> bool {
		match self {
			Expr::Call(CallExpr { callee: Callee::Super(..), .. }) => true,
