...
impl VisitMut for Generator {
	...
	fn visit_mut_arrow_expr(&mut self, e:&mut ArrowExpr) { e.params.visit_mut_with(self); }

	fn visit_mut_function(&mut self, e:&mut Function) { e.params.visit_mut_with(self); }

	fn visit_mut_getter_prop(&mut self, _:&mut GetterProp) {}

	fn visit_mut_setter_prop(&mut self, e:&mut SetterProp) { e.param.visit_mut_with(self); }
	...
	fn visit_mut_new_expr(&mut self, node:&mut NewExpr) {
		if contains_yield(&node.args) {
			...
			*node =
				NewExpr { span:node.span, callee:Box::new(apply), args:None, ..Default::default() };
...
impl Generator {
	...
	fn reduce_property(
		...
		{
			self.emit_stmt(
				ExprStmt { span:DUMMY_SP, expr:Expr::from_exprs(expressions.take()) }.into(),
			);
		}
				...
				for (i, clause) in node.cases.iter_mut().enumerate().skip(clauses_written) {
					if clause.test.is_some() {
						...
						pending_clauses.push(SwitchCase {
							...
							cons:vec![
								self.create_inline_break(clause_labels[i], Some(span)).into(),
							],
						})
					} else {
	...
	fn begin_catch_block(&mut self, variable:VarDeclarator) {
		...
		let mut exception = RefMut::map(exception, |v| {
			match v {
	...
	fn create_label(&mut self, label:Option<Label>) -> Box<Expr> {
		if let Some(label) = label {
			if label.0 > 0 {
				...
				return Invalid { span:Span::new(BytePos(label.0 as _), BytePos(label.0 as _)) }
					.into();
			}
		}
	...
	fn emit_nop(&mut self) { self.emit_worker(OpCode::Nop, None, None); }
	...
	/// semantics.
	fn emit_endfinally(&mut self) { self.emit_worker(OpCode::Endfinally, None, None); }
	...
	fn build_stmts(&mut self) -> Vec<Stmt> {
		...
		if let Some(clauses) = self.clauses.take() {
			...
			let switch_stmt =
				SwitchStmt { span:DUMMY_SP, discriminant:label_expr.into(), cases:clauses };
	...
	fn is_final_label_reachable(&self, op_index:usize) -> bool {
		...
		// is reachable.
		for (label, label_offset) in
			self.label_offsets.as_ref().unwrap().iter().copied().enumerate()
		{
	...
	fn append_label(&mut self, mark_label_end:bool) {
		...
		let stmts = if let Some(mut stmts) = self.stmts.take() {
			if self.with_block_stack.is_some() {
				...
				for (_i, with_block) in
					self.with_block_stack.as_ref().unwrap().iter().enumerate().rev()
				{
			...
			if let Some(current_exception_block) = self.current_exception_block.take() {
				...
				let ExceptionBlock { start_label, catch_label, finally_label, end_label, .. } =
					match &*b {
						CodeBlock::Exception(v) => v,
						_ => {
							unreachable!()
	...
	fn create_call_binding(&mut self, expr:Box<Expr>, is_new_call:bool) -> (Box<Expr>, Box<Expr>) {
		...
		match &mut *callee {
			Expr::Ident(..) => {
				(callee.clone(), if is_new_call { callee } else { Expr::undefined(DUMMY_SP) })
			},
...
impl Visit for YieldFinder {
	...
	fn visit_yield_expr(&mut self, _:&YieldExpr) { self.found = true; }

	fn visit_arrow_expr(&mut self, f:&ArrowExpr) { f.params.visit_with(self); }
