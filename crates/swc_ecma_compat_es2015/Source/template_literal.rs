...
impl Parallel for TemplateLiteral {
	fn create(&self) -> Self { Self::default() }

	fn merge(&mut self, other:Self) { self.added.extend(other.added); }
}
	...
	fn visit_mut_expr(&mut self, e:&mut Expr) {
		...
		match e {
			Expr::Tpl(Tpl { span, exprs, quasis, .. }) => {
				...
				let mut obj:Box<Expr> = Box::new(
					Lit::Str({
						...
						Str { span:quasis[0].span, value:JsWord::from(&*s), raw:None }
					})
					.into(),
				);
					...
					let expr = if i % 2 == 0 {
						...
						match quasis.next() {
							Some(TplElement { span, cooked, raw, .. }) => {
								...
								Box::new(
									Lit::Str(Str { span:*span, value:(&*s).into(), raw:None })
										.into(),
								)
							},
			...
			Expr::TaggedTpl(TaggedTpl { tag, tpl, .. }) => {
				...
				self.added.push(
					FnDecl { declare:false, ident:fn_ident.clone(), function:f.into() }.into(),
				);
