...
where
	...
	(
		...
		if !c.typescript { Some(object_super()) } else { None },
		shorthand(),
		function_name(),
		for_of(c.for_of),
...
mod tests {
	...
	test!(
		...
		|t| es2015(Mark::fresh(Mark::root()), Some(t.comments.clone()), Default::default()),
		issue_169,
		r#"
export class Foo {
	func(a, b = Date.now()) {
...
"#
  ...
  fn2(...args);
	...
	test!(
		::swc_ecma_parser::Syntax::default(),
		|_| (block_scoped_functions(), resolver(Mark::new(), Mark::new(), false)),
		issue_271,
		"
function foo(scope) {
