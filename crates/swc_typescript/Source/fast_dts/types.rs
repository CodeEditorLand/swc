...
impl FastDts {
	pub(crate) fn transform_expr_to_ts_type(&mut self, expr:&Expr) -> Option<Box<TsType>> {
		match expr {
			...
			Expr::Tpl(tpl) => self.tpl_to_string(tpl).map(|string| ts_lit_type(TsLit::Str(string))),
	...
	pub(crate) fn transform_fn_to_ts_type(
		...
		return_type.map(|return_type| {
			Box::new(TsType::TsFnOrConstructorType(TsFnOrConstructorType::TsFnType(TsFnType {
				span:DUMMY_SP,
			...
			Box::new(TsType::TsFnOrConstructorType(TsFnOrConstructorType::TsFnType(TsFnType {
				...
				params:self.transform_fn_params_to_ts_type(
					&arrow
						...
						.iter()
						.map(|pat| {
							Param { span:pat.span(), decorators:Vec::new(), pat:pat.clone() }
						})
						.collect::<Vec<_>>(),
				),
	...
	pub(crate) fn transform_object_to_ts_type(
				...
				PropOrSpread::Prop(prop) => {
					match prop.as_ref() {
						...
						Prop::Method(method) => {
							...
							if is_const {
								let (key, computed) =
									self.transform_property_name_to_expr(&method.key);
		...
		Some(Box::new(TsType::TsTypeLit(TsTypeLit { span:DUMMY_SP, members })))
	}
	...
	pub(crate) fn transform_array_to_ts_type(&mut self, array:&ArrayLit) -> Option<Box<TsType>> {
		...
		for elem in &array.elems {
			...
			if let Some(type_ann) = self.transform_expr_to_ts_type(&elem.expr) {
				elements.push(TsTupleElement { span:DUMMY_SP, label:None, ty:type_ann });
			} else {
