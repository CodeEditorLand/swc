...
impl FastDts {
		...
		for mut member in body {
			match &mut member {
				ClassMember::Constructor(constructor) => {
					...
					class
						.body
						.splice(0..0, self.transform_constructor_params(&mut constructor.params));
		...
		if has_private_key {
			class.body.insert(
				...
				ClassMember::PrivateProp(PrivateProp {
					...
					key:PrivateName { span:DUMMY_SP, name:"private".into() },
					value:None,
					type_ann:None,
					is_static:false,
					decorators:Vec::new(),
	...
	pub(crate) fn transform_constructor_ts_param(
		...
		let ident = (match &ts_param_prop.param {
			...
			TsParamPropParam::Assign(assign_pat) => {
				assign_pat.left.as_ident().map(|binding_ident| binding_ident.id.clone())
			},
		})?;
	...
	pub(crate) fn transform_class_property(&mut self, prop:&mut ClassProp) {
		if prop
			.accessibility
			.map_or(true, |accessibility| accessibility != Accessibility::Private)
		{
				...
				if let Some(value) = prop.value.as_ref() {
					if prop.readonly {
						if Self::need_to_infer_type_from_expression(value) {
							...
							prop.value =
								self.tpl_to_string(tpl).map(|s| Box::new(Expr::Lit(Lit::Str(s))));
						}
					} else {
