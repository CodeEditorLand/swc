...
impl FastDts {
	pub fn new(filename:Arc<FileName>) -> Self {
		Self { filename, is_top_level:false, id_counter:0, diagnostics:Vec::new() }
	}

	fn mark_diagnostic(&mut self, diagnostic:DtsIssue) { self.diagnostics.push(diagnostic) }

	fn source_range_to_range(&self, range:Span) -> SourceRange {
		SourceRange { filename:self.filename.clone(), span:range }
	}
...
impl FastDts {
		...
		for mut item in orig_items {
			let is_overload = match &item {
				...
				| ModuleItem::Stmt(Stmt::Decl(decl)) => {
					match decl {
						Decl::Fn(FnDecl { function, declare, .. }) => {
			...
			match &mut item {
				...
				ModuleItem::ModuleDecl(ModuleDecl::ExportDecl(ExportDecl {
					...
					if let Some(()) = self.decl_to_type_decl(decl) {
						new_items.push(ExportDecl { decl:decl.take(), span:*span }.into());
					} else {
						self.mark_diagnostic(DtsIssue::UnableToInferType {
				...
				ModuleItem::ModuleDecl(ModuleDecl::ExportDefaultExpr(export)) => {
					...
					let type_ann =
						self.expr_to_ts_type(export.expr.clone(), false, true).map(type_ann);
					...
					if let Some(type_ann) = type_ann {
						...
						new_items.push(
							ExportDefaultExpr { span:export.span, expr:name_ident.into() }.into(),
						)
					} else {
						new_items.push(
							ExportDefaultExpr { span:export.span, expr:export.expr.take() }.into(),
						)
					}
				},
	...
	fn expr_to_ts_type(
		...
		match *e {
				...
				for elems in arr.elems {
					if let Some(expr_or_spread) = elems {
						...
						elem_types.push(ts_tuple_element(Box::new(TsType::TsKeywordType(
							TsKeywordType { kind:TsKeywordTypeKind::TsAnyKeyword, span:DUMMY_SP },
						))))
					}
				}

				let mut result =
					Box::new(TsType::TsTupleType(TsTupleType { span:arr.span, elem_types }));
			...
			Expr::Object(obj) => {
				...
				Some(Box::new(TsType::TsTypeLit(TsTypeLit { span:obj.span, members })))
			},
			Expr::Lit(lit) => {
				if as_const {
			...
			Expr::Fn(fn_expr) => {
				let return_type = fn_expr.function.return_type.map_or(any_type_ann(), |val| val);
				...
				Some(Box::new(TsType::TsFnOrConstructorType(TsFnOrConstructorType::TsFnType(
					TsFnType {
			...
			Decl::TsEnum(ts_enum) => {
				...
				for member in &mut ts_enum.members {
					if let Some(init) = &member.init {
						...
						member.init =
							if self.valid_enum_init_expr(init) { Some(init.clone()) } else { None };
					}
				}
	...
	fn pat_to_ts_fn_param(&mut self, pat:Pat) -> Option<TsFnParam> {
		match pat {
			...
			Pat::Assign(assign_pat) => {
				self.expr_to_ts_type(assign_pat.right, false, false).map(|param| {
...
fn any_type_ann() -> Box<TsTypeAnn> { type_ann(ts_keyword_type(TsKeywordTypeKind::TsAnyKeyword)) }

fn type_ann(ts_type:Box<TsType>) -> Box<TsTypeAnn> {
	Box::new(TsTypeAnn { span:DUMMY_SP, type_ann:ts_type })
}
...
fn ts_tuple_element(ts_type:Box<TsType>) -> TsTupleElement {
	TsTupleElement { label:None, span:DUMMY_SP, ty:ts_type }
}

fn ts_keyword_type(kind:TsKeywordTypeKind) -> Box<TsType> {
	Box::new(TsType::TsKeywordType(TsKeywordType { span:DUMMY_SP, kind }))
}

fn ts_lit_type(lit:TsLit) -> Box<TsType> {
	Box::new(TsType::TsLitType(TsLitType { lit, span:DUMMY_SP }))
}

fn regex_type() -> Box<TsType> { Box::new(TsType::TsTypeRef(type_ref("RegExp".into()))) }
