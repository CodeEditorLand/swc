...
fn parse_config(s:&str) -> Result<Config, serde_json::Error> { serde_json::from_str(s) }
...
fn get_files_list(
	...
	let files = if let Some(input_dir) = input_dir {
		...
		WalkDir::new(input_dir)
			...
			.filter(|e| e.is_file())
			.filter(|e| {
				extensions.iter().any(|ext| e.extension().map(|v| v == &**ext).unwrap_or(false))
			})
			.collect()
	} else {
...
fn emit_output(
	...
	if let Some(out_dir) = out_dir {
		...
		if let Some(ref source_map) = output.map {
			...
			output.code.push_str(&source_map_path.file_name().unwrap().to_string_lossy());
		...
		let source_map = if let Some(ref source_map) = output.map { &**source_map } else { "" };
...
fn collect_stdin_input() -> Option<String> {
	...
	if result.is_ok() && !buffer.is_empty() { Some(buffer) } else { None }
}
...
impl CompileOptions {
	fn build_transform_options(&self, file_path:&Option<&Path>) -> anyhow::Result<Options> {
		...
		if let Some(file_path) = *file_path {
			file_path.to_str().unwrap_or_default().clone_into(&mut options.filename);
		}
		...
		if let Some(source_maps) = &self.source_maps {
			...
			self.source_file_name.clone_into(&mut options.source_file_name);
	...
	fn execute_inner(&self) -> anyhow::Result<()> {
		...
		if let Some(single_out_file) = self.out_file.as_ref() {
			let result:anyhow::Result<Vec<TransformOutput>> = inputs
				.into_par_iter()
				.map(|InputContext { compiler, fm, options, .. }| execute(compiler, fm, options))
				.collect();
					...
					if buf_srcmap.is_none() {
						let map_out_file = if let Some(source_map_target) = &self.source_map_target
						{
							...
							source_map_path = Some(
								map_out_file.file_name().unwrap().to_string_lossy().to_string(),
							);
							map_out_file
						};
			...
			inputs.into_par_iter().try_for_each(
				|InputContext { compiler, fm, options, file_path, file_extension }| {
