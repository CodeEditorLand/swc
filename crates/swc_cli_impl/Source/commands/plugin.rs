...
fn write_ignore_file(base_path:&Path) -> Result<()> {
	let ignore_list:Vec<String> =
		["/target", "^target/", "target"].iter().map(|v| v.to_string()).collect();
	...
	let mut f = OpenOptions::new().append(true).create(true).open(&ignore_file_path)?;
...
"#
		...
		let dist_output_path =
			format!("target/{}/release/{}.wasm", build_target, name.replace('-', "_"));
