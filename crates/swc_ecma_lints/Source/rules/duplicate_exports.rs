...
pub fn duplicate_exports() -> Box<dyn Rule> { visitor_rule(DuplicateExports::default()) }
			...
			if !self.exports.is_empty() {
				...
				HANDLER.with(|handler| {
					handler
						.struct_span_err(
							span,
							r#"An export assignment cannot be used in a module with other exported elements."#,
						)
						.emit()
				});
			}
