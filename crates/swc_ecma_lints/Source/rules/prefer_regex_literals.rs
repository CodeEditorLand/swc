...
pub fn prefer_regex_literals(
	...
	let disallow_redundant_wrapping =
		config.get_rule_config().disallow_redundant_wrapping.unwrap_or(false);

	match rule_reaction {
		LintRuleReaction::Off => None,
		_ => {
			Some(visitor_rule(PreferRegexLiterals::new(
				rule_reaction,
...
impl PreferRegexLiterals {
	...
	fn set_state(&mut self, call_span:Span, args:&[ExprOrSpread]) {
		...
		if let Some(ExprOrSpread { expr, .. }) = args.first() {
			self.first_arg =
				Some(extract_arg_val(self.unresolved_ctxt, expr.unwrap_seqs_and_parens()));
		}
