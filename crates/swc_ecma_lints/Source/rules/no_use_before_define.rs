...
impl NoUseBeforeDefine {
	...
	fn insert_ident_to_current_scope(&mut self, id:Id, span:Span) {
		...
		self.scoped_indents.get_mut(current_scope).unwrap().insert(id.clone());

		let spans_block_id = SpansScopeId { ident_id:id, block_span:*current_scope };
	...
	fn get_ident_spans_in_current_scope(&self, id:&Id) -> &AHashSet<Span> {
		...
		let spans_block_id = SpansScopeId { ident_id:id.clone(), block_span:*current_block };
	...
	fn emit_report(&self, es6_var_check:bool, span:Span, name:&str) {
		...
		let expected_reaction =
			if es6_var_check { LintRuleReaction::Error } else { self.expected_reaction };
