use swc_common::{Span, collections::AHashMap, errors::HANDLER};
use swc_ecma_ast::*;
use swc_ecma_visit::{Visit, VisitWith, noop_visit_type};

use crate::rule::{Rule, visitor_rule};

pub fn const_assign() -> Box<dyn Rule> { visitor_rule(ConstAssign::default()) }
	...
	fn check(&mut self, id:&Ident) {
		...
		if let Some(&decl_span) = self.const_vars.get(&id.to_id()) {
			HANDLER.with(|handler| {
				handler
					.struct_span_err(id.span, "cannot reassign to a variable declared with `const`")
...
impl Visit for ConstAssign {
	...
	fn visit_binding_ident(&mut self, n:&BindingIdent) { self.check(&Ident::from(n)); }
