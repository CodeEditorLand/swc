...
impl DefaultParamLast {
	pub fn new(config:&RuleConfig<()>) -> Self {
		Self { expected_reaction:config.get_rule_reaction() }
	}
...
impl Visit for DefaultParamLast {
	...
	fn visit_constructor(&mut self, constructor:&Constructor) {
		constructor.params.iter().fold(false, |seen, param| {
			match param {
				ParamOrTsParamProp::Param(Param { pat, span, .. }) => {
					match pat {
				...
				ParamOrTsParamProp::TsParamProp(TsParamProp { param, span, .. }) => {
					match param {
	...
	fn visit_class_method(&mut self, class_method:&ClassMethod) {
		self.check(class_method.function.params.iter().map(|param| (&param.pat, param.span)));
