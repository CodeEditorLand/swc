...
pub fn all(lint_params:LintParams) -> Vec<Box<dyn Rule>> {
	...
	#[cfg(feature = "non_critical_lints")]
	{
		...
		rules.extend(no_console::no_console(&lint_config.no_console, unresolved_ctxt));

		rules.extend(no_alert::no_alert(&lint_config.no_alert, unresolved_ctxt, es_version));
		...
		rules.extend(dot_notation::dot_notation(program, &lint_config.dot_notation));
		...
		rules.extend(no_empty_pattern::no_empty_pattern(&lint_config.no_empty_pattern));
		...
		rules.extend(no_restricted_syntax::no_restricted_syntax(&lint_config.no_restricted_syntax));

		rules.extend(radix::radix(unresolved_ctxt, &lint_config.radix));

		rules.extend(no_bitwise::no_bitwise(&lint_config.no_bitwise));

		rules.extend(default_param_last::default_param_last(&lint_config.default_param_last));

		rules.extend(yoda::yoda(&lint_config.yoda));

		rules.extend(no_new_symbol::no_new_symbol(unresolved_ctxt, &lint_config.no_new_symbol));

		rules.extend(use_is_nan::use_is_nan(unresolved_ctxt, &lint_config.use_isnan));

		rules.extend(valid_typeof::valid_typeof(&lint_config.valid_typeof));

		rules.extend(no_param_reassign::no_param_reassign(&lint_config.no_param_reassign));
		...
		rules.extend(no_obj_calls::no_obj_calls(unresolved_ctxt, &lint_config.no_obj_calls));

		rules.extend(no_throw_literal::no_throw_literal(&lint_config.no_throw_literal));

		rules.extend(no_var::no_var(&lint_config.no_var));

		rules.extend(prefer_const::prefer_const(&lint_config.prefer_const));

		rules.extend(no_compare_neg_zero::no_compare_neg_zero(&lint_config.no_compare_neg_zero));

		rules.extend(constructor_super::constructor_super(&lint_config.constructor_super));

		rules.extend(no_sparse_arrays::no_sparse_arrays(&lint_config.no_sparse_arrays));

		rules.extend(default_case_last::default_case_last(&lint_config.default_case_last));

		rules.extend(no_await_in_loop::no_await_in_loop(&lint_config.no_await_in_loop));
		...
		rules.extend(no_new_object::no_new_object(unresolved_ctxt, &lint_config.no_new_object));
