...
use crate::rule::{Rule, visitor_rule};

pub fn duplicate_bindings() -> Box<dyn Rule> { visitor_rule(DuplicateBindings::default()) }
...
impl DuplicateBindings {
	...
	fn is_unique_var_kind(&self) -> bool {
		matches!(self.var_decl_kind, Some(VarDeclKind::Const) | Some(VarDeclKind::Let))
	}
	...
	fn visit_with_stmts(&mut self, s:&[Stmt], lexical_function:bool) {
		...
		if lexical_function {
			self.visit_with_stmt_like(s, |s| {
				match s {
					Stmt::Decl(Decl::Fn(FnDecl { ident, function: f, .. })) if f.body.is_some() => {
						Some(ident.clone())
					},
					_ => None,
				}
...
impl Visit for DuplicateBindings {
	...
	fn visit_function(&mut self, f:&Function) {
		...
		let Function { body, params, decorators, .. } = f;
	...
	fn visit_static_block(&mut self, c:&StaticBlock) { self.visit_with_stmts(&c.body.stmts, false) }

	// block stmt and case block
	fn visit_stmts(&mut self, b:&[Stmt]) { self.visit_with_stmts(b, true) }
	...
	fn visit_class_decl(&mut self, d:&ClassDecl) {
		...
		self.add(
			d.ident.sym.clone(),
			BindingInfo { span:d.ident.span, ctxt:d.ident.ctxt, unique:true, is_function:false },
		);
	...
	fn visit_ts_import_equals_decl(&mut self, s:&TsImportEqualsDecl) {
		...
		if !s.is_type_only && !self.type_bindings.contains(&s.id.to_id()) {
			self.add(
				s.id.sym.clone(),
				BindingInfo { span:s.id.span, ctxt:s.id.ctxt, unique:true, is_function:false },
			);
		}
	}

	fn visit_module(&mut self, m:&Module) {
		m.visit_with(&mut TypeCollector { type_bindings:&mut self.type_bindings });

		self.lexical_function = true;

		self.visit_with_stmt_like(&m.body, |s| {
			match s {
				ModuleItem::Stmt(Stmt::Decl(Decl::Fn(FnDecl { ident, function: f, .. })))
				| ModuleItem::ModuleDecl(
					ModuleDecl::ExportDecl(ExportDecl {
						decl: Decl::Fn(FnDecl { ident, function: f, .. }),
						..
					})
					| ModuleDecl::ExportDefaultDecl(ExportDefaultDecl {
						decl: DefaultDecl::Fn(FnExpr { ident: Some(ident), function: f }),
						..
					}),
	...
	fn visit_script(&mut self, s:&Script) {
		s.visit_with(&mut TypeCollector { type_bindings:&mut self.type_bindings });
	...
	fn visit_param(&mut self, p:&Param) { self.visit_with_kind(p, Some(VarDeclKind::Var)) }
}
...
fn emit_error(name:&str, span:Span, prev_span:Span) {
	HANDLER.with(|handler| {
		handler
			.struct_span_err(span, &format!("the name `{}` is defined multiple times", name))
