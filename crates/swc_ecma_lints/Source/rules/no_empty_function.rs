...
use swc_common::{SourceMap, Span, collections::AHashSet, errors::HANDLER};
use swc_ecma_ast::*;
...
pub fn no_empty_function(
	...
	match config.get_rule_reaction() {
		...
		_ => Some(visitor_rule(NoEmptyFunction::new(source_map.clone(), config))),
	}
}
...
impl NoEmptyFunction {
	...
	fn emit_report(&self, span:Span, target_type:&str, function_type:Option<&FunctionModifiers>) {
		let message = if let Some(fn_modifier) = function_type {
			format!("Unexpected empty {} {} pattern", fn_modifier.get_human_readable(), target_type)
		} else {
		...
		if let Some(BlockStmt { span, stmts, .. }) = &constructor.body {
			...
			if stmts.is_empty() {
				let (count, modifiers) = get_modifiers::<1>(FunctionMarkers {
					...
					is_protected:constructor.accessibility.eq(&Some(Accessibility::Protected)),
					..Default::default()
				});
		...
		if let Some(BlockStmt { span, stmts, .. }) = &method.body {
			...
			if stmts.is_empty() {
				let (count, modifiers) = get_modifiers::<3>(FunctionMarkers {
					...
					is_protected:class_method.accessibility.eq(&Some(Accessibility::Protected)),
					..Default::default()
				});
