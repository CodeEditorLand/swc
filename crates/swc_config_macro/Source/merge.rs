...
pub fn expand(input:DeriveInput) -> TokenStream {
	match &input.data {
		...
		syn::Data::Enum(_) => {
			unimplemented!("derive(Merge) does not support an enum")
		},
		syn::Data::Union(_) => {
			unimplemented!("derive(Merge) does not support a union")
		},
	}
}
...
fn call_merge_for_fields(obj:&dyn ToTokens, fields:&Fields) -> Vec<Stmt> {
	...
	match fields {
		...
		Fields::Unit => {
			unimplemented!("derive(Merge) does not support a unit struct")
		},
	}
}
