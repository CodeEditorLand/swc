...
use swc_ecma_ast::{AssignOp, EsVersion, Ident, op};

use self::{
	comments_buffer::CommentsBuffer,
	state::State,
	table::{BYTE_HANDLERS, ByteHandler},
	util::*,
};
pub use self::{
	input::Input,
...
impl From<char> for Char {
	fn from(c:char) -> Self { Char(c as u32) }
}

impl From<u32> for Char {
	fn from(c:u32) -> Self { Char(c) }
}
...
impl<'a> Lexer<'a> {
	...
	fn read_token(&mut self) -> LexResult<Option<Token>> {
		...
		match handler {
			...
			None => {
				...
				self.error_span(pos_span(start), SyntaxError::UnexpectedChar { c:byte as _ })
			},
		}
	}
	...
	fn read_token_dot(&mut self) -> LexResult<Token> {
		...
		if next.is_ascii_digit() {
			return self.read_number(true).map(|v| {
				match v {
	...
	fn read_token_zero(&mut self) -> LexResult<Token> {
		...
		let bigint = match next {
			...
			_ => {
				return self.read_number(false).map(|v| {
					match v {
						Left((value, raw)) => Token::Num { value, raw },
						Right((value, raw)) => Token::BigInt { value, raw },
					}
	...
	fn read_token_logical(&mut self, c:u8) -> LexResult<Token> {
		...
		let token = if c == b'&' { BinOpToken::BitAnd } else { BinOpToken::BitOr };
	...
	fn read_token_plus_minus(&mut self, c:u8) -> LexResult<Option<Token>> {
		...
		Ok(Some(if self.input.cur() == Some(c as char) {
			...
			if c == b'+' { Token::PlusPlus } else { Token::MinusMinus }
		} else if self.input.eat_byte(b'=') {
			Token::AssignOp(if c == b'+' { AssignOp::AddAssign } else { AssignOp::SubAssign })
		} else {
			Token::BinOp(if c == b'+' { BinOpToken::Add } else { BinOpToken::Sub })
		}))
	}
...
impl Lexer<'_> {
	...
	fn read_slash(&mut self) -> LexResult<Option<Token>> {
		...
		Ok(Some(if self.eat(b'=') { tok!("/=") } else { tok!('/') }))
	}
	...
	fn read_token_lt_gt(&mut self) -> LexResult<Option<Token>> {
		...
		let mut op = if c == '<' { BinOpToken::Lt } else { BinOpToken::Gt };

		// '<<', '>>'
		if self.cur() == Some(c) {
			self.bump();

			op = if c == '<' { BinOpToken::LShift } else { BinOpToken::RShift };
	...
	fn read_word_with(
		...
		if has_escape && self.ctx.is_reserved(&word) {
			self.error(start, SyntaxError::EscapeInReservedWord { word:word.into() })?
		} else {
				...
				if let Some(c) = l.input.cur_as_ascii() {
					...
					if c == b'\\' {
						...
						if let Some(c) = chars.first() {
							let valid = if first { c.is_ident_start() } else { c.is_ident_part() };
		...
		let c = match self.read_int_u32::<16>(if is_curly { 0 } else { 4 }) {
			...
			_ => {
				...
				self.error(
					start,
					SyntaxError::BadCharacterEscapeSequence {
						expected:if is_curly { "1-6 hex characters" } else { "4 hex characters" },
					},
				)?
			},
		};
	...
	fn read_str_lit(&mut self) -> LexResult<Token> {
		...
		self.with_buf(|l, buf| {
			...
			Ok(Token::Str { value:l.atoms.atom(&*buf), raw:l.atoms.atom(raw) })
		})
	}
		...
		let flags = {
			match self.cur() {
				Some(c) if c.is_ident_start() => {
					self.read_word_as_str_with(|l, s, _, _| l.atoms.atom(s)).map(Some)
				},
				_ => Ok(None),
			}
		}?
	...
	fn read_tmpl_token(&mut self, start_of_tpl:BytePos) -> LexResult<Token> {
		...
		while let Some(c) = self.cur() {
			if c == '`' || (c == '$' && self.peek() == Some('{')) {
				...
				return Ok(Token::Template { cooked, raw:self.atoms.atom(raw) });
			}
	...
	pub fn had_line_break_before_last(&self) -> bool { self.state.had_line_break }

	#[inline]
	pub fn set_expr_allowed(&mut self, allow:bool) { self.state.is_expr_allowed = allow; }

	#[inline]
	pub fn set_next_regexp(&mut self, start:Option<BytePos>) { self.state.next_regexp = start; }
}

fn pos_span(p:BytePos) -> Span { Span::new(p, p) }

fn fixed_len_span(p:BytePos, len:u32) -> Span { Span::new(p, p + BytePos(len)) }
