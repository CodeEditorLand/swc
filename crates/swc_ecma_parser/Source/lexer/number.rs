...
impl Lexer<'_> {
	...
	pub(super) fn read_number(
		...
		let val = if starts_with_dot {
			...
			if self.eat(b'n') {
				...
				return Ok(Either::Right((Box::new(s.into_value()), self.atoms.atom(raw))));
			}
		...
		match self.cur() {
			Some('e') | Some('E') => {
				...
				val = if exp == f64::INFINITY {
					if positive && val != 0.0 { f64::INFINITY } else { 0.0 }
				} else {
					let end = self.cur_pos();
	...
	pub(super) fn read_int<const RADIX: u8>(&mut self, len:u8) -> LexResult<Option<f64>> {
		...
		if len != 0 && count != len as u16 { Ok(None) } else { Ok(v) }
	}
	...
	pub(super) fn read_int_u32<const RADIX: u8>(&mut self, len:u8) -> LexResult<Option<u32>> {
		...
		if len != 0 && count != len { Ok(None) } else { Ok(v) }
	}
...
mod tests {
	...
	where
		...
		crate::with_test_sess(s, |_, input| {
			let mut l = Lexer::new(Syntax::Es(Default::default()), Default::default(), input, None);
	...
	fn num(s:&'static str) -> (f64, Atom) {
		lex(s, |l| l.read_number(s.starts_with('.')).unwrap().left().unwrap())
	}

	fn int<const RADIX: u8>(s:&'static str) -> u32 {
		lex(s, |l| l.read_int_u32::<RADIX>(0).unwrap().expect("read_int returned None"))
	}
	...
	fn num_very_big_exp() {
		...
		assert_eq!(num(LARGE_POSITIVE_EXP), (f64::INFINITY, LARGE_POSITIVE_EXP.into()));

		assert_eq!(num(LARGE_NEGATIVE_EXP), (0.0, LARGE_NEGATIVE_EXP.into()));

		assert_eq!(num(ZERO_WITH_LARGE_POSITIVE_EXP), (0.0, ZERO_WITH_LARGE_POSITIVE_EXP.into()));

		assert_eq!(num(ZERO_WITH_LARGE_NEGATIVE_EXP), (0.0, ZERO_WITH_LARGE_NEGATIVE_EXP.into()));
	...
	#[test]
	fn issue_480() { assert_eq!((9.09, "9.09".into()), num("9.09")) }
	...
	fn read_bigint() {
		assert_eq!(
			lex("10000000000000000000000000000000000000000000000000000n", |l| {
				l.read_number(false).unwrap().right().unwrap()
			}),
			(
				Box::new(
	...
	fn large_bin_number() {
		...
		assert_eq!(
			lex(VERY_LARGE_BINARY_NUMBER, |l| {
				l.read_radix_number::<2>().unwrap().left().unwrap()
			}),
	...
	fn test_floats(strict:bool, success:bool, cases:&'static [&'static str]) {
		for case in cases {
			println!("Testing {} (when strict = {}); Expects success = {}", case, strict, success);
			...
			let expected:f64 = (i64::from_str_radix(case, 8).map(|v| v as f64))
				...
				.unwrap_or_else(|err| {
					panic!("failed to parse '{}' as float using str.parse(): {}", case, err)
				});
			...
			if success {
				...
				assert_ne!(
					vec![Token::Num { value:expected, raw:expected.to_string().into() }],
					vec
				)
			}
		}
