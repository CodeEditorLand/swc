...
enum TokenType {
	...
	JSXTagEnd,
	Arrow,
	Other { before_expr:bool, can_have_trailing_comment:bool },
}
impl TokenType {
	#[inline]
				...
				TokenType::Other {
					...
					can_have_trailing_comment:matches!(
						...
						TokenKind::Num { .. }
							...
							| TokenKind::Regex | TokenKind::BigInt { .. }
							| TokenKind::JSXText { .. }
							| TokenKind::RBrace
					),
				}
...
impl Tokens for Lexer<'_> {
	...
	#[inline]
	fn set_expr_allowed(&mut self, allow:bool) { self.set_expr_allowed(allow) }

	#[inline]
	fn set_next_regexp(&mut self, start:Option<BytePos>) { self.state.next_regexp = start; }

	#[inline]
	fn token_context(&self) -> &TokenContexts { &self.state.context }

	#[inline]
	fn token_context_mut(&mut self) -> &mut TokenContexts { &mut self.state.context }

	#[inline]
	fn set_token_context(&mut self, c:TokenContexts) { self.state.context = c; }
	...
	fn end_pos(&self) -> BytePos { self.input.end_pos() }
}
...
impl Iterator for Lexer<'_> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		token.map(|token| {
			...
			TokenAndSpan { token, had_line_break:self.had_line_break_before_last(), span }
		})
	}
}
...
impl State {
	pub fn can_skip_space(&self) -> bool {
		!self.context.current().map(|t| t.preserve_space()).unwrap_or(false)
	}
	...
	pub fn can_have_trailing_comment(&self) -> bool {
		match self.token_type {
			...
			Some(TokenType::Other { can_have_trailing_comment, .. }) => can_have_trailing_comment,
			_ => false,
		}
	}
	...
	fn update(&mut self, start:BytePos, next:TokenKind) {
		if cfg!(feature = "debug") {
			trace!("updating state: next={:?}, had_line_break={} ", next, self.had_line_break);
		}
...
impl TokenContexts {
	...
	fn is_brace_block(
		...
		if had_line_break {
			if let Some(TokenType::Other { before_expr: false, .. }) = prev {
				return true;
			}
		}
	...
	#[inline]
	pub fn current(&self) -> Option<TokenContext> { self.0.last().cloned() }
...
pub(crate) fn lex_tokens(syntax:Syntax, s:&'static str) -> Vec<Token> {
	with_lexer(syntax, Default::default(), s, |l| Ok(l.map(|ts| ts.token).collect())).unwrap()
}
