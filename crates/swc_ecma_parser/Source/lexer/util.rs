...
use super::{
	...
	LexResult,
	Lexer,
	comments_buffer::BufferedComment,
	input::Input,
...
impl Lexer<'_> {
 ...
 start = {}, end = {}",
	...
	#[inline(always)]
	pub(super) fn is(&mut self, c:u8) -> bool { self.input.is_byte(c) }

	#[inline(always)]
	pub(super) fn is_str(&self, s:&str) -> bool { self.input.is_str(s) }

	#[inline(always)]
	pub(super) fn eat(&mut self, c:u8) -> bool { self.input.eat_byte(c) }

	#[inline(always)]
	pub(super) fn cur(&mut self) -> Option<char> { self.input.cur() }

	#[inline(always)]
	pub(super) fn peek(&mut self) -> Option<char> { self.input.peek() }

	#[inline(always)]
	pub(super) fn peek_ahead(&mut self) -> Option<char> { self.input.peek_ahead() }

	#[inline(always)]
	pub(super) fn cur_pos(&mut self) -> BytePos { self.input.cur_pos() }

	#[inline(always)]
	pub(super) fn last_pos(&self) -> BytePos { self.input.last_pos() }
	...
	pub(super) fn skip_space<const LEX_COMMENTS: bool>(&mut self) {
		loop {
			let (offset, newline) = {
				let mut skip =
					SkipWhitespace { input:self.input.as_str(), newline:false, offset:0 };
	...
	pub(super) fn skip_line_comment(&mut self, start_skip:usize) {
		...
		let idx = self.input.as_str().find(['\r', '\n', '\u{2028}', '\u{2029}']).map_or(
			self.input.as_str().len(),
			|v| {
				self.state.had_line_break = true;
	...
	#[inline(never)]
	fn store_comment(&mut self, is_for_next:bool, start:BytePos, end:BytePos, slice_start:BytePos) {
		if let Some(comments) = self.comments_buffer.as_mut() {
			let src = unsafe {
...
impl CharExt for Char {
	#[inline(always)]
	fn to_char(self) -> Option<char> { char::from_u32(self.0) }
}

impl CharExt for char {
	#[inline(always)]
	fn to_char(self) -> Option<char> { Some(self) }
}
