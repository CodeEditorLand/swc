...
trait LineBreak: Into<TokenAndSpan> {
	fn lb(self) -> TokenAndSpan { TokenAndSpan { had_line_break:true, ..self.into() } }
}
...
trait WithSpan: Sized {
	...
	where
		R: SpanRange, {
		TokenAndSpan { token:self.into_token(), had_line_break:false, span:span.into_span() }
	}
...
impl WithSpan for usize {
	fn into_token(self) -> Token { Num { value:self as f64, raw:self.to_string().into() } }
}
impl WithSpan for f64 {
	fn into_token(self) -> Token { Num { value:self, raw:self.to_string().into() } }
}
impl WithSpan for &str {
	fn into_token(self) -> Token { Word(Word::Ident(self.into())) }
}
impl WithSpan for Keyword {
	fn into_token(self) -> Token { Word(Word::Keyword(self)) }
}
impl WithSpan for Word {
	fn into_token(self) -> Token { Word(self) }
}
impl WithSpan for BinOpToken {
	fn into_token(self) -> Token { BinOp(self) }
}
impl WithSpan for AssignOp {
	fn into_token(self) -> Token { AssignOp(self) }
}
...
fn test262_lexer_error_0001() {
	assert_eq!(
		...
		vec![
			TokenAndSpan {
				token:Num { value:123.0, raw:"123.".into() },
				had_line_break:true,
...
fn test262_lexer_error_0002() {
	assert_eq!(
		...
		vec![
			Token::Str { value:"use strict".into(), raw:"'use\\x20strict'".into() }
				.span(0..15)
				.lb(),
			Semi.span(15..16),
		]
...
fn ident_escape_unicode() {
	assert_eq!(lex(Syntax::default(), r"a\u0061"), vec!["aa".span(0..7).lb()]);
}
...
fn ident_escape_unicode_2() {
	...
	assert_eq!(lex(Syntax::default(), r"℘\u2118"), vec!["℘℘".span(0..9).lb()]);
}
...
fn tpl_raw_unicode_escape() {
	assert_eq!(
		...
		vec![
			tok!('`'),
			Token::Template { cooked:Ok(format!("{}", '\u{0010}').into()), raw:"\\u{0010}".into() },
			tok!('`'),
		]
	);
}
...
fn tpl_invalid_unicode_escape() {
		...
		vec![
			...
			Token::Template {
				cooked:Err(Error::new(
					Span { lo:BytePos(2), hi:BytePos(4) },
					SyntaxError::BadCharacterEscapeSequence { expected:"4 hex characters" }
				)),
				raw:"\\unicode".into(),
			},
			tok!('`'),
	...
	assert_eq!(
		...
		vec![
			...
			Token::Template {
				cooked:Err(Error::new(
					Span { lo:BytePos(2), hi:BytePos(5) },
					SyntaxError::BadCharacterEscapeSequence { expected:"1-6 hex characters" }
				)),
				raw:"\\u{".into(),
			},
			tok!('`'),
	...
	assert_eq!(
		...
		vec![
			...
			Token::Template {
				cooked:Err(Error::new(
					Span { lo:BytePos(2), hi:BytePos(4) },
					SyntaxError::BadCharacterEscapeSequence { expected:"2 hex characters" }
				)),
				raw:"\\xhex".into(),
			},
			tok!('`'),
...
fn str_escape_3() {
	assert_eq!(
		...
		vec![Token::Str { value:"\x00".into(), raw:"'\\x00'".into() }]
	);
}
...
fn str_escape_hex() {
	assert_eq!(
		...
		vec![Token::Str { value:"a".into(), raw:"'\\x61'".into() }.span(0..6).lb(),]
	);
}
...
fn str_escape_octal() {
	assert_eq!(
		...
		vec![
			Token::Str { value:"Hello\nWorld".into(), raw:"'Hello\\012World'".into() }
				.span(0..16)
				.lb(),
		]
	)
...
fn str_escape_unicode_long() {
	assert_eq!(
		...
		vec![
			Token::Str { value:"4".into(), raw:"'\\u{00000000034}'".into() }
				.span(0..17)
				.lb(),
		]
	);
...
fn ident_paren() {
	assert_eq!(
		...
		vec!["a".span(0).lb(), LParen.span(1), "bc".span(2..4), RParen.span(4), Semi.span(5),]
	);
}
	...
	assert_eq!(
		...
		vec![
			TokenAndSpan {
				...
				had_line_break:true,
				span:Span { lo:BytePos(1), hi:BytePos(2) },
			},
			42.span(1..3),
			BinOp(Div).span(3)
		]
...
fn complex_regex() {
	assert_eq!(
		...
		vec![
			...
			RBrace,
			BinOp(Div),
			Num { value:42.0, raw:Atom::new("42") },
			BinOp(Div),
...
/hi/g.exec(c).map(d);"
	...
	assert_eq!(lex_tokens(Syntax::default(), "a = b / hi / g.exec(c).map(d);"), expected);
}
...
fn migrated_0006() {
	...
	assert_eq!(
		...
		vec![
			...
			BinOp(Div).span(3),
			TokenAndSpan {
				token:Num { value:42.0, raw:"42".into() },
...
fn str_lit() {
	assert_eq!(
		...
		vec![Token::Str { value:"abcde".into(), raw:"'abcde'".into() }],
	);
	...
	assert_eq!(
		...
		vec![Token::Str { value:"русский".into(), raw:"'русский'".into() }],
	);

	assert_eq!(
		lex_tokens(Syntax::default(), "'\\x32'"),
		vec![Token::Str { value:"2".into(), raw:"'\\x32'".into() }],
	);

	assert_eq!(
		lex_tokens(Syntax::default(), "'\\u1111'"),
		vec![Token::Str { value:"ᄑ".into(), raw:"'\\u1111'".into() }],
	);

	assert_eq!(
		lex_tokens(Syntax::default(), "'\\u{1111}'"),
		vec![Token::Str { value:"ᄑ".into(), raw:"'\\u{1111}'".into() }],
	);

	assert_eq!(
		lex_tokens(Syntax::default(), "'\t'"),
		vec![Token::Str { value:"\t".into(), raw:"'\t'".into() }],
	);
	...
	assert_eq!(
		lex_tokens(Syntax::default(), "'\\\nabc'"),
		vec![Token::Str { value:"abc".into(), raw:"'\\\nabc'".into() }]
	);
	...
	assert_eq!(
		...
		vec![Token::Str { value:"\u{0000}".into(), raw:"'\\0'".into() }],
	);
	...
	assert_eq!(
		lex_tokens(Syntax::default(), "'\\r'"),
		vec![Token::Str { value:"\r".into(), raw:"'\\r'".into() }],
	);
	...
	assert_eq!(
		...
		vec![Token::Str { value:"\u{0007}".into(), raw:"'\\07'".into() }],
	);

	assert_eq!(
		lex_tokens(Syntax::default(), "'\\08'"),
		vec![Token::Str { value:"\u{0000}8".into(), raw:"'\\08'".into() }],
	);
}
...
fn tpl_empty() {
	assert_eq!(
		lex_tokens(Syntax::default(), r#"``"#),
		vec![tok!('`'), Template { raw:"".into(), cooked:Ok("".into()) }, tok!('`')]
	)
}
...
fn tpl() {
	assert_eq!(
		...
		vec![
			tok!('`'),
			Template { raw:"".into(), cooked:Ok("".into()) },
			tok!("${"),
			Word(Word::Ident("a".into())),
			tok!('}'),
			Template { raw:"".into(), cooked:Ok("".into()) },
	...
	assert_eq!(
		...
		vec![
			...
			Word(Word::Ident("a".into())),
			tok!('}'),
			Template { raw:"test".into(), cooked:Ok("test".into()) },
			tok!('`'),
		]
	);
}
...
fn jsx_01() {
	assert_eq!(
		lex_tokens(crate::Syntax::Es(crate::EsSyntax { jsx:true, ..Default::default() }), "<a />"),
		vec![
			Token::JSXTagStart,
...
fn jsx_02() {
	assert_eq!(
		...
		vec![
			...
			Token::JSXName { name:"a".into() },
			Token::JSXTagEnd,
			Token::JSXText { raw:"foo".into(), value:"foo".into() },
			Token::JSXTagStart,
			tok!('/'),
			Token::JSXName { name:"a".into() },
			Token::JSXTagEnd,
...
#[test]
fn max_integer() { lex_tokens(crate::Syntax::default(), "1.7976931348623157e+308"); }
...
fn issue_191() {
	assert_eq!(
		...
		vec![
			tok!('`'),
			Token::Template { raw:"".into(), cooked:Ok("".into()) },
			tok!("${"),
			Token::Word(Word::Ident("foo".into())),
			tok!('}'),
			Token::Template { raw:"<bar>".into(), cooked:Ok("<bar>".into()) },
			tok!('`')
		]
	);
}
...
fn issue_299_01() {
	assert_eq!(
		...
		vec![
			Token::JSXTagStart,
			Token::JSXName { name:"Page".into() },
			Token::JSXName { name:"num".into() },
			tok!('='),
			Token::Str { value:"\\ ".into(), raw:"'\\ '".into() },
			Token::JSXTagEnd,
			JSXText { raw:"ABC".into(), value:"ABC".into() },
			JSXTagStart,
			tok!('/'),
			JSXName { name:"Page".into() },
			JSXTagEnd,
			Semi,
		]
	);
...
fn jsx_string_1() {
	assert_eq!(
		...
		vec![
			...
			Token::JSXName { name:"num".into() },
			tok!('='),
			Token::Str { value:"abc".into(), raw:"'abc'".into() },
			Token::JSXTagEnd,
			JSXText { raw:"ABC".into(), value:"ABC".into() },
			JSXTagStart,
			tok!('/'),
...
fn jsx_string_4() {
	assert_eq!(
		...
		vec![
			...
			Token::JSXName { name:"num".into() },
			tok!('='),
			Token::Str { value:"³".into(), raw:"'&sup3;'".into() },
			Token::JSXTagEnd,
			JSXText { raw:"ABC".into(), value:"ABC".into() },
			JSXTagStart,
			tok!('/'),
...
fn issue_401() {
	assert_eq!(
		...
		vec![Token::Str { value:"17".into(), raw:"'17'".into() }, tok!("as"), tok!("const")],
	);
}
...
fn issue_481() {
	assert_eq!(
		...
		vec![
			Token::JSXTagStart,
			Token::JSXName { name:"span".into() },
			Token::JSXTagEnd,
			JSXText { raw:" ".into(), value:" ".into() },
			LBrace,
			Word(Word::Ident("foo".into())),
			RBrace,
			JSXTagStart,
			BinOp(Div),
			JSXName { name:"span".into() },
			JSXTagEnd,
		]
	);
}
...
fn bench_simple(b:&mut Bencher, s:&str) { bench(b, Default::default(), s) }
...
fn issue_2853_1_js() {
	...
	assert_eq!(
		...
		vec![
			...
			Token::Str { value:"\u{0000}a".into(), raw:"\"\\0a\"".into() }
		],
	);
}
...
fn issue_2853_2_ts() {
	let (tokens, errors) =
		lex_errors(crate::Syntax::Typescript(Default::default()), "const a = \"\\0a\"");
...
fn issue_2853_3_js() {
	let (tokens, errors) =
		lex_errors(crate::Syntax::Es(Default::default()), "const a = \"\u{0000}a\"");
	...
	assert_eq!(
		...
		vec![
			...
			Word(Word::Ident("a".into())),
			Token::AssignOp(AssignOp::Assign),
			Token::Str { value:"\u{0000}a".into(), raw:"\"\u{0000}a\"".into() }
		],
	);
}
...
fn issue_2853_4_ts() {
	let (tokens, errors) =
		lex_errors(crate::Syntax::Typescript(Default::default()), "const a = \"\u{0000}a\"");
...
fn issue_2853_6_tsx() {
	let (tokens, errors) = lex_errors(
		crate::Syntax::Typescript(crate::TsSyntax { tsx:true, ..Default::default() }),
		"const a = \"\\0a\"",
	);
