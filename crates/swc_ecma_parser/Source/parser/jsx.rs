...
impl<I:Tokens> Parser<I> {
	...
	pub(super) fn parse_jsx_empty_expr(&mut self) -> PResult<JSXEmptyExpr> {
		...
		Ok(JSXEmptyExpr { span:Span::new(start, start) })
	}
	...
	pub(super) fn parse_jsx_spread_child(&mut self) -> PResult<JSXSpreadChild> {
		...
		Ok(JSXSpreadChild { span:span!(self, start), expr })
	}
	...
	pub(super) fn parse_jsx_expr_container(&mut self, _:BytePos) -> PResult<JSXExprContainer> {
		...
		Ok(JSXExprContainer { span:span!(self, start), expr })
	}
	...
	pub(super) fn parse_jsx_attr(&mut self) -> PResult<JSXAttrOrSpread> {
		...
		let value = if eat!(self, '=') {
			let ctx =
				Context { in_cond_expr:false, will_expect_colon_for_cond:false, ..self.ctx() };
		...
		Ok(JSXAttr { span:span!(self, start), name, value }.into())
	}
	...
	pub(super) fn parse_jsx_opening_element_at(
		...
		self.parse_jsx_opening_element_after_name(start, name).map(Either::Right)
	}
	...
	pub(super) fn parse_jsx_opening_element_after_name(
		...
		Ok(JSXOpeningElement { span:span!(self, start), name, attrs, self_closing, type_args })
	}
	...
	fn parse_jsx_closing_element_at(
		...
		Ok(Either::Right(JSXClosingElement { span:span!(self, start), name }))
	}
		...
		self.with_ctx(ctx).parse_with(|p| {
				...
				'contents: loop {
					match *cur!(p, true) {
						Token::JSXTagStart => {
							...
							children.push(p.parse_jsx_element_at(start).map(|e| {
								match e {
						...
						tok!('{') => {
							...
							if peeked_is!(p, "...") {
								...
								children.push(
									p.parse_jsx_expr_container(start).map(JSXElementChild::from)?,
								);
							}
						},
			...
			Ok(match (opening_element, closing_element) {
				...
				(Either::Left(opening), Some(Either::Left(closing))) => {
					Either::Left(JSXFragment { span, opening, children, closing })
				},
				(Either::Right(opening), None) => {
					Either::Right(JSXElement { span, opening, children, closing:None })
				},
				(Either::Right(opening), Some(Either::Right(closing))) => {
					...
					Either::Right(JSXElement { span, opening, children, closing:Some(closing) })
				},
	...
	pub(super) fn parse_jsx_element(&mut self) -> PResult<Either<JSXFragment, JSXElement>> {
		...
		let ctx = Context { in_cond_expr:false, will_expect_colon_for_cond:false, ..self.ctx() };
...
fn get_qualified_jsx_name(name:&JSXElementName) -> JsWord {
	fn get_qualified_obj_name(obj:&JSXObject) -> JsWord {
		match *obj {
			...
			JSXObject::JSXMemberExpr(ref member) => {
				format!("{}.{}", get_qualified_obj_name(&member.obj), member.prop.sym).into()
			},
		}
	}
