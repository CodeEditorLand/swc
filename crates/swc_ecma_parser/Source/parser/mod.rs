...
use swc_common::{BytePos, Span, collections::AHashMap, comments::Comments, input::StringInput};
use swc_ecma_ast::*;
...
impl<I:Tokens> Parser<I> {
	pub fn new_from(mut input:I) -> Self {
		#[cfg(feature = "typescript")]
		let in_declare = matches!(input.syntax(), Syntax::Typescript(TsSyntax { dts:true, .. }));
		#[cfg(not(feature = "typescript"))]
		let in_declare = false;

		let ctx = Context { in_declare, ..input.ctx() };

		input.set_ctx(ctx);

		Parser { state:Default::default(), input:Buffer::new(input) }
	}

	pub fn take_errors(&mut self) -> Vec<Error> { self.input().take_errors() }
	...
	pub fn parse_script(&mut self) -> PResult<Script> {
		...
		let ctx = Context { module:false, ..self.ctx() };
		...
		self.parse_block_body(true, true, None)
			.map(|body| Script { span:span!(self, start), body, shebang })
	}
	...
	pub fn parse_typescript_module(&mut self) -> PResult<Module> {
		...
		let ctx = Context { module:true, strict:false, ..self.ctx() };
		...
		self.parse_block_body(true, true, None)
			.map(|body| Module { span:span!(self, start), body, shebang })
	}
	...
	pub fn parse_program(&mut self) -> PResult<Program> {
		...
		let ctx = Context { can_be_module:true, ..self.ctx() };
		...
		let has_module_item = self.state.found_module_item
			|| body.iter().any(|item| matches!(item, ModuleItem::ModuleDecl(..)));

		if has_module_item && !self.ctx().module {
			let ctx = Context { module:true, can_be_module:true, strict:true, ..self.ctx() };
			// Emit buffered strict mode / module code violations
			self.input.set_ctx(ctx);
		}

		Ok(if has_module_item {
			Program::Module(Module { span:span!(self, start), body, shebang })
		} else {
			let body = body
				.into_iter()
				.map(|item| {
			...
			Program::Script(Script { span:span!(self, start), body, shebang })
		})
	}
	...
	fn ctx(&self) -> Context { self.input.get_ctx() }
