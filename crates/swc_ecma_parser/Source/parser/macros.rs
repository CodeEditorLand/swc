macro_rules! unexpected {
	($p:expr, $expected:literal) => {{
		...
		syntax_error!($p, $p.input.cur_span(), SyntaxError::Unexpected { got, expected:$expected })
	}};
}
	...
	($p:expr,false) => {{
		match $p.input.cur() {
			...
			None => {
				...
				Err(crate::error::Error::new(last, crate::error::SyntaxError::Eof))
			},
		}
	}};
...
macro_rules! make_error {
	($p:expr, $span:expr, $err:expr) => {{ crate::error::Error::new($span, $err) }};
}
