...
fn module(src:&'static str) -> Module { program(src).expect_module() }

/// Assert that Parser.parse_program returns [Program::Script].
fn script(src:&'static str) -> Script { program(src).expect_script() }
...
fn illegal_language_mode_directive1() {
	test_parser(r#"function f(a = 0) { "use strict"; }"#, Default::default(), |p| {
		let program = p.parse_program()?;
		...
		assert_eq!(
			errors,
			vec![Error::new(
				Span { lo:BytePos(21), hi:BytePos(34) },
...
fn illegal_language_mode_directive2() {
	test_parser(r#"let f = (a = 0) => { "use strict"; }"#, Default::default(), |p| {
		let program = p.parse_program()?;
		...
		assert_eq!(
			errors,
			vec![Error::new(
				Span { lo:BytePos(22), hi:BytePos(35) },
				crate::parser::SyntaxError::IllegalLanguageModeDirective
			)]
		);
...
#[test]
fn parse_non_strict_for_loop() { script("for (var v1 = 1 in v3) {}"); }
...
fn parse_program_take_script_module_errors() {
	test_parser(r#"077;"#, Default::default(), |p| {
		...
		assert_eq!(
			...
			vec![Error::new(
				Span { lo:BytePos(1), hi:BytePos(4) },
