use either::Either;
use swc_common::{Spanned, ast_node, util::take::Take};
...
impl<I:Tokens> Parser<I> {
	pub fn parse_expr(&mut self) -> PResult<Box<Expr>> {
		...
		if is!(self, ',') {
			...
			return Ok(SeqExpr { span:span!(self, start), exprs }.into());
		}
	...
	fn parse_assignment_expr_base(&mut self) -> PResult<Box<Expr>> {
		...
		{
			let ctx = Context { will_expect_colon_for_cond:false, ..self.ctx() };
			...
			let res = self.with_ctx(ctx).try_parse_ts(|p| {
				...
				match *arrow {
					Expr::Arrow(ArrowExpr { ref mut span, ref mut type_params, .. }) => {
		...
		match self.input.cur() {
			Some(&Token::AssignOp(op)) => {
				let left = if op == AssignOp::Assign {
					...
					if self.input.syntax().typescript()
						&& cond.as_ident().map(|i| i.is_reserved_in_strict_bind()).unwrap_or(false)
					{
	...
	fn parse_cond_expr(&mut self) -> PResult<Box<Expr>> {
		...
		if eat!(self, '?') {
			...
			let ctx = Context { in_cond_expr:true, will_expect_colon_for_cond:false, ..self.ctx() };

			let alt = self.with_ctx(ctx).parse_assignment_expr()?;

			let span = Span::new(start, alt.span_hi());

			Ok(CondExpr { span, test, cons, alt }.into())
		} else {
			Ok(test)
		}
	}
	...
	pub(super) fn parse_primary_expr(&mut self) -> PResult<Box<Expr>> {
		...
		let can_be_arrow = self.state.potential_arrow_start.map(|s| s == start).unwrap_or(false);
		...
		if let Some(tok) = self.input.cur() {
			match tok {
				tok!("this") => {
					...
					return Ok(ThisExpr { span:span!(self, start) }.into());
				},
		...
		if eat!(self, '#') {
			...
			return Ok(PrivateName { span:span!(self, start), name:id.sym }.into());
		}
	...
	fn parse_array_lit(&mut self) -> PResult<Box<Expr>> {
		...
		while !eof!(self) && !is!(self, ']') {
			...
			if !is!(self, ']') {
				...
				if is!(self, ']') {
					self.state.trailing_commas.insert(start, self.input.prev_span());
				}
			}
		}
	...
	fn parse_member_expr_or_new_expr(&mut self, is_new_expr:bool) -> PResult<Box<Expr>> {
		let ctx = Context { should_not_lex_lt_or_gt_as_type:true, ..self.ctx() };

		self.with_ctx(ctx).parse_member_expr_or_new_expr_inner(is_new_expr)
	}
	...
	fn parse_member_expr_or_new_expr_inner(&mut self, is_new_expr:bool) -> PResult<Box<Expr>> {
		...
		if eat!(self, "new") {
			if eat!(self, '.') {
				if eat!(self, "target") {
					...
					let expr = MetaPropExpr { span, kind:MetaPropKind::NewTarget }.into();
			...
			if is_new_expr {
				match *callee {
					Expr::OptChain(OptChainExpr { span, optional: true, .. }) => {
						syntax_error!(self, span, SyntaxError::OptChainCannotFollowConstructorCall)
					},

					Expr::Member(MemberExpr { ref obj, .. }) => {
						if let Expr::OptChain(OptChainExpr { span, optional: true, .. }) = **obj {
							syntax_error!(
								self,
								span,
			...
			let type_args = if self.input.syntax().typescript() && is!(self, '<') {
				self.try_parse_ts(|p| {
					let ctx = Context { should_not_lex_lt_or_gt_as_type:false, ..p.ctx() };
		...
		if eat!(self, "super") {
			let base = Callee::Super(Super { span:span!(self, start) });
		...
		let obj = if let Some(type_args) = type_args {
			...
			TsInstantiation { expr:obj, type_args, span:span!(self, start) }.into()
		} else {
			obj
		};
	...
	pub(super) fn parse_expr_or_spread(&mut self) -> PResult<ExprOrSpread> {
		...
		if eat!(self, "...") {
			...
			self.parse_assignment_expr().map(|expr| ExprOrSpread { spread:None, expr })
		}
	}
	...
	fn parse_paren_expr_or_arrow_fn(
		...
		let (paren_items, trailing_comma) =
			self.with_ctx(ctx).include_in_expr(true).parse_args_or_pats()?;

		let has_pattern =
			paren_items.iter().any(|item| matches!(item, AssignTargetOrSpread::Pat(..)));
		...
		if has_pattern || return_type.is_some() || is!(self, "=>") {
			if self.input.had_line_break_before_cur() {
				syntax_error!(self, span!(self, expr_start), SyntaxError::LineBreakBeforeArrow);
			}
		...
		if expr_or_spreads.len() == 1 {
			let expr = match expr_or_spreads.into_iter().next().unwrap() {
				ExprOrSpread { spread: Some(..), ref expr } => {
					syntax_error!(self, expr.span(), SyntaxError::SpreadInParenExpr)
				},
				ExprOrSpread { expr, .. } => expr,
			};

			Ok(ParenExpr { span:span!(self, expr_start), expr }.into())
		} else {
			...
			let seq_expr = SeqExpr {
				span:Span::new(exprs.first().unwrap().span_lo(), exprs.last().unwrap().span_hi()),
				exprs,
			}
			.into();

			Ok(ParenExpr { span:span!(self, expr_start), expr:seq_expr }.into())
		}
	}
	...
	fn parse_tagged_tpl(
		...
		Ok(TaggedTpl { span, tag, type_params, tpl, ..Default::default() })
	}
	...
	pub(super) fn parse_tpl_element(&mut self, is_tagged_tpl:bool) -> PResult<TplElement> {
		...
		Ok(TplElement { span:span!(self, start), raw, tail, cooked })
	}
	...
	fn parse_subscript(
		...
		if self.input.syntax().typescript() {
			if !self.input.had_line_break_before_cur() && is!(self, '!') {
				...
				return Ok((TsNonNullExpr { span:span!(self, start), expr }.into(), true));
			}
			...
			if matches!(obj, Callee::Expr(..)) && is!(self, '<') {
				...
				let ctx:Context = Context { should_not_lex_lt_or_gt_as_type:true, ..self.ctx() };
				...
				|| eat!(self, '['))
		{
			...
			let prop =
				ComputedPropName { span:Span::new(bracket_lo, self.input.last_pos()), expr:prop };
				...
				Box::new(match obj {
					...
					Callee::Super(obj) => {
							...
							&& !self.input.syntax().allow_super_outside_method()
						{
							...
							SuperPropExpr { span, obj, prop:SuperProp::Computed(prop) }.into()
						}
					},
					...
					Callee::Expr(obj) => {
						...
						let expr = MemberExpr { span, obj, prop:MemberProp::Computed(prop) };
			...
			|| (!no_call && (is!(self, '(')))
		{
			...
			return if question_dot_token.is_some()
				...
				Ok((
					CallExpr { span:span!(self, start), callee:obj, args, ..Default::default() }
						.into(),
					true,
				))
			};
		...
		// $obj.name
		if eat!(self, '.') {
			let prop = self.parse_maybe_private_name().map(|e| {
				match e {
					...
					callee @ Callee::Import(_) => {
						match prop {
							MemberProp::Ident(IdentName { sym, .. }) => {
								...
								match &*sym {
									"meta" => {
										MetaPropExpr { span, kind:MetaPropKind::ImportMeta }.into()
									},
									_ => {
							...
							&& !self.input.syntax().allow_super_outside_method()
						{
							...
							match prop {
								MemberProp::Ident(ident) => {
									SuperPropExpr { span, obj, prop:SuperProp::Ident(ident) }.into()
								},
		...
		match obj {
			Callee::Expr(expr) => {
				let expr = if let Some(type_args) = type_args {
					TsInstantiation { expr, type_args, span:span!(self, start) }.into()
				} else {
					expr
				};
	...
	pub(super) fn parse_lhs_expr(&mut self) -> PResult<Box<Expr>> {
		...
		if self.input.syntax().jsx() {
			...
			match *cur!(self, true) {
				Token::JSXText { .. } => {
					return self.parse_jsx_text().map(Lit::JSXText).map(Expr::Lit).map(Box::new);
				},
		...
		if eat!(self, "super") {
			let obj = Callee::Super(Super { span:span!(self, start) });
		...
		let type_args = if self.input.syntax().typescript() && is_one_of!(self, '<', "<<") {
			self.try_parse_ts(|p| {
				...
				if is!(p, '(') { Ok(Some(type_args)) } else { Ok(None) }
			})
		} else {
			None
		};
		...
		if is!(self, '(') {
			...
			let (callee, is_import) = match callee {
				_ if callee.is_ident_ref_to("import") => {
					(Callee::Import(Import { span:callee.span(), phase:Default::default() }), true)
				},
	...
	pub(super) fn parse_for_head_prefix(&mut self) -> PResult<Box<Expr>> { self.parse_expr() }
	...
	pub(super) fn parse_args_or_pats(
		...
		self.with_ctx(Context { will_expect_colon_for_cond:false, ..self.ctx() })
	...
	fn parse_args_or_pats_inner(&mut self) -> PResult<(Vec<AssignTargetOrSpread>, Option<Span>)> {
		...
		while !eof!(self) && !is!(self, ')') {
			...
			let is_async = is!(self, "async")
				&& matches!(peek!(self), Some(tok!('(') | tok!("function") | Token::Word(..)));
			...
			let mut arg = {
				...
				{
					let spread =
						if eat!(self, "...") { Some(self.input.prev_span()) } else { None };
			...
			let optional = if self.input.syntax().typescript() {
				if is!(self, '?') {
						...
						|| peeked_is!(self, '=')
					{
						...
						arg = ExprOrSpread {
							spread:None,
							expr:CondExpr { span:Span::new(start, alt.span_hi()), test, cons, alt }
								.into(),
						};
			...
			if optional || (self.input.syntax().typescript() && is!(self, ':')) {
				...
				match pat {
					...
					| Pat::Array(ArrayPat { ref mut type_ann, ref mut span, .. })
					| Pat::Object(ObjectPat { ref mut type_ann, ref mut span, .. })
					| Pat::Rest(RestPat { ref mut type_ann, ref mut span, .. }) => {
				...
				if eat!(self, '=') {
					...
					pat =
						AssignPat { span:span!(self, pat_start), left:Box::new(pat), right }.into();
				}
			...
			if eat!(self, "=>") && {
				...
				let params:Vec<Pat> =
					self.parse_paren_items_as_params(items.clone(), None)?.into_iter().collect();
...
impl<I:Tokens> Parser<I> {
	fn parse_yield_expr(&mut self) -> PResult<Box<Expr>> {
		...
		if is!(self, ';')
			|| (!is!(self, '*')
				&& !is!(self, '/')
				&& !is!(self, "/=")
				&& !cur!(self, false).map(|t| t.kind().starts_expr()).unwrap_or(true))
		{
			Ok(YieldExpr { span:span!(self, start), arg:None, delegate:false }.into())
		} else {
			...
			Ok(YieldExpr { span:span!(self, start), arg:Some(arg), delegate:has_star }.into())
		}
	}
	...
	pub(super) fn parse_lit(&mut self) -> PResult<Lit> {
		...
		let v = match cur!(self, true) {
			...
			Token::Str { .. } => {
				match bump!(self) {
					Token::Str { value, raw } => {
						Lit::Str(Str { span:span!(self, start), value, raw:Some(raw) })
					},
					_ => unreachable!(),
				}
			},
			Token::Num { .. } => {
				match bump!(self) {
					Token::Num { value, raw } => {
						Lit::Num(Number { span:span!(self, start), value, raw:Some(raw) })
					},
					_ => unreachable!(),
				}
			},
			Token::BigInt { .. } => {
				match bump!(self) {
					Token::BigInt { value, raw } => {
						Lit::BigInt(BigInt { span:span!(self, start), value, raw:Some(raw) })
					},
					_ => unreachable!(),
				}
			},
		...
		if eat!(self, '.') {
			...
			match &*ident.sym {
				"meta" => {
					...
					let expr = MetaPropExpr { span, kind:MetaPropKind::ImportMeta };
	...
	fn parse_dynamic_import_call(
		...
		let import = Callee::Import(Import { span:span!(self, start), phase });
