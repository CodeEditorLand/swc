...
impl<I:Tokens> Parser<I> {
	pub fn parse_pat(&mut self) -> PResult<Pat> { self.parse_binding_pat_or_ident(false) }
	...
	pub(super) fn parse_binding_element(&mut self) -> PResult<Pat> {
		...
		if eat!(self, '=') {
			...
			return Ok(AssignPat { span:span!(self, start), left:Box::new(left), right }.into());
		}
	...
	fn parse_array_binding_pat(&mut self) -> PResult<Pat> {
		...
		while !eof!(self) && !is!(self, ']') {
			...
			if eat!(self, "...") {
				...
				let pat =
					RestPat { span:rest_span, dot3_token, arg:Box::new(pat), type_ann:None }.into();
		...
		Ok(ArrayPat { span:span!(self, start), elems, optional, type_ann:None }.into())
	}
	...
	pub(super) fn parse_formal_param_pat(&mut self) -> PResult<Pat> {
		...
		if self.input.syntax().typescript() {
			if eat!(self, '?') {
				match pat {
					Pat::Ident(BindingIdent { id: Ident { ref mut optional, .. }, .. })
					| Pat::Array(ArrayPat { ref mut optional, .. })
					| Pat::Object(ObjectPat { ref mut optional, .. }) => {
						*optional = true;
		...
		let pat = if eat!(self, '=') {
			...
			AssignPat { span:span!(self, start), left:Box::new(pat), right }.into()
		} else {
			pat
		};
	...
	pub(super) fn parse_constructor_params(&mut self) -> PResult<Vec<ParamOrTsParamProp>> {
		...
		while !eof!(self) && !is!(self, ')') {
			...
			if eat!(self, "...") {
				...
				let pat =
					RestPat { span:rest_span, dot3_token, arg:Box::new(pat), type_ann }.into();
	...
	pub(super) fn parse_formal_params(&mut self) -> PResult<Vec<Param>> {
		...
		while !eof!(self) && !is!(self, ')') {
			...
			params.push(Param { span:span!(self, param_start), decorators, pat });
...
impl<I:Tokens> Parser<I> {
	...
	fn reparse_expr_as_pat_inner(&mut self, pat_ty:PatType, expr:Box<Expr>) -> PResult<Pat> {
		...
		match *expr {
			...
			Expr::Assign(assign_expr @ AssignExpr { op: AssignOp::Assign, .. }) => {
				let AssignExpr { span, left, right, .. } = assign_expr;
			...
			Expr::Object(ObjectLit { span: object_span, props }) => {
				// {}
			...
			Expr::Array(ArrayLit { elems: mut exprs, .. }) => {
				if exprs.is_empty() {
					return Ok(
						ArrayPat { span, elems:Vec::new(), optional:false, type_ann:None }.into()
					);
				}
				...
				for expr in exprs.drain(..idx_of_rest_not_allowed) {
					match expr {
						Some(expr @ ExprOrSpread { spread: Some(..), .. }) => {
				...
				if count_of_trailing_comma == 0 {
					...
					let last = match expr {
						// Rest
						Some(ExprOrSpread { spread: Some(dot3_token), expr }) => {
				...
				Ok(ArrayPat { span, elems:params, optional:false, type_ann:None }.into())
			},
	...
	pub(super) fn parse_paren_items_as_params(
		...
		for expr in exprs.drain(..len - 1) {
			match expr {
				AssignTargetOrSpread::ExprOrSpread(ExprOrSpread { spread: Some(..), .. })
				| AssignTargetOrSpread::Pat(Pat::Rest(..)) => self.emit_err(expr.span(), SyntaxError::TS1014),
		...
		let last = match expr {
			// Rest
			AssignTargetOrSpread::ExprOrSpread(ExprOrSpread { spread: Some(dot3_token), expr }) => {
				if let Expr::Assign(_) = *expr {
					self.emit_err(outer_expr_span, SyntaxError::TS1048)
				};
				...
				self.reparse_expr_as_pat(pat_ty, expr).map(|pat| {
					RestPat { span:expr_span, dot3_token, arg:Box::new(pat), type_ann:None }.into()
				})?
			},
...
mod tests {
	...
	fn object_pat(s:&'static str) -> Pat {
		test_parser(s, Syntax::default(), |p| p.parse_binding_pat_or_ident(false))
	}

	fn ident(s:&str) -> Ident { Ident::new_no_ctxt(s.into(), span) }

	fn ident_name(s:&str) -> IdentName { IdentName::new(s.into(), span) }

	fn rest() -> Option<Pat> {
		Some(RestPat { span, dot3_token:span, type_ann:None, arg:ident("tail").into() }.into())
	}
	...
	fn array_binding_pattern_tail() {
		assert_eq_ignore_span!(
			array_pat("[...tail]"),
			Pat::Array(ArrayPat { span, optional:false, elems:vec![rest()], type_ann:None })
		);
	}
				...
				props:vec![ObjectPatProp::Assign(AssignPatProp {
					...
					value:Some(Box::new(Expr::Object(ObjectLit {
						...
						props:vec![
							...
							prop(
								PropName::Str(Str { span, value:"".into(), raw:Some("''".into()) }),
								"sym",
