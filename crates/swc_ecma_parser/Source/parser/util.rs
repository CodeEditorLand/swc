...
impl<I:Tokens> Parser<I> {
	...
	pub(super) fn with_state(&mut self, state:State) -> WithState<I> {
		...
		WithState { orig_state, inner:self }
	}

	pub(super) fn set_ctx(&mut self, ctx:Context) { self.input.set_ctx(ctx); }

	pub(super) fn strict_mode(&mut self) -> WithCtx<I> {
		let ctx = Context { strict:true, ..self.ctx() };
	...
	pub(super) fn include_in_expr(&mut self, include_in_expr:bool) -> WithCtx<I> {
		let ctx = Context { include_in_expr, ..self.ctx() };
	...
	pub(super) fn syntax(&self) -> Syntax { self.input.syntax() }
}
...
impl<I:Tokens> Deref for WithState<'_, I> {
	...
	fn deref(&self) -> &Parser<I> { self.inner }
}
impl<I:Tokens> DerefMut for WithState<'_, I> {
	fn deref_mut(&mut self) -> &mut Parser<I> { self.inner }
}
impl<I:Tokens> Drop for WithState<'_, I> {
	fn drop(&mut self) { std::mem::swap(&mut self.inner.state, &mut self.orig_state); }
}
...
impl<I:Tokens> Drop for WithCtx<'_, I> {
	fn drop(&mut self) { self.inner.set_ctx(self.orig_ctx); }
}
