...
impl<I:Tokens> Parser<I> {
	...
	fn parse_class_inner(
		...
		self.strict_mode().parse_with(|p| {
			...
			let ident = p.parse_maybe_opt_binding_ident(is_ident_required, true)?.map(Ident::from);
			...
			let body = p
				.with_ctx(Context { has_super_class:super_class.is_some(), ..p.ctx() })
				.parse_class_body()?;

			if p.input.cur().is_none() {
				let eof_text = p.input.dump_cur();

				p.emit_err(p.input.cur_span(), SyntaxError::Expected(&Token::RBrace, eof_text));
			} else {
				expect!(p, '}');
			}
	...
	where
		...
		let (ident, mut class) = self
			.with_ctx(Context { in_class:true, ..self.ctx() })
		...
		if is_abstract {
			...
			for member in class.body.iter() {
				match member {
					ClassMember::ClassProp(ClassProp { is_abstract: true, span, .. })
					| ClassMember::Method(ClassMethod { span, is_abstract: true, .. }) => {
	...
	fn parse_super_class(&mut self) -> PResult<(Box<Expr>, Option<Box<TsTypeParamInstantiation>>)> {
		...
		match *super_class {
			Expr::TsInstantiation(TsInstantiation { expr, type_args, .. }) => {
	...
	fn parse_decorator(&mut self) -> PResult<Decorator> {
		...
		let expr = if eat!(self, '(') {
			...
			let expr = self.parse_ident(false, false).map(Expr::from).map(Box::new)?;
		...
		Ok(Decorator { span:span!(self, start), expr })
	}
	...
	fn parse_class_body(&mut self) -> PResult<Vec<ClassMember>> {
		...
		while !eof!(self) && !is!(self, '}') {
			if eat_exact!(self, ';') {
				...
				elems.push(ClassMember::Empty(EmptyStmt { span:Span::new(span.lo, span.hi) }));

				continue;
			}

			let mut p = self.with_ctx(Context { allow_direct_super:true, ..self.ctx() });

			let elem = p.parse_class_member()?;

			if !p.ctx().in_declare {
				if let ClassMember::Constructor(Constructor { body: Some(..), span, .. }) = elem {
	...
	fn parse_class_member(&mut self) -> PResult<ClassMember> {
		...
		let static_token = {
			...
			if eat!(self, "static") { Some(span!(self, start)) } else { None }
		};

		let accessor_token = if self.syntax().auto_accessors() {
			let start = cur_pos!(self);

			if eat!(self, "accessor") { Some(span!(self, start)) } else { None }
		} else {
			None
		};
		...
		if let Some(accessor_token) = accessor_token {
			...
			if self.is_class_method() {
				let key =
					Key::Public(PropName::Ident(IdentName::new("accessor".into(), accessor_token)));
		...
		if let Some(static_token) = static_token {
			...
			if self.is_class_method() {
				let key =
					Key::Public(PropName::Ident(IdentName::new("static".into(), static_token)));
	...
	fn parse_class_member_with_is_static(
		...
		let key = if readonly.is_some() && is_one_of!(self, '!', ':') {
			Key::Public(PropName::Ident(IdentName::new("readonly".into(), readonly.unwrap())))
		} else {
			...
			if is_constructor {
				...
				if self.syntax().typescript() && body.is_none() {
					...
					for p in &params {
						...
						let span = match *p {
							ParamOrTsParamProp::Param(ref param) => {
								match param.pat {
	...
	fn make_property(
		...
		if key.is_private() {
			if declare {
				self.emit_err(key.span(), SyntaxError::PrivateNameModifier("declare".into()))
			}

			if is_abstract {
				self.emit_err(key.span(), SyntaxError::PrivateNameModifier("abstract".into()))
			}
		}
		...
		let ctx = Context { include_in_expr:true, in_class_field:true, ..self.ctx() };
	...
	fn is_class_property(&mut self, asi:bool) -> bool {
		(self.input.syntax().typescript() && is_one_of!(self, '!', ':'))
			|| is_one_of!(self, '=', '}')
			|| if asi { is!(self, ';') } else { is_exact!(self, ';') }
	}
	...
	fn parse_fn_inner(
		...
		let ident = if is_fn_expr {
			...
			self.with_ctx(Context { allow_direct_super:false, in_class_field:false, ..self.ctx() })
				.parse_maybe_opt_binding_ident(is_ident_required, false)?
		}
		.map(Ident::from);
	...
	fn parse_maybe_opt_binding_ident(
		...
		if required {
			self.parse_binding_ident(disallow_let).map(|v| v.id).map(Some)
		} else {
			self.parse_opt_binding_ident(disallow_let).map(|v| v.map(|v| v.id))
		}
	}
	...
	where
		...
		let ctx = Context { in_async:is_async, in_generator:is_generator, ..self.ctx() };
		...
		self.with_ctx(ctx).parse_with(|p| {
			...
			let body:Option<_> =
				p.parse_fn_body(is_async, is_generator, false, params.is_simple_parameter_list())?;
	...
	where
		...
		let state = State { labels:Vec::new(), ..Default::default() };
...
impl<I:Tokens> Parser<I> {
	...
	where
		...
		let function = self
			.with_ctx(Context { allow_direct_super:true, in_class_field:false, ..self.ctx() })
			.parse_with(|p| {
...
impl OutputType for ExportDefaultDecl {
	...
	fn finish_fn(
		...
		Ok(ExportDefaultDecl { span, decl:DefaultDecl::Fn(FnExpr { ident, function }) })
	}

	fn finish_class(span:Span, ident:Option<Ident>, class:Box<Class>) -> Result<Self, SyntaxError> {
		Ok(ExportDefaultDecl { span, decl:DefaultDecl::Class(ClassExpr { ident, class }) })
	}
}
...
impl OutputType for Decl {
	...
	fn finish_fn(
		...
		Ok(FnDecl { declare:false, ident, function }.into())
	}

	fn finish_class(_:Span, ident:Option<Ident>, class:Box<Class>) -> Result<Self, SyntaxError> {
		let ident = ident.ok_or(SyntaxError::ExpectedIdent)?;

		Ok(ClassDecl { declare:false, ident, class }.into())
	}
}
...
fn has_use_strict(block:&BlockStmt) -> Option<Span> {
	block
		...
		.find_map(|s| if s.is_use_strict() { Some(s.span()) } else { None })
}
impl<I:Tokens> FnBodyParser<Box<BlockStmtOrExpr>> for Parser<I> {
	fn parse_fn_body_inner(
		...
		if is!(self, '{') {
			...
			self.parse_assignment_expr().map(BlockStmtOrExpr::Expr).map(Box::new)
		}
	}
}

impl<I:Tokens> FnBodyParser<Option<BlockStmt>> for Parser<I> {
	fn parse_fn_body_inner(&mut self, is_simple_parameter_list:bool) -> PResult<Option<BlockStmt>> {
		...
		self.iter().all(|param| {
			matches!(
				...
				ParamOrTsParamProp::TsParamProp(..)
					| ParamOrTsParamProp::Param(Param { pat:Pat::Ident(_), .. })
			)
		})
	}
}
