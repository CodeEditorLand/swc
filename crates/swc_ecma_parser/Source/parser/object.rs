...
impl<I:Tokens> Parser<I> {
	...
	pub(super) fn parse_prop_name(&mut self) -> PResult<PropName> {
		...
		let ctx = self.ctx();
		self.with_ctx(Context { in_property_name:true, ..ctx }).parse_with(|p| {
			let start = cur_pos!(p);
			...
			let v = match *cur!(p, true) {
				Token::Str { .. } => {
					match bump!(p) {
						Token::Str { value, raw } => {
							PropName::Str(Str { span:span!(p, start), value, raw:Some(raw) })
						},
						_ => unreachable!(),
					}
				},
				Token::Num { .. } => {
					match bump!(p) {
						Token::Num { value, raw } => {
							PropName::Num(Number { span:span!(p, start), value, raw:Some(raw) })
						},
						_ => unreachable!(),
					}
				},
				Token::BigInt { .. } => {
					match bump!(p) {
						Token::BigInt { value, raw } => {
							PropName::BigInt(BigInt { span:span!(p, start), value, raw:Some(raw) })
						},
						_ => unreachable!(),
					}
				},
				...
				tok!('[') => {
					...
					if p.syntax().typescript() && is!(p, ',') {
						...
						expr = Box::new(SeqExpr { span:span!(p, inner_start), exprs }.into());
					}

					expect!(p, ']');

					PropName::Computed(ComputedPropName { span:span!(p, start), expr })
				},
				_ => {
					unexpected!(
						p,
	...
	fn parse_object_prop(&mut self) -> PResult<Self::Prop> {
		...
		if eat!(self, '*') {
			...
			return self
				...
				.map(|function| {
					PropOrSpread::Prop(Box::new(Prop::Method(MethodProp { key:name, function })))
				});
		}
			...
			&& !(is!(self, '}') && matches!(key, PropName::Ident(..)))
		{
			...
			return Ok(PropOrSpread::Prop(Box::new(Prop::KeyValue(KeyValueProp {
				key,
				value:Invalid { span:span!(self, start) }.into(),
			}))));
		}
		// {[computed()]: a,}
		// { 'a': a, }
		...
		if eat!(self, ':') {
			...
			return Ok(PropOrSpread::Prop(Box::new(Prop::KeyValue(KeyValueProp { key, value }))));
		}
...
impl<I:Tokens> ParseObject<Pat> for Parser<I> {
	...
	fn make_object(
		...
		Ok(ObjectPat { span, props, optional, type_ann:None }.into())
	}
	...
	fn parse_object_prop(&mut self) -> PResult<Self::Prop> {
		...
		let value = if eat!(self, '=') {
			self.include_in_expr(true).parse_assignment_expr().map(Some)?
		} else {
