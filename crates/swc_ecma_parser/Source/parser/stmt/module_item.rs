...
impl<I:Tokens> Parser<I> {
	...
	fn parse_imported_default_binding(&mut self) -> PResult<Ident> { self.parse_imported_binding() }
	...
	fn parse_export(&mut self, mut decorators:Vec<Decorator>) -> PResult<ModuleDecl> {
		...
		if !type_only && eat!(self, "default") {
			...
			if self.input.syntax().typescript() {
				...
				if is!(self, "interface") {
					...
					let decl =
						self.parse_ts_interface_decl(interface_start).map(DefaultDecl::from)?;
						...
						.unwrap_or(false))
		{
			...
			if let Some(default) = default {
				...
				specifiers
					.push(ExportSpecifier::Default(ExportDefaultSpecifier { exported:default }))
			}
			...
			return Ok(
				NamedExport { span:span!(self, start), specifiers, src, type_only, with }.into()
			);
		};
