...
use swc_common::{DUMMY_SP as span, FileName, SourceMap};
use swc_ecma_visit::assert_eq_ignore_span;

use super::*;
use crate::{EsSyntax, parse_file_as_expr};

fn syntax() -> Syntax {
	Syntax::Es(EsSyntax { allow_super_outside_method:true, ..Default::default() })
}

fn lhs(s:&'static str) -> Box<Expr> { test_parser(s, syntax(), |p| p.parse_lhs_expr()) }

fn new_expr(s:&'static str) -> Box<Expr> { test_parser(s, syntax(), |p| p.parse_new_expr()) }

fn member_expr(s:&'static str) -> Box<Expr> { test_parser(s, syntax(), |p| p.parse_member_expr()) }

fn expr(s:&'static str) -> Box<Expr> {
	test_parser(s, syntax(), |p| {
		p.parse_stmt(true).map(|stmt| {
			match stmt {
...
fn regex_expr() -> Box<Expr> {
	AssignExpr {
		...
		op:AssignOp::Assign,
		right:Box::new(Lit::Regex(Regex { span, exp:"w+".into(), flags:"".into() }).into()),
	}
	.into()
}
#[test]
...
fn object_rest_pat() {
	assert_eq_ignore_span!(
		...
		Box::new(Expr::Arrow(ArrowExpr {
			...
			body:Box::new(BlockStmtOrExpr::BlockStmt(BlockStmt { span, ..Default::default() })),
			..Default::default()
		}))
	);
}
...
fn new_no_paren() {
	assert_eq_ignore_span!(
		expr("new a"),
		Box::new(Expr::New(NewExpr { span, callee:expr("a"), args:None, ..Default::default() }))
	);
}
...
fn issue_319_1() {
	assert_eq_ignore_span!(
		...
		Box::new(Expr::Call(CallExpr {
			...
			args:vec![ExprOrSpread { spread:None, expr:expr("({ async f() { await g(); } })") }],
			..Default::default()
		}))
	);
}
...
fn issue_328() {
	assert_eq_ignore_span!(
		test_parser("import('test')", Syntax::Es(Default::default()), |p| { p.parse_stmt(true) }),
		Stmt::Expr(ExprStmt {
			span,
...
#[test]
fn issue_337() {
	test_parser("const foo = 'bar' in bas ? 'beep' : 'boop';", Default::default(), |p| {
		p.parse_module()
	});
}
...
#[test]
fn issue_675() { expr("fn = function () { Object.setPrototypeOf(this, new.target.prototype); }"); }
	...
	assert_eq_ignore_span!(
		...
		Box::new(Expr::Call(CallExpr {
			...
			callee:Callee::Expr(Box::new(Expr::SuperProp(SuperPropExpr {
				span,
				obj:Super { span },
				prop:SuperProp::Ident(IdentName { span, sym:"foo".into() })
			}))),
			..Default::default()
		}))
	);
...
fn issue_6781() {
	...
	let expr = parse_file_as_expr(&fm, Default::default(), Default::default(), None, &mut errors);
...
fn bench_new_expr_ts(b:&mut Bencher) {
	bench_parser(b, "new Foo()", Syntax::Typescript(Default::default()), |p| {
...
fn bench_member_expr_ts(b:&mut Bencher) {
	bench_parser(b, "a.b.c.d.e.f", Syntax::Typescript(Default::default()), |p| {
		black_box(p.parse_expr()?);
