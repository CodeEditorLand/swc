...
pub trait Tokens: Clone + Iterator<Item = TokenAndSpan> {
	...
	fn start_pos(&self) -> BytePos { BytePos(0) }
...
impl Tokens for TokensInput {
	...
	fn ctx(&self) -> Context { self.ctx }

	fn syntax(&self) -> Syntax { self.syntax }

	fn target(&self) -> EsVersion { self.target }

	fn start_pos(&self) -> BytePos { self.start_pos }

	fn set_expr_allowed(&mut self, _:bool) {}

	fn set_next_regexp(&mut self, _:Option<BytePos>) {}

	fn token_context(&self) -> &TokenContexts { &self.token_ctx }

	fn token_context_mut(&mut self) -> &mut TokenContexts { &mut self.token_ctx }

	fn set_token_context(&mut self, c:TokenContexts) { self.token_ctx = c; }

	fn add_error(&self, error:Error) { self.errors.borrow_mut().push(error); }
	...
	fn take_errors(&mut self) -> Vec<Error> { take(&mut self.errors.borrow_mut()) }
	...
	fn end_pos(&self) -> BytePos {
		self.iter.as_slice().last().map(|t| t.span.hi).unwrap_or(self.start_pos)
	}
}
...
impl<I:Tokens> Clone for Capturing<I> {
	fn clone(&self) -> Self {
		Capturing { inner:self.inner.clone(), captured:self.captured.clone() }
	}
}

impl<I:Tokens> Capturing<I> {
	pub fn new(input:I) -> Self { Capturing { inner:input, captured:Default::default() } }

	pub fn tokens(&self) -> Rc<RefCell<Vec<TokenAndSpan>>> { self.captured.clone() }

	/// Take captured tokens
	pub fn take(&mut self) -> Vec<TokenAndSpan> { mem::take(&mut *self.captured.borrow_mut()) }
}
...
impl<I:Tokens> Tokens for Capturing<I> {
	fn set_ctx(&mut self, ctx:Context) { self.inner.set_ctx(ctx) }

	fn ctx(&self) -> Context { self.inner.ctx() }

	fn syntax(&self) -> Syntax { self.inner.syntax() }

	fn target(&self) -> EsVersion { self.inner.target() }

	fn start_pos(&self) -> BytePos { self.inner.start_pos() }

	fn set_expr_allowed(&mut self, allow:bool) { self.inner.set_expr_allowed(allow) }

	fn set_next_regexp(&mut self, start:Option<BytePos>) { self.inner.set_next_regexp(start); }

	fn token_context(&self) -> &TokenContexts { self.inner.token_context() }

	fn token_context_mut(&mut self) -> &mut TokenContexts { self.inner.token_context_mut() }

	fn set_token_context(&mut self, c:TokenContexts) { self.inner.set_token_context(c) }

	fn add_error(&self, error:Error) { self.inner.add_error(error); }

	fn add_module_mode_error(&self, error:Error) { self.inner.add_module_mode_error(error) }

	fn take_errors(&mut self) -> Vec<Error> { self.inner.take_errors() }

	fn take_script_module_errors(&mut self) -> Vec<Error> { self.inner.take_script_module_errors() }

	fn end_pos(&self) -> BytePos { self.inner.end_pos() }
}
...
impl<I:Tokens> Parser<I> {
	pub fn input(&mut self) -> &mut I { &mut self.input.iter }

	pub(crate) fn input_ref(&self) -> &I { &self.input.iter }
}
...
impl<I:Tokens> Buffer<I> {
	pub fn new(lexer:I) -> Self {
		...
		Buffer { iter:lexer, cur:None, prev_span:Span::new(start_pos, start_pos), next:None }
	}
	...
	pub fn store(&mut self, token:Token) {
		...
		self.cur = Some(TokenAndSpan { span, token, had_line_break:false });
	}

	#[allow(dead_code)]
	pub fn cur_debug(&self) -> Option<&Token> { self.cur.as_ref().map(|it| &it.token) }
	...
	pub fn knows_cur(&self) -> bool { self.cur.is_some() }
	...
	pub fn had_line_break_before_cur(&mut self) -> bool {
		...
		self.cur.as_ref().map(|it| it.had_line_break).unwrap_or_else(|| true)
	}
	...
	pub fn has_linebreak_between_cur_and_peeked(&mut self) -> bool {
		...
		self.next.as_ref().map(|item| item.had_line_break).unwrap_or({
			// return true on eof.
			true
		})
	}
	...
	pub fn merge_lt_gt(&mut self) {
		...
		self.cur = Some(TokenAndSpan { token, span, had_line_break:cur.had_line_break });
	}
	...
	pub fn cur_pos(&mut self) -> BytePos {
		...
		self.cur.as_ref().map(|item| item.span.lo).unwrap_or_else(|| {
			// eof
			self.last_pos()
		})
	}
	...
	pub fn cur_span(&self) -> Span {
		let data = self.cur.as_ref().map(|item| item.span).unwrap_or(self.prev_span);
	...
	#[inline]
	pub fn last_pos(&self) -> BytePos { self.prev_span.hi }

	/// Returns span of the previous token.
	#[inline]
	pub fn prev_span(&self) -> Span { self.prev_span }

	#[inline]
	pub(crate) fn get_ctx(&self) -> Context { self.iter.ctx() }

	#[inline]
	pub(crate) fn set_ctx(&mut self, ctx:Context) { self.iter.set_ctx(ctx); }

	#[inline]
	pub fn syntax(&self) -> Syntax { self.iter.syntax() }

	#[inline]
	pub fn target(&self) -> EsVersion { self.iter.target() }

	#[inline]
	pub(crate) fn set_expr_allowed(&mut self, allow:bool) { self.iter.set_expr_allowed(allow) }

	#[inline]
	pub fn set_next_regexp(&mut self, start:Option<BytePos>) { self.iter.set_next_regexp(start); }

	#[inline]
	pub(crate) fn token_context(&self) -> &lexer::TokenContexts { self.iter.token_context() }
