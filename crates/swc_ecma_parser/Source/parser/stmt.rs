...
impl<'a, I:Tokens> Parser<I> {
	pub fn parse_module_item(&mut self) -> PResult<ModuleItem> { self.parse_stmt_like(true, true) }
	...
	where
		...
		while {
			if self.input.cur().is_none() && end.is_some() {
				...
				self.emit_err(self.input.cur_span(), SyntaxError::Expected(end.unwrap(), eof_text));
			...
			if allow_directives {
				...
				if stmt.is_use_strict() {
					let ctx = Context { strict:true, ..old_ctx };
	...
	where
		...
		let ctx = Context { will_expect_colon_for_cond:false, allow_using_decl:true, ..self.ctx() };
	...
	fn parse_stmt_internal(
		...
		if is_typescript && is!(self, "const") && peeked_is!(self, "enum") {
			...
			return self.parse_ts_enum_decl(start, true).map(Decl::from).map(Stmt::from);
		}
		...
		match cur!(self, true) {
			tok!("await") if include_decl || top_level => {
				...
				if peeked_is!(self, "using") {
					...
					let expr = self.include_in_expr(true).parse_bin_op_recursively(expr, 0)?;
			...
			tok!("debugger") => {
				...
				return Ok(DebuggerStmt { span:span!(self, start) }.into());
			},
			...
			tok!("class") => {
				...
				return self.parse_class_decl(start, start, decorators, false).map(Stmt::from);
			},
			...
			tok!("catch") => {
				...
				return Ok(ExprStmt { span, expr:Invalid { span }.into() }.into());
			},
			...
			tok!('{') => {
				let ctx = Context { allow_using_decl:true, ..self.ctx() };
		...
		if eat_exact!(self, ';') {
			return Ok(EmptyStmt { span:span!(self, start) }.into());
		}
		...
		if let Expr::Ident(ref ident) = *expr {
			if &*ident.sym == "interface" && self.input.had_line_break_before_cur() {
				...
				return Ok(ExprStmt { span:span!(self, start), expr }.into());
			}
		...
		if eat!(self, ';') {
			Ok(ExprStmt { span:span!(self, start), expr }.into())
		} else {
			...
			syntax_error!(self, SyntaxError::ExpectedSemiForExprStmt { expr:expr.span() });
		}
	}
	...
	fn parse_if_stmt(&mut self) -> PResult<IfStmt> {
		...
		let ctx = Context { ignore_else_clause:false, ..self.ctx() };

		let test = self.with_ctx(ctx).include_in_expr(true).parse_expr().map_err(|err| {
			Error::new(
				err.span(),
		...
		let alt = if self.ctx().ignore_else_clause {
			...
			let ctx = Context { ignore_else_clause:true, ..self.ctx() };
	...
	fn parse_return_stmt(&mut self) -> PResult<Stmt> {
		...
		let stmt = self.parse_with(|p| {
			...
			let arg =
				if is!(p, ';') { None } else { p.include_in_expr(true).parse_expr().map(Some)? };

			expect!(p, ';');

			Ok(ReturnStmt { span:span!(p, start), arg }.into())
		});
	...
	fn parse_switch_stmt(&mut self) -> PResult<Stmt> {
		...
		let ctx = Context { is_break_allowed:true, ..self.ctx() };
		...
		Ok(SwitchStmt { span:span!(self, switch_start), discriminant, cases }.into())
	}
	...
	fn parse_try_stmt(&mut self) -> PResult<Stmt> {
		...
		Ok(TryStmt { span, block, handler, finalizer }.into())
	}
	...
	fn parse_catch_clause(&mut self) -> PResult<Option<CatchClause>> {
		...
		Ok(if eat!(self, "catch") {
			...
			self.parse_block(false)
				.map(|body| CatchClause { span:span!(self, start), param, body })
				.map(Some)?
		} else {
			None
		})
	...
	fn parse_finally_block(&mut self) -> PResult<Option<BlockStmt>> {
		Ok(if eat!(self, "finally") { self.parse_block(false).map(Some)? } else { None })
	}
	...
	fn parse_catch_param(&mut self) -> PResult<Option<Pat>> {
		if eat!(self, '(') {
			...
			if self.syntax().typescript() && eat!(self, ':') {
				let ctx = Context { in_type:true, ..self.ctx() };
	...
	pub(super) fn parse_using_decl(
		...
		Ok(Some(Box::new(UsingDecl { span:span!(self, start), is_await, decls })))
	}
	...
	pub(super) fn parse_var_stmt(&mut self, for_loop:bool) -> PResult<Box<VarDecl>> {
		...
		while first || eat!(self, ',') {
			...
			let ctx = if should_include_in {
				Context { include_in_expr:true, ..self.ctx() }
			} else {
				self.ctx()
			};
	...
	fn parse_var_declarator(&mut self, for_loop:bool, kind:VarDeclKind) -> PResult<VarDeclarator> {
		let start = cur_pos!(self);
		...
		if self.input.syntax().typescript() && is!(self, ':') {
			...
			match name {
				Pat::Array(ArrayPat { ref mut type_ann, .. })
				| Pat::Ident(BindingIdent { ref mut type_ann, .. })
				| Pat::Object(ObjectPat { ref mut type_ann, .. })
				| Pat::Rest(RestPat { ref mut type_ann, .. }) => {
		...
		Ok(VarDeclarator { span:span!(self, start), name, init, definite })
	}
	...
	fn parse_do_stmt(&mut self) -> PResult<Stmt> {
		...
		let ctx = Context { is_break_allowed:true, is_continue_allowed:true, ..self.ctx() };
	...
	fn parse_with_stmt(&mut self) -> PResult<Stmt> {
		...
		let ctx = Context { in_function:true, ..self.ctx() };
	...
	pub(super) fn parse_block(&mut self, allow_directives:bool) -> PResult<BlockStmt> {
		...
		Ok(BlockStmt { span, stmts, ctxt:Default::default() })
	}

	fn parse_labelled_stmt(&mut self, l:Ident) -> PResult<Stmt> {
		let ctx = Context { is_break_allowed:true, allow_using_decl:false, ..self.ctx() };
		...
		self.with_ctx(ctx).parse_with(|p| {
			...
			for lb in &p.state.labels {
				if l.sym == *lb {
					errors.push(Error::new(l.span, SyntaxError::DuplicateLabel(l.sym.clone())));
				}
			}
			...
			Ok(LabeledStmt { span:span!(p, start), label:l, body }.into())
		})
	}
	...
	fn parse_for_stmt(&mut self) -> PResult<Stmt> {
		...
		let await_token = if eat!(self, "await") { Some(span!(self, await_start)) } else { None };
		...
		Ok(match head {
			TempForHead::For { init, test, update } => {
				...
				ForStmt { span, init, test, update, body }.into()
			},
			...
			TempForHead::ForIn { left, right } => {
				...
				ForInStmt { span, left, right, body }.into()
			},

			TempForHead::ForOf { left, right } => {
				ForOfStmt { span, is_await:await_token.is_some(), left, right, body }.into()
			},
		})
	}
	...
	fn parse_for_head(&mut self) -> PResult<TempForHead> {
		...
		if self.input.syntax().explicit_resource_management() {
			...
			// await using foo
			if !maybe_using_decl
				&& init.as_await_expr().filter(|e| e.arg.is_ident_ref_to("using")).is_some()
			{
...
enum TempForHead {
	For { init:Option<VarDeclOrExpr>, test:Option<Box<Expr>>, update:Option<Box<Expr>> },
	ForIn { left:ForHead, right:Box<Expr> },
	ForOf { left:ForHead, right:Box<Expr> },
}
...
where
	T: IsDirective,
{
	fn as_ref(&self) -> Option<&Stmt> { T::as_ref(&**self) }
}
...
pub(super) trait StmtLikeParser<'a, Type:IsDirective> {
	fn handle_import_export(&mut self, top_level:bool, decorators:Vec<Decorator>) -> PResult<Type>;
}
...
mod tests {
	...
	fn stmt(s:&'static str) -> Stmt { test_parser(s, Syntax::default(), |p| p.parse_stmt(true)) }

	fn module_item(s:&'static str) -> ModuleItem {
		test_parser(s, Syntax::default(), |p| p.parse_stmt_like(true, true))
	}

	fn expr(s:&'static str) -> Box<Expr> { test_parser(s, Syntax::default(), |p| p.parse_expr()) }

	#[test]
	fn expr_stmt() {
		assert_eq_ignore_span!(
			stmt("a + b + c"),
			Stmt::Expr(ExprStmt { span, expr:expr("a + b + c") })
		)
	}
	...
	fn catch_rest_pat() {
		assert_eq_ignore_span!(
			...
			Stmt::Try(Box::new(TryStmt {
				span,
				block:BlockStmt { span, ..Default::default() },
				handler:Some(CatchClause {
					...
					.into(),
					body:BlockStmt { span, ..Default::default() }
				}),
				finalizer:None
			}))
		);
	...
	fn throw_this() {
		assert_eq_ignore_span!(
			stmt("throw this"),
			Stmt::Throw(ThrowStmt { span, arg:expr("this") })
		)
	}
	...
	fn no_empty_without_semi() {
		...
		assert_eq_ignore_span!(
			stmt("{ 1; }"),
			Stmt::Block(BlockStmt { span, stmts:vec![stmt("1")], ..Default::default() })
		);
	}
	...
	fn class_decorator() {
		assert_eq_ignore_span!(
            ...
            class Foo {}
            ",
				Syntax::Es(EsSyntax { decorators:true, ..Default::default() }),
			...
			Stmt::Decl(Decl::Class(ClassDecl {
				...
				class:Box::new(Class {
					span,
					decorators:vec![
						Decorator { span, expr:expr("decorator") },
						Decorator { span, expr:expr("dec2") }
					],
					super_class:None,
					body:Vec::new(),
					is_abstract:false,
...
"#;
		...
		test_parser(src, Syntax::Es(EsSyntax { jsx:true, ..Default::default() }), |p| {
			p.parse_module()
		});
	}
...
export default App"#;
	...
	fn export_default() {
		...
		test_parser(
			src,
			Syntax::Es(EsSyntax { export_default_from:true, ..Default::default() }),
			|p| p.parse_module(),
		);
	}
...
let x = 4";
	...
	#[test]
	fn empty() { test_parser("", Syntax::Es(Default::default()), |p| p.parse_module()); }
	...
	fn issue_4369_1() {
		test_parser(
			...
			Syntax::Es(EsSyntax { export_default_from:false, ..Default::default() }),
			|p| p.parse_module(),
		);
	}
...
export default function waitUntil(callback, options = {}) {
  let timeout = 'timeout' in options ? options.timeout : 1000;
	...
	#[test]
	fn issue_340_fn() {
		test_parser("export default function(){};", Default::default(), |p| p.parse_module());
	}

	#[test]
	fn issue_340_async_fn() {
		test_parser("export default async function(){};", Default::default(), |p| p.parse_module());
	}
	...
	#[test]
	fn issue_340_class() {
		test_parser("export default class {};", Default::default(), |p| p.parse_module());
	}
	...
	#[test]
	fn issue_380_1() {
		test_parser("import(filePath).then(bar => {})", Syntax::Es(Default::default()), |p| {
			p.parse_module()
		});
	}
	...
	#[test]
	fn top_level_await() {
		test_parser("await foo", Syntax::Es(Default::default()), |p| p.parse_module());
	}
...
";
  ...
  __dirname: string
	...
	fn class_static_blocks() {
		...
		assert_eq_ignore_span!(
			...
			Box::new(Expr::Class(ClassExpr {
				ident:Some(Ident { span, sym:"Foo".into(), ..Default::default() }),
				class:Box::new(Class {
					span,
					decorators:Vec::new(),
					super_class:None,
					...
					body:vec!(ClassMember::StaticBlock(StaticBlock {
						span,
						body:BlockStmt { span, stmts:vec!(stmt("1 + 1;")), ..Default::default() }
					})),
					..Default::default()
				})
			}))
		...
		assert_eq_ignore_span!(
			...
			Box::new(Expr::Class(ClassExpr {
				...
				class:Box::new(Class {
					...
					body:vec!(ClassMember::StaticBlock(StaticBlock {
						span,
						body:BlockStmt { span, stmts:Vec::new(), ..Default::default() }
					})),
					..Default::default()
				})
			}))
	...
	fn class_static_blocks_in_ts() {
		...
		test_parser(src, Syntax::Typescript(Default::default()), |p| p.parse_expr());
	}
	...
	fn no_error_for_trailing_comma_inside_dynamic_import_with_import_assertions() {
		...
		test_parser(
			src,
			Syntax::Es(EsSyntax { import_attributes:true, ..Default::default() }),
			|p| p.parse_expr(),
		);
	}
	...
	fn type_only_star_exports_with_name() {
		...
		test_parser(src, Syntax::Typescript(Default::default()), |p| p.parse_module());
	}
...
const foo;"#;
		...
		test_parser(src, Syntax::Typescript(Default::default()), |p| p.parse_script());
	}
