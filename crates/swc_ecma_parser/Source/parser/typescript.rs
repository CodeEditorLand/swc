...
impl<I:Tokens> Parser<I> {
	...
	fn parse_ts_type_ref(&mut self) -> PResult<TsTypeRef> {
		...
		let type_params = if !self.input.had_line_break_before_cur() && is!(self, '<') {
			let ctx = Context { should_not_lex_lt_or_gt_as_type:false, ..self.ctx() };
		...
		Ok(TsTypeRef { span:span!(self, start), type_name, type_params })
	}
	...
	fn parse_ts_this_type_node(&mut self) -> PResult<TsThisType> {
		...
		Ok(TsThisType { span:self.input.prev_span() })
	}
	...
	fn parse_ts_import_type(&mut self) -> PResult<TsImportType> {
		...
		let arg = match cur!(self, true) {
			Token::Str { .. } => {
				match bump!(self) {
					Token::Str { value, raw } => Str { span:arg_span, value, raw:Some(raw) },
					_ => unreachable!(),
				}
			},
			_ => {
				...
				Str { span:arg_span, value:"".into(), raw:Some("\"\"".into()) }
			},
		};
		...
		let qualifier =
			if eat!(self, '.') { self.parse_ts_entity_name(false).map(Some)? } else { None };

		let type_args = if is!(self, '<') { self.parse_ts_type_args().map(Some)? } else { None };

		Ok(TsImportType { span:span!(self, start), arg, qualifier, type_args })
	}
	...
	fn parse_ts_type_query(&mut self) -> PResult<TsTypeQuery> {
		...
		Ok(TsTypeQuery { span:span!(self, start), expr_name, type_args })
	}
	...
	pub(super) fn parse_ts_type_params(
		...
		self.in_type().parse_with(|p| {
			p.ts_in_no_context(|p| {
				...
				Ok(Box::new(TsTypeParamDecl { span:span!(p, start), params }))
			})
		})
	}
	...
	pub(super) fn parse_ts_type_or_type_predicate_ann(
		...
		self.in_type().parse_with(|p| {
			...
			Ok(Box::new(TsTypeAnn { span:span!(p, return_token_start), type_ann:node }))
		})
	}
	...
	where
		...
		let ctx = Context { ignore_error:true, ..self.input.get_ctx() };
		...
		match res {
			Ok(Some(res)) if res => {
				*self = cloned;
				...
				let ctx = Context { ignore_error:prev_ignore_error, ..self.input.get_ctx() };
	...
	pub(super) fn parse_ts_type_ann(
		...
		self.in_type().parse_with(|p| {
			...
			Ok(Box::new(TsTypeAnn { span:span!(p, start), type_ann }))
		})
	}
		...
		self.in_type().parse_with(|p| {
			if !p.input.eat(token) {
				...
				syntax_error!(
					p,
					p.input.cur_span(),
					SyntaxError::Unexpected { got, expected:token_str }
				);
			}
	...
	fn parse_ts_enum_member(&mut self) -> PResult<TsEnumMember> {
		...
		let id = match *cur!(self, true) {
			...
			_ => self.parse_ident_name().map(Ident::from).map(TsEnumMemberId::from)?,
		};
		...
		Ok(TsEnumMember { span:span!(self, start), id, init })
	}
	...
	fn parse_ts_module_block(&mut self) -> PResult<TsModuleBlock> {
		...
		Ok(TsModuleBlock { span:span!(self, start), body })
	}
	...
	pub(super) fn parse_ts_type(&mut self) -> PResult<Box<TsType>> {
		...
		self.with_ctx(Context { disallow_conditional_types:false, ..self.ctx() })
			.parse_with(|p| {
				...
				let extends_type = {
					p.with_ctx(Context { disallow_conditional_types:true, ..p.ctx() })
	...
	fn parse_ts_non_conditional_type(&mut self) -> PResult<Box<TsType>> {
		...
		if self.is_ts_start_of_fn_type()? {
			return self.parse_ts_fn_or_constructor_type(true).map(TsType::from).map(Box::new);
		}

		if (is!(self, "abstract") && peeked_is!(self, "new")) || is!(self, "new") {
			// As in `new () => Date`
			return self.parse_ts_fn_or_constructor_type(false).map(TsType::from).map(Box::new);
		}
	...
	pub(super) fn parse_ts_type_assertion(&mut self, start:BytePos) -> PResult<TsTypeAssertion> {
		...
		Ok(TsTypeAssertion { span:span!(self, start), type_ann, expr })
	}
	...
	fn parse_ts_heritage_clause_element(&mut self) -> PResult<TsExprWithTypeArgs> {
		...
		if !matches!(&*expr, Expr::Ident(..) | Expr::Member(..) | Expr::TsInstantiation(..)) {
			self.emit_err(span!(self, start), SyntaxError::TS2499);
		}

		match *expr {
			Expr::TsInstantiation(v) => {
				Ok(TsExprWithTypeArgs { span:v.span, expr:v.expr, type_args:Some(v.type_args) })
			},
			_ => {
				let type_args =
					if is!(self, '<') { Some(self.parse_ts_type_args()?) } else { None };

				Ok(TsExprWithTypeArgs { span:span!(self, start), expr, type_args })
			},
		}
	}
	...
	pub(super) fn parse_ts_interface_decl(
		...
		let body = self.in_type().parse_with(|p| p.parse_ts_object_type_members())?;

		let body = TsInterfaceBody { span:span!(self, body_start), body };
	...
	fn parse_ts_module_ref(&mut self) -> PResult<TsModuleRef> {
		...
		if self.is_ts_external_module_ref()? {
			...
			self.parse_ts_entity_name(/* allow_reserved_words */ false).map(From::from)
		}
	}
	...
	fn parse_ts_external_module_ref(&mut self) -> PResult<TsExternalModuleRef> {
		...
		Ok(TsExternalModuleRef { span:span!(self, start), expr })
	}
	...
	where
		...
		let ctx = Context { ignore_error:true, ..cloned.ctx() };
	...
	fn parse_ts_property_name(&mut self) -> PResult<(bool, Box<Expr>)> {
		let (computed, key) = if eat!(self, '[') {
			...
			let ctx = Context { in_property_name:true, ..self.ctx() };
			...
			self.with_ctx(ctx).parse_with(|p| {
				...
				let key = match *cur!(p, true) {
					...
					_ => {
						p.parse_maybe_private_name().map(|e| {
							match e {
								Either::Left(e) => {
	...
	fn parse_ts_property_or_method_signature(
		...
		if is_one_of!(self, '(', '<') {
			...
			let type_ann = if is!(self, ':') {
				self.parse_ts_type_or_type_predicate_ann(&tok!(':')).map(Some)?
			} else {
				None
			};
			// -----
	...
	fn parse_ts_type_member(&mut self) -> PResult<TsTypeElement> {
		...
		self.parse_ts_property_or_method_signature(start, readonly).map(|e| {
			match e {
				Either::Left(e) => e.into(),
				Either::Right(e) => e.into(),
			}
	...
	fn parse_ts_type_lit(&mut self) -> PResult<TsTypeLit> {
		...
		Ok(TsTypeLit { span:span!(self, start), members })
	}
	...
	fn parse_ts_mapped_type(&mut self) -> PResult<TsMappedType> {
		...
		if is_one_of!(self, '+', '-') {
			readonly =
				Some(if is!(self, '+') { TruePlusMinus::Plus } else { TruePlusMinus::Minus });
		...
		let name_type = if eat!(self, "as") { Some(self.parse_ts_type()?) } else { None };
	...
	fn parse_ts_tuple_type(&mut self) -> PResult<TsTupleType> {
		...
		for elem in elems.iter() {
			match *elem.ty {
				...
				_ if seen_optional_element => {
					syntax_error!(self, span!(self, start), SyntaxError::TsRequiredAfterOptional)
				},
		...
		Ok(TsTupleType { span:span!(self, start), elem_types:elems })
	}
	...
	fn try_parse_ts_tuple_element_name(&mut self) -> Option<Pat> {
		...
		self.try_parse_ts(|p| {
			...
			let rest = if eat!(p, "...") { Some(p.input.prev_span()) } else { None };
			...
			Ok(Some(if let Some(dot3_token) = rest {
				RestPat { span:span!(p, start), dot3_token, arg:ident.into(), type_ann:None }.into()
			} else {
				ident.into()
			}))
		})
	...
	fn parse_ts_tuple_element_type(&mut self) -> PResult<TsTupleElement> {
		...
		if eat!(self, "...") {
			...
			return Ok(TsTupleElement {
				span:span!(self, start),
				label,
				ty:Box::new(TsType::TsRestType(TsRestType { span:span!(self, start), type_ann })),
			});
		}
		...
		Ok(TsTupleElement { span:span!(self, start), label, ty })
	}
	...
	fn parse_ts_parenthesized_type(&mut self) -> PResult<TsParenthesizedType> {
		...
		Ok(TsParenthesizedType { span:span!(self, start), type_ann })
	}
	...
	fn parse_ts_fn_or_constructor_type(
		...
		let is_abstract = if !is_fn_type { eat!(self, "abstract") } else { false };
	...
	fn parse_ts_lit_type_node(&mut self) -> PResult<TsLitType> {
		...
		Ok(TsLitType { span:span!(self, start), lit })
	}
	...
	fn parse_ts_tpl_lit_type(&mut self) -> PResult<TsTplLitType> {
		...
		Ok(TsTplLitType { span:span!(self, start), types, quasis })
	}
		...
		for param in params {
			let item = match param.pat {
				...
				_ => {
					unexpected!(
						self,
						"an identifier, [ for an array pattern, { for an object patter or ... for \
						 a rest pattern"
					)
				},
			};
	...
	pub(super) fn try_parse_ts_type_params(
		...
		if is!(self, '<') {
			return self.parse_ts_type_params(permit_in_out, permit_const).map(Some);
		}
	...
	fn parse_ts_non_array_type(&mut self) -> PResult<Box<TsType>> {
		...
		match *cur!(self, true) {
			...
			| tok!("false")
			| tok!('`') => {
				return self.parse_ts_lit_type_node().map(TsType::from).map(Box::new);
			},
			...
			tok!('-') => {
				...
				let lit = match lit {
					Lit::Num(Number { span, value, raw }) => {
						...
						TsLit::Number(Number { span, value:-value, raw:Some(new_raw.into()) })
					},
				...
				return Ok(Box::new(TsType::TsLitType(TsLitType { span:span!(self, start), lit })));
			},
			...
			tok!('(') => {
				return self.parse_ts_parenthesized_type().map(TsType::from).map(Box::new);
			},
	...
	fn parse_ts_type_operator(&mut self, op:TsTypeOperatorOp) -> PResult<TsTypeOperator> {
		...
		Ok(TsTypeOperator { span:span!(self, start), op, type_ann })
	}
	...
	fn parse_ts_infer_type(&mut self) -> PResult<TsInferType> {
		...
		Ok(TsInferType { span:span!(self, start), type_param })
	}
	...
	fn parse_ts_type_operator_or_higher(&mut self) -> PResult<Box<TsType>> {
		...
		match operator {
			Some(operator) => self.parse_ts_type_operator(operator).map(TsType::from).map(Box::new),
			None => {
	...
	pub(super) fn parse_ts_expr_stmt(
		...
		match &*expr.sym {
			"declare" => {
				...
				if let Some(decl) = decl { Ok(Some(make_decl_declare(decl))) } else { Ok(None) }
			},
			"global" => {
				...
				if is!(self, '{') {
					...
					let body = self.parse_ts_module_block().map(TsNamespaceBody::from).map(Some)?;

					Ok(Some(
						TsModuleDecl { span:span!(self, start), global, declare:false, id, body }
							.into(),
					))
				} else {
					Ok(None)
	...
	pub(super) fn try_parse_ts_declare(
		...
		if self.ctx().in_declare
			&& matches!(self.syntax(), Syntax::Typescript(TsSyntax { dts:false, .. }))
		{
		...
		let ctx = Context { in_declare:true, ..self.ctx() };
							...
							Decl::Fn(f) => {
								FnDecl {
									...
									function:Box::new(Function {
										span:Span { lo:declare_start, ..f.function.span },
										..*f.function
									}),
									..f
								}
						...
						match decl {
							Decl::Class(c) => {
								ClassDecl {
									declare:true,
									class:Box::new(Class {
										span:Span { lo:declare_start, ..c.class.span },
										..*c.class
									}),
									..c
								}
			...
			if is!(p, "const") && peeked_is!(p, "enum") {
				...
				return p
					...
					.map(|decl| {
						TsEnumDecl {
							declare:true,
							span:Span { lo:declare_start, ..decl.span },
							..*decl
						}
					})
					.map(Box::new)
		...
		match &*value {
			...
			"interface" => {
				if next || (is!(self, IdentRef)) {
					...
					return self.parse_ts_interface_decl(start).map(From::from).map(Some);
				}
			},
			...
			"module" if !self.input.had_line_break_before_cur() => {
				...
				if matches!(*cur!(self, true), Token::Str { .. }) {
					...
					return self.parse_ts_module_or_ns_decl(start).map(From::from).map(Some);
				}
			},
			...
			"type" => {
				if next || (!self.input.had_line_break_before_cur() && is!(self, IdentRef)) {
					...
					return self.parse_ts_type_alias_decl(start).map(From::from).map(Some);
				}
			},
	...
	pub(super) fn try_parse_ts_generic_async_arrow_fn(
		...
		let res = if is_one_of!(self, '<', JSXTagStart) {
			self.try_parse_ts(|p| {
				...
				let params:Vec<Pat> = p.parse_formal_params()?.into_iter().map(|p| p.pat).collect();
		...
		let ctx = Context { in_async:true, in_generator:false, ..self.ctx() };
	...
	pub fn parse_ts_type_args(&mut self) -> PResult<Box<TsTypeParamInstantiation>> {
		...
		Ok(Box::new(TsTypeParamInstantiation { span:span!(self, start), params }))
	}
...
impl<I:Tokens> Parser<I> {
	...
	where
		...
		self.input.set_token_context(TokenContexts(smallvec::smallvec![cloned.0[0]]));
...
mod tests {
	...
	use crate::{Capturing, Parser, Syntax, lexer::Lexer, test_parser, token::*};

	#[test]
	fn issue_708_1() {
		let actual = test_parser("type test = -1;", Syntax::Typescript(Default::default()), |p| {
			p.parse_module()
		});
	...
	#[test]
	fn issue_708_2() {
		let actual = test_parser("const t = -1;", Syntax::Typescript(Default::default()), |p| {
			p.parse_module()
		});
	...
	fn issue_751() {
		crate::with_test_sess("t ? -(v >>> 1) : v >>> 1", |handler, input| {
			let lexer =
				Lexer::new(Syntax::Typescript(Default::default()), EsVersion::Es2019, input, None);
