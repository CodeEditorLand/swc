...
use swc_atoms::{Atom, AtomStore, atom};
...
impl std::str::FromStr for KnownIdent {
	...
	fn from_str(s:&str) -> Result<Self, Self::Err> { STR_TO_KNOWN_IDENT.get(s).cloned().ok_or(()) }
}
...
impl From<Keyword> for WordKind {
	#[inline(always)]
	fn from(kwd:Keyword) -> Self { Self::Keyword(kwd) }
}
...
impl BinOpToken {
	pub(crate) const fn starts_expr(self) -> bool { matches!(self, Self::Add | Self::Sub) }

	pub(crate) const fn before_expr(self) -> bool { true }
}
...
impl From<Keyword> for Word {
	fn from(kwd:Keyword) -> Self { Word::Keyword(kwd) }
}
...
impl Keyword {
	pub(crate) const fn before_expr(self) -> bool {
		matches!(
			self,
			Self::Await
				| Self::Case | Self::Default_
				| Self::Do | Self::Else
				| Self::Return
				| Self::Throw
				| Self::New | Self::Extends
				| Self::Yield
				| Self::In | Self::InstanceOf
				| Self::TypeOf
				| Self::Void | Self::Delete
		)
	}
