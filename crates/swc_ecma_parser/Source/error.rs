...
impl Spanned for Error {
	fn span(&self) -> Span { (*self.error).0 }
}
...
impl Error {
	...
	pub fn kind(&self) -> &SyntaxError { &self.error.1 }

	pub fn into_kind(self) -> SyntaxError { self.error.1 }
}
...
impl SyntaxError {
	...
	pub fn msg(&self) -> Cow<'static, str> {
		match self {
			...
			SyntaxError::ExpectedDigit { radix } => {
				format!(
					"Expected {} digit",
					match radix {
						2 => "a binary",
			...
			SyntaxError::InvalidIdentInStrict(identifier_name) => {
				format!("`{}` cannot be used as an identifier in strict mode", identifier_name)
					.into()
			},
			...
			SyntaxError::DecoratorOnExport => {
				"Using the export keyword between a decorator and a class is not allowed. Please \
				 use `export @dec class` instead."
					.into()
			},
			...
			SyntaxError::UsingDeclNotEnabled => {
				"Using declaration is not enabled. Set jsc.parser.explicitResourceManagement to \
				 true"
					.into()
			},
			...
			SyntaxError::InvalidNewTarget => {
				"'new.target' is only allowed in the body of a function declaration, function \
				 expression, or class."
					.into()
			},
			...
			SyntaxError::ImportBindingIsString(s) => {
				format!(
			...
			SyntaxError::TS1115 => {
				"A 'continue' statement can only jump to a label of an enclosing iteration \
				 statement"
					.into()
			},
			SyntaxError::TS1116 => {
			...
			SyntaxError::TS1274(word) => {
				format!(
					"'{}' modifier can only appear on a type parameter of a class, interface or \
					 type alias",
					word
				)
				.into()
			},
			SyntaxError::TS1277(word) => {
				format!(
					"'{}' modifier can only appear on a type parameter of a function, method or \
					 class",
					word
				)
				.into()
			...
			SyntaxError::TS2206 => {
				"The 'type' modifier cannot be used on a named import when 'import type' is used \
				 on its import statement."
					.into()
			},
			SyntaxError::TS2207 => {
				"The 'type' modifier cannot be used on a named export when 'export type' is used \
				 on its export statement."
					.into()
			},
			SyntaxError::TS2369 => {
			...
			SyntaxError::TS2406 => {
				"The left-hand side of an assignment expression must be a variable or a property \
				 access."
					.into()
			},
			SyntaxError::TS2410 => {
				"The 'with' statement is not supported. All symbols in a 'with' block will have \
				 type 'any'."
					.into()
			},
			...
			SyntaxError::TS4112 => {
				"This member cannot have an 'override' modifier because its containing class does \
				 not extend another class."
					.into()
			},
			SyntaxError::TS8038 => {
				"Decorators may not appear after `export` or `export default` if they also appear \
				 before `export`."
					.into()
			},
			SyntaxError::TS18010 => {
			...
			SyntaxError::ReservedTypeAssertion => {
				"This syntax is reserved in files with the .mts or .cts extension. Use an `as` \
				 expression instead."
					.into()
			},
			SyntaxError::ReservedArrowTypeParam => {
				"This syntax is reserved in files with the .mts or .cts extension. Add a trailing \
				 comma, as in `<T,>() => ...`."
					.into()
			},
