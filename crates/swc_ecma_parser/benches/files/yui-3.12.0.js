    ...
    **/
			_getEnv: function () {
				...
				return (this._env = {
					...
					async:
						(doc && doc.createElement("script").async === true) ||
						ua.ie >= 10,
						...
						if (subs.hasOwnProperty(i)) {
							...
							if (s.lang && s.lang.length) {
								...
								for (j = 0; j < langs.length; j++) {
									...
									flatSup =
										flatSup || yArray.hash(smod.supersedes);
			...
			resolve: function (calc, s) {
				...
				for (i = 0; i < len; i++) {
					...
					if (groupName && group) {
						...
						m.maxURLLength =
							group.maxURLLength || self.maxURLLength;
					} else {
						if (!self.combine) {
							//This is not a combo module, skip it and load it singly later.
							addSingle(m);
					...
					if (resCombos.hasOwnProperty(j)) {
						...
						for (type in resCombos[base]) {
							if (type === JS || type === CSS) {
								...
								resolved[type + "Mods"] =
									resolved[type + "Mods"].concat(mods);
							}
						}
					}
				}
