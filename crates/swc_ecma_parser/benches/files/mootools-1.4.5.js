...
Function.implement({
	...
	bindWithEvent: function (bind, args) {
		...
		return function (event) {
			return self.apply(
				bind,
				args == null ? arguments : [event].concat(args),
			);
		};
	},
...
(function () {
					...
					: root && root.compareDocumentPosition
						? function (context, node) {
								return (
									context === node ||
									!!(
										context.compareDocumentPosition(node) &
										16
									)
								);
							}
	...
	local.createNTHPseudo = function (child, sibling, positions, ofType) {
		return function (node, argument) {
			...
			argument = argument || "n";
			var parsed =
				this.cacheNTH[argument] || this.parseNTHArgument(argument);
			if (!parsed) return false;
			var a = parsed.a,
				b = parsed.b,
	...
	var pseudos = {
		...
		"contains": function (node, text) {
			return (
				(node.innerText || node.textContent || "").indexOf(text) > -1
			);
		},
	...
	Document.implement({
		...
		id: (function () {
			...
			return function (el, nocash, doc) {
				...
				var type = typeOf(el);
				return types[type]
					? types[type](el, nocash, doc || document)
					: null;
			};
		})(),
	});
		...
		function (combinator, method) {
			Element.implement(method, function (expression) {
				return this.getElement(
					injectCombinator(expression, combinator),
				);
			});
		},
	);
	...
	var getOpacity = hasOpacity
		...
		: hasFilter
			? function (element) {
					...
					return opacity == null || filter == null
						? 1
						: opacity[1] / 100;
				}
			: function (element) {
	...
	var Fx = (this.Fx = new Class({
		...
		*/
		...
		start: function (from, to) {
			...
			this.frames =
				frames || Math.round(this.duration / this.frameInterval);
...
(function () {
	var Swiff = (this.Swiff = new Class({
		...
		initialize: function (path, options) {
			...
			build += "</object>";
			this.object = (
				container ? container.empty() : new Element("div")
			).set("html", build).firstChild;
		},
