	...
	fn parse(input:ParseStream) -> syn::Result<Self> {
		fn update(c:&mut Config, meta:Meta) {
			if let Meta::List(list) = &meta {
				if list.path.get_ident().map(|i| *i == "exclude").unwrap_or(false) {
					//
					macro_rules! fail {
					...
					for lit in input.input {
						c.exclude_patterns.push(Regex::new(&lit.value()).unwrap_or_else(|err| {
			...
			unimplemented!("Expected something like {}\nGot wrong meta tag: {:?}", expected, meta,)
		}

		let pattern:LitStr = input.parse()?;
		let pattern = pattern.value();

		let mut config = Self { pattern, exclude_patterns:Vec::new() };
...
pub fn expand(callee:&Ident, attr:Config) -> Result<Vec<TokenStream>, Error> {
	...
	'add: for path in paths {
		...
		let path_for_name = path.strip_prefix(&base_dir).with_context(|| {
			format!("Failed to strip prefix `{}` from `{}`", base_dir.display(), path.display())
		})?;
		...
		let test_name = format!(
			...
			callee,
			RE.replace_all(
				path_for_name.to_string_lossy().replace(['\\', '/'], "__").as_str(),
				"_",
			)
		)
		.replace("___", "__");
...
impl Parse for InputParen {
	fn parse(input:ParseStream) -> syn::Result<Self> {
		Ok(Self { input:input.call(Punctuated::parse_terminated)? })
	}
}
