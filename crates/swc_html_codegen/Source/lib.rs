	...
	W: HtmlWriter,
{
	...
	fn basic_emit_element(
				...
				"body"
					if n.children.is_empty()
						|| (match n.children.first() {
							...
							Some(Child::Element(Element { namespace, tag_name, .. }))
								if *namespace == Namespace::HTML
									&& matches!(
										&**tag_name,
										"base"
											| "basefont" | "bgsound" | "frameset"
											| "link" | "meta" | "noframes" | "noscript"
					...
					&& matches!(n.namespace, Namespace::HTML))
			{
				if self.config.minify {
					let need_space = match n.attributes.last() {
						Some(Attribute { value: Some(value), .. }) => {
							!value.chars().any(|c| {
								match c {
		...
		if let Some(content) = &n.content {
			...
			if self.tag_omission {
				self.with_ctx(ctx).emit_list_for_tag_omission(TagOmissionParent::Element(n))?;
			} else {
				self.with_ctx(ctx).emit_list(&n.children, ListFormat::NotDelimited)?;
			}
		}
			...
			|| (self.tag_omission
				...
				&& match &*n.tag_name {
					...
					"p" => {
						match next {
							...
							None if match parent {
								Some(Element { namespace, tag_name, .. })
									if is_html_tag_name(*namespace, tag_name)
										&& !matches!(
											&**tag_name,
											"a" | "audio"
												| "acronym" | "big" | "del" | "font" | "ins"
												| "tt" | "strike" | "map" | "noscript"
												| "video" | "kbd" | "rbc"
										) =>
								{
									true
					...
					} =>
					{
						match next {
							Some(Child::Element(Element { namespace, tag_name, .. }))
								if *namespace == Namespace::HTML && *tag_name == "li" =>
							{
								true
							},
							None => true,
					...
					"optgroup" => {
						match next {
							Some(Child::Element(Element { namespace, tag_name, .. }))
								if *namespace == Namespace::HTML && *tag_name == "optgroup" =>
							{
								true
							},
							None => true,
					...
					"tr" => {
						match next {
							Some(Child::Element(Element { namespace, tag_name, .. }))
								if *namespace == Namespace::HTML && *tag_name == "tr" =>
							{
								true
							},
							None => true,
