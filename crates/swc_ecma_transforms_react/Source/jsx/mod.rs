...
use swc_ecma_utils::{ExprFactory, StmtLike, drop_span, prepend_stmt, private_ident, quote_ident};
...
impl Default for Runtime {
	fn default() -> Self { Runtime::Classic }
}
...
pub fn default_import_source() -> String { "react".into() }

pub fn default_pragma() -> String { "React.createElement".into() }

pub fn default_pragma_frag() -> String { "React.Fragment".into() }

fn default_throw_if_namespace() -> bool { true }
...
pub fn parse_expr_for_jsx(
	...
	let fm = cm.new_source_file(FileName::Internal(format!("jsx-config-{}.js", name)).into(), src);

	parse_file_as_expr(&fm, Syntax::default(), Default::default(), None, &mut Vec::new())
		.map_err(|e| {
			if HANDLER.is_set() {
				HANDLER.with(|h| e.into_diagnostic(h).note("Failed to parse jsx pragma").emit())
			}
		})
		.map(drop_span)
		.map(|mut expr| {
		...
		.map(Arc::new)
		.unwrap_or_else(|()| {
			panic!("failed to parse jsx option {}: '{}' is not an expression", name, fm.src,)
		})
}
...
where
	...
	visit_mut_pass(Jsx {
		...
		import_source:options.import_source.unwrap_or_else(default_import_source).into(),
		import_jsx:None,
		import_jsxs:None,
		...
		throw_if_namespace:options.throw_if_namespace.unwrap_or_else(default_throw_if_namespace),
		top_level_node:true,
	})
}
...
impl JsxDirectives {
	pub fn from_comments(cm:&SourceMap, _:Span, comments:&[Comment], top_level_mark:Mark) -> Self {
	...
	C: Comments,
{
	...
	where
		...
		if self.runtime == Runtime::Automatic {
			if let Some(local) = self.import_create_element.take() {
				inject(vec![(local, quote_ident!("createElement"))], &self.import_source, body);
			}
			...
			let imports = if self.development {
				imports
					...
					.into_iter()
					.chain(
						self.import_fragment.take().map(|local| (local, quote_ident!("Fragment"))),
					)
					.collect::<Vec<_>>()
			} else {
				imports
					...
					.into_iter()
					.chain(self.import_jsxs.take().map(|local| (local, quote_ident!("jsxs"))))
					.chain(
						self.import_fragment.take().map(|local| (local, quote_ident!("Fragment"))),
					)
					.collect::<Vec<_>>()
			...
			if !imports.is_empty() {
				let jsx_runtime = if self.development { "jsx-dev-runtime" } else { "jsx-runtime" };
		...
		match self.runtime {
			Runtime::Automatic => {
				let jsx = if use_jsxs && !self.development {
					self.import_jsxs.get_or_insert_with(|| private_ident!("_jsxs")).clone()
				} else {
					let jsx = if self.development { "_jsxDEV" } else { "_jsx" };

					self.import_jsx.get_or_insert_with(|| private_ident!(jsx)).clone()
				};

				let fragment =
					self.import_fragment.get_or_insert_with(|| private_ident!("_Fragment")).clone();

				let mut props_obj = ObjectLit { span:DUMMY_SP, props:Vec::new() };
				...
				match (children.len(), use_jsxs) {
					(0, _) => {},
					(1, false) => {
						props_obj.props.push(PropOrSpread::Prop(Box::new(Prop::KeyValue(
							KeyValueProp {
					...
					_ => {
						props_obj.props.push(PropOrSpread::Prop(Box::new(Prop::KeyValue(
							KeyValueProp {
								key:PropName::Ident(quote_ident!("children")),
								value:ArrayLit { span:DUMMY_SP, elems:children }.into(),
							},
						))));
					},
				}
				...
				CallExpr { span, callee:jsx.as_callee(), args, ..Default::default() }.into()
			},
							...
							match attr.name {
								JSXAttrName::Ident(i) => {
									...
									if !use_create_element
										&& *i.sym == *"__source" && self.development
									{
									...
									let key = if i.sym.contains('-') {
										PropName::Str(Str { span:i.span, raw:None, value:i.sym })
									} else {
										PropName::Ident(i)
									};
								...
								JSXAttrName::JSXNamespacedName(JSXNamespacedName {
									...
									let key = Str { span, raw:None, value:str_value.into() };
				...
				match children.len() {
					...
					_ => {
						props_obj.props.push(PropOrSpread::Prop(Box::new(Prop::KeyValue(
							KeyValueProp {
								key:PropName::Ident(quote_ident!("children")),
								value:ArrayLit { span:DUMMY_SP, elems:children.take() }.into(),
							},
						))));
					},
				}
			...
			Runtime::Classic => {
				CallExpr {
					...
					args:iter::once(name.as_arg())
						...
						.chain({
							// Children
							el.children.into_iter().filter_map(|c| self.jsx_elem_child_to_expr(c))
						})
						.collect(),
					..Default::default()
				}
	...
	fn jsx_elem_child_to_expr(&mut self, c:JSXElementChild) -> Option<ExprOrSpread> {
		...
		Some(match c {
			JSXElementChild::JSXText(text) => {
				...
				let s = Str { span:text.span, raw:None, value };
			...
			JSXElementChild::JSXSpreadChild(JSXSpreadChild { span, expr, .. }) => {
				ExprOrSpread { spread:Some(span), expr }
			},
		})
	}
	...
	fn fold_attrs_for_classic(&mut self, attrs:Vec<JSXAttrOrSpread>) -> Box<Expr> {
		...
		let obj = ObjectLit { span:DUMMY_SP, props };
	...
	fn attr_to_prop(&mut self, a:JSXAttr) -> Prop {
		...
		let value = a
			...
			.map(|v| {
				match v {
					JSXAttrValue::Lit(Lit::Str(s)) => {
						...
						Lit::Str(Str { span:s.span, raw:None, value:value.into() }).into()
					},
					...
					JSXAttrValue::JSXFragment(fragment) => {
						Box::new(self.jsx_frag_to_expr(fragment))
					},
			...
			.unwrap_or_else(|| Lit::Bool(Bool { span:key.span(), value:true }).into());
	...
	C: Comments,
{
	...
	fn parse_directives(&mut self, span:Span) -> bool {
		...
		let JsxDirectives { runtime, import_source, pragma, pragma_frag } = directives;
	...
	C: Comments,
{
	...
	fn visit_mut_expr(&mut self, expr:&mut Expr) {
		...
		} else if let Expr::Paren(ParenExpr { expr: inner_expr, .. }) = expr {
			...
			self.inject_runtime(&mut module.body, |imports, src, stmts| {
				...
				prepend_stmt(
					...
					ImportDecl {
						span:DUMMY_SP,
						specifiers,
						src:Str { span:DUMMY_SP, raw:None, value:src.into() }.into(),
		...
		match name {
			JSXElementName::Ident(i) => {
				...
				if i.as_ref().starts_with(|c:char| c.is_ascii_lowercase()) {
					Lit::Str(Str { span, raw:None, value:i.sym }).into()
				} else {
					i.into()
				}
			},
			...
			JSXElementName::JSXNamespacedName(JSXNamespacedName { ref ns, ref name, .. }) => {
				...
				Lit::Str(Str { span, raw:None, value:value.into() }).into()
			},
			...
			JSXElementName::JSXMemberExpr(JSXMemberExpr { obj, prop, .. }) => {
				...
				MemberExpr { span, obj:convert_obj(obj), prop:MemberProp::Ident(prop) }.into()
			},
		}
	}
}
...
fn to_prop_name(n:JSXAttrName) -> PropName {
	...
	match n {
		JSXAttrName::Ident(i) => {
			if i.sym.contains('-') {
				PropName::Str(Str { span, raw:None, value:i.sym })
			} else {
				PropName::Ident(i)
			}
		},
		...
		JSXAttrName::JSXNamespacedName(JSXNamespacedName { ns, name, .. }) => {
			...
			PropName::Str(Str { span, raw:None, value:value.into() })
		},
	}
}
...
fn jsx_text_to_str(t:Atom) -> JsWord {
	...
	for (is_last, (i, line)) in replaced.lines().enumerate().identify_last() {
		...
		let line = if i != 0 { Cow::Borrowed(line.trim_start_matches(' ')) } else { line };

		let line = if is_last { line } else { Cow::Borrowed(line.trim_end_matches(' ')) };
...
fn jsx_attr_value_to_expr(v:JSXAttrValue) -> Option<Box<Expr>> {
	Some(match v {
		...
		JSXAttrValue::Lit(lit) => Box::new(lit.into()),
		JSXAttrValue::JSXExprContainer(e) => {
			match e.expr {
