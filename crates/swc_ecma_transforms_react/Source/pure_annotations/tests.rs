...
fn parse(
	...
	let program = {
		...
		let res = p.parse_module().map_err(|e| e.into_diagnostic(tester.handler).emit());
...
fn emit(
	...
	let mut buf = std::vec::Vec::new();
	{
		...
		let mut emitter =
			Emitter { cfg:Default::default(), comments:Some(&comments), cm:source_map, wr:writer };
...
fn run_test(input:&str, expected:&str) {
	Tester::run(|tester| {
		...
		let actual = actual.apply(&mut resolver(unresolved_mark, top_level_mark, false)).apply(
			&mut crate::react(
				actual_sm.clone(),
...
macro_rules! test {
	($test_name:ident, $input:expr, $expected:expr) => {
		#[test]
		fn $test_name() { run_test($input, $expected) }
	};
}
