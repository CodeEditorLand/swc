...
pub fn display_name() -> impl Fold + VisitMut { as_folder(DisplayName) }
...
impl VisitMut for DisplayName {
	...
	fn visit_mut_assign_expr(&mut self, expr:&mut AssignExpr) {
		...
		if let AssignTarget::Simple(
			SimpleAssignTarget::Member(MemberExpr { prop: MemberProp::Ident(prop), .. })
			| SimpleAssignTarget::SuperProp(SuperPropExpr { prop: SuperProp::Ident(prop), .. }),
		) = &expr.left
		{
			return expr.right.visit_mut_with(&mut Folder {
				name:Some(
					Lit::Str(Str { span:prop.span, raw:None, value:prop.sym.clone() }).into(),
				),
			});
		};
	...
	fn visit_mut_module_decl(&mut self, decl:&mut ModuleDecl) {
		...
		if let ModuleDecl::ExportDefaultExpr(e) = decl {
			e.visit_mut_with(&mut Folder {
				name:Some(Lit::Str(Str { span:DUMMY_SP, raw:None, value:"input".into() }).into()),
			});
		}
	}
			...
			value.visit_mut_with(&mut Folder {
				name:Some(match key {
					PropName::Ident(ref i) => {
						Lit::Str(Str { span:i.span, raw:None, value:i.sym.clone() }).into()
					},
	...
	fn visit_mut_var_declarator(&mut self, decl:&mut VarDeclarator) {
		if let Pat::Ident(ref ident) = decl.name {
			decl.init.visit_mut_with(&mut Folder {
				name:Some(
					Lit::Str(Str { span:ident.span, value:ident.sym.clone(), raw:None }).into(),
				),
			});
		}
	}
...
fn add_display_name(call:&mut CallExpr, name:Box<Expr>) {
	let props = match call.args.first_mut() {
		Some(&mut ExprOrSpread { ref mut expr, .. }) => {
