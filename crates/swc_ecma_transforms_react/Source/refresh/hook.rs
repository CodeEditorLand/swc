...
impl HookSig {
	fn new(hooks:Vec<Hook>) -> Self { HookSig { handle:private_ident!("_s"), hooks } }
}
...
impl<'a> HookRegister<'a> {
	...
	fn wrap_with_register(&self, handle:Ident, func:Expr, hooks:Vec<Hook>) -> Expr {
		...
		args.push(Lit::Str(Str { span:DUMMY_SP, raw:None, value:sign.into() }).as_arg());
		...
		for hook in custom_hook {
			...
			if !ident.map(|id| self.current_scope.contains(&id.ctxt)).unwrap_or(false) {
				...
				Function {
					...
					body:Some(BlockStmt {
						...
						stmts:vec![Stmt::Return(ReturnStmt {
							span:DUMMY_SP,
							arg:Some(Box::new(Expr::Array(ArrayLit { span:DUMMY_SP, elems }))),
						})],
						..Default::default()
					}),
					..Default::default()
		...
		CallExpr { span:DUMMY_SP, callee:handle.as_callee(), args, ..Default::default() }.into()
	}
...
fn collect_hooks(stmts:&mut Vec<Stmt>, cm:&SourceMap) -> Option<HookSig> {
	let mut hook = HookCollector { state:Vec::new(), cm };
...
impl<'a> HookCollector<'a> {
	fn get_hook_from_call_expr(&self, expr:&CallExpr, lhs:Option<&Pat>) -> Option<Hook> {
		...
		let ident = match callee {
			...
			Expr::Member(MemberExpr { obj, prop: MemberProp::Ident(ident), .. }) => {
				hook_call = Some(HookCall::Member(*obj.clone(), ident.clone()));
				Some(&ident.sym)
			},
			_ => None,
		}?;
		let name = if is_hook_like(ident) { Some(ident) } else { None }?;
		...
		if *name == "useState" && !expr.args.is_empty() {
			...
			let _ = write!(
				key,
				"({})",
				self.cm.span_to_snippet(expr.args[0].span()).unwrap_or_default()
			);
			...
			let _ = write!(
				key,
				"({})",
				self.cm.span_to_snippet(expr.args[1].span()).unwrap_or_default()
			);
		}
