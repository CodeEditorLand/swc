use swc_common::{DUMMY_SP, Spanned, SyntaxContext, collections::AHashSet};
use swc_ecma_ast::*;
...
pub fn is_body_arrow_fn(body:&BlockStmtOrExpr) -> bool {
	if let BlockStmtOrExpr::Expr(body) = body { body.is_arrow() } else { false }
}
...
pub fn make_assign_stmt(handle:Ident, expr:Box<Expr>) -> Expr {
	...
	AssignExpr { span:expr.span(), op:op!("="), left:handle.into(), right:expr }.into()
}

pub fn make_call_stmt(handle:Ident) -> Stmt {
	ExprStmt { span:DUMMY_SP, expr:Box::new(make_call_expr(handle)) }.into()
}

pub fn make_call_expr(handle:Ident) -> Expr {
	CallExpr { span:DUMMY_SP, callee:handle.as_callee(), args:Vec::new(), ..Default::default() }
		.into()
}

pub fn is_import_or_require(expr:&Expr) -> bool {
	match expr {
		Expr::Call(CallExpr { callee: Callee::Expr(expr), .. }) => {
		...
		Expr::Call(CallExpr { callee: Callee::Import(_), .. }) => true,
		_ => false,
	}
}
	...
	fn visit_call_expr(&mut self, n:&CallExpr) {
		...
		if let Callee::Expr(expr) = &n.callee {
			let ident = match expr.as_ref() {
				...
				Expr::Member(MemberExpr { prop: MemberProp::Ident(ident), .. }) => {
					(ident.sym.clone(), SyntaxContext::empty())
				},
				_ => return,
			};

			if matches!(ident.0.as_ref(), "createElement" | "jsx" | "jsxDEV" | "jsxs") {
