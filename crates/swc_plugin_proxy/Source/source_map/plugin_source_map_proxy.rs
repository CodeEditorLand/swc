...
impl SourceMapper for PluginSourceMapProxy {
	...
	fn lookup_char_pos(&self, pos:BytePos) -> Loc {
		#[cfg(target_arch = "wasm32")]
		{
			let should_request_source_file = if self.source_file.get().is_none() { 1 } else { 0 };
			...
			if self.source_file.get().is_none() {
				if let Some(source_file) = partial_loc.source_file {
					self.source_file.set(source_file).expect("Should able to set source file");
				}
			}

			return Loc {
				file:self.source_file.get().expect("SourceFile should exist").clone(),
	...
	fn span_to_lines(&self, sp:Span) -> FileLinesResult {
		#[cfg(target_arch = "wasm32")]
		{
			...
			return partial_files.map(|files| {
				FileLines {
					file:self.source_file.get().expect("SourceFile should exist").clone(),
					lines:files.lines,
				}
	...
	fn merge_spans(&self, sp_lhs:Span, sp_rhs:Span) -> Option<Span> {
		...
		unsafe {
			...
			let span = Span { lo:BytePos(0), hi:BytePos(0) };
