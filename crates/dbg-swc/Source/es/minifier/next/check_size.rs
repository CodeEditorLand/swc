...
impl CheckSizeCommand {
	pub fn run(self, cm:Arc<SourceMap>) -> Result<()> {
		...
		for file in &files {
			println!(
				...
				file.path.strip_prefix(self.workspace.join("inputs")).unwrap().display(),
				file.swc,
				file.terser
			);
		}
		...
		let items = files
			...
			.map(|f| {
				format!(
					...
					f.path.strip_prefix(self.workspace.join("inputs")).unwrap().display(),
	...
	fn build_app(&self, app_dir:&Path) -> Result<Vec<InputFile>> {
		wrap_task(|| {
			...
			let output = c.output().context("failed to get output of `npm run build`")?;
...
where
	...
	wrap_task(|| {
		...
		let json_str = c.output().context("failed to parse json loosely using node")?.stdout;
