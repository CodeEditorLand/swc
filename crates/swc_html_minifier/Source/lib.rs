...
static COMMA_SEPARATED_SVG_ATTRIBUTES:&[(&str, &str)] =
	&[("style", "media"), ("polyline", "points"), ("polygon", "points")];
...
#[inline(always)]
fn is_whitespace(c:char) -> bool { matches!(c, '\x09' | '\x0a' | '\x0c' | '\x0d' | '\x20') }
...
impl<C:MinifyCss> Minifier<'_, C> {
		...
		match element.namespace {
			Namespace::HTML => {
				match &*attribute.name {
					...
					_ => {
						COMMA_SEPARATED_HTML_ATTRIBUTES
							.contains(&(&element.tag_name, &attribute.name))
					},
				}
			},
			Namespace::SVG => {
	...
	fn is_attribute_value_unordered_set(&self, element:&Element, attribute:&Attribute) -> bool {
		if matches!(&*attribute.name, "class" | "part" | "itemprop" | "itemref" | "itemtype") {
			return true;
		}
	...
	fn is_crossorigin_attribute(&self, current_element:&Element, attribute:&Attribute) -> bool {
		matches!(
			(current_element.namespace, &*current_element.tag_name, &*attribute.name,),
			(Namespace::HTML, "img" | "audio" | "video" | "script" | "link", "crossorigin",)
	...
	fn is_type_text_javascript(&self, value:&str) -> bool {
		...
		let value = if let Some(next) = value.split(';').next() { next } else { &value };
			...
			_ => {
				matches!(
					...
					|(Namespace::MATHML, "math", "xmlns", "http://www.w3.org/1998/math/mathml")| {
						(Namespace::MATHML, "math", "xlink", "http://www.w3.org/1999/xlink")
					}
				)
			},
		}
		...
		if only_first {
			if let Some(last) = children.first_mut() {
				match last {
					...
					Child::Element(Element { namespace, tag_name, children, .. })
	...
	fn get_whitespace_minification_for_tag(&self, element:&Element) -> WhitespaceMinificationMode {
		...
		match element.namespace {
			Namespace::HTML => {
				match &*element.tag_name {
					...
					_ => {
						...
						{
							WhitespaceMinificationMode { collapse:false, trim:false }
						} else {
							WhitespaceMinificationMode {
			...
			Namespace::SVG => {
				match &*element.tag_name {
					"script" | "style" => WhitespaceMinificationMode { collapse:false, trim:true },
					// https://svgwg.org/svg2-draft/render.html#Definitions
					_ if matches!(
						&*element.tag_name,
						"a" | "circle"
							| "ellipse" | "foreignObject"
							| "g" | "image" | "line"
							| "path" | "polygon" | "polyline"
							| "rect" | "svg" | "switch"
							| "symbol" | "text" | "textPath"
							| "tspan" | "use"
					) =>
					{
						WhitespaceMinificationMode { collapse:default_collapse, trim:default_trim }
					},
			...
			_ => WhitespaceMinificationMode { collapse:false, trim:default_trim },
		}
	}
	...
	fn merge_text_children(&self, left:&Element, right:&Element) -> Option<Vec<Child>> {
		...
		Some(vec![Child::Text(Text { span:DUMMY_SP, data:data.into(), raw:None })])
	}
							...
							) =>
					{
						...
						if matches!(
							...
							let prev_display = match prev {
								...
								Some(Child::Comment(_)) => {
									match need_remove_metadata_whitespaces {
										true => None,
										_ => Some(Display::None),
									}
								},
	...
	fn merge_js(&self, left:String, right:String, is_modules:bool) -> Option<String> {
		...
		match &mut left_program {
			swc_ecma_ast::Program::Module(left_program) => {
				match right_program {
			...
			swc_ecma_ast::Program::Script(left_program) => {
				match right_program {
	...
	fn minify_js(&self, data:String, is_module:bool, is_attribute:bool) -> Option<String> {
		...
		let mut program = if is_module {
			match swc_ecma_parser::parse_file_as_module(
				...
				if options.parser.comments { Some(&comments) } else { None },
				&mut errors,
			) {
				Ok(module) => swc_ecma_ast::Program::Module(module),
				_ => return None,
		...
		let program =
			program.apply(swc_ecma_transforms_base::fixer::paren_remover(Some(&comments)));
...

		{
			...
			let mut emitter = swc_ecma_codegen::Emitter {
				cfg:options.codegen,
				cm,
				comments:if options.parser.comments { Some(&comments) } else { None },
				wr,
			};
	...
	fn minify_sizes(&self, value:&str) -> Option<String> {
		let values = value.rsplitn(2, ['\t', '\n', '\x0C', '\r', ' ']).collect::<Vec<&str>>();
	...
	fn minify_css(&self, data:String, mode:CssMinificationMode) -> Option<String> {
		self.css_minifier.minify_css(&self.options.minify_css, data, mode)
	}
		...
		let mut document_or_document_fragment = match mode {
			...
			HtmlMinificationMode::DocumentIframeSrcdoc => {
				match swc_html_parser::parse_file_as_document(
					&fm,
					ParserConfig { iframe_srcdoc:true, ..Default::default() },
					&mut errors,
				) {
			...
			match (element.namespace, &*element.tag_name, &*n.name) {
				(Namespace::HTML, "iframe", "srcdoc") => {
					if let Some(minified) = self
						.minify_html(value.to_string(), HtmlMinificationMode::DocumentIframeSrcdoc)
					{
				...
				{
					n.value = Some(value.split_whitespace().collect::<Vec<_>>().join(" ").into());
				},
...
impl<C:MinifyCss> VisitMut for Minifier<'_, C> {
	...
	fn visit_mut_element(&mut self, n:&mut Element) {
		...
		if let Some(attribute_name_counter) = &self.attribute_name_counter {
			n.attributes.sort_by(|a, b| {
				let ordeing =
					attribute_name_counter.get(&b.name).cmp(&attribute_name_counter.get(&a.name));
	...
	fn visit_mut_text(&mut self, n:&mut Text) {
		...
		match text_type {
			...
			Some(MinifierType::Html) => {
				let minified = match self
					.minify_html(n.data.to_string(), HtmlMinificationMode::ConditionalComments)
				{
					Some(minified) => minified,
					None => return,
				};
	...
	fn visit_mut_comment(&mut self, n:&mut Comment) {
		...
		if self.is_conditional_comment(&n.data) && n.data.len() > 0 {
			...
			let html = n.data.chars().skip(start_pos).take(end_pos - start_pos).collect();
				...
				match swc_css_parser::parse_file::<Vec<swc_css_ast::DeclarationOrAtRule>>(
					...
					Ok(list_of_declarations) => {
						let declaration_list:Vec<swc_css_ast::ComponentValue> =
							list_of_declarations.into_iter().map(|node| node.into()).collect();
							...
							rules:vec![swc_css_ast::Rule::AtRule(
								swc_css_ast::AtRule {
									...
									block:Some(swc_css_ast::SimpleBlock {
										...
										// TODO make the `compress_empty` option for CSS minifier
										// and remove it
...
pub fn minify_document_with_custom_css_minifier<C:MinifyCss>(
	...
	if options.sort_attributes {
		let mut attribute_name_counter = AttributeNameCounter { tree:Default::default() };
