...
fn default_ts_syntax() -> TsSyntax { TsSyntax { decorators:true, ..Default::default() } }
...
pub fn operate(
	...
	let filename = options.filename.map_or(FileName::Anon, |f| FileName::Real(f.into()));
	...
	let lexer =
		Capturing::new(Lexer::new(syntax, target, StringInput::from(&*fm), Some(&comments)));
	...
	match options.mode {
		Mode::StripOnly => {
			...
			if replacements.is_empty() && overwrites.is_empty() {
				return Ok(TransformOutput { code:fm.src.to_string(), map:Default::default() });
			}
					...
					match c {
						...
						_ => {
							match c.len_utf8() {
								...
								4 => {
									// We do not have a 4-byte space character in the Unicode
									// standard.
			...
			Ok(TransformOutput { code, map:Default::default() })
		},
		...
		Mode::Transform => {
...

			{
				let mut emitter = swc_ecma_codegen::Emitter {
					...
					comments:if options.source_map { Some(&comments) } else { None },
					cm:cm.clone(),
				...
				let map = src_map_buf
					.map(|map| {
						...
						map.to_writer(&mut s).context("failed to write source map")?;
...
impl TsStrip {
	fn new(src:Lrc<String>, tokens:std::vec::Vec<TokenAndSpan>) -> Self {
		TsStrip { src, replacements:Default::default(), overwrites:Default::default(), tokens }
	}
}

impl TsStrip {
	fn add_replacement(&mut self, span:Span) { self.replacements.push((span.lo, span.hi)); }

	fn add_overwrite(&mut self, pos:BytePos, value:u8) { self.overwrites.push((pos, value)); }
	...
	fn fix_asi(&mut self, span:Span) {
		...
		let TokenAndSpan { token: prev_token, span: prev_span, .. } = &self.tokens[index - 1];
		...
		let TokenAndSpan { token, had_line_break, .. } = &self.tokens[index + 1];
	...
	fn strip_class_modifier(&mut self, mut start_pos:BytePos, key_pos:BytePos) {
		...
		while start_pos < key_pos {
			...
			if !matches!(
				next.token,
				Token::LBracket
					| Token::LBrace | Token::BinOp(BinOpToken::Mul)
					| Token::DotDotDot
					| Token::Hash | Token::Word(_)
					| Token::Str { .. }
					| Token::Num { .. }
					| Token::BigInt { .. }
			) {
	...
	fn strip_definite_mark(&mut self, index:usize) { self.strip_token(index, Token::Bang); }

	fn strip_optional_mark(&mut self, index:usize) { self.strip_token(index, Token::QuestionMark); }
...
impl Visit for TsStrip {
	...
	fn visit_class_method(&mut self, n:&ClassMethod) {
		...
		let start_pos = n.function.decorators.last().map_or(n.span.lo, |d| d.span.hi);
	...
	fn visit_ts_as_expr(&mut self, n:&TsAsExpr) {
		...
		let TokenAndSpan { token, span: as_span, .. } = self.get_next_token(n.expr.span_hi());

		debug_assert_eq!(token, &Token::Word(Word::Ident(IdentLike::Known(KnownIdent::As))));
	...
	fn visit_ts_index_signature(&mut self, n:&TsIndexSignature) { self.add_replacement(n.span); }
	...
	fn visit_ts_enum_decl(&mut self, e:&TsEnumDecl) {
		HANDLER.with(|handler| {
			handler.span_err(e.span, "TypeScript enum is not supported in strip-only mode");
		});
	}
	...
	fn visit_ts_satisfies_expr(&mut self, n:&TsSatisfiesExpr) {
		...
		debug_assert_eq!(token, &Token::Word(Word::Ident(IdentLike::Known(KnownIdent::Satisfies))));
	...
	fn visit_ts_type_ann(&mut self, n:&TsTypeAnn) { self.add_replacement(n.span); }
	...
	fn visit_ts_type_param_decl(&mut self, n:&TsTypeParamDecl) { self.add_replacement(n.span); }
...
impl IsTsDecl for Stmt {
	fn is_ts_declare(&self) -> bool { self.as_decl().map_or(false, IsTsDecl::is_ts_declare) }
}
...
fn span(lo:BytePos, hi:BytePos) -> Span { Span::new(lo, hi) }
