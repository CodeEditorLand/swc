...
impl<T:?Sized> From<std::boxed::Box<T, FastAlloc>> for Box<T> {
	#[inline(always)]
	fn from(v:std::boxed::Box<T, FastAlloc>) -> Self { Box(v) }
}
...
where
	T: Default,
{
	fn default() -> Self { Box::new(Default::default()) }
}
...
impl<T> Box<T> {
	...
	#[inline(always)]
	pub fn new(value:T) -> Self { Self::new_in(value, Default::default()) }
	...
	/// ```
	#[inline(always)]
	pub fn new_in(value:T, alloc:FastAlloc) -> Self { Self(std::boxed::Box::new_in(value, alloc)) }

	/// Moves the value out of the box.
	pub fn unbox(self) -> T { std::boxed::Box::into_inner(self.0) }
}
...
impl<T:?Sized> Box<T> {
	...
	///
	/// [memory layout]: self#memory-layout
	pub fn into_raw(b:Self) -> *mut T { std::boxed::Box::into_raw(b.0) }
...
impl<T:?Sized> fmt::Pointer for Box<T> {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result { fmt::Pointer::fmt(&self.0, f) }
}
	...
	T: fmt::Write,
{
	...
	fn write_fmt(&mut self, args:fmt::Arguments) -> fmt::Result { self.0.write_fmt(args) }
}
	...
	T: io::Write,
{
	...
	fn write_fmt(&mut self, fmt:fmt::Arguments) -> io::Result<()> { self.0.write_fmt(fmt) }
}
