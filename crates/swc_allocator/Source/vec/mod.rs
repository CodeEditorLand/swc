...
impl<T> Vec<T> {
	...
	#[inline(always)]
	pub fn new() -> Self { Self::new_in(Default::default()) }
	...
	#[inline(always)]
	pub fn new_in(alloc:FastAlloc) -> Self { Self(std::vec::Vec::new_in(alloc)) }
	...
	/// ```
	#[inline(always)]
	pub fn into_boxed_slice(self) -> Box<[T]> { self.0.into_boxed_slice().into() }
	...
	/// ```
	#[inline(always)]
	pub fn leak(self) -> &'static mut [T] { self.0.leak() }
	...
	pub unsafe fn from_raw_parts(ptr:*mut T, length:usize, capacity:usize) -> Self {
		Self(std::vec::Vec::from_raw_parts_in(ptr, length, capacity, FastAlloc::default()))
	}
}
...
impl<T> Default for Vec<T> {
	#[inline(always)]
	fn default() -> Self { Self(std::vec::Vec::new_in(FastAlloc::default())) }
}
...
impl<T> From<Box<[T]>> for Vec<T> {
	#[inline(always)]
	fn from(v:Box<[T]>) -> Self { Self(std::vec::Vec::from(v.0)) }
}

impl<T> From<Vec<T>> for Box<[T]> {
	#[inline(always)]
	fn from(v:Vec<T>) -> Self { Box(v.0.into()) }
}
...
impl io::Write for Vec<u8> {
	#[inline(always)]
	fn write(&mut self, buf:&[u8]) -> io::Result<usize> { io::Write::write(&mut self.0, buf) }

	#[inline(always)]
	fn flush(&mut self) -> io::Result<()> { io::Write::flush(&mut self.0) }

	#[inline(always)]
	fn write_all(&mut self, buf:&[u8]) -> io::Result<()> { io::Write::write_all(&mut self.0, buf) }
