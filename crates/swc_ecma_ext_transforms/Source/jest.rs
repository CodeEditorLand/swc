...
pub fn jest() -> impl Fold + VisitMut { as_folder(Jest::default()) }
...
impl Jest {
	...
	fn should_hoist(&self, e:&Expr) -> bool {
		match e {
			...
			Expr::Member(callee @ MemberExpr { prop: MemberProp::Ident(prop), .. }) => {
...
fn is_global_jest(e:&Expr) -> bool {
	match e {
		...
		Expr::Call(CallExpr { callee: Callee::Expr(callee), .. }) => is_global_jest(callee),
		_ => false,
	}
}
