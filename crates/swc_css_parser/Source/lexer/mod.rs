...
impl<I:Input> Iterator for Lexer<'_, I> {
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		match token {
			Ok(token) => {
				let end = self.override_pos.take().unwrap_or_else(|| self.input.last_pos());
	...
	I: Input,
{
	...
	fn start_pos(&mut self) -> BytePos { self.input.last_pos() }

	fn state(&mut self) -> Self::State { LexerState { pos:self.input.last_pos() } }
	...
	fn take_errors(&mut self) -> Vec<Error> { self.errors.take() }
	...
	fn atom(&self, s:Cow<str>) -> JsWord { self.atoms.atom(s) }
}
...
{
	#[inline(always)]
	fn cur(&mut self) -> Option<char> { self.cur }

	#[inline(always)]
	fn next(&mut self) -> Option<char> { self.input.cur() }

	#[inline(always)]
	fn next_next(&mut self) -> Option<char> { self.input.peek() }

	#[inline(always)]
	fn next_next_next(&mut self) -> Option<char> { self.input.peek_ahead() }
	...
	fn emit_error(&mut self, kind:ErrorKind) {
		self.errors
			.borrow_mut()
			.push(Error::new(Span::new(self.cur_pos, self.input.last_pos()), kind));
	}
		...
		match self.consume() {
			...
			Some(c) if is_whitespace(c) => {
				self.with_buf(|l, buf| {
					...
					return Ok(Token::WhiteSpace { value:l.atoms.atom(&**buf) });
				})
			},
			...
			Some('@') => {
				...
				if self.would_start_ident(first, second, third) {
					...
					return Ok(Token::AtKeyword { value:ident_sequence.0, raw:ident_sequence.1 });
				}
	...
	fn read_numeric(&mut self) -> LexResult<Token> {
		...
		// a <percentage-token> with the same value as number, and return it.
			...
			return Ok(Token::Percentage { value:number.0, raw:number.1 });
		}

		// Otherwise, create a <number-token> with the same value and type flag as
		// number, and return it.
		Ok(Token::Number { value:number.0, raw:number.1, type_flag:number.2 })
	}
	...
	fn read_ident_like(&mut self) -> LexResult<Token> {
		...
		if matches_eq_ignore_ascii_case!(ident_sequence.0, "url") && self.next() == Some('(') {
			...
			match self.next() {
						...
						&& (self.next_next() == Some('"') || self.next_next() == Some('\'')) =>
				{
					...
					return Ok(Token::Function { value:ident_sequence.0, raw:ident_sequence.1 });
				},
				Some('"' | '\'') => {
					return Ok(Token::Function { value:ident_sequence.0, raw:ident_sequence.1 });
				},
		...
		// it.
		Ok(Token::Ident { value:ident_sequence.0, raw:ident_sequence.1 })
	}
	...
	fn read_str(&mut self, maybe_ending_code_point:Option<char>) -> LexResult<Token> {
		self.with_buf_and_raw_buf(|l, buf, raw| {
			...
			loop {
				match l.consume() {
					...
					Some(c) if is_newline(c) => {
						...
						return Ok(Token::BadString { raw:l.atoms.atom(&**raw) });
					},
			...
			Ok(Token::String { value:l.atoms.atom(&**buf), raw:l.atoms.atom(&**raw) })
		})
	}
...
#[inline(always)]
fn is_uppercase_letter(c:char) -> bool { c.is_ascii_uppercase() }

#[inline(always)]
fn is_lowercase_letter(c:char) -> bool { c.is_ascii_lowercase() }

#[inline(always)]
fn is_letter(c:char) -> bool { is_uppercase_letter(c) || is_lowercase_letter(c) }

#[inline(always)]
fn is_non_ascii(c:char) -> bool { c as u32 >= 0x80 }
...
#[inline(always)]
fn is_name(c:char) -> bool { is_name_start(c) || matches!(c, c if c.is_ascii_digit() || c == '-') }
...
#[inline(always)]
fn is_newline(c:char) -> bool { matches!(c, '\n' | '\r' | '\x0C') }

#[inline(always)]
fn is_whitespace(c:char) -> bool { matches!(c, c if c == ' ' || c == '\t' || is_newline(c)) }
