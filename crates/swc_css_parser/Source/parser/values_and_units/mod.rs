									...
									_ => {
										if !has_variable_before {
											Err(Error::new(
												...
												ErrorKind::Expected(
													"percentage, number, function (math \
									...
									_ => {
										if !has_variable_before {
											Err(Error::new(
												parser.input.cur_span(),
												ErrorKind::Expected(
													"number, dimension, function (math functions) \
													 or ident (with 'none' value) token",
												),
											))
										} else {
											Ok(None)
									...
									_ => {
										if !has_variable_before {
											Err(Error::new(
												parser.input.cur_span(),
												ErrorKind::Expected(
													"percentage, function (math functions) or \
								...
								_ => {
									if !has_variable_before {
										Err(Error::new(
											parser.input.cur_span(),
											ErrorKind::Expected(
												"percentage, function (math functions) or number \
												 token",
											),
										))
									} else {
										...
										_ => {
											if !has_variable_before {
												Err(Error::new(
													parser.input.cur_span(),
													ErrorKind::Expected(
														"percentage, functions (math functions) \
														 or ident (with 'none' value) token",
													),
												))
											} else {
										...
										_ => {
											if !has_variable_before {
												Err(Error::new(
													parser.input.cur_span(),
													ErrorKind::Expected(
														"number, functions (math functions) or \
														 ident (with 'none' value) token",
													),
												))
											} else {
										...
										_ => {
											if !has_variable_before {
												Err(Error::new(
													parser.input.cur_span(),
													ErrorKind::Expected(
														"number, function (math functions) or \
														 ident (with 'none' value) token",
													),
												))
											} else {
												...
												return Err(Error::new(
													...
													ErrorKind::Expected(
														"number, dimension, functions (math \
														 functions) or ident (with 'none' value) \
														 token",
													),
												));
											} else {
								...
								_ => {
									if !has_variable_before {
										Err(Error::new(
											parser.input.cur_span(),
											ErrorKind::Expected(
												"percentage, number, functions (math functions) \
												 or ident (with 'none' value) token",
											),
										))
									} else {
						...
						return Err(Error::new(
							...
							ErrorKind::Expected(
								"hash, ident (with named color, system color, 'transparent' or \
								 'currentColor' value) or function (with color function name) \
								 token",
							),
						));
					},
	...
	fn parse(&mut self) -> PResult<Percentage> {
		...
		match bump!(self) {
			Token::Percentage { value, raw } => {
				let value =
					Number { span:Span::new(span.lo, span.hi - BytePos(1)), value, raw:Some(raw) };
	...
	I: ParserInput,
{
	fn parse(&mut self) -> PResult<CalcProduct> {
		...
		let mut end = match calc_value {
			CalcValueOrOperator::Value(ref calc_value) => {
				match calc_value {
...
fn is_time_unit(unit:&JsWord) -> bool { matches_eq_ignore_ascii_case!(unit, "s", "ms") }

fn is_frequency_unit(unit:&JsWord) -> bool { matches_eq_ignore_ascii_case!(unit, "hz", "khz") }

fn is_resolution_unit(unit:&JsWord) -> bool {
	matches_eq_ignore_ascii_case!(unit, "dpi", "dpcm", "dppx", "x")
}

fn is_flex_unit(unit:&JsWord) -> bool { matches_eq_ignore_ascii_case!(unit, "fr") }
