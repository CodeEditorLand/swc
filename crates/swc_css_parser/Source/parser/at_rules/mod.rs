	...
	I: ParserInput,
{
	pub(super) fn parse_at_rule_prelude(&mut self, name:&str) -> PResult<Option<AtRulePrelude>> {
			...
			"layer" => {
				...
				if is!(self, Ident) {
					...
					let res = if name_list.len() == 1 {
						Some(AtRulePrelude::LayerPrelude(LayerPrelude::Name(name_list.remove(0))))
					} else {
		...
		if !is!(self, EOF) {
			...
			return Err(Error::new(span, ErrorKind::Unexpected("tokens in at-rule prelude")));
		}
	...
	fn parse(&mut self) -> PResult<KeyframeSelector> {
		match cur!(self) {
			...
			_ => {
				...
				return Err(Error::new(span, ErrorKind::Expected("'from', 'to' or percentage")));
			},
		}
	}
}
	...
	I: ParserInput,
{
	fn parse(&mut self) -> PResult<SupportsOr> {
		...
		let keyword = match cur!(self) {
			...
			_ => {
				return Err(Error::new(span, ErrorKind::Expected("ident (with 'or' value) token")));
			},
		};
	...
	fn parse(&mut self) -> PResult<SupportsFeature> {
		match cur!(self) {
			tok!("(") => {
				...
				let declaration = match self.try_to_parse_declaration_in_parens() {
					...
					None => {
						...
						return Err(Error::new(span, ErrorKind::Expected("declaration in parens")));
					},
				};
			...
			_ => {
				...
				Err(Error::new(span, ErrorKind::Expected("'(' or 'function' token")))
			},
		}
	}
}
	...
	I: ParserInput,
{
	fn parse(&mut self) -> PResult<DocumentPreludeMatchingFunction> {
		match cur!(self) {
			...
			Token::Function { value: function_name, .. } => {
	...
	I: ParserInput,
{
	fn parse(&mut self) -> PResult<PageSelector> {
		...
		let page_type = if is!(self, Ident) { Some(self.parse()?) } else { None };
