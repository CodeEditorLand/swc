	...
	I: ParserInput,
{
	pub fn new(mut input:I) -> Self {
		...
		Buffer { cur:None, peeked:None, input, last_pos }
	}

	pub fn atom<'a>(&self, s:impl Into<Cow<'a, str>>) -> Atom { self.input.atom(s.into()) }
	...
	pub fn bump(&mut self) -> Option<TokenAndSpan> {
		debug_assert!(self.cur.is_some(), "bump() is called without checking current token");
	...
	pub(super) fn skip_ws(&mut self) {
		...
		if let Some(TokenAndSpan { token: tok!(" "), span }) = &self.cur {
...

			{
				...
				match &self.cur {
					Some(TokenAndSpan { token: tok!(" "), span }) => {
						self.last_pos = span.hi;
	...
	pub(super) fn state(&mut self) -> WrappedState<I::State> {
		WrappedState { cur:self.cur.clone(), inner:self.input.state() }
	}
...
impl<'a> Input<'a> {
	pub fn new(input:InputType<'a>) -> Self {
		...
		Input { input, idx, balance_stack, atoms:Default::default() }
	}
					...
					Some(token_or_block) => {
						match token_or_block {
							...
							TokenOrBlock::Function(function) => {
								...
								TokenAndSpan {
									span:function.0,
									token:Token::Function { value:name.0, raw:name.1 },
								}
							},

							TokenOrBlock::LBracket(span) => {
								TokenAndSpan {
									span:Span::new(span.0, span.1),
									token:Token::LBracket,
								}
							},
							TokenOrBlock::LBrace(span) => {
								TokenAndSpan { span:Span::new(span.0, span.1), token:Token::LBrace }
							},
							TokenOrBlock::LParen(span) => {
								TokenAndSpan { span:Span::new(span.0, span.1), token:Token::LParen }
							},
							TokenOrBlock::RBracket(span) => {
								TokenAndSpan {
									span:Span::new(span.0, span.1),
									token:Token::RBracket,
								}
							},
							TokenOrBlock::RBrace(span) => {
								TokenAndSpan { span:Span::new(span.0, span.1), token:Token::RBrace }
							},
							TokenOrBlock::RParen(span) => {
								TokenAndSpan { span:Span::new(span.0, span.1), token:Token::RParen }
							},
						}
					},
					None => {
...
impl ParserInput for Input<'_> {
	...
	fn state(&mut self) -> Self::State {
		State { idx:self.idx.clone(), balance_stack:self.balance_stack.clone() }
	}
	...
	fn take_errors(&mut self) -> Vec<Error> { Vec::new() }

	fn skip_ws(&mut self) -> Option<BytePos> {
		let mut last_pos = None;

		while let Ok(TokenAndSpan { token: tok!(" "), span }) = self.cur().as_deref() {
	...
	fn atom(&self, s:Cow<str>) -> Atom { self.atoms.borrow_mut().atom(s) }
}
