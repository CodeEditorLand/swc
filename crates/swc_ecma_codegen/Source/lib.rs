...
/// Generate a code from a syntax node using default options.
pub fn to_code(node:&impl Node) -> String { to_code_with_comments(None, node) }
	...
	let mut matched_indexes = chars
		...
		.filter(|(index, byte)| {
			byte == &&b'<'
				...
				&& chars[index + 1..index + pattern_len]
	...
	S: SourceMapperExt,
{
	...
	pub fn emit_program(&mut self, node:&Program) -> Result {
		match *node {
			...
			// TODO: reenable once experimental_metadata breaking change is
			// merged _ => unreachable!(),
		}
	}
	...
	fn emit_str_lit(&mut self, node:&Str) -> Result {
		...
		let mut value =
			get_quoted_utf16(&node.value, self.cfg.ascii_only, target);
			...
			match &num.raw {
				Some(raw) => {
					if raw.len() > 2 && self.cfg.target < EsVersion::Es2015 && {
						...
						slice == b"0b"
							|| slice == b"0o" || slice == b"0B"
							|| slice == b"0O"
					} {
						if num.value.is_infinite() && num.raw.is_some() {
							self.wr.write_str_lit(
								DUMMY_SP,
								num.raw.as_ref().unwrap(),
							)?;
						} else {
		...
		match &*node.obj {
			...
			Expr::Lit(Lit::Num(num)) => {
				needs_2dots_for_property_access =
					self.emit_num_lit_internal(num, true)?;
			},
			_ => {
				emit!(node.obj);
			},
	...
	fn emit_template_for_tagged_template(&mut self, node:&Tpl) -> Result {
		...
		for i in 0..(node.quasis.len() + node.exprs.len()) {
			if i % 2 == 0 {
				self.emit_template_element_for_tagged_template(
					&node.quasis[i / 2],
				)?;
			} else {
				punct!(self, "${");
	...
	fn emit_object_lit(&mut self, node:&ObjectLit) -> Result {
		...
		let emit_new_line = !self.cfg.minify
			&& !(node.props.is_empty()
				&& is_empty_comments(&node.span(), &self.comments));

		if emit_new_line {
			self.wr.write_line()?;
		}

		let mut list_format = ListFormat::ObjectLiteralExpressionProperties
			| ListFormat::CanSkipTrailingComma;
	...
	fn emit_ident_like(
		...
		if self.cfg.ascii_only {
			if self.wr.can_ignore_invalid_unicodes() {
				self.wr.write_symbol(
					DUMMY_SP,
					&get_ascii_only_ident(sym, false, self.cfg.target),
				)?;
			} else {
				self.wr.write_symbol(
					DUMMY_SP,
					&get_ascii_only_ident(
						&handle_invalid_unicodes(sym),
						false,
						self.cfg.target,
					),
				)?;
	...
	fn emit_first_of_list5(
		...
		if children.is_none()
			&& format.contains(ListFormat::OptionalIfUndefined)
		{
			return Some(Ok(()));
		}

		let is_empty =
			children.is_none() || start > children.unwrap() || count == 0;
	...
	fn emit_pre_child_for_list5(
		...
		if let Some(previous_sibling) = previous_sibling {
			...
			//          a
			// /* End of parameter a */
			// -> this comment isn't considered to be trailing comment of
			// parameter "a" due to newline ,
	...
	fn emit_list_finisher_of_list5(
		...
		let has_trailing_comma = format
			.contains(ListFormat::ForceTrailingComma)
		...
		if has_trailing_comma
			...
			&& (!self.cfg.minify
				|| !format.contains(ListFormat::CanSkipTrailingComma))
		{
			punct!(self, ",");
			formatting_space!(self);
		}

		{
			...
			let emit_trailing_comments = {
				...
				// contains(EmitFlags::NoTrailingComments))
	...
	fn emit_list5<N:Node>(
		...
		if let Some(result) = self.emit_first_of_list5(
			parent_node,
			children.map(|v| v.len()),
			format,
			start,
			count,
		) {
		...
		let is_empty =
			children.is_none() || start > children.unwrap().len() || count == 0;
		...
		if is_empty {
			...
			let mut should_emit_intervening_comments =
			...
			for i in 0..count {
				...
				if should_emit_intervening_comments {
					...
					should_emit_intervening_comments =
						may_emit_intervening_comments;
				}
		...
		match arg {
			Expr::Call(c) => {
				let has_leading = match &c.callee {
					Callee::Super(callee) => {
						span_has_leading_comment(cmt, callee.span)
					},
			...
			Expr::Assign(e) => {
				...
				let has_leading = match &e.left {
					AssignTarget::Simple(e) => {
						self.simple_assign_target_has_leading_comment(e)
					},

					AssignTarget::Pat(p) => {
						match p {
							AssignTargetPat::Array(a) => {
								span_has_leading_comment(cmt, a.span)
							},
							AssignTargetPat::Object(o) => {
								span_has_leading_comment(cmt, o.span)
							},
...
fn should_emit_whitespace_before_operand(node:&UnaryExpr) -> bool {
	...
	match &*node.arg {
		...
		Expr::Lit(Lit::Num(v))
			if v.value.is_sign_negative() && node.op == op!(unary, "-") =>
		{
			true
		},
...
fn get_template_element_from_raw(s:&str, ascii_only:bool) -> String {
	...
	while let Some(c) = iter.next() {
				...
				match iter.next() {
					Some(c) => {
						match c {
							...
							'\u{2028}' | '\u{2029}' => None,
							// `\t` and `\h` are special cases, because they can
							// be replaced on real characters `\xXX` can
							// be replaced on character
							'\\'
							| 'r'
							| 'v'
							| 'b'
		...
		match unescape {
			...
			// Octal doesn't supported in template literals, except in tagged
			// templates, but we don't use this for tagged templates, they
			// are printing as is
		...
		match c {
			...
			_ => {
				if c.is_ascii() {
					...
					// unicode that is safe for the target Es5 does not
					// support code point escapes and so surrograte formula must
					// be used
					...
					Some('u') => {
						...
						if let Some(c @ 'D' | c @ 'd') = next {
							...
							if is_valid {
								...
								let v = u32::from_str_radix(val_str, 16)
									.unwrap_or_else(|err| {
										unreachable!(
											"failed to parse {} as a hex \
											 value: {:?}",
											val_str, err
										)
									});
...
fn require_space_before_rhs(rhs:&Expr, op:&BinaryOp) -> bool {
	match rhs {
		Expr::Lit(Lit::Num(v))
			if v.value.is_sign_negative() && *op == op!(bin, "-") =>
		{
			true
		},
...
fn is_empty_comments(span:&Span, comments:&Option<&dyn Comments>) -> bool {
	span.is_dummy()
		|| comments
			.map_or(true, |c| !c.has_leading(span.span_hi() - BytePos(1)))
}
