	...
	S: SourceMapperExt,
{
	...
	fn emit_num_lit(&mut self, num:&Number) -> Result { self.emit_num_lit_internal(num, false)?; }
	...
	#[emitter]
	fn emit_new_expr(&mut self, node:&NewExpr) -> Result { self.emit_new(node, true)?; }
	...
	fn emit_yield_expr(&mut self, node:&YieldExpr) -> Result {
		...
		if let Some(ref arg) = node.arg {
			let need_paren =
				node.arg.as_deref().map(|expr| self.has_leading_comment(expr)).unwrap_or(false);
	...
	S: SourceMapperExt,
{
	...
	fn emit_array_pat(&mut self, node:&ArrayPat) -> Result {
		...
		self.emit_list(node.span(), Some(&node.elems), ListFormat::ArrayBindingPatternElements)?;
	...
	S: SourceMapperExt,
{
	...
	fn emit_return_stmt(&mut self, n:&ReturnStmt) -> Result {
		...
		if let Some(arg) = n.arg.as_deref() {
			let need_paren =
				n.arg.as_deref().map(|expr| self.has_leading_comment(expr)).unwrap_or(false);
...
fn get_ascii_only_ident(sym:&str, may_need_quote:bool, target:EsVersion) -> Cow<str> {
	...
	if need_quote { Cow::Owned(format!("\"{}\"", buf)) } else { Cow::Owned(buf) }
}
...
fn clz(s:&str) -> usize { s.as_bytes().iter().take_while(|&&c| c == b'0').count() }
