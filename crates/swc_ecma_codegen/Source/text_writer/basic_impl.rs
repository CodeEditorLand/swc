...
impl<'a, W:Write> JsWriter<'a, W> {
	...
	pub fn set_indent_str(&mut self, indent_str:&'static str) { self.indent_str = indent_str; }
	...
	fn srcmap(&mut self, byte_pos:BytePos) {
		...
		if let Some(ref mut srcmap) = self.srcmap {
			if self.srcmap_done.insert((byte_pos, self.line_count as _, self.line_pos as _)) {
				let loc = LineCol { line:self.line_count as _, col:self.line_pos as _ };
...
impl<'a, W:Write> WriteJs for JsWriter<'a, W> {
	...
	fn care_about_srcmap(&self) -> bool { self.srcmap.is_some() }
	...
	fn commit_pending_semi(&mut self) -> Result { Ok(()) }
...
fn compute_line_starts(s:&str) -> LineStart {
	...
	LineStart { line_count:count, byte_pos:line_start }
}
