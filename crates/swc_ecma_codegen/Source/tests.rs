...
fn parse_then_emit(from:&str, cfg:Config, syntax:Syntax) -> String {
	::testing::run_test(false, |cm, handler| {
		...
		let res = {
			...
			let res = parser.parse_module().map_err(|e| e.into_diagnostic(handler).emit());
...
pub(crate) fn assert_min_target(from:&str, to:&str, target:EsVersion) {
	let out = parse_then_emit(
		from,
		Config { minify:true, omit_last_semi:true, target, ..Default::default() },
		Syntax::default(),
	);
...
pub(crate) fn assert_pretty(from:&str, to:&str) {
	let out = parse_then_emit(
		from,
		Config { minify:false, target:EsVersion::latest(), ..Default::default() },
		Syntax::default(),
	);
...
fn test_from_to(from:&str, expected:&str) {
	let out = parse_then_emit(
		from,
		Config { target:EsVersion::latest(), ..Default::default() },
		Syntax::default(),
	);
	...
	assert_eq!(DebugUsingDisplay(out.trim()), DebugUsingDisplay(expected.trim()),);
}

fn test_identical(from:&str) { test_from_to(from, from) }

fn test_from_to_custom_config(from:&str, to:&str, cfg:Config, syntax:Syntax) {
	let out = parse_then_emit(from, Config { omit_last_semi:true, ..cfg }, syntax);

	assert_eq!(DebugUsingDisplay(out.trim()), DebugUsingDisplay(to.trim()),);
}

#[test]
fn empty_stmt() { test_from_to(";", ";"); }
...
#[test]
fn comment_4() { test_from_to("/** foo */ a", "/** foo */ a;"); }
...
#[test]
fn empty_named_export() { test_from_to("export { }", "export { };"); }
...
fn empty_named_export_min() {
	test_from_to_custom_config(
		"export { }",
		"export{}",
		Config { minify:true, ..Default::default() },
		Default::default(),
	);
}

#[test]
fn empty_named_export_from() { test_from_to("export { } from 'foo';", "export { } from 'foo';"); }
...
#[test]
fn named_export_from() { test_from_to("export { bar } from 'foo';", "export { bar } from 'foo';"); }
...
fn named_and_namespace_export_from() {
	test_from_to_custom_config(
		...
		"export * as Foo, { bar } from 'foo';",
		Default::default(),
		Syntax::Es(EsSyntax { export_default_from:true, ..EsSyntax::default() }),
	);
}
...
#[test]
fn issue_639() { test_from_to(r"`\x1b[33m Yellow \x1b[0m`;", r"`\x1b[33m Yellow \x1b[0m`;"); }
...
#[test]
fn tpl_escape_1() {
	test_from_to("`${parent.path}\x00${request}`", "`${parent.path}\x00${request}`;")
}

#[test]
fn tpl_escape_2() { test_from_to("`${arg}\0`", "`${arg}\0`;"); }

#[test]
fn tpl_escape_3() {
	test_from_to(r"`${resolvedDevice.toLowerCase()}\\`", r"`${resolvedDevice.toLowerCase()}\\`;");
}
...
#[test]
fn issue_915_1() { test_identical(r"relResolveCacheIdentifier = `${parent.path}\x00${request}`;"); }

#[test]
fn issue_915_2() { test_identical(r"relResolveCacheIdentifier = `${parent.path}\x00${request}`;"); }

#[test]
fn issue_915_3() { test_identical(r#"encoder.encode("\\r\\n");"#); }

#[test]
fn issue_915_4() { test_identical(r"`\\r\\n--${this.boundary}`;"); }
...
#[test]
fn deno_8162() { test_from_to(r#""\x00\r\n\x85\u2028\u2029";"#, r#""\x00\r\n\x85\u2028\u2029";"#); }
...
#[test]
fn deno_8925() { assert_pretty("const 𝒫 = 2;", "const 𝒫 = 2;"); }
...
fn test_get_quoted_utf16() {
	...
	fn es2020(src:&str, expected:&str) {
		assert_eq!(super::get_quoted_utf16(src, true, EsVersion::Es2020), expected)
	}
...
#[test]
fn issue_1452_1() { assert_min("async foo => 0", "async foo=>0"); }

#[test]
fn issue_1619_1() {
	assert_min_target("\"\\x00\" + \"\\x31\"", "\"\\0\"+\"1\"", EsVersion::latest());
}
...
#[test]
fn issue_1619_3() {
	assert_eq!(get_quoted_utf16("\x00\x31", true, EsVersion::Es3), "\"\\x001\"");
}
...
#[test]
fn invalid_unicode_in_ident() { check_latest("\\ud83d;", "\\ud83d;"); }
...
#[test]
fn issue_2213() { assert_min("a - -b * c", "a- -b*c") }
...
fn issue_3617() {
	...
	let out = parse_then_emit(
		from,
		Config { ascii_only:true, target:EsVersion::Es5, ..Default::default() },
		Syntax::default(),
	);
...
module.exports = '\u0009\u000A\u000B\u000C\u000D\u0020\u00A0\u1680\u2000\u2001\u2002' + '\u2003\u2004\u2005\u2006\u2007\u2008\u2009\u200A\u202F\u205F\u3000\u2028\u2029\uFEFF' + '\u{a0}';";
	...
	let out = parse_then_emit(
		from,
		Config { target:EsVersion::Es2022, ..Default::default() },
		Syntax::default(),
	);
...
fn test_all(src:&str, expected:&str, expected_minified:&str, config:Config) {
	{
		let out = parse_then_emit(
			src,
			Config { minify:false, omit_last_semi:true, ..config },
			Syntax::default(),
		);
	...
	}
	{
		...
		let out = parse_then_emit(
			src,
			Config { minify:true, omit_last_semi:true, ..config },
			Syntax::default(),
		);
		...
		assert_eq!(DebugUsingDisplay(out.trim()), DebugUsingDisplay(expected_minified.trim()),);
	}
}
...
fn ascii_only_str_1() {
	test_all(
		...
		"'😊❤️';\n",
		r#""😊❤️""#,
		Config { ascii_only:false, target:EsVersion::Es2015, ..Default::default() },
	);
}
...
fn ascii_only_str_2() {
	test_all(
		...
		r#""\u{1F60A}\u2764\uFE0F";"#,
		r#""\u{1F60A}\u2764\uFE0F""#,
		Config { ascii_only:true, ..Default::default() },
	);
}
...
fn emit_type_import_statement_named() {
	...
	let out = parse_then_emit(from, Default::default(), Syntax::Typescript(Default::default()));
...
fn issue_8491_2() {
    ...
    "a\xf3b": "\xf3"
		...
		r#"console.log({"a\xf3b":"\xf3"})"#,
		Config { ascii_only:true, target:EsVersion::Es2015, ..Default::default() },
	);
}
...
fn test_str_lit(input:PathBuf) { test_str_lit_inner(input) }

/// Print text back using `console.log`
fn run_node(code:&str) -> String {
	exec_node_js(code, JsExecOptions { cache:true, module:false, args:Default::default() })
...
fn test_str_lit_inner(input:PathBuf) {
	...
	let output_code = parse_then_emit(
		&input_code,
		Config::default().with_target(EsVersion::latest()).with_minify(true),
		Syntax::default(),
	);
