...
impl Builder {
	pub fn with<'a, F, Ret>(
		self,
		_:&str,
		s:&'a mut std::vec::Vec<u8>,
		op:F,
	) -> Ret
	where
		F: for<'aa> FnOnce(
			&mut Emitter<'aa, Box<(dyn WriteJs + 'aa)>, SourceMap>,
		) -> Ret,
		Ret: 'static, {
...
fn parse_then_emit(from:&str, cfg:Config, syntax:Syntax) -> String {
	::testing::run_test(false, |cm, handler| {
		let src = cm.new_source_file(
			FileName::Real("custom.js".into()).into(),
			from.to_string(),
		);
		println!(
			"--------------------\nSource: \n{}\nPos: {:?} ~ {:?}\n",
		...
		let res = {
			let mut parser =
				Parser::new(syntax, StringInput::from(&*src), Some(&comments));
			let res = parser
				.parse_module()
		...
		let out = Builder { cfg, cm, comments }
...
fn test_from_to_custom_config(from:&str, to:&str, cfg:Config, syntax:Syntax) {
	let out =
		parse_then_emit(from, Config { omit_last_semi:true, ..cfg }, syntax);
...
fn issue_546() {
	test_from_to(
		"import availabilities, * as availabilityFunctions from \
		 'reducers/availabilities';",
		"import availabilities, * as availabilityFunctions from \
		 'reducers/availabilities';",
	);
}
...
fn integration_01() {
    ...
    `Unexpected ${unexpectedKeys.length > 1 ? 'keys' : 'key'} ` + \
		 `\"${unexpectedKeys.join('\", \"')}\" found in ${argumentName}. ` + \
		 `Expected to find one of the known reducer keys instead: ` + \
		 `\"${reducerKeys.join('\", \"')}\". Unexpected keys will be \
		 ignored.`;
        ",
	);
}
...
fn issue_3617() {
    ...
    module.exports = '\u0009\u000A\u000B\u000C\u000D\u0020\u00A0\u1680\u2000\u2001\u2002' +
      '\u2003\u2004\u2005\u2006\u2007\u2008\u2009\u200A\u202F\u205F\u3000\u2028\u2029\uFEFF' + '\u{a0}';";
	let expected = "// a string of all valid unicode \
	                whitespaces\nmodule.exports = \
	                '\\u0009\\u000A\\u000B\\u000C\\u000D\\u0020\\u00A0\\u1680\\
	                \u2000\\u2001\\u2002' + \
	                '\\u2003\\u2004\\u2005\\u2006\\u2007\\u2008\\u2009\\u200A\\
...
fn run_node(code:&str) -> String {
	exec_node_js(
		code,
		JsExecOptions { cache:true, module:false, args:Default::default() },
	)
