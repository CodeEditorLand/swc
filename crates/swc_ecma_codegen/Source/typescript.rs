	...
	W: WriteJs,
{
	...
	fn emit_ts_intersection_type(&mut self, n:&TsIntersectionType) -> Result {
		...
		self.emit_list(n.span, Some(&n.types), ListFormat::IntersectionTypeConstituents)?;
	}
	...
	fn emit_ts_type_lit(&mut self, n:&TsTypeLit) -> Result {
		...
		punct!("{");
		self.emit_list(n.span, Some(&n.members), ListFormat::MultiLineTypeLiteralMembers)?;
		punct!("}");
	}
...
mod tests {
	...
	#[test]
	fn qualified_type() {
		assert_min_typescript("var memory: WebAssembly.Memory;", "var memory:WebAssembly.Memory");
	}

	#[test]
	fn type_arg() { assert_min_typescript("do_stuff<T>()", "do_stuff<T>()"); }

	#[test]
	fn no_type_arg() { assert_min_typescript("do_stuff()", "do_stuff()"); }
}
