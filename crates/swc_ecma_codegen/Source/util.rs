...
impl EndsWithAlphaNum for Expr {
	fn ends_with_alpha_num(&self) -> bool {
		match self {
			...
			| Expr::Lit(Lit::Str(..))
			| Expr::Paren(..)
			| Expr::Member(MemberExpr { prop: MemberProp::Computed(..), .. })
			| Expr::Tpl(..)
			| Expr::TaggedTpl(..)
			| Expr::Call(..) => false,
			...
			Expr::New(NewExpr { args: Some(args), .. }) => args.is_empty(),
...
impl StartsWithAlphaNum for ExprOrSpread {
	fn starts_with_alpha_num(&self) -> bool {
		match *self {
			ExprOrSpread { spread: Some(_), .. } => false,
			ExprOrSpread { spread: None, ref expr } => expr.starts_with_alpha_num(),
		}
	}
}
