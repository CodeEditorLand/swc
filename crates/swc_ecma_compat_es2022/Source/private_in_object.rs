...
pub fn private_in_object() -> impl Pass { visit_mut_pass(PrivateInObject::default()) }
...
impl Default for Mode {
	fn default() -> Self { Self::ClassDecl { vars:Default::default() } }
}
		...
		match self {
			Mode::ClassExpr { vars, init_exprs } => {
				...
				if let Some(init) = init {
					init_exprs.push(
						AssignExpr { span:DUMMY_SP, op:op!("="), left:n.into(), right:init }.into(),
					);
				}
			},
...
impl CompilerPass for PrivateInObject {
	fn name(&self) -> Cow<'static, str> { Cow::Borrowed("private-in-object") }
}
	...
	fn visit_mut_class(&mut self, n:&mut Class) {
		...
		if !self.cls.constructor_exprs.is_empty() {
			...
			if !has_constructor {
				...
				n.body.push(ClassMember::Constructor(default_constructor_with_span(
					has_super, n.span,
				)));
			}
