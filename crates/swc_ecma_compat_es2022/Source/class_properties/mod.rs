...
impl VisitMut for ClassProperties {
	...
	fn visit_mut_block_stmt_or_expr(&mut self, body:&mut BlockStmtOrExpr) {
		match body {
			BlockStmtOrExpr::Expr(expr) if expr.is_class() => {
				...
				stmts.push(ReturnStmt { span:DUMMY_SP, arg:Some(ident.into()) }.into());
	...
	fn visit_mut_expr(&mut self, expr:&mut Expr) {
		if let Expr::Class(ClassExpr { ident: orig_ident, class }) = expr {
			...
			let class = ClassExpr { ident:orig_ident.clone(), class:decl.class }.into();
		...
		for stmt in stmts.drain(..) {
			match T::try_into_stmt(stmt) {
						...
						Ok(mut decl) => {
							match decl {
								...
								ModuleDecl::ExportDecl(ExportDecl {
									span,
									decl: Decl::Class(ClassDecl { ident, declare: false, class }),
									..
								}) => {
									...
									extra.merge_with(
										&mut buf,
										match T::try_from_module_decl(
											ExportDecl { span, decl:decl.into() }.into(),
										) {
											Ok(t) => t,
											Err(..) => unreachable!(),
										},
				...
				Ok(mut stmt) => {
					// Fold class
					match stmt {
						Stmt::Decl(Decl::Class(ClassDecl { ident, class, declare: false })) => {
							let (decl, extra) = self.visit_mut_class_as_decl(ident, class);
...
impl ClassProperties {
	fn visit_mut_class_as_decl(
		...
		class.body.visit_mut_with(&mut BrandCheckHandler { private:&self.private });

		let should_create_vars_for_method_names = class.body.iter().any(|m| {
			match m {
			...
			match member {
				...
				ClassMember::Method(method) => {
					...
					let key = match method.key {
						PropName::Computed(ComputedPropName { span: c_span, mut expr })
							if should_create_vars_for_method_names && !is_literal(&*expr) =>
						{
							...
							// string.
							PropName::Computed(ComputedPropName { span:c_span, expr:ident.into() })
						},
				...
				ClassMember::ClassProp(mut prop) => {
					...
					prop.key.visit_mut_with(&mut ClassNameTdzFolder { class_name:&class_ident });

					if !prop.is_static {
						prop.key
							.visit_with(&mut UsedNameCollector { used_names:&mut used_key_names });

						prop.value
							.visit_with(&mut UsedNameCollector { used_names:&mut used_names });
					}
					...
					if prop.is_static {
						...
						value.visit_mut_with(&mut ThisInStaticFolder { ident:class_ident.clone() });
					}

					let init =
						MemberInit::PubProp(PubProp { span:prop_span, name:prop.key, value });
				...
				ClassMember::PrivateProp(mut prop) => {
					...
					prop.value.visit_with(&mut UsedNameCollector { used_names:&mut used_names });

					if prop.is_static {
						prop.value
							.visit_mut_with(&mut ThisInStaticFolder { ident:class_ident.clone() });
					}
				...
				ClassMember::PrivateMethod(mut method) => {
						...
						(MethodKind::Getter | MethodKind::Setter, false) => {
							...
							let inserted =
								constructor_inits.push(MemberInit::PrivAccessor(PrivAccessor {
									...
									getter:if is_getter { Some(fn_name.clone()) } else { None },
									setter:if !is_getter { Some(fn_name.clone()) } else { None },
								}));

							if inserted { Some(quote_ident!("WeakMap")) } else { None }
						},
					...
					private_method_fn_decls.push(
						FnDecl { ident:fn_name, function:method.function, declare:false }.into(),
					)
				},
		...
		(
			ClassDecl {
				ident:class_ident,
				declare:false,
				class:Class { body:members, ..*class }.into(),
			},
			ClassExtra { vars, lets, stmts:extra_stmts },
		)
	}
...
impl Visit for ShouldWork {
	...
	fn visit_class_method(&mut self, _:&ClassMethod) { self.found = true; }

	fn visit_class_prop(&mut self, _:&ClassProp) { self.found = true; }

	fn visit_private_prop(&mut self, _:&PrivateProp) { self.found = true; }

	fn visit_private_method(&mut self, _:&PrivateMethod) { self.found = true; }

	fn visit_constructor(&mut self, _:&Constructor) { self.found = true; }
}
...
impl Visit for SuperVisitor {
	...
	fn visit_super(&mut self, _:&Super) { self.found = true; }
}
