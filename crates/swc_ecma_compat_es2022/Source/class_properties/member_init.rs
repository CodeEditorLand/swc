...
impl MemberInitRecord {
	pub fn new(c:Config) -> Self { Self { c, record:Vec::new() } }
	...
	pub fn into_init(self) -> Vec<Box<Expr>> {
		...
		for init in self.record {
			match init {
				MemberInit::PrivMethod(PrivMethod { span, name, fn_name }) => {
					...
					normal_init.push(CallExpr { span, callee, args, ..Default::default() }.into())
				},
				...
				MemberInit::PrivAccessor(PrivAccessor { span, name, getter, setter }) => {
					normal_init.push(
						CallExpr {
							span,
							callee:if self.c.private_as_properties {
...
fn obj_def_prop() -> Callee {
	quote_ident!("Object").make_member(quote_ident!("defineProperty")).as_callee()
}
