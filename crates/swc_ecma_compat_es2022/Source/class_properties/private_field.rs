...
#[swc_trace]
impl PrivateRecord {
	pub fn new() -> Self { PrivateRecord(Vec::new()) }

	pub fn curr_class(&self) -> &Ident { &self.0.last().unwrap().class_name }

	pub fn cur_mark(&self) -> Mark { self.0.last().unwrap().mark }

	pub fn push(&mut self, p:Private) { self.0.push(p) }
...
impl PrivateKind {
	fn is_readonly(&self) -> bool { self.is_method && !self.has_setter }
	...
	fn is_method(&self) -> bool { self.is_method && !self.has_getter && !self.has_setter }
}
	...
	fn visit_mut_expr(&mut self, e:&mut Expr) {
		if let Expr::OptChain(opt) = e {
			let is_private_access = match &*opt.base {
				OptChainBase::Member(MemberExpr { prop: MemberProp::PrivateName(..), .. }) => true,
				OptChainBase::Call(OptCall { callee, .. }) => {
					matches!(
						&**callee,
						Expr::Member(MemberExpr { prop:MemberProp::PrivateName(..), .. })
					)
				},
				_ => false,
			};
		...
		if self.c.private_as_properties {
			if let Expr::Member(MemberExpr { span, obj, prop: MemberProp::PrivateName(n) }) = e {
				obj.visit_mut_children_with(self);
				let (mark, ..) = self.private.get(n.span, &n.name);
				let ident = Ident::new(
					format!("_{}", n.name).into(),
					n.span,
					SyntaxContext::empty().apply_mark(mark),
		...
		match e {
			...
			Expr::Assign(AssignExpr { span, left, op, right })
				if left.as_simple().is_some() && left.as_simple().unwrap().is_member() =>
			{
				...
				let n = match &left.prop {
					...
					_ => {
						*e =
							AssignExpr { span:*span, left:left.into(), op:*op, right:right.take() }
								.into();
				...
				if kind.is_static {
					*e = CallExpr {
					...
					.into();
					*e = SeqExpr { span:*span, exprs:vec![this, value, err] }.into();
				} else {
			...
			Expr::Assign(AssignExpr { left: AssignTarget::Pat(left), right, .. }) => {
				right.visit_mut_with(self);
				let old_access_type = self.private_access_type;
		...
		if kind.is_static {
			...
			if kind.is_method() {
				...
				return (
					CallExpr {
						span:DUMMY_SP,
						callee:h,
						args:vec![obj.as_arg(), class_name.clone().as_arg(), method_name.as_arg()],
						..Default::default()
					}
					.into(),
			...
			match self.private_access_type {
				...
				PrivateAccessType::Get if kind.is_writeonly() => {
					...
					(SeqExpr { span:DUMMY_SP, exprs:vec![obj.clone(), expr] }.into(), Some(*obj))
				},
				...
				PrivateAccessType::Get => {
					...
					match &*obj {
						...
						_ => {
							...
							(
								CallExpr { span:DUMMY_SP, callee:get, args, ..Default::default() }
									.into(),
								Some(var.into()),
							)
						},
