...
impl<T> BoolOrDataConfig<T> {
	pub fn from_bool(v:bool) -> Self { Self(Some(BoolOr::Bool(v))) }

	pub fn from_obj(v:T) -> Self { v.into() }
	...
	pub fn is_true(&self) -> bool { matches!(self.0, Some(BoolOr::Bool(true))) }

	pub fn is_false(&self) -> bool { matches!(self.0, Some(BoolOr::Bool(false))) }

	pub fn is_obj(&self) -> bool { matches!(self.0, Some(BoolOr::Data(_))) }

	pub fn into_inner(self) -> Option<BoolOr<T>> { self.0 }
...
impl<T> From<T> for BoolOrDataConfig<T> {
	fn from(v:T) -> Self { Self(Some(BoolOr::Data(v))) }
}
