...
use swc_common::{FileName, collections::AHashMap};
...
impl Config {
	pub fn codegen_target(&self) -> Option<EsVersion> {
		self.options.as_ref().and_then(|options| options.codegen_target())
	}
}
	...
	fn from(c:EntryConfig) -> Self {
		...
		match c {
			EntryConfig::File(f) => {
				...
				let file_name =
					path.file_name().expect("entry must be a file, instead of a directory");
...
impl<T, Ret> fmt::Debug for JsCallback<T, Ret> {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result { write!(f, "<js function>") }
}
