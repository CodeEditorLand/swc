...
impl SwcLoader {
	...
	fn env_map(&self) -> Lrc<AHashMap<JsWord, Expr>> {
		...
		let envs_map:AHashMap<_, _> = match envs {
			...
			GlobalInliningPassEnvs::List(envs) => {
				envs.into_iter()
					.map(|name| {
		...
		for (k, v) in envs_map {
			m.insert(k, Lit::Str(Str { span:DUMMY_SP, raw:None, value:v }).into());
		}
	...
	fn load_with_handler(&self, handler:&Handler, name:&FileName) -> Result<ModuleData, Error> {
		...
		if let FileName::Custom(id) = name {
			...
			if id.starts_with("node:") {
				let fm = self.compiler.cm.new_source_file(name.clone().into(), "".to_string());
			...
			// set to `false`
				...
				.unwrap();
				return Ok(ModuleData { fm, module, helpers:Default::default() });
			}
		}
		...
		match program {
			Program::Module(module) => Ok(ModuleData { fm, module, helpers }),
			_ => unreachable!(),
		}
	}
}
