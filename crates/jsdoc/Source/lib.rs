...
pub fn parse_tag_item(i:Input) -> IResult<Input, TagItem> {
	...
	let tag = match &*tag_name.value {
		...
		"memberof" | "memberof!" => {
			...
			i = input;
			Tag::MemberOf(MemberOfTag { span, parent_name_path })
		},
		...
		"param" | "arg" | "argument" => {
			...
			i = input;
			Tag::Parameter(ParameterTag { span, ty, name, desc })
		},
		...
		"property" | "prop" => {
			...
			let (input, desc) = parse_line(input)?;
			i = input;
			Tag::Property(PropertyTag { span, ty, name_path, desc })
		},
		...
		"returns" | "return" => {
			...
			i = input;
			Tag::Return(ReturnTag { span, ty, description })
		},
		...
		"typedef" => {
			...
			let (input, name_path) = parse_name_path(input)?;
			i = input;
			Tag::TypeDef(TypeDefTag { span, ty, name_path })
		},
		...
		"yields" | "yield" => {
			...
			let (input, description) = parse_line(input)?;
			i = input;
			Tag::Yield(YieldTag { span, value, description })
		},
