...
impl<'a> From<&'a Comment> for Input<'a> {
	fn from(c:&'a Comment) -> Self { Self::new(c.span.lo, c.span.hi, &c.text) }
}
...
impl InputLength for Input<'_> {
	fn input_len(&self) -> usize { self.src.as_bytes().len() }
}
...
impl<'a> InputIter for Input<'a> {
	...
	fn iter_indices(&self) -> Self::Iter { self.src.iter_indices() }

	fn iter_elements(&self) -> Self::IterElem { self.src.iter_elements() }
...
impl Deref for Input<'_> {
	...
	fn deref(&self) -> &Self::Target { self.src }
}
