...
impl VisitMut for Umd {
	...
	fn visit_mut_module(&mut self, module:&mut Module) {
		...
		stmts.extend(module_items.take().into_iter().filter_map(|i| {
			match i {
		...
		*module_items = vec![
			adapter_fn_expr
				.as_call(
					DUMMY_SP,
					vec![ThisExpr { span:DUMMY_SP }.as_arg(), factory_fn_expr.as_arg()],
				)
				.into_stmt()
				.into(),
		]
...
impl Umd {
	fn handle_import_export(
		...
		link.into_iter().for_each(
			|(src, LinkItem(src_span, link_specifier_set, mut link_flag))| {
				...
				if need_new_var {
					let stmt:Stmt =
						import_expr.into_var_decl(self.const_var_kind, new_var_ident.into()).into();
	...
	fn adapter(&mut self, module_span:Span, is_export_assign:bool) -> (FnExpr, Vec<Param>) {
		...
		let module =
			quote_ident!(SyntaxContext::empty().apply_mark(self.unresolved_mark), "module");

		let require =
			quote_ident!(SyntaxContext::empty().apply_mark(self.unresolved_mark), "require");
		let define =
			quote_ident!(SyntaxContext::empty().apply_mark(self.unresolved_mark), "define");
		let global_this =
			quote_ident!(SyntaxContext::empty().apply_mark(self.unresolved_mark), "globalThis");
