...
pub fn import_analyzer(import_interop:ImportInterop, ignore_dynamic:bool) -> impl Fold + VisitMut {
...
impl VisitMut for ImportAnalyzer {
	...
	fn visit_mut_module(&mut self, module:&mut Module) { self.visit_module(&*module); }
}
...
impl Visit for ImportAnalyzer {
	...
	fn visit_module_items(&mut self, n:&[ModuleItem]) {
		...
		if self.import_interop.is_swc()
			&& flag_record.values().any(|flag| flag.interop() && !flag.has_named())
		{
	...
	fn visit_import(&mut self, _:&Import) { self.dynamic_import_found = true; }
}
