...
impl VisitMut for ModuleDeclStrip {
	...
	fn visit_mut_named_export(&mut self, n:&mut NamedExport) {
		...
		if let Some(src) = src {
			...
			self.export.extend(specifiers.into_iter().map(|e| {
				match e {
					...
					ExportSpecifier::Named(ExportNamedSpecifier { orig, exported, .. }) => {
						...
						if let Some(exported) = exported {
							let (export_name, export_name_span) = match exported {
								ModuleExportName::Ident(Ident { ctxt, span, sym, .. }) => {
									(sym, (span, ctxt))
								},
	...
	fn visit_mut_export_default_decl(&mut self, n:&mut ExportDefaultDecl) {
		match &mut n.decl {
			...
			DefaultDecl::Fn(fn_expr) => {
				let ident =
					fn_expr.ident.get_or_insert_with(|| private_ident!(n.span, "_default")).clone();
	...
	fn visit_mut_export_default_expr(&mut self, n:&mut ExportDefaultExpr) {
		...
		self.export_default =
			Some(n.expr.take().into_var_decl(self.const_var_kind, ident.into()).into());
	}
...
pub enum LinkSpecifier {
	...
	ExportNamed { orig:(JsWord, SpanCtx), exported:Option<(JsWord, SpanCtx)> },
	...
	fn from(e:ExportSpecifier) -> Self {
		match e {
			...
			ExportSpecifier::Named(ExportNamedSpecifier {
				...
				let exported = exported.map(|exported| {
					match exported {
						ModuleExportName::Ident(Ident { span, sym, .. })
						| ModuleExportName::Str(Str { span, value: sym, .. }) => {
							(sym, (span, SyntaxContext::empty().apply_mark(Mark::new())))
...
impl LinkFlag {
	pub fn interop(&self) -> bool { self.intersects(Self::DEFAULT) }

	pub fn has_named(&self) -> bool { self.intersects(Self::NAMED) }

	pub fn namespace(&self) -> bool { self.contains(Self::NAMESPACE) }

	pub fn need_raw_import(&self) -> bool { self.interop() && self.intersects(Self::IMPORT_EQUAL) }

	pub fn export_star(&self) -> bool { self.intersects(Self::EXPORT_STAR) }
}
			...
			match s {
				LinkSpecifier::ImportNamed { imported, local } => {
					*ref_to_mod_ident = true;
					...
					import_map
						.insert(local.clone(), (mod_ident.clone(), imported.or(Some(local.0))));
				},
				...
				LinkSpecifier::ImportDefault(id) => {
					*ref_to_mod_ident = true;
					...
					import_map.insert(
						id,
						(mod_ident.clone(), (!default_nowrap).then(|| "default".into())),
					);
				},
				...
				LinkSpecifier::ExportDefaultAs(_, key, span) => {
					*ref_to_mod_ident = true;
					...
					// foo -> mod.default
					import_map
						.insert((key.clone(), span.1), (mod_ident.clone(), Some("default".into())));
				...
				LinkSpecifier::ImportEqual(id) => {
					*ref_to_mod_ident = true;
					...
					import_map.insert(
						id,
						(raw_mod_ident.clone().unwrap_or_else(|| mod_ident.clone()), None),
					);
				},
...
impl ExportItem {
	...
	pub fn export_name_span(&self) -> SpanCtx { self.0 }

	pub fn into_local_ident(self) -> Ident { self.1 }
}
