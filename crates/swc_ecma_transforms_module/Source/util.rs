...
impl Config {
	pub fn default_js_ext() -> String { "js".to_string() }
}
...
const fn default_strict_mode() -> bool { true }
...
impl LazyObjectConfig {
	pub fn is_lazy(&self, src:&JsWord) -> bool { self.patterns.iter().any(|pat| pat.is_match(src)) }
}
...
impl Default for Lazy {
	fn default() -> Self { Lazy::Bool(false) }
}
...
impl VecStmtLike for [ModuleItem] {
	...
	fn as_ref(&self) -> &[Self::StmtLike] { self }
}
...
pub(super) fn use_strict() -> Stmt { Lit::Str(quote_str!("use strict")).into_stmt() }
