...
impl Resolver {
	...
	pub(crate) fn make_require_call(
		...
		CallExpr {
			span:DUMMY_SP,
			callee:quote_ident!(SyntaxContext::empty().apply_mark(unresolved_mark), "require")
				.as_callee(),
	...
	R: Resolve,
{
	...
	pub fn new(resolver:R) -> Self { Self::with_config(resolver, Default::default()) }

	#[deprecated(note = "Use `with_config`")]
	pub fn with_base_dir(resolver:R, base_dir:Option<PathBuf>) -> Self {
		Self::with_config(resolver, Config { base_dir, ..Default::default() })
	}
	...
	R: Resolve,
{
	fn to_specifier(&self, mut target_path:PathBuf, orig_filename:Option<&str>) -> JsWord {
		...
		if let Some(orig_filename) = orig_filename {
			let is_resolved_as_index =
				if let Some(stem) = target_path.file_stem() { stem == "index" } else { false };

			let is_resolved_as_non_js =
				if let Some(ext) = target_path.extension() { ext != "js" } else { false };

			let is_resolved_as_js =
				if let Some(ext) = target_path.extension() { ext == "js" } else { false };
	...
	fn try_resolve_import(&self, base:&FileName, module_specifier:&str) -> Result<JsWord, Error> {
		...
		let mut base = match base {
			FileName::Real(v) => {
				Cow::Borrowed(v.parent().ok_or_else(|| anyhow!("failed to get parent of {:?}", v))?)
			},
			FileName::Anon => {
