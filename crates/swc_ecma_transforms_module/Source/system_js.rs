...
use swc_common::{DUMMY_SP, Mark, Span, SyntaxContext, collections::AHashMap};
use swc_ecma_ast::*;
use swc_ecma_utils::{
	ExprFactory,
	member_expr,
...
impl SystemJs {
	...
	fn replace_assign_expr(&mut self, assign_expr:AssignExpr) -> Expr {
		match &assign_expr.left {
			AssignTarget::Simple(pat_or_expr) => {
				match pat_or_expr {
					...
					body:Box::new(Stmt::Block(BlockStmt {
						...
						stmts:vec![Stmt::If(IfStmt {
							...
							test:Box::new(Expr::Bin(BinExpr {
								...
								op:op!("&&"),
								left:Box::new(
									key_ident.clone().make_bin(op!("!=="), quote_str!("default")),
								),
								right:Box::new(
									key_ident
										.clone()
				...
				if let Some(init) = for_stmt.init {
					if let VarDeclOrExpr::VarDecl(var_decl) = init {
						if var_decl.kind == VarDeclKind::Var {
							...
							ForStmt { init:Some(VarDeclOrExpr::VarDecl(var_decl)), ..for_stmt }
								.into()
						}
					} else {
	...
	fn fold_expr(&mut self, expr:Expr) -> Expr {
		...
		match expr {
				...
				let assign_expr = AssignExpr {
					right:match *assign.right {
						...
						Expr::Assign(AssignExpr { op: AssignOp::Assign, .. }) => {
							return self
								.replace_assign_expr(AssignExpr { right:assign.right, ..assign });
						},
			...
			Expr::MetaProp(meta_prop_expr) => {
				match meta_prop_expr.kind {
					MetaPropKind::ImportMeta => {
						self.context_ident.clone().make_member(quote_ident!("meta")).into()
					},
											...
											AssignExpr {
												...
												right:MemberExpr {
													...
													prop:match specifier.imported {
														Some(m) => {
															get_module_export_member_prop(&m)
														},
														None => {
															MemberProp::Ident(
																specifier.local.into(),
															)
														},
													},
												}
										...
										match specifier {
											ExportSpecifier::Named(specifier) => {
												export_names.push(match &specifier.exported {
													...
													None => {
														get_module_export_name(&specifier.orig).0
													},
												});
											...
											ExportSpecifier::Namespace(specifier) => {
												export_names.push(
													get_module_export_name(&specifier.name).0,
												);
									...
									for specifier in decl.specifiers {
										if let ExportSpecifier::Named(specifier) = specifier {
												...
												.any(|i| i.sym == id.0)
											{
												...
												self.export_values.push(Box::new(
