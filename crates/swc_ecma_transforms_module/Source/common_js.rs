...
impl VisitMut for Cjs {
	...
	fn visit_mut_module(&mut self, n:&mut Module) {
		...
		// Collect directives
		stmts.extend(
			&mut n.body.iter_mut().take_while(|i| i.directive_continue()).map(|i| i.take()),
		);
		...
		stmts.extend(n.body.take().into_iter().filter(|item| {
			match item {
...
impl Cjs {
	fn handle_import_export(
		...
		let lexer_reexport =
			if export_interop_annotation { self.emit_lexer_reexport(&link) } else { None };
		...
		link.into_iter().for_each(
			|(src, LinkItem(src_span, link_specifier_set, mut link_flag))| {
				...
				// require("mod");
				let import_expr =
					self.resolver.make_require_call(self.unresolved_mark, src, src_span.0);

				// _export_star(require("mod"), exports);
				let import_expr = if link_flag.export_star() {
					helper_expr!(export_star)
						.as_call(DUMMY_SP, vec![import_expr.as_arg(), self.exports().as_arg()])
				} else {
					import_expr
				};
				...
				let import_expr = {
					match import_interop {
						ImportInterop::Swc if link_flag.interop() => {
							if link_flag.namespace() {
	...
	fn handle_ts_import_equals(
		...
		match module_decl {
				...
				) =>
			{
				...
				let Str { span: src_span, value: src, .. } =
	...
	fn emit_lexer_exports_init(&mut self, export_id_list:&[ExportKV]) -> Option<Stmt> {
		match export_id_list.len() {
			...
			1 => {
				...
				let expr =
					BinExpr { span:DUMMY_SP, op:op!("&&"), left:0.into(), right:Box::new(expr) };
	...
	fn emit_lexer_reexport(&self, link:&Link) -> Option<Stmt> {
		link.iter()
			...
			.map(|(src, ..)| {
				let import_expr =
					self.resolver.make_require_call(self.unresolved_mark, src.clone(), DUMMY_SP);
			...
			.map(|expr| {
				BinExpr { span:DUMMY_SP, op:op!("&&"), left:0.into(), right:expr.into() }
					.into_stmt()
			})
	}
}
...
fn cjs_import_meta_url(span:Span, require:Ident, unresolved_mark:Mark) -> Expr {
	require
		...
		.as_call(
			DUMMY_SP,
			vec![
				quote_ident!(SyntaxContext::empty().apply_mark(unresolved_mark), "__filename")
					.as_arg(),
			],
		)
