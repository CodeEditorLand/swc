...
impl Expr {
	...
	pub fn undefined(span:Span) -> Box<Expr> {
		UnaryExpr {
			span,
			op:op!("void"),
			arg:Lit::Num(Number { span, value:0.0, raw:None }).into(),
		}
		.into()
	}
	...
	pub fn unwrap_parens(&self) -> &Expr {
		self.unwrap_with(|e| if let Expr::Paren(expr) = e { Some(&expr.expr) } else { None })
	}
	...
	pub fn unwrap_parens_mut(&mut self) -> &mut Expr {
		self.unwrap_mut_with(
			|e| {
				if let Expr::Paren(expr) = e { Some(&mut expr.expr) } else { None }
			},
		)
	}
...
impl Take for Expr {
	fn dummy() -> Self { Invalid { span:DUMMY_SP }.into() }
}

impl Default for Expr {
	fn default() -> Self { Expr::Invalid(Default::default()) }
}
...
impl Take for ThisExpr {
	fn dummy() -> Self { ThisExpr { span:DUMMY_SP } }
}
...
impl Take for ArrayLit {
	fn dummy() -> Self { ArrayLit { span:DUMMY_SP, elems:Default::default() } }
}
...
impl ObjectLit {
	...
	pub fn as_import_with(&self) -> Option<ImportWith> {
		...
		Some(ImportWith { span:self.span, values })
	}
}
...
impl ImportWith {
	pub fn get(&self, key:&str) -> Option<&Str> {
		self.values.iter().find_map(
			|item| {
				if item.key.sym == key { Some(&item.value) } else { None }
			},
		)
	}
}
...
impl Take for ObjectLit {
	fn dummy() -> Self { ObjectLit { span:DUMMY_SP, props:Default::default() } }
}
...
impl Take for PropOrSpread {
	fn dummy() -> Self {
		PropOrSpread::Spread(SpreadElement { dot3_token:DUMMY_SP, expr:Take::dummy() })
	}
}
...
impl Take for SpreadElement {
	fn dummy() -> Self { SpreadElement { dot3_token:DUMMY_SP, expr:Take::dummy() } }
}
...
impl Take for UnaryExpr {
	fn dummy() -> Self { UnaryExpr { span:DUMMY_SP, op:op!("!"), arg:Take::dummy() } }
}
...
impl Take for UpdateExpr {
	fn dummy() -> Self {
		UpdateExpr { span:DUMMY_SP, op:op!("++"), prefix:false, arg:Take::dummy() }
	}
}
...
impl Take for BinExpr {
	fn dummy() -> Self {
		BinExpr { span:DUMMY_SP, op:op!("*"), left:Take::dummy(), right:Take::dummy() }
	}
}
...
impl Take for FnExpr {
	fn dummy() -> Self { FnExpr { ident:None, function:Take::dummy() } }
}

impl From<Box<Function>> for FnExpr {
	fn from(function:Box<Function>) -> Self { Self { ident:None, function } }
}
...
impl Take for ClassExpr {
	fn dummy() -> Self { ClassExpr { ident:None, class:Take::dummy() } }
}

impl From<Box<Class>> for ClassExpr {
	fn from(class:Box<Class>) -> Self { Self { ident:None, class } }
}
...
impl Take for AssignExpr {
	fn dummy() -> Self {
		AssignExpr { span:DUMMY_SP, op:op!("="), left:Take::dummy(), right:Take::dummy() }
	}
}

impl AssignExpr {
	pub fn is_simple_assign(&self) -> bool { self.op == op!("=") && self.left.as_ident().is_some() }
}
...
impl Take for MemberExpr {
	fn dummy() -> Self { MemberExpr { span:DUMMY_SP, obj:Take::dummy(), prop:Take::dummy() } }
}
...
impl Default for MemberProp {
	fn default() -> Self { MemberProp::Ident(Default::default()) }
}

impl Take for SuperProp {
	fn dummy() -> Self { SuperProp::Ident(Default::default()) }
}

impl Default for SuperProp {
	fn default() -> Self { SuperProp::Ident(Default::default()) }
}
...
impl Take for CondExpr {
	fn dummy() -> Self {
		CondExpr { span:DUMMY_SP, test:Take::dummy(), cons:Take::dummy(), alt:Take::dummy() }
	}
}
...
impl Take for SeqExpr {
	fn dummy() -> Self { SeqExpr { span:DUMMY_SP, exprs:Take::dummy() } }
}
...
impl Take for ArrowExpr {
	fn dummy() -> Self { ArrowExpr { ..Default::default() } }
}
...
impl Take for YieldExpr {
	fn dummy() -> Self { YieldExpr { span:DUMMY_SP, arg:Take::dummy(), delegate:false } }
}
...
impl Take for Tpl {
	fn dummy() -> Self { Tpl { span:DUMMY_SP, exprs:Take::dummy(), quasis:Take::dummy() } }
}
...
pub struct TplElement {
	...
	///
	/// If you are going to use codegen right after creating a [TplElement],
...
impl Take for TplElement {
	fn dummy() -> Self {
		TplElement { span:DUMMY_SP, tail:Default::default(), cooked:None, raw:Default::default() }
	}
}
...
impl<'a> arbitrary::Arbitrary<'a> for TplElement {
	fn arbitrary(u:&mut arbitrary::Unstructured<'_>) -> arbitrary::Result<Self> {
		...
		Ok(Self { span, tail:false, cooked, raw })
	}
}
...
impl Take for ParenExpr {
	fn dummy() -> Self { ParenExpr { span:DUMMY_SP, expr:Take::dummy() } }
}
...
impl Default for Callee {
	fn default() -> Self { Callee::Super(Default::default()) }
}

impl Take for Callee {
	fn dummy() -> Self { Callee::Super(Take::dummy()) }
}
...
impl Take for Super {
	fn dummy() -> Self { Super { span:DUMMY_SP } }
}
...
impl Take for Import {
	fn dummy() -> Self { Import { span:DUMMY_SP, phase:ImportPhase::default() } }
}
...
impl Spanned for ExprOrSpread {
	...
	#[inline]
	fn span_hi(&self) -> BytePos { self.expr.span_hi() }
}

impl From<Box<Expr>> for ExprOrSpread {
	fn from(expr:Box<Expr>) -> Self { Self { expr, spread:None } }
}
...
impl Default for BlockStmtOrExpr {
	fn default() -> Self { BlockStmtOrExpr::BlockStmt(Default::default()) }
}
...
where
	T: Into<Expr>,
{
	fn from(e:T) -> Self { Self::Expr(Box::new(e.into())) }
}

impl Take for BlockStmtOrExpr {
	fn dummy() -> Self { BlockStmtOrExpr::Expr(Take::dummy()) }
}
...
impl TryFrom<Box<Pat>> for AssignTarget {
	...
	fn try_from(p:Box<Pat>) -> Result<Self, Self::Error> { (*p).try_into().map_err(Box::new) }
}
...
impl Default for AssignTargetPat {
	fn default() -> Self { AssignTargetPat::Invalid(Take::dummy()) }
}
...
impl From<AssignTargetPat> for Box<Pat> {
	fn from(pat:AssignTargetPat) -> Self { Box::new(pat.into()) }
}
...
impl Take for SimpleAssignTarget {
	fn dummy() -> Self { SimpleAssignTarget::Invalid(Take::dummy()) }
}
...
impl AssignTarget {
	pub fn as_ident(&self) -> Option<&BindingIdent> { self.as_simple()?.as_ident() }
...
impl Default for AssignTarget {
	fn default() -> Self { SimpleAssignTarget::dummy().into() }
}
...
impl Default for OptChainBase {
	fn default() -> Self { OptChainBase::Member(Default::default()) }
}
...
impl From<OptChainBase> for Expr {
	fn from(opt:OptChainBase) -> Self {
		match opt {
			OptChainBase::Call(OptCall { span, ctxt, callee, args, type_args }) => {
				Self::Call(CallExpr { callee:Callee::Expr(callee), args, span, type_args, ctxt })
			},
...
impl Take for OptCall {
	fn dummy() -> Self { Self { ..Default::default() } }
}

impl From<OptCall> for CallExpr {
	fn from(OptCall { span, ctxt, callee, args, type_args }:OptCall) -> Self {
		Self { span, callee:Callee::Expr(callee), args, type_args, ctxt }
	}
}
