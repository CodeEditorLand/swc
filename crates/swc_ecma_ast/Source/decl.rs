...
boxed!(
	Decl,
	[VarDecl, UsingDecl, TsInterfaceDecl, TsTypeAliasDecl, TsEnumDecl, TsModuleDecl]
);
...
decl_from_boxed!(VarDecl, UsingDecl, TsInterfaceDecl, TsTypeAliasDecl, TsEnumDecl, TsModuleDecl);

impl Take for Decl {
	fn dummy() -> Self { Decl::Var(Default::default()) }
}
...
impl Take for FnDecl {
	fn dummy() -> Self {
		FnDecl { ident:Take::dummy(), declare:Default::default(), function:Take::dummy() }
	}
}
...
impl Take for ClassDecl {
	fn dummy() -> Self {
		ClassDecl { ident:Take::dummy(), declare:Default::default(), class:Take::dummy() }
	}
}
...
impl Take for UsingDecl {
	fn dummy() -> Self { Self { span:DUMMY_SP, is_await:Default::default(), decls:Take::dummy() } }
}
