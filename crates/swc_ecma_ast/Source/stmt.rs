...
impl Take for BlockStmt {
	fn dummy() -> Self { BlockStmt { span:DUMMY_SP, stmts:Vec::new(), ctxt:Default::default() } }
}
...
impl Default for Stmt {
	fn default() -> Self { Self::Empty(EmptyStmt { span:DUMMY_SP }) }
}
...
impl Take for SwitchCase {
	fn dummy() -> Self { Self { span:DUMMY_SP, test:None, cons:Vec::new() } }
}
...
impl Default for ForHead {
	fn default() -> Self { ForHead::Pat(Take::dummy()) }
}
...
impl Take for VarDeclOrExpr {
	fn dummy() -> Self { VarDeclOrExpr::Expr(Take::dummy()) }
}
