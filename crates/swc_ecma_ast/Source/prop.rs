...
impl Default for PropName {
	fn default() -> Self { PropName::Ident(Default::default()) }
}

impl Take for PropName {
	fn dummy() -> Self { PropName::Ident(Take::dummy()) }
}
...
impl From<PropName> for MemberProp {
	fn from(p:PropName) -> Self {
		match p {
			...
			PropName::Str(p) => {
				MemberProp::Computed(ComputedPropName { span:DUMMY_SP, expr:p.into() })
			},
			PropName::Num(p) => {
				MemberProp::Computed(ComputedPropName { span:DUMMY_SP, expr:p.into() })
			},
...
impl Take for ComputedPropName {
	fn dummy() -> Self { Self { span:DUMMY_SP, expr:Take::dummy() } }
}
