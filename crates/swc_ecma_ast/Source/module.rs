...
impl Take for Program {
	fn dummy() -> Self { Program::Script(Script::default()) }
}
...
impl<'a> arbitrary::Arbitrary<'a> for Module {
	fn arbitrary(u:&mut arbitrary::Unstructured<'_>) -> arbitrary::Result<Self> {
		...
		Ok(Self { span, body, shebang:None })
	}
}

impl Take for Module {
	fn dummy() -> Self { Module { span:DUMMY_SP, body:Take::dummy(), shebang:Take::dummy() } }
}
...
impl Take for Script {
	fn dummy() -> Self { Script { span:DUMMY_SP, body:Take::dummy(), shebang:Take::dummy() } }
}
...
impl Take for ModuleItem {
	fn dummy() -> Self { Self::Stmt(Default::default()) }
}
