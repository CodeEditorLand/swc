...
where
	...
	let pass = add!(pass, ClassStaticBlock, es2022::static_blocks(static_blocks_mark));
	let pass = add!(
		pass,
		ClassProperties,
	...
	let pass =
		add!(pass, BugfixAsyncArrowsInClass, bugfixes::async_arrows_in_class(unresolved_mark));
		...
		let required = match self.mode {
			...
			Some(Mode::Entry) => {
				match self.corejs {
					...
					Version { major: 3, .. } => {
						let mut v =
							corejs3::Entry::new(self.targets, self.corejs, !self.regenerator);
						m.visit_mut_with(&mut v);
						v.imports
					},
