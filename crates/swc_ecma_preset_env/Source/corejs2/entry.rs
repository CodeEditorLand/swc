...
use swc_common::{DUMMY_SP, collections::ARandomState};
use swc_ecma_ast::*;
...
impl Entry {
	pub fn new(target:Versions, regenerator:bool) -> Self {
		...
		let mut v =
			Entry { is_any_target:target.is_any_target(), target, imports:Default::default() };
	...
	fn visit_mut_module_items(&mut self, items:&mut Vec<ModuleItem>) {
		items.retain_mut(|item| {
			...
			if let ModuleItem::Stmt(Stmt::Expr(ExprStmt { expr, .. })) = &item {
				if let Expr::Call(CallExpr { callee: Callee::Expr(callee), ref args, .. }) = &**expr
				{
					if callee.is_ident_ref_to("require")
						&& args.len() == 1 && if let ExprOrSpread { spread: None, expr } = &args[0] {
