...
impl UsageVisitor {
	pub fn new(target:Versions) -> Self {
		...
		Self { is_any_target:target.is_any_target(), target, required:Default::default() }
	}

	/// Add imports
	fn add(&mut self, features:&'static [&'static str]) {
		let UsageVisitor { is_any_target, target, .. } = self;
	...
	fn visit_object_pat_props(&mut self, obj:&Expr, props:&[ObjectPatProp]) {
		...
		for p in props {
			match p {
				ObjectPatProp::KeyValue(KeyValuePatProp { key: PropName::Ident(i), .. }) => {
...
impl Visit for UsageVisitor {
	...
	fn visit_call_expr(&mut self, e:&CallExpr) {
		...
		if match &e.callee {
			Callee::Expr(callee) => {
				matches!(&**callee, Expr::Member(MemberExpr {
