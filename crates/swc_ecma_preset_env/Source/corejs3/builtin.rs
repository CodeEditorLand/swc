...
use crate::util::{CoreJSPolyfillDescriptor, ObjectMap, ObjectMap2, descriptor};
...
static SYMBOL_DEPENDENCIES:&[&str] = &["es.symbol", "es.symbol.description", "es.object.to-string"];
...
static WEAK_MAP_DEPENDENCIES:&[&str] = &concat2::<7>(
	&["es.weak-map", "esnext.weak-map.delete-all", "esnext.weak-map.emplace"],
	COMMON_ITERATORS_WITH_TAG,
);

static WEAK_SET_DEPENDENCIES:&[&str] = &concat2::<7>(
	&["es.weak-set", "esnext.weak-set.add-all", "esnext.weak-set.delete-all"],
	COMMON_ITERATORS_WITH_TAG,
);
...
fn typed_array_static_methods(base:&'static [&'static str]) -> ObjectMap<CoreJSPolyfillDescriptor> {
	map!(Map {
		from:define(null, ["es.typed-array.from", base, TYPED_ARRAY_DEPENDENCIES]),
		fromAsync:define(
			null,
			[
...
fn uint8_typed_array_static_methods() -> ObjectMap<CoreJSPolyfillDescriptor> {
	...
	map.extend(map!(Map {
		fromBase64:define(null, ["esnext.uint8-array.from-base64", TYPED_ARRAY_DEPENDENCIES,]),
		fromHex:define(null, ["esnext.uint8-array.from-hex", TYPED_ARRAY_DEPENDENCIES,]),
	}));

	map
}

static DATA_VIEW_DEPENDENCIES:&[&str] = &concat2::<8>(&["es.data-view"], ARRAY_BUFFER_DEPENDENCIES);
