...
pub fn decorators(c:Config) -> impl Pass {
	if c.legacy {
		...
		Either::Right(fold_pass(Decorators { is_in_strict:false, vars:Default::default() }))
	}
}
...
impl Fold for Decorators {
	...
	fn fold_decl(&mut self, decl:Decl) -> Decl {
		...
		match decl {
			Decl::Class(ClassDecl { ident, declare: false, class }) => {
				if !contains_decorator(&class) {
					return ClassDecl { ident, declare:false, class }.into();
				}
	...
	fn fold_expr(&mut self, expr:Expr) -> Expr {
		...
		match expr {
			Expr::Class(ClassExpr { ident, class }) => {
				...
				self.fold_class_inner(ident.unwrap_or_else(|| quote_ident!("_class").into()), class)
			},
	...
	fn fold_module_decl(&mut self, decl:ModuleDecl) -> ModuleDecl {
		...
		match decl {
			ModuleDecl::ExportDefaultDecl(ExportDefaultDecl {
				...
				ExportDefaultExpr { span, expr:decorate_call }.into()
			},
		...
		items.into_iter().for_each(|item| {
			...
			macro_rules! handle_class {
				($cls:expr, $ident:expr) => {{
					...
					buf.push(ModuleItem::ModuleDecl(ModuleDecl::ExportNamed(NamedExport {
						span:DUMMY_SP,
						specifiers:vec![
			...
			match item {
				ModuleItem::ModuleDecl(ModuleDecl::ExportDefaultDecl(ExportDefaultDecl {
					decl: DefaultDecl::Class(ClassExpr { ident: Some(ident), class }),
					..
...
impl Decorators {
	...
	fn fold_class_inner(&mut self, ident:Ident, mut class:Box<Class>) -> Expr {
		...
		let super_class_ident =
			class.super_class.as_ref().map(|expr| alias_ident_for(expr, "_super"));
		...
		let constructor = {
			...
			let pos = class.body.iter().position(|member| {
				matches!(*member, ClassMember::Constructor(Constructor { body:Some(..), .. }))
			});
			...
			match pos {
				...
				None => {
					...
					c.body.as_mut().unwrap().stmts.push(initialize_call.into_stmt());
				...
				match member {
					...
					ClassMember::ClassProp(prop) => {
						...
						let key_prop_value = match prop.key {
							PropName::Ident(i) => {
								Lit::Str(Str { span:i.span, raw:None, value:i.sym }).into()
							},
			...
			iter::once({
				...
				Function {
					...
					params:iter::once(initialize.into())
						...
						.map(|pat| Param { span:DUMMY_SP, decorators:Vec::new(), pat })
						.collect(),
...
fn make_decorate_call(
	...
	CallExpr {
		...
		args:iter::once(
			ArrayLit {
				span:DUMMY_SP,
				elems:decorators.into_iter().map(|dec| Some(dec.expr.as_arg())).collect(),
			}
			.as_arg(),
		)
		.chain(args)
...
impl Visit for DecoratorFinder {
	fn visit_decorator(&mut self, _:&Decorator) { self.found = true }
}
