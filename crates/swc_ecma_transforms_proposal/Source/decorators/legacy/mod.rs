...
use swc_common::{BytePos, DUMMY_SP, collections::AHashMap, util::take::Take};
use swc_ecma_ast::*;
...
impl TscDecorator {
		...
		for mut s in stmts.take() {
			...
			new.extend(
				self.prepended_exprs
					.drain(..)
					.map(|expr| ExprStmt { span:DUMMY_SP, expr }.into())
					.map(T::from),
			);
	...
	fn key(&mut self, k:&mut PropName) -> Expr {
		match k {
			...
			PropName::Ident(i) => {
				return Lit::Str(Str { span:DUMMY_SP, raw:None, value:i.sym.clone() }).into();
			},
	...
	fn visit_ts_enum_decl(&mut self, e:&TsEnumDecl) {
		let enum_kind = e
			...
			.fold(None, |opt:Option<EnumKind>, item| {
				...
				if a == b { Some(a) } else { Some(EnumKind::Mixed) }
			});
...
impl VisitMut for TscDecorator {
		...
		if !appended_private.is_empty() {
			let expr = if appended_private.len() == 1 {
				*appended_private.into_iter().next().unwrap()
				...
				SeqExpr { exprs:appended_private, ..Default::default() }.into()
			};

			n.body.push(
				StaticBlock {
					body:BlockStmt { stmts:vec![expr.into_stmt()], ..Default::default() },
					..Default::default()
				}
				.into(),
			)
		...
		if let Some(class_name) = self.class_name.clone() {
			if !n.decorators.is_empty() {
				...
				let decorated = CallExpr {
					...
					args:vec![
						decorators,
						class_name.clone().with_pos(BytePos::DUMMY, BytePos::DUMMY).as_arg(),
					],
					..Default::default()
				}
				.into();
	...
	fn visit_mut_class_expr(&mut self, n:&mut ClassExpr) {
		...
		let ident = n.ident.get_or_insert_with(|| private_ident!("_class")).clone();
