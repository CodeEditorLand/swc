...
impl VisitMut for ParamMetadata {
	fn visit_mut_class(&mut self, cls:&mut Class) {
		...
		cls.body = cls.body.take().move_map(|m| {
			match m {
...
impl VisitMut for Metadata<'_> {
	fn visit_mut_class(&mut self, c:&mut Class) {
		...
		let constructor = c.body.iter().find_map(|m| {
			match m {
	...
	fn visit_mut_class_method(&mut self, m:&mut ClassMethod) {
...

		{
			let type_arg = match m.kind {
				...
				MethodKind::Setter => {
					serialize_type(self.class_name, get_type_ann_of_pat(&m.function.params[0].pat))
						.as_arg()
				},
			};
		...
		match *expr {
			...
			_ => {
				BinExpr {
					...
					op:op!("==="),
					right:Box::new(
						Lit::Str(Str { span:DUMMY_SP, value:"undefined".into(), raw:None }).into(),
					),
				}
				.into()
			},
...
fn ts_entity_to_member_expr(type_name:&TsEntityName) -> Expr {
	match type_name {
		TsEntityName::TsQualifiedName(q) => {
			...
			MemberExpr { span:DUMMY_SP, obj:obj.into(), prop:MemberProp::Ident(q.right.clone()) }
				.into()
		},
