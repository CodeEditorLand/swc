...
pub(crate) fn decorator_impl(version:DecoratorVersion) -> impl VisitMut + Fold {
	as_folder(DecoratorPass { version, ..Default::default() })
}
...
impl DecoratorPass {
	...
	fn preserve_side_effect_of_decorator(&mut self, dec:Box<Expr>) -> Box<Expr> {
		...
		self.pre_class_inits.push(
			AssignExpr { span:DUMMY_SP, op:op!("="), left:ident.clone().into(), right:dec }.into(),
		);
	...
	fn consume_inits(&mut self) {
		...
		combined_args
			.push(ArrayLit { span:DUMMY_SP, elems:self.state.class_decorators.take() }.as_arg());
	...
	fn ensure_constructor<'a>(&mut self, c:&'a mut Class) -> &'a mut Constructor {
		...
		c.body.insert(insert_index, default_constructor(c.super_class.is_some()).into());
	...
	fn handle_class_decl(&mut self, c:&mut ClassDecl) -> Stmt {
		...
		let has_static_member = body.iter().any(|m| {
			match m {
...
impl VisitMut for DecoratorPass {
				...
				ClassMember::AutoAccessor(mut accessor) => {
					...
					let private_field = PrivateProp {
						...
						key:match &mut accessor.key {
							Key::Private(k) => {
								name =
									Lit::Str(Str { span:DUMMY_SP, value:k.name.clone(), raw:None })
										.into();
							...
							Key::Public(k) => {
								...
								field_name_like =
									format!("__{}", init.sym).replacen("init", "private", 1).into();
								...
								elems:match &accessor.key {
									Key::Private(_) => {
										let data = vec![
											...
											Some(name.as_arg()),
											Some(
												FnExpr { ident:None, function:getter_function }
													.as_arg(),
											),
											Some(
												FnExpr { ident:None, function:setter_function }
													.as_arg(),
											),
										];
	...
	fn visit_mut_module_items(&mut self, n:&mut Vec<ModuleItem>) {
		...
		for (index, n) in n.iter_mut().enumerate() {
			...
			if !self.pre_class_inits.is_empty() {
				insert_builder.push_back(
					index,
					ExprStmt { span:DUMMY_SP, expr:Expr::from_exprs(self.pre_class_inits.take()) }
						.into(),
				);
			}
		}
	...
	fn visit_mut_stmts(&mut self, n:&mut Vec<Stmt>) {
		...
		if !self.extra_vars.is_empty() {
			let insert_pos =
				n.iter().position(|stmt| !is_maybe_branch_directive(stmt)).unwrap_or(0);
			insert_builder.push_front(
				insert_pos,
				VarDecl {
					span:DUMMY_SP,
...
impl<T> InsertPassBuilder<T> {
	pub fn new() -> Self { Self { inserts:Default::default() } }
