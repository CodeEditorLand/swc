	...
	where
		...
		for stmt in stmts.take() {
			match stmt.try_into_stmt() {
				...
				Err(stmt) => {
					match stmt.try_into_module_decl() {
		...
		let assign_error = AssignExpr {
			span:DUMMY_SP,
			op:op!("="),
			left:state.using_ctx.clone().make_member(quote_ident!("e")).into(),
		...
		let dispose_expr = CallExpr {
			span:DUMMY_SP,
			callee:state.using_ctx.clone().make_member(quote_ident!("d")).as_callee(),
			args:Vec::new(),
			..Default::default()
		};
		let dispose_stmt = if state.has_await {
			AwaitExpr { span:DUMMY_SP, arg:Box::new(dispose_expr.into()) }.into()
		} else {
		...
		let try_stmt = TryStmt {
			span:DUMMY_SP,
			block:BlockStmt { span:DUMMY_SP, stmts:try_body, ..Default::default() },
			handler:Some(CatchClause {
				span:DUMMY_SP,
				param:Some(state.catch_var.into()),
				body:BlockStmt { span:DUMMY_SP, stmts:vec![assign_error], ..Default::default() },
			}),
...
impl VisitMut for ExplicitResourceManagement {
	...
	fn visit_mut_stmt(&mut self, s:&mut Stmt) {
		...
		if let Stmt::Decl(Decl::Using(decl)) = s {
			...
			*s = VarDecl {
				span:DUMMY_SP,
				kind:if self.is_not_top_level { VarDeclKind::Const } else { VarDeclKind::Var },
				declare:Default::default(),
				decls:decl
					.decls
					.take()
					...
					.map(|d| {
						...
						VarDeclarator { init:Some(init.into()), ..d }
					})
					.collect(),
				..Default::default()
			}
