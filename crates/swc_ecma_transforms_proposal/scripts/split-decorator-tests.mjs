...
const ts = fs.readFileSync(
	"./tests/decorator-tests/decorator-tests.ts",
	"utf8",
);
...
const mainFile = program.getSourceFile(
	"./tests/decorator-tests/decorator-tests.ts",
);
// console.log('mainFile', mainFile)

const testVarStmt = mainFile.statements.find(
	(s) => s.kind === typescript.SyntaxKind.VariableStatement,
);
...
for (const property of properties) {
	...
	const code = `(${value})()`;
	const transpiiled = typescript.transpile(code, {
