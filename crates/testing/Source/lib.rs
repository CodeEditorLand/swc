...
pub fn find_executable(name:&str) -> Option<PathBuf> {
	...
	if path.is_none() {
		...
		path = Command::new("yarn").arg("bin").arg(name).output().ok().and_then(|output| {
...
where
	...
	let result = swc_common::GLOBALS
		.set(&swc_common::Globals::new(), || HANDLER.set(&handler, || op(cm, &handler)));
...
pub fn print_left_right(left:&dyn Debug, right:&dyn Debug) -> String {
	...
	let test_name = cur.name().expect("rustc sets test name as the name of thread");
