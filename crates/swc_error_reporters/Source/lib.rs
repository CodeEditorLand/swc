...
impl PrettyEmitter {
	pub fn new(
		...
		Self { cm, wr:WriterWrapper(wr), reporter, config }
	}
}
...
impl Write for WriterWrapper {
	...
	fn write_char(&mut self, c:char) -> fmt::Result { self.0.write_char(c) }

	fn write_fmt(&mut self, args:fmt::Arguments<'_>) -> fmt::Result { self.0.write_fmt(args) }
}
...
impl Emitter for PrettyEmitter {
	fn emit(&mut self, db:&DiagnosticBuilder) {
		...
		let diagnostic = MietteDiagnostic { source_code, d, children };

		self.reporter.render_report(&mut self.wr, &diagnostic).unwrap();
	}
}
...
impl miette::Diagnostic for MietteDiagnostic<'_> {
	...
	fn severity(&self) -> Option<Severity> { level_to_severity(self.d.level) }
	...
	fn url<'a>(&'a self) -> Option<Box<dyn fmt::Display + 'a>> { None }
	...
	fn related<'a>(&'a self) -> Option<Box<dyn Iterator<Item = &'a dyn miette::Diagnostic> + 'a>> {
		if self.children.is_empty() {
			...
			Some(Box::new(self.children.iter().map(|d| d as &dyn miette::Diagnostic)))
		}
	}
}
...
impl fmt::Debug for MietteDiagnostic<'_> {
	fn fmt(&self, f:&mut fmt::Formatter) -> fmt::Result { fmt::Debug::fmt(&self.d, f) }
}
...
impl miette::Diagnostic for MietteSubdiagnostic<'_> {
	fn code<'a>(&'a self) -> Option<Box<dyn fmt::Display + 'a>> { None }

	fn severity(&self) -> Option<Severity> { level_to_severity(self.d.level) }

	fn help<'a>(&'a self) -> Option<Box<dyn fmt::Display + 'a>> { None }

	fn url<'a>(&'a self) -> Option<Box<dyn fmt::Display + 'a>> { None }

	fn source_code(&self) -> Option<&dyn SourceCode> { Some(&self.source_code) }
...
impl<'a> SpanContents<'a> for SpanContentsImpl<'a> {
	fn data(&self) -> &'a [u8] { self.data.as_bytes() }
