...
impl<N:Eq + Hash + fmt::Debug, E:fmt::Debug, Ty:EdgeType> fmt::Debug for FastGraphMap<N, E, Ty> {
	fn fmt(&self, f:&mut fmt::Formatter) -> fmt::Result { self.nodes.fmt(f) }
}
...
impl PartialEq<Direction> for CompactDirection {
	fn eq(&self, rhs:&Direction) -> bool { (*self as usize) == (*rhs as usize) }
}
	...
	Ty: EdgeType,
{
	...
	pub fn capacity(&self) -> (usize, usize) { (self.nodes.capacity(), self.edges.capacity()) }

	/// Use their natural order to map the node pair (a, b) to a canonical edge
	/// id.
	#[inline]
	fn edge_key(a:N, b:N) -> (N, N) { if Ty::is_directed() || a <= b { (a, b) } else { (b, a) } }
	...
	pub fn contains_node(&self, n:N) -> bool { self.nodes.contains_key(&n) }
	...
	/// graph.
	pub fn contains_edge(&self, a:N, b:N) -> bool { self.edges.contains_key(&Self::edge_key(a, b)) }

	/// Return an iterator over the nodes of the graph.
	///
	/// Iterator element type is `N`.
	pub fn nodes(&self) -> Nodes<N> { Nodes { iter:self.nodes.keys().cloned() } }
	...
	pub fn edges(&self, from:N) -> Edges<N, E, Ty> {
		Edges { from, iter:self.neighbors(from), edges:&self.edges }
	}

	/// Return a reference to the edge weight connecting `a` with `b`, or
	/// `None` if the edge does not exist in the graph.
	pub fn edge_weight(&self, a:N, b:N) -> Option<&E> { self.edges.get(&Self::edge_key(a, b)) }
	...
	pub fn all_edges(&self) -> AllEdges<N, E, Ty> {
		AllEdges { inner:self.edges.iter(), ty:self.ty }
	}
	...
	pub fn all_edges_mut(&mut self) -> AllEdgesMut<N, E, Ty> {
		AllEdgesMut { inner:self.edges.iter_mut(), ty:self.ty }
	}
	...
	Ty: EdgeType,
{
	...
	fn next(&mut self) -> Option<Self::Item> { self.inner.next().map(|(&(a, b), v)| (a, b, v)) }
...
where
	...
	Ty: EdgeType,
{
	fn default() -> Self { FastGraphMap::with_capacity(0, 0) }
}
...
impl<'b, T> PartialEq for Ptr<'b, T> {
	...
	fn eq(&self, other:&Ptr<'b, T>) -> bool { ptr_eq(self.0, other.0) }
}

impl<'b, T> PartialOrd for Ptr<'b, T> {
	fn partial_cmp(&self, other:&Ptr<'b, T>) -> Option<Ordering> { Some(self.cmp(other)) }
}
	...
	Ty: EdgeType,
{
	...
	fn next(&mut self) -> Option<Self::Item> { self.iter.next().map(|(&n, _)| n) }
}
	...
	Ty: EdgeType,
{
	...
	fn next(&mut self) -> Option<Self::Item> { self.iter.next().map(|(n, _)| (*n, n)) }
}
	...
	Ty: EdgeType,
{
	...
	fn reset_map(&self, map:&mut Self::Map) { map.clear(); }
}
	...
	Ty: EdgeType,
{
	...
	fn neighbors(self, n:Self::NodeId) -> Self::Neighbors { self.neighbors(n) }
}
	...
	Ty: EdgeType,
{
	...
	fn node_identifiers(self) -> Self::NodeIdentifiers {
		NodeIdentifiers { iter:self.nodes.iter(), ty:self.ty, edge_ty:PhantomData }
	}
}
	...
	Ty: EdgeType,
{
	...
	fn from_index(&self, ix:usize) -> Self::NodeId {
		assert!(ix < self.nodes.len(), "The requested index {} is out-of-bounds.", ix);
