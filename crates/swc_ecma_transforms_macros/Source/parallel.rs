...
pub fn expand(attr:TokenStream, mut item:ItemImpl) -> ItemImpl {
	...
	let explode = meta.as_ref().map(|v| v.path().is_ident("explode")).unwrap_or(false);

	item.items
		.push(ImplItem::Fn(make_par_visit_method(mode, "module_items", explode)));

	item.items.push(ImplItem::Fn(make_par_visit_method(mode, "stmts", explode)));
...
fn make_par_visit_method(mode:Mode, suffix:&str, explode:bool) -> ImplItemFn {
	...
	match (mode, explode_method_name) {
		(Mode::Fold, Some(explode_method_name)) => {
			parse_quote!(
		...
		(Mode::VisitMut, Some(explode_method_name)) => {
			parse_quote!(
