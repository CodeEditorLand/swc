...
pub fn generate(crate_name:&Ident, node_types:&[&Item]) -> File {
	let mut output = File { shebang:None, attrs:Vec::new(), items:Vec::new() };
	...
	for &kind in [TraitKind::Visit, TraitKind::VisitMut, TraitKind::Fold].iter() {
		for &variant in [Variant::Normal, Variant::AstPath].iter() {
			...
			output.items.extend(g.implement_visit_with_for_node_types(node_types));

			output
				.items
				.extend(g.implement_visit_with_for_non_node_types(&field_only_types));

			output.items.extend(g.implement_visit_with_for_generic_types());
		}
	}
...
impl Variant {
	pub fn method_suffix(self, is_visitor_method:bool) -> &'static str {
		if self == Variant::Normal || is_visitor_method { "" } else { "_ast_path" }
	}
}
...
impl Generator {
	...
	fn trait_name(&self, with:bool) -> Ident {
		...
		let name = if with { format!("{}With", name) } else { name.to_string() };
	...
	fn declare_visit_trait(&self, all_types:&[FieldType]) -> Vec<Item> {
		...
		for ty in all_types {
			...
			let else_block = if self.kind == TraitKind::Fold { quote!(node) } else { quote!() };
	...
	fn declare_visit_with_trait(&self) -> Vec<Item> {
...

		{
			...
			let visit_with_name = Ident::new(
				&format!("{}_with{}", self.kind.method_prefix(), self.variant.method_suffix(false)),
				Span::call_site(),
			);
			let visit_with_children_name = Ident::new(
				&format!(
	...
	fn implement_visit_with_for_node_types(&self, node_types:&[&Item]) -> Vec<Item> {
		...
		for node_type in node_types {
			...
			let visit_with_doc =
				doc(&format!("Calls [{visitor_trait_name}`::{}`] with `self`.", visit_method_name));
	...
	fn default_visit_body(
		...
		let enum_ast_path = match enum_variant_name {
			Some(variant_name) if self.variant == Variant::AstPath => {
				let field_variant =
					Ident::new(&variant_name.to_string().to_pascal_case(), Span::call_site());
...
fn doc(s:&str) -> Attribute { parse_quote!(#[doc = #s]) }
...
fn extract_vec(ty:&Type) -> Option<&Type> { extract_generic("Vec", ty) }
